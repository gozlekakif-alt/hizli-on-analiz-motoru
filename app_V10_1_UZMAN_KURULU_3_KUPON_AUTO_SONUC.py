...
# GitHub veri.txt yeni sonucu aldıysa, kullanıcı sonucu ayrıca yapıştırmasa bile
# sonuçtan ÖNCE dondurulmuş snapshot ile son gerçekleşen çekilişi otomatik karşılaştır.
def _auto_latest_eval_from_data(df):
    if df is None or df.empty:
        return None
    z=df.sort_values(["_dt","draw_no"],ascending=False)
    for _,r in z.head(20).iterrows():
        draw=int(r["draw_no"])
        snap=find_snapshot_for_draw(draw)
        if not snap or int(snap.get("target_draw",-1))!=draw:
            continue
        actual=set(map(int,r["numbers"]))
        wanted=[
            ("V10 ANA 5",snap.get("v10_final5",[])),
            ("BAĞIMSIZ 5 — V9.5",snap.get("independent5",snap.get("v9_final5",[]))),
            ("3. KUPON — V7",snap.get("v7_final5",[])),
        ...
        for name,arr in wanted:
            arr=list(map(int,arr or [])); hits=sorted(set(arr)&actual)
            cards.append({"name":name,"score":f"{len(hits)}/{len(arr) if arr else 5}",
                          "hits":"-".join(map(str,hits))})
        # Karnenin GitHub/yerel kaydı da otomatik güncellensin.
        try:
            summ,_,_=eval_snapshot(snap,r["numbers"])
            save_eval_log(snap,r["numbers"],summ)
        except Exception:
            pass
        return {"draw":draw,"date":str(r["date"]),"time":str(r["time"]),
                "cards":cards,"persist":"GitHub veri.txt sonucundan otomatik kontrol",
                "learn_msg":"Sonuç, önceden dondurulmuş kuponlarla otomatik karşılaştırıldı."}
    return None

auto_eval=_auto_latest_eval_from_data(df)
if auto_eval is not None:
    # Elle eklenen daha yeni bir değerlendirme varsa onu ezme; aksi halde otomatik sonucu göster.
    if not isinstance(last_eval,dict) or int(auto_eval.get("draw",0)) >= int(last_eval.get("draw",0)):
        last_eval=auto_eval
        st.session_state["v10_simple_last_eval"]=auto_eval
