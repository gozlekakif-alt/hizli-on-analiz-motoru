...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.0",
    page_icon="🎯",
    layout="wide",
)
...
st.title("🎯 Hızlı On Ultimate AI Studio V18.0")
...
if page == "✅ Kontrol":
    st.success("Çalışan sürüm: V18.0 — Ana dosya: app.py")
    ...
    if len(df) < 100:
        ...
        if st.button(
            ...
            st.session_state["v180_start_draw"] = (
                int(df.iloc[-1].Cekilis_No) + 1
            )
        ...
        if items and start_draw:
            ...
            if evaluated.empty:
                st.info(
                    f"#{start_draw} ve sonrası "
                    "ilk gerçek sonuç bekleniyor."
                )
            else:
                st.dataframe(
                    evaluated.sort_values(
...
st.caption(
    "V18.0 V17'nin tüm özelliklerini korur; merkezi takvim, kalıcı öğrenme, güven puanı, hata analizi ve kapalı öğrenme döngüsünü ekler. "
