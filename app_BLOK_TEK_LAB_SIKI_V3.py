...
def evaluate_dynamic_selector(cmp, min_history=60, window=120, min_bucket=12):
    ...
    # SIKI V3: 750 testin ilk %65'inde seçilip son %35'inde ayrıca doğrulanan
    # basit ek fren. Gerçek hedef sonucu tahmin anında kullanılmaz.
    out["Sıkı V3 Seçici"] = "PAS"
    strict_v3_mask = (
        out["Sıkı Seçici"].eq("KONUŞ")
        & (out["Güven"].astype(float) >= 91.0)
        & (out["DEVAM olay"].astype(float) >= 0.80)
    )
    out.loc[strict_v3_mask, "Sıkı V3 Seçici"] = "KONUŞ"
    out["SIKI V3 Top10"] = np.where(
        out["Sıkı V3 Seçici"].eq("KONUŞ"), out["YENİ Top10"], np.nan
    )
    return out
...
st.title("🧪 BLOK TEK LAB — HAFİF + SIKI V3")
st.caption("Dinamik seçici ve SIKI V2 korunur; SIKI V3 daha seçici üçüncü katmandır.")
st.success("✅ Uygulama hazır. Ağır hesap henüz başlamadı.")
st.info("Telefon için hafif mod: Test yalnız aşağıdaki düğmeye bastığında çalışır. SIKI V3, V2 sinyallerini Güven ve DEVAM kanıtıyla daha da sıkılaştırır.")
...
if isinstance(selected, pd.DataFrame) and not selected.empty:
    ...
    strict_v3_spoken = selected[selected["Sıkı V3 Seçici"] == "KONUŞ"].copy() if "Sıkı V3 Seçici" in selected.columns else pd.DataFrame()
    ...
    st.subheader("🔥 SIKI V3 — kronolojik doğrulamalı seçici")
    if len(strict_v3_spoken) and "YENİ Top10" in strict_v3_spoken.columns:
        v1,v2,v3,v4 = st.columns(4)
        v1.metric("V3 KONUŞ", len(strict_v3_spoken))
        v2.metric("V3 Top10 ort.", f"{strict_v3_spoken['YENİ Top10'].mean():.3f}")
        v3.metric("V3 5+", int((strict_v3_spoken['YENİ Top10'] >= 5).sum()))
        v4.metric("V3 0–1", int((strict_v3_spoken['YENİ Top10'] <= 1).sum()))
        st.success("SIKI V3 = SIKI V2 + Güven ≥ 91 + DEVAM olay ≥ 0.80.")
    else:
        st.info("SIKI V3 bu testte KONUŞ koşulu bulamadı.")
    ...
    st.download_button(
        ...
        data=csv_selected,
        file_name="BLOK_TEK_LAB_SIKI_V3_TEST.csv",
        mime="text/csv",
