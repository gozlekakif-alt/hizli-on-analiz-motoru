...
st.set_page_config(page_title="KOMSU TEK LAB FINAL", page_icon="🧲", layout="wide")
APP_VERSION = "KOMSU TEK LAB FINAL — IZOLASYONLU WALK-FORWARD"

st.title("🧲 KOMSU TEK LAB FINAL")
st.caption("Yalnız KOMŞU uzmanı çalışır. Eski BLOK/Birleşim ekranları bu dosyada yoktur.")
...
try:
    st.success(f"Veri hazır • {len(df)} çekiliş • son çekiliş #{int(df['Cekilis_No'].iloc[-1])}")
except Exception:
    pass

if st.button("🧲 KOMSU TAM WALK-FORWARD TESTINI CALISTIR", type="primary"):
    with st.spinner("KOMSU motoru bütün uygun çekilişlerde test ediliyor..."):
        strict_df, kom_wf = komsu_strict_walkforward(eng, 140)
        score_sum, rank_sum = komsu_calibration_report(kom_wf)

    st.session_state["komsu_strict"] = strict_df
    st.session_state["komsu_raw"] = kom_wf
    st.session_state["komsu_score"] = score_sum
    st.session_state["komsu_rank"] = rank_sum

if "komsu_strict" in st.session_state:
    strict_df = st.session_state["komsu_strict"]
    kom_wf = st.session_state["komsu_raw"]
    score_sum = st.session_state["komsu_score"]
    rank_sum = st.session_state["komsu_rank"]
    ...
    c2.metric("Aday satırı", len(kom_wf))
    ...
    st.subheader("Skor kalibrasyonu")
    ...
    st.dataframe(strict_df.sort_values("Çekiliş", ascending=False),
                 use_container_width=True, hide_index=True)

    st.download_button(
        "⬇️ 1) KOMSU WALK-FORWARD KARARLARI",
    ...
    st.download_button(
        "⬇️ 2) KOMSU TUM ADAY KALIBRASYONU",
    ...
    st.info(
        ...
        f"ADAY KALİBRASYON={len(kom_wf)} satır. "
        "İkinci dosya 80 aday x uygun çekilişler olduğu için çok daha büyüktür."
    )
