...
tabs=st.tabs(["🎯 CANLI 4/5","🧱 ARDIŞIK 9","🧠 TEK BEYİN","🧬 UZMANLAR","🌙 REJİM & BANT","🧪 KÖR TEST","🩺 CERRAHİ","🧫 GÜNLÜK KADAVRA","➕ YENİ ÇEKİLİŞ","📥 SONUÇ & ÖĞREN","📋 MANİFESTO","💾 KAYDET"])
...
with tabs[1]:
    ...
    b.write("3'lü anatomi:",ce["triple_meta"])
...
with tabs[8]:
    st.subheader("➕ Sonuç / çekiliş ekle — tek veya çoklu")
    ...
    if mode=="⚡ Tek çekiliş":
        ...
        save_gh=st.checkbox("GitHub veri.txt'ye de kalıcı yaz",value=False,key="single_save_gh")
            ...
            if mdf.empty:
                ...
                if not rep.empty:
                    ...
                    if not piv.empty:
                        ...
                        st.dataframe(summary.round(3),use_container_width=True,hide_index=True)
        multi_save_gh=st.checkbox("Çoklu sonuçları GitHub veri.txt'ye de kalıcı yaz",value=False,key="multi_save_gh")
        if st.button("📚 TÜMÜNÜ EKLE",type="primary",use_container_width=True):
            if mdf.empty:
                ...
                msg=f"{len(mdf)} çekiliş oturuma eklendi · {local_msg}."
                if multi_save_gh:
                    try:
...
with tabs[11]:
    ...
    row={"saved_at":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),"target_draw":target_draw,"target_date":target_date,"target_time":target_time,"night_character":p["char"]["label"],"core4":"-".join(map(str,p["core4"])),"alt4":"-".join(map(str,p["alt4"])),"core5":"-".join(map(str,p["core5"])),"alt5":"-".join(map(str,p["alt5"])),"live5":"-".join(map(str,p["surgery"]["live5"])),"v10_ana5":"-".join(map(str,p.get("v10final5",[]))),"bagimsiz5":"-".join(map(str,p.get("independent5",p.get("v9final5",[])))),"pool16":"-".join(map(str,p.get("pool16",p["tab"].head(16)["Sayı"].astype(int).tolist()))),"snapshot_json":json.dumps(snapshot,ensure_ascii=False,separators=(",",":"))}
    ...
    if SAVE_FILE.exists():
        try:
            saved=pd.read_csv(SAVE_FILE); showcols=[c for c in ["saved_at","target_draw","target_date","target_time","night_character","v10_ana5","bagimsiz5","core4","core5","live5","pool16"] if c in saved.columns]
