...
def blind_test(df,ntest=40,min_train=120):
    ...
    for i in range(start,len(df)):
        ...
        rows.append({
            ...
            "Gece":p["char"]["label"],"Gerçek20":"-".join(map(str,sorted(actual))),
            "4 Çekirdek":len(set(p["core4"])&actual),"4 Alternatif":len(set(p["alt4"])&actual),
...
# UI — V26
# ------------------------------------------------------------
st.title("🧠 Hızlı On — V26 Mikro Uzman Ortak Beyin")
st.caption(
...
with tabs[0]:
    ...
    show=p["tab"].head(pool_size).copy(); cols=["Sayı","Bağımsız Aile","Güçlü Aile","Ana Puan","Uzman Oy","Güçlü Oy","Gap","Yaşam Durumu","Yaşam İzi","Kaynaklar"]
...
with tabs[7]:
    ...
    if SAVE_FILE.exists():
        try:
            ...
            st.download_button("⬇️ Kupon kayıt CSV",saved.to_csv(index=False).encode("utf-8-sig"),file_name="v26_kupon_kayitlari.csv",mime="text/csv")
        except Exception: pass
