...
st.set_page_config(page_title='Hızlı On — Aile Araştırma Laboratuvarı V4.1', layout='wide')
st.title('🧬 Hızlı On — Aile Araştırma Laboratuvarı V4.1')
...
def family_occurrences(daydf,k):
    # Exact recurring-family lifecycle (>=2 sightings). For 4/5, avoid rescanning
    # all 217 draws for every candidate: intersect per-number posting lists instead.
...
# Tek tık başlatır; her Streamlit çalıştırmasında yalnızca BİR gün işlenir.
# Böylece uzun tek-request kilitlenmesi yerine günlük checkpoint + otomatik rerun kullanılır.
if 'v4_running' not in st.session_state: st.session_state['v4_running']=False
if st.button('▶️ TEK TUŞ — 14 GÜNLÜK AİLE ARAŞTIRMASINI BAŞLAT',type='primary',width='stretch',disabled=st.session_state['v4_running']):
    st.session_state['v4_running']=True
    st.session_state['v4_error']=''
    st.rerun()

def valid_day_pkl(day):
    p=STORE/f'{day}_v4.pkl'
    if not p.exists(): return False
    try:
        with open(p,'rb') as f:
            r=pickle.load(f)
        return isinstance(r,dict) and all(k in r for k in ('life','events','growth','types','singles'))
    except Exception:
        p.unlink(missing_ok=True); return False

done=[d for d in DAYS if valid_day_pkl(d)]
st.progress(len(done)/14,text=f'{len(done)}/14 gün güvenli checkpoint tamamlandı')
if done: st.caption('Tamamlanan: '+', '.join(done))

if st.session_state['v4_running']:
    next_day=next((d for d in DAYS if d not in done),None)
    if next_day is not None:
        box=st.status(f'🔬 {next_day} araştırılıyor…',expanded=True)
        try:
            r=analyze_day(df[df.date==next_day].copy())
            # Günlük ZIP daha burada doğrulanır; bozuk sonuç checkpoint sayılmaz.
            dz=make_day_zip(next_day,r)
            if not zip_ok(dz): raise RuntimeError('Günlük ZIP CRC testi geçmedi')
            pkl=STORE/f'{next_day}_v4.pkl'; tmp=pkl.with_suffix('.tmp')
            with open(tmp,'wb') as f: pickle.dump(r,f,pickle.HIGHEST_PROTOCOL)
            os.replace(tmp,pkl)
            box.update(label=f'✅ {next_day} tamamlandı ve doğrulandı',state='complete',expanded=False)
            del r,dz; gc.collect()
            st.rerun()
        except Exception as e:
            st.session_state['v4_running']=False
            st.session_state['v4_error']=f'{next_day}: {type(e).__name__}: {e}'
            box.update(label='❌ Günlük araştırma durduruldu',state='error',expanded=True)
            st.error(st.session_state['v4_error'])
    else:
        # 14 checkpoint hazır. Parçaları ve Master'ı yalnız bir kez üret.
        try:
            daily={}
            for d in DAYS:
                with open(STORE/f'{d}_v4.pkl','rb') as f: daily[d]=pickle.load(f)
            part_files=[]
            for pi,idxs in enumerate(PARTS,1):
                days=[DAYS[i] for i in idxs]
                pb=make_part_zip(pi,days,daily)
                path=STORE/f'AILE_V4_PARCA_{pi}.zip'; tmp=path.with_suffix('.tmp')
                tmp.write_bytes(pb); os.replace(tmp,path); part_files.append(path)
            ...
            with zipfile.ZipFile(master,'w',zipfile.ZIP_DEFLATED) as z:
                ...
                z.writestr('14_GUN_KARAKTER_OZET.csv',csvb(char))
                z.writestr('14_GUN_BUYUME_OZET.csv',csvb(gg))
                z.writestr('14_GUN_TIP_OZET.csv',csvb(agg['types']))
                manifest={'version':'V4.1','draws':len(df),'days':DAYS,'parts':[p.name for p in part_files],
            ...
            mb=master.getvalue()
            if not zip_ok(mb): raise RuntimeError('MASTER ZIP CRC testi geçmedi')
            mpath=STORE/'HIZLI_ON_14_GUN_AILE_ARASTIRMA_V4_1_MASTER.zip'; tmp=mpath.with_suffix('.tmp')
            tmp.write_bytes(mb); os.replace(tmp,mpath)
            st.session_state['v4_running']=False; st.session_state['v4_ready']=True
            del daily,agg,char,gg,mb; gc.collect()
            st.success('14/14 tamamlandı. 4 parça ve Master CRC kontrolünden geçti.')
            st.rerun()
        except Exception as e:
            st.session_state['v4_running']=False
            st.session_state['v4_error']=f'Master oluşturma: {type(e).__name__}: {e}'
            st.error(st.session_state['v4_error'])

if st.session_state.get('v4_error'):
    st.warning('Son hata: '+st.session_state['v4_error'])
...
mp=STORE/'HIZLI_ON_14_GUN_AILE_ARASTIRMA_V4_1_MASTER.zip'
