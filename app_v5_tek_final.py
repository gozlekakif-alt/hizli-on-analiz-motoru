...
def final_ticket_v5(train, target_slot, tab, regime_model, carry_pool, return_pool, narrow_pool, size=10):
    """
    Tek final kuponu:
    - güncel kanıtları toplar,
    - geçmiş walk-forward başarısına göre kanalları ödüllendirir/cezalandırır,
    - taşıma/dönüş sayısını beklenen taşıma rejimine göre dengeler.
    """
    ...
    if rel:
        w = rel["weights"]
        learned = sum(w.get(c,0.0) * t[c] for c in w)
        denom = max(1.0, sum(abs(x) for x in w.values()))
        t["Öğrenilmiş"] = learned / denom
        t["FinalPuan"] = 0.72*t["FinalHam"] + 0.28*pct_rank(t["Öğrenilmiş"])
    else:
        t["Öğrenilmiş"] = 0.0
        t["FinalPuan"] = t["FinalHam"]

    # Beklenen gerçek taşıma adedi final kuponuna oransal çevrilir.
    expected = float(regime_model["expected_carry"])
    carry_n = int(round(size * expected / 20.0))
    carry_n = max(1, min(size-1, carry_n))
    return_n = size - carry_n

    carry = t[t["Kaynakta"]].sort_values(["FinalPuan","Kanıt"], ascending=False)
    ret = t[~t["Kaynakta"]].sort_values(["FinalPuan","Kanıt"], ascending=False)

    selected = carry.head(carry_n)["Sayı"].astype(int).tolist()
    selected += ret.head(return_n)["Sayı"].astype(int).tolist()

    # En iyi genel adaylar, zayıf kota adayını geçiyorsa değiştirebilsin.
    chosen = set(selected)
    ranked = t.sort_values(["FinalPuan","Kanıt"], ascending=False)
    ...
    final = (
        t[t["Sayı"].isin(chosen)]
        .sort_values(["FinalPuan","Kanıt"], ascending=False)
    ...
    for n in final:
        ...
        if r["KarsiRank"] >= .75: tags.append("Karşı")
        if r["RejimUzlasi"] >= .70: tags.append("Rejim-Uzlaşı")
        reasons[n] = "/".join(tags[:4]) or "Birleşik"

    return final, reasons, t.sort_values(["FinalPuan","Kanıt"], ascending=False), rel
