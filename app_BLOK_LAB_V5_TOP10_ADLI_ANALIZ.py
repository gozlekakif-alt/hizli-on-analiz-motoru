...
# -------------------------------------------------------------------
st.title("🧪 Ardışık + Blok Motor Laboratuvarı V5 — Top10 Adli Analiz")
st.caption(
    "META kapalı. V4, DEVAM sayılarını korur; BÜYÜME+KAYMA kanal çakışmasının "
    "yanlış blok-kenarı adaylarını aşırı yükseltmesini frenler. Seçici kapı ayrıca test edilir."
)
...
def block_predict_v2(engine, t, k=7, neighbors=70):
    ...
    # V4 sayı-sıralama düzeltmesi:
    # BÜYÜME + KAYMA aynı blok-kenarı adayını birlikte şişirebiliyor.
    # Gerçek DEVAM adaylarını koru, saf kenar çakışmasını frenle.
    devam = channel["DEVAM"]
    buyume = channel["BÜYÜME"]
    kayma = channel["KAYMA"]
    dogum = channel["DOĞUM"]

    pure_edge_collision = (1.0 - devam) * buyume * kayma
    score = (
        score
        + 0.12 * devam
        + 0.03 * dogum
        + 0.03 * (devam * dogum)
        - 0.06 * pure_edge_collision
    )
    ...
    for i in range(80):
        rows.append({
            ...
            "Saf Kenar Çakışması": round(float(pure_edge_collision[i])*100,2),
...
BLOCK_REGIME_RULES = [
    {
        "name": "R1 — Kullanıcı güçlü DEVAM",
        "conds": [("Güven", ">=", 84.0), ("Ort benzerlik", ">=", 0.93), ("DEVAM olay", ">=", 0.90)],
        "preferred_k": 10,
    },
    {
        "name": "R2 — Yüksek güven + aktif blok",
        "conds": [("Güven", ">=", 83.8), ("Sinyal", ">=", 4.0)],
        "preferred_k": 10,
    },
    {
        "name": "R3 — Çok yüksek güven + güçlü sinyal",
        "conds": [("Güven", ">=", 85.7), ("Sinyal", ">=", 6.0)],
        "preferred_k": 9,
    },
    {
        "name": "R4 — Benzerlik yüksek + doğum sakin",
        "conds": [("Ort benzerlik", ">=", 0.904), ("DOĞUM olay", "<=", 0.975)],
        "preferred_k": 10,
    },
    {
        "name": "R5 — Büyüme baskısı düşük + sinyal kontrollü",
        "conds": [("BÜYÜME olay", "<=", 0.889), ("Sinyal", "<=", 9.0)],
        "preferred_k": 10,
    },
    {
        "name": "R6 — Kayma düşük + blok sinyali aktif",
        "conds": [("KAYMA olay", "<=", 0.78), ("Sinyal", ">=", 6.0)],
        "preferred_k": 9,
    },
]

def _rule_match_values(values, rule):
    for col, op, th in rule["conds"]:
        v = float(values[col])
        if op == ">=" and not (v >= th):
            return False
        if op == "<=" and not (v <= th):
...
def _rule_mask_df(hist, rule):
    m = np.ones(len(hist), dtype=bool)
    for col, op, th in rule["conds"]:
        if op == ">=":
            m &= (hist[col].astype(float).to_numpy() >= th)
        else:
            m &= (hist[col].astype(float).to_numpy() <= th)
    return m

def select_regime_from_history(hist_bt, current_values, min_events=12, min_uplift=0.12):
    """
    Hard-coded gate tek başına yetmez.
    Her kural, sadece hedef çekilişten ÖNCEKİ test geçmişinde yeterli destek ve
    rastgele üstü net avantaj gösteriyorsa aktif olabilir.
    """
    candidates = []
    if hist_bt is None or hist_bt.empty:
        return None, pd.DataFrame()

    for rule in BLOCK_REGIME_RULES:
        if not _rule_match_values(current_values, rule):
            continue

        mask = _rule_mask_df(hist_bt, rule)
        sub = hist_bt.loc[mask]
        k = int(rule["preferred_k"])
        n = len(sub)
        if n < min_events:
            continue

        avg = float(sub[f"Top{k}"].mean())
        rnd = k * 20 / 80
        uplift = avg - rnd

        # Son dönem kontrolü: kural yakın geçmişte tamamen bozulduysa fren.
        recent = sub.tail(min(20, n))
        recent_uplift = float(recent[f"Top{k}"].mean()) - rnd

        if uplift >= min_uplift and recent_uplift > -0.05:
            candidates.append({
                "Rejim": rule["name"],
                "Koltuk": k,
                "Geçmiş olay": n,
                ...
                "Net avantaj": round(uplift, 3),
                "Son dönem net": round(recent_uplift, 3),
                "Skor": uplift * np.sqrt(n) + 0.35 * max(recent_uplift, 0),
            })

    tab = pd.DataFrame(candidates)
    if tab.empty:
        return None, tab
    tab = tab.sort_values(["Skor", "Geçmiş olay"], ascending=False).reset_index(drop=True)
    return tab.iloc[0].to_dict(), tab

def current_block_values(info):
    return {
...
def backtest_v2(engine, tests=250, neighbors=70):
    ...
    for t in range(start, len(engine.A)):
        ...
        # V5 adli kayıt: ilk 10 ve hemen arkasındaki 11-15 adayları.
        top10_pred = pred10[:10]
        top15_order = [int(i+1) for i in np.argsort(-score, kind="mergesort")[:15]]
        rank11_15 = top15_order[10:15]
        rec["Top10 tahmin"]=" - ".join(map(str,top10_pred))
        rec["Top10 tutan"]=" - ".join(map(str,sorted(set(top10_pred)&actual))) or "Yok"
        rec["11-15 adaylar"]=" - ".join(map(str,rank11_15))
        rec["11-15 gerçek"]=" - ".join(map(str,sorted(set(rank11_15)&actual))) or "Yok"
        rec["11-15 gerçek adet"]=len(set(rank11_15)&actual)
        rows.append(rec)
    return pd.DataFrame(rows)
...
def selective_walkforward_v3(engine, tests=250, neighbors=70, min_events=12, min_uplift=0.12):
    """
    Önce V2 ayrıntı kayıtlarını sırayla üretir.
    Her hedefte rejim seçimi yalnız o hedeften önce oluşmuş laboratuvar kayıtlarını kullanır.
    """
    start = max(120, len(engine.A) - tests)
    history_rows = []
    out_rows = []
    ...
    for t in range(start, len(engine.A)):
        ...
        for k in range(3, 11):
            rec[f"Top{k}"] = len(set(pred10[:k]) & actual)

        hist_df = pd.DataFrame(history_rows)
        chosen, candidates = select_regime_from_history(
            hist_df, rec, min_events=min_events, min_uplift=min_uplift
        )
        ...
        if chosen is None:
            ...
            k = int(chosen["Koltuk"])
            hits = len(set(pred10[:k]) & actual)
            rec.update({
                "Aktif": 1,
                "Rejim": chosen["Rejim"],
                "Koltuk": k,
                "İsabet": hits,
                "Net": round(hits - k * 20 / 80, 3),
                "Tahmin": " - ".join(map(str, pred10[:k])),
            })

        out_rows.append(rec.copy())
        history_rows.append(rec.copy())
...
st.header("3️⃣ V3 Seçici Uzman — yalnız doğru rejimde konuş")
st.caption(
    "Rejim eşikleri tek başına yeterli değildir. Her kural geçmişte en az belirlenen sayıda "
    "olay görmüş ve rastgele üstü avantaj üretmişse aktif olabilir; aksi halde motor 0 koltuk ister."
)
min_events_v3 = st.slider("Bir rejim için minimum geçmiş olay", 8, 30, 12)
min_uplift_v3 = st.slider("Minimum geçmiş net avantaj", 0.00, 0.60, 0.12, 0.01)

if st.button("🎯 V3 seçici walk-forward testi", type="primary", key="v3_selective_run"):
    with st.spinner("Her çekilişte yalnız önceki rejim başarıları kullanılıyor..."):
        sbt = selective_walkforward_v3(
            eng, min(tests, len(df)-120), neighbors=neighbors,
            min_events=min_events_v3, min_uplift=min_uplift_v3
        )

    active = sbt[sbt["Aktif"] == 1]
    q1,q2,q3,q4 = st.columns(4)
    q1.metric("Aktif olduğu el", f"{len(active)}/{len(sbt)}")
    if len(active):
        total_slots = int(active["Koltuk"].sum())
        total_hits = int(active["İsabet"].sum())
        random_total = total_slots * 0.25
        q2.metric("Aktif toplam isabet", total_hits)
        q3.metric("Rastgele karşılığı", f"{random_total:.1f}")
        q4.metric("Net ek isabet", f"{total_hits-random_total:+.1f}")

        rs = active.groupby(["Rejim","Koltuk"], as_index=False).agg(
            Olay=("İsabet","size"),
            Ort_İsabet=("İsabet","mean"),
            Net_Ort=("Net","mean"),
            Maks=("İsabet","max"),
        ).sort_values("Net_Ort", ascending=False)
        st.subheader("Hangi rejim gerçekten para ediyor?")
        st.dataframe(rs, use_container_width=True, hide_index=True)
    else:
        q2.metric("Aktif toplam isabet", 0)
        q3.metric("Rastgele karşılığı", 0)
        q4.metric("Net ek isabet", 0)
        st.info("Seçilen minimum destek/avantaj kurallarıyla motor hiçbir elde konuşmadı.")

    st.subheader("V3 çekiliş çekiliş karar kaydı")
    st.dataframe(sbt.sort_values("Çekiliş", ascending=False), use_container_width=True, hide_index=True)
    st.download_button(
        "⬇️ V3 seçici test CSV indir",
        sbt.to_csv(index=False).encode("utf-8-sig"),
        file_name="blok_motor_lab_v3_secici_test.csv",
        mime="text/csv",
    )
