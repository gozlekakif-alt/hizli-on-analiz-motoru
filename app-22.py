...
st.title("🧬 Sayı Laboratuvarı V7.1 — Kalıcı Taşıma/Dönüş Uzman Motoru")
st.caption(
    "Taşıma ve dönüşü iki ayrı uzman motor öğrenir; doğum, devam, sönüş, dinlenme ve "
    "aynı-nota geçiş karakterini ayrı ayrı değerlendirip tek FINAL-10 üretir."
)
...
with tabs[0]:
    st.subheader("⚡ Sonucu yapıştır → KALICI KAYDET → sonraki V7 analizi")
    paste=st.text_area(
        "Gelen sonuç",
        height=300,
        placeholder=(
    ...
    if st.button("⚡ KALICI KAYDET + V7 ÜRET",type="primary",use_container_width=True):
        try:
            r=parse_block(paste)
            ...
            st.error(f"V7 işlem durduruldu: {e}")
    ...
    if pred:
        ...
        if ft is not None:
            ...
            for c in ["Uzman Olasılığı","Yaşam Olasılığı","Sönme Riski","FinalPuanV7"]:
            ...
            life_cols = [
                ...
                "Yaşam Olasılığı","Sönme Riski","FinalPuanV7"
            ]
