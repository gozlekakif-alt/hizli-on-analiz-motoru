...
st.set_page_config(
    page_title="Hızlı On Ultimate V22 — Öğrenilen Gerçekleşme Bandı",
    page_icon="🎯",
    layout="wide",
)
...
def persistent_save_panel(df_to_save, key_prefix):
    """V18.5.10 — çekiliş/veri havuzu kalıcı kaydı PIN istemeden tek dokunuşla yapılır."""
...
def transition_expert_bundle_v18510(df, target_time=None, size=7, lookback=500):
    """V18.5.10 — dört Geçiş profilini kendi özgün formüllerini bozmadan bağımsız uzman ailesi olarak üretir."""
    if df is None or len(df) < 40:
        return [], pd.DataFrame(), Counter()
    work = df.sort_values("Cekilis_No").reset_index(drop=True)
    latest_set = sorted(int(work.iloc[-1][c]) for c in NUM_COLS)
    if target_time is None:
        _, _, target_time = next_draw_defaults(work)
    _, transition_candidates, pair_counts = transition_statistics(
        work,
        latest_set,
        min(int(lookback), len(work)),
    )
    if transition_candidates is None or transition_candidates.empty:
        return [], pd.DataFrame(), pair_counts
    hybrid = hybrid_transition_table(work, transition_candidates, target_time)
    profiles = [
        "Dengeli",
        "Tekrar ağırlıklı",
        "Yerine geçme ağırlıklı",
    ...
    for i, profile in enumerate(profiles):
        coupon = build_profile_coupon(
            ...
            size,
            profile,
            excluded_coupons=used,
            diversity_seed=i,
        )
        if coupon:
            used.append(coupon)
            items.append({"Profil":profile,"Kupon":coupon})
    return items, hybrid, pair_counts


def transition_walkforward_v18510(df, test_count=12, analysis_window=500, size=7):
    """Geleceği görmeyen walk-forward: her hedefte dört Geçiş uzmanını ayrı ayrı ölçer."""
    if df is None or len(df) < 80:
        return pd.DataFrame(), pd.DataFrame()
    work=df.sort_values("Cekilis_No").reset_index(drop=True)
    test_count=int(min(max(1,test_count), max(1,len(work)-60)))
    start=len(work)-test_count
    rows=[]
    for i in range(start,len(work)):
        ...
        target=work.iloc[i]
        try:
            items, _, _ = transition_expert_bundle_v18510(
                train,
                target_time=str(target.Saat),
                size=size,
                lookback=min(int(analysis_window),len(train)),
            )
        except Exception:
            continue
        actual=set(int(target[c]) for c in NUM_COLS)
        for item in items:
            coupon=sorted(set(int(x) for x in item.get("Kupon",[])))
            hits=sorted(set(coupon)&actual)
            rows.append({
                "Çekiliş":int(target.Cekilis_No),
                "Tarih":str(target.Tarih),
                "Saat":str(target.Saat),
                "Faz":period_name(str(target.Saat)),
                "Profil":str(item.get("Profil","")),
    ...
    summary=(detail.groupby("Profil",as_index=False)
             .agg(Test=("İsabet","size"),Ortalama=("İsabet","mean"),En_Yüksek=("İsabet","max")))
    extra=[]
    for profile,g in detail.groupby("Profil"):
        extra.append({
            "Profil":profile,
            "3+":int((g["İsabet"]>=3).sum()),
            "4+":int((g["İsabet"]>=4).sum()),
            "Boş":int((g["İsabet"]==0).sum()),
        })
    summary=summary.merge(pd.DataFrame(extra),on="Profil",how="left")
    summary["Ortalama"]=summary["Ortalama"].round(3)
    # Bağlamsal meta ağırlık: ortalama + yüksek isabet sıklığı; aşırı oynamayı sınırlı tut.
    base=max(float(summary["Ortalama"].mean()),0.01)
    summary["Meta Ağırlık"]=((summary["Ortalama"]/base)*0.75 + (1.0+summary["3+"]/summary["Test"].clip(lower=1))*0.25).clip(0.70,1.35).round(3)
    return detail, summary.sort_values(["Ortalama","3+"],ascending=False)


def upper_meta_master_v18510(living_items, transition_items, size=7, transition_weights=None):
    """Living Engine ve 4 Geçiş uzmanını yalnız üst katta birleştirir; alt motorların formüllerine dokunmaz."""
    size=int(size)
    living_items=living_items or []
    transition_items=transition_items or []
    transition_weights=transition_weights or {}
    living_master=set()
    living_cons=set()
    living_regular=[]
    for it in living_items:
        name=str(it.get("Senaryo",""))
        cp=set(int(x) for x in it.get("Kupon",[]))
        if name.startswith("🏆 OYNA"):
            living_master=cp
        elif "Konsensüs" in name:
            living_cons=cp
        else:
            living_regular.append(cp)
    nums=range(1,81)
    rows=[]
    for n in nums:
        reg_support=sum(n in cp for cp in living_regular)
        reg_den=max(len(living_regular),1)
        trans_support=0.0
        trans_count=0
        trans_names=[]
        for it in transition_items:
            profile=str(it.get("Profil",""))
            if n in set(int(x) for x in it.get("Kupon",[])):
                w=float(transition_weights.get(profile,1.0))
                trans_support+=w
                trans_count+=1
                trans_names.append(profile)
        trans_den=sum(float(transition_weights.get(str(it.get("Profil","")),1.0)) for it in transition_items) or 1.0
        living_score=(0.50*(n in living_master)+0.22*(n in living_cons)+0.28*(reg_support/reg_den))
        transition_score=trans_support/trans_den
        cross=(living_score>0.22 and transition_score>0.0)
        score=55.0*living_score + 40.0*transition_score + (8.0 if cross else 0.0)
        # Tek aile baskınsa küçük fren; iki aile bağımsız onayında bonus korunur.
        if transition_score>0.70 and living_score<0.10:
            score-=5.0
        if living_score>0.75 and transition_score==0:
            score-=3.0
        rows.append({
            "Sayı":n,"Üst Puan":score,"Living":living_score,"Geçiş":transition_score,
            "Living Uzman Desteği":reg_support,"Geçiş Profil Desteği":trans_count,
            "Çapraz Aile":bool(cross),"Geçiş Profilleri":", ".join(trans_names),
        })
    table=pd.DataFrame(rows).sort_values(["Üst Puan","Çapraz Aile","Sayı"],ascending=[False,False,True])
    selected=[]
    band_counts=Counter()
    def band(n): return ten_band_name(int(n))
    def runlen(vals):
        a=sorted(set(vals)); best=cur=1 if a else 0
        for x,y in zip(a,a[1:]):
            cur=cur+1 if y==x+1 else 1; best=max(best,cur)
        return best
    for _,r in table.iterrows():
        n=int(r["Sayı"])
        if band_counts[band(n)]>=3:
            continue
        if runlen(selected+[n])>3:
            continue
        selected.append(n); band_counts[band(n)]+=1
        if len(selected)>=size:
            break
    if len(selected)<size:
        for n in table["Sayı"].astype(int):
            if n not in selected:
                selected.append(n)
                if len(selected)>=size: break
    chosen=table[table["Sayı"].isin(selected)].copy().sort_values("Üst Puan",ascending=False)
    return sorted(selected[:size]), chosen


def _clock_minutes_v18511(value):
    try:
...
def transition_profile_rank_table_v18511(hybrid_candidates, profile):
    """Bir Geçiş uzmanının 80 aday içindeki kendi ham puan/sıra tablosunu üretir."""
    if hybrid_candidates is None or hybrid_candidates.empty:
        return pd.DataFrame(columns=["Sayı", "Profil", "Uzman Puanı", "Uzman Puanı 0-100", "Uzman Sırası"])
    work = hybrid_candidates.copy()
    work["Uzman Puanı"] = work.apply(lambda row: transition_profile_score(row, profile), axis=1)
    raw = pd.to_numeric(work["Uzman Puanı"], errors="coerce").fillna(0.0)
    lo, hi = float(raw.min()), float(raw.max())
    if hi > lo:
        work["Uzman Puanı 0-100"] = ((raw - lo) / (hi - lo) * 100.0).round(2)
    else:
        work["Uzman Puanı 0-100"] = 50.0
    work = work.sort_values(["Uzman Puanı", "Sayı"], ascending=[False, True]).reset_index(drop=True)
    work["Uzman Sırası"] = np.arange(1, len(work) + 1)
    work["Profil"] = str(profile)
    return work[["Sayı", "Profil", "Uzman Puanı", "Uzman Puanı 0-100", "Uzman Sırası"]]


def enrich_transition_items_v18511(transition_items, hybrid_candidates):
    """Dört Geçiş kuponuna, kendi uzmanının iç puanını ve sırasını ekler."""
    out = []
    for item in transition_items or []:
        profile = str(item.get("Profil", ""))
        coupon = sorted(set(int(x) for x in item.get("Kupon", [])))
        rank_table = transition_profile_rank_table_v18511(hybrid_candidates, profile)
        indexed = rank_table.set_index("Sayı") if not rank_table.empty else pd.DataFrame()
        score_map, rank_map = {}, {}
        for n in coupon:
            if isinstance(indexed, pd.DataFrame) and not indexed.empty and n in indexed.index:
                score_map[n] = float(indexed.loc[n, "Uzman Puanı 0-100"])
                rank_map[n] = int(indexed.loc[n, "Uzman Sırası"])
            else:
                score_map[n] = 0.0
                rank_map[n] = 99
        new_item = dict(item)
        new_item["Uzman Puanları 511"] = score_map
        new_item["Uzman Sıraları 511"] = rank_map
        new_item["Uzman Puan Tablosu 511"] = rank_table
        out.append(new_item)
    return out


def transition_three_memory_weights_v18511(detail, target_time=None):
    """V18.5.11 üç hafıza: kısa dönem + aynı faz/saat + uzun dönem uzman güveni.

    Kaynak kodu değiştirmez; her yeni çekilişte geçmiş walk-forward sonuçlarından
    ağırlıkları yeniden öğrenir. Böylece uzman formülleri özgün kalır.
    """
    if detail is None or detail.empty:
        return {}, pd.DataFrame()
    work = detail.copy()
    work["İsabet"] = pd.to_numeric(work["İsabet"], errors="coerce").fillna(0.0)
    target_phase = period_name(str(target_time or "00:00"))
    target_min = _clock_minutes_v18511(target_time)
    profiles = sorted(work["Profil"].dropna().astype(str).unique())
    rows = []
    for profile in profiles:
        g = work[work["Profil"].astype(str) == profile].copy()
        g = g.sort_values("Çekiliş")
        long_avg = float(g["İsabet"].mean()) if len(g) else 0.0
        short_g = g.tail(min(12, len(g)))
        short_avg = float(short_g["İsabet"].mean()) if len(short_g) else long_avg
        phase_g = g[g["Faz"].astype(str) == str(target_phase)] if "Faz" in g.columns else g.iloc[0:0]
        phase_avg = float(phase_g["İsabet"].mean()) if len(phase_g) >= 3 else long_avg
        near_g = g.iloc[0:0]
        if target_min is not None and "Saat" in g.columns:
            mins = g["Saat"].map(_clock_minutes_v18511)
            dist = mins.map(lambda m: min(abs(m-target_min), 1440-abs(m-target_min)) if m is not None else 9999)
            near_g = g[dist <= 60]
        near_avg = float(near_g["İsabet"].mean()) if len(near_g) >= 3 else phase_avg
        # 7 sayılık kupon için rastgele beklenti 1.75 civarıdır; güveni bunun çevresinde ölçekle.
        blended = 0.38 * short_avg + 0.27 * near_avg + 0.20 * phase_avg + 0.15 * long_avg
        weight = float(np.clip(0.72 + (blended / 1.75) * 0.28, 0.72, 1.42))
        stability = float(np.clip(1.0 - (g["İsabet"].std(ddof=0) if len(g) > 1 else 0.0) / 4.0, 0.55, 1.0))
        weight = float(np.clip(weight * (0.88 + 0.12 * stability), 0.70, 1.42))
        rows.append({
            "Profil": profile,
            "Kısa Hafıza": round(short_avg, 3),
            "Saat/Faz Hafıza": round(near_avg, 3),
            "Faz Hafıza": round(phase_avg, 3),
            "Uzun Hafıza": round(long_avg, 3),
            "Kararlılık": round(stability, 3),
            "V18.5.11 Güven": round(weight, 3),
        })
    table = pd.DataFrame(rows).sort_values(["V18.5.11 Güven", "Profil"], ascending=[False, True])
    return dict(zip(table["Profil"], table["V18.5.11 Güven"])), table


def upper_meta_master_v18511(living_items, transition_items, size=7, transition_weights=None, reserve_size=5):
    """V18.5.11 Havuzdan 7'ye Akıllı Sıkıştırma Motoru.

    Living ve dört Geçiş uzmanının özgün formüllerine dokunmaz. Üst katmanda:
    uzman içi sıra/puan, üç-hafızalı bağlamsal güven, çapraz aile onayı,
    sürü freni, korumalı Living çekirdeği, bağımsız uzman şampiyonu ve rezerv
    sıralamasıyla 7 koltuk kurar.
    """
    size = int(size)
    reserve_size = int(max(0, reserve_size))
    living_items = living_items or []
    transition_items = transition_items or []
    transition_weights = transition_weights or {}

    living_master, living_cons, living_regular = set(), set(), []
    for it in living_items:
        name = str(it.get("Senaryo", ""))
        cp = set(int(x) for x in it.get("Kupon", []))
        if name.startswith("🏆 OYNA"):
            ...
            living_cons = cp
        elif not name.startswith("👑 ÜST MASTER"):
            living_regular.append(cp)
    ...
    for n in range(1, 81):
        reg_support = sum(n in cp for cp in living_regular)
        reg_den = max(len(living_regular), 1)
        living_score = 0.48 * float(n in living_master) + 0.22 * float(n in living_cons) + 0.30 * (reg_support / reg_den)

        trans_support = 0.0
        trans_count = 0
        trans_names, rank_qualities, expert_scores = [], [], []
        for it in transition_items:
            profile = str(it.get("Profil", ""))
            coupon = set(int(x) for x in it.get("Kupon", []))
            if n not in coupon:
                continue
            w = float(transition_weights.get(profile, 1.0))
            rank = int(it.get("Uzman Sıraları 511", {}).get(n, 99))
            expert_score = float(it.get("Uzman Puanları 511", {}).get(n, 0.0)) / 100.0
            # Kupon içindeki 1-7 sıra niteliği; rank tablosu 80 adaylık olsa da seçilen sayı için
            # uzman ham puanı ayrıca kullanılır.
            rank_quality = max(0.0, 1.0 - (max(rank, 1) - 1) / 20.0)
            quality = 0.58 * expert_score + 0.42 * rank_quality
            trans_support += w * (0.55 + 0.45 * quality)
            trans_count += 1
            trans_names.append(profile)
            rank_qualities.append(rank_quality)
            expert_scores.append(expert_score)
        transition_score = trans_support / trans_den
        avg_rank_quality = float(np.mean(rank_qualities)) if rank_qualities else 0.0
        avg_expert_score = float(np.mean(expert_scores)) if expert_scores else 0.0
        cross = living_score >= 0.18 and transition_score > 0.0

        score = 52.0 * living_score + 38.0 * transition_score
        score += 10.0 if cross else 0.0
        score += 6.0 * avg_expert_score + 4.0 * avg_rank_quality

        # Sürü freni: çok profil söylüyor ama kendi uzman puanı düşükse ortaklaşma tek başına ödül değildir.
        herd_penalty = 0.0
        if trans_count >= 3 and avg_expert_score < 0.55:
            herd_penalty = 5.5
            score -= herd_penalty
        if transition_score > 0.72 and living_score < 0.08:
            score -= 3.5
        # Living'in gerçekten güçlü çekirdeğini Geçiş ailesi topluca sökmesin.
        living_protect = (n in living_master) and (n in living_cons or reg_support >= max(2, reg_den // 2))
        if living_protect:
            score += 5.0

        rows.append({
            "Sayı": n,
            "V18.5.11 Üst Puan": round(score, 3),
            "Living Gücü": round(living_score, 3),
            "Geçiş Gücü": round(transition_score, 3),
            "Living Uzman Desteği": int(reg_support),
            "Geçiş Profil Desteği": int(trans_count),
            "Uzman İçi Puan": round(avg_expert_score * 100.0, 2),
            "Uzman Sıra Kalitesi": round(avg_rank_quality * 100.0, 2),
            "Çapraz Aile": bool(cross),
            "Living Korumalı": bool(living_protect),
            "Sürü Cezası": round(herd_penalty, 2),
            "Geçiş Profilleri": ", ".join(trans_names),
        })

    table = pd.DataFrame(rows).sort_values(["V18.5.11 Üst Puan", "Çapraz Aile", "Sayı"], ascending=[False, False, True]).reset_index(drop=True)
    selected, reasons = [], {}
    band_counts = Counter()
    def _band(n): return ten_band_name(int(n))
    def _can_take(n):
        if n in selected:
            return False
        if band_counts[_band(n)] >= 3:
            return False
        trial = sorted(set(selected + [n]))
        run = 1
        best = 1
    ...
    def _take(n, why):
        if _can_take(int(n)):
            selected.append(int(n)); band_counts[_band(n)] += 1; reasons[int(n)] = why; return True
        return False

    # Koltuk 1-2: Living çekirdeği korunur.
    living_candidates = table[table["Sayı"].isin(living_master)].sort_values(["Living Korumalı", "V18.5.11 Üst Puan"], ascending=[False, False])
    for n in living_candidates["Sayı"].astype(int):
        if len(selected) >= min(2, size): break
        _take(n, "Living korumalı çekirdek")

    # Koltuk 3-4: Geçiş ailesinin güçlü ve kendi içinde yüksek puanlı adayları.
    trans_candidates = table[table["Geçiş Profil Desteği"] > 0].sort_values(["Uzman İçi Puan", "Geçiş Gücü", "V18.5.11 Üst Puan"], ascending=False)
    target_trans = min(4, size)
    for n in trans_candidates["Sayı"].astype(int):
        if len(selected) >= target_trans: break
        _take(n, "Geçiş uzmanı yüksek iç puan")

    # Sonraki koltuk: çapraz aile köprüsü.
    cross_candidates = table[table["Çapraz Aile"]].sort_values("V18.5.11 Üst Puan", ascending=False)
    if len(selected) < size:
        for n in cross_candidates["Sayı"].astype(int):
            if _take(n, "Living + Geçiş çapraz aile köprüsü"): break

    # Bağımsız uzman şampiyonu: en yüksek güvenli profilin kuponundaki en yüksek iç puan.
    if len(selected) < size and transition_items:
        best_profile = None
        if transition_weights:
            best_profile = max(transition_weights, key=lambda k: transition_weights[k])
        champion_rows = []
        for it in transition_items:
            if best_profile is not None and str(it.get("Profil", "")) != str(best_profile):
                continue
            for n in it.get("Kupon", []):
                champion_rows.append((float(it.get("Uzman Puanları 511", {}).get(int(n), 0.0)), int(n)))
        for _, n in sorted(champion_rows, reverse=True):
            if _take(n, "Bağlamda güçlü uzman şampiyonu"): break

    # Kalan koltuklar: global üst puan; bu koltuk aynı zamanda keşif/sigorta rolü görür.
    for n in table["Sayı"].astype(int):
        if len(selected) >= size: break
        _take(n, "Global meta / keşif-sigorta")
    if len(selected) < size:
        for n in table["Sayı"].astype(int):
            if n not in selected:
                selected.append(n); reasons[n] = "Kısıt sonrası tamamlayıcı"
                if len(selected) >= size: break

    table["Seçim Nedeni 511"] = table["Sayı"].map(reasons).fillna("")
    table["MASTER 511"] = table["Sayı"].isin(selected)
    chosen = table[table["MASTER 511"]].copy().sort_values("V18.5.11 Üst Puan", ascending=False)
    reserve = table[~table["MASTER 511"]].head(reserve_size).copy()
    if not reserve.empty:
        reserve["Rezerv Sırası"] = np.arange(1, len(reserve) + 1)
    return sorted(selected[:size]), chosen, reserve, table


def seven_seat_selector_v18512(items, memory_df=None, size=7):
    """V18.5.12 — mevcut uzman havuzunu geçmiş performans + fikir ayrılığıyla 7 koltuğa sıkıştırır.

    Hedef çekilişin sonucunu kullanmaz. Senaryo güveni yalnız daha önce sonuçlanmış
    motor hafızasından öğrenilir; hafıza yoksa nötr ağırlık kullanılır.
    """
    items = [it for it in (items or []) if it.get("Kupon")]
    if not items:
        return [], pd.DataFrame(), pd.DataFrame()

    # Geçmiş senaryo formu: 7 sayılık kolonda rastgele beklenti ~1.75 isabet.
    scenario_weight = {}
    if isinstance(memory_df, pd.DataFrame) and not memory_df.empty and {"Senaryo", "Isabet"}.issubset(memory_df.columns):
        mem = memory_df.copy()
        mem["Isabet"] = pd.to_numeric(mem["Isabet"], errors="coerce")
        mem = mem.dropna(subset=["Isabet"])
        for name, g in mem.groupby(mem["Senaryo"].astype(str)):
            g = g.tail(40)
            long_avg = float(g["Isabet"].mean()) if len(g) else 1.75
            short_avg = float(g.tail(min(10, len(g)))["Isabet"].mean()) if len(g) else long_avg
            blended = 0.62 * short_avg + 0.38 * long_avg
            scenario_weight[name] = float(np.clip(0.72 + 0.28 * (blended / 1.75), 0.68, 1.38))

    rows = []
    family_count = len(items)
    for n in range(1, 81):
        supporters, weighted_support = [], 0.0
        master_support = 0.0
        for it in items:
            name = str(it.get("Senaryo", ""))
            cp = set(int(x) for x in it.get("Kupon", []))
            if n not in cp:
                continue
            w = float(scenario_weight.get(name, 1.0))
            supporters.append(name)
            weighted_support += w
            if name.startswith("🏆 OYNA") or name.startswith("👑 ÜST MASTER"):
                master_support += 1.0

        support_count = len(supporters)
        if support_count == 0:
            continue
        consensus = weighted_support / max(family_count, 1)
        # Azınlık sinyali: tek/iki uzman tarafından seçilen aday tamamen ezilmesin;
        # fakat konsensüsün önüne de geçmesin.
        minority = 1.0 if support_count == 1 else (0.55 if support_count == 2 else 0.0)
        cross_master = min(master_support, 2.0) / 2.0
        score = 72.0 * consensus + 18.0 * cross_master + 10.0 * minority
        rows.append({
            "Sayı": n,
            "V18.5.12 Koltuk Puanı": round(score, 3),
            "Uzman Desteği": support_count,
            "Ağırlıklı Destek": round(weighted_support, 3),
            "MASTER Desteği": int(master_support),
            "Azınlık Sinyali": round(minority, 2),
            "Destekleyenler": " | ".join(supporters),
        })

    table = pd.DataFrame(rows).sort_values(
        ["V18.5.12 Koltuk Puanı", "MASTER Desteği", "Uzman Desteği", "Sayı"],
        ascending=[False, False, False, True],
    ).reset_index(drop=True)
    selected, band_counts = [], Counter()
    reasons = {}
    for _, row in table.iterrows():
        n = int(row["Sayı"])
        band = ten_band_name(n)
        if band_counts[band] >= 3:
            continue
        trial = sorted(selected + [n])
        run = best = 1
        for a, b in zip(trial, trial[1:]):
            run = run + 1 if b == a + 1 else 1
            best = max(best, run)
        ...
        selected.append(n)
        band_counts[band] += 1
        if int(row["MASTER Desteği"]) > 0:
            reasons[n] = "MASTER + uzman havuzu"
        elif int(row["Uzman Desteği"]) >= 3:
            reasons[n] = "çoklu uzman konsensüsü"
        elif float(row["Azınlık Sinyali"]) > 0:
            reasons[n] = "azınlık/keşif sinyali"
        else:
            reasons[n] = "global koltuk puanı"
    ...
    table["Seçim Nedeni 512"] = table["Sayı"].map(reasons).fillna("")
    table["MASTER 512"] = table["Sayı"].isin(selected)
    chosen = table[table["MASTER 512"]].copy().sort_values("V18.5.12 Koltuk Puanı", ascending=False)
    return sorted(selected[:int(size)]), chosen, table



def seven_seat_selector_v18513(items, memory_df=None, size=7):
    """V18.5.13 — aile-tekilleştirmeli 7 koltuk seçici.

    Aynı kökten gelen kuponları bağımsız oy gibi saymaz; MASTER çekirdeğini korur,
    geçmişte sonuçlanmış senaryo formunu kullanır ve bir koltuğu kontrollü azınlık
    sinyaline ayırır. Hedef çekiliş sonucu bu seçimde kullanılmaz.
    """
    items = [it for it in (items or []) if it.get("Kupon") and not str(it.get("Senaryo", "")).startswith("🧠 V18.5.12")]
    if not items:
        return [], pd.DataFrame(), pd.DataFrame()

    def family(name):
        name = str(name)
        if name.startswith("🏆 OYNA"): return "MASTER"
        if name.startswith("👑 ÜST MASTER"): return "UPPER_MASTER"
        if "Geçiş" in name: return "TRANSITION"
        return "LIVING"
    ...
    if isinstance(memory_df, pd.DataFrame) and not memory_df.empty and {"Senaryo", "Isabet"}.issubset(memory_df.columns):
        ...
        for name, g in mem.groupby(mem["Senaryo"].astype(str)):
            g = g.tail(60)
            if len(g):
                long_avg = float(g["Isabet"].mean())
                short_avg = float(g.tail(min(12, len(g)))["Isabet"].mean())
                blended = 0.65 * short_avg + 0.35 * long_avg
                # Güveni küçült: az örnekli senaryolar nötre yaklaşır.
                reliability = min(1.0, len(g) / 24.0)
                raw = np.clip(0.72 + 0.28 * (blended / 1.75), 0.70, 1.35)
                scenario_weight[name] = float(1.0 + reliability * (raw - 1.0))

    families = sorted(set(family(it.get("Senaryo", "")) for it in items))
    rows = []
    for n in range(1, 81):
        fam_votes = {}
        supporters = []
        ...
        for it in items:
            name = str(it.get("Senaryo", ""))
            cp = set(int(x) for x in it.get("Kupon", []))
            if n not in cp: continue
            supporters.append(name)
            fam = family(name)
            w = float(scenario_weight.get(name, 1.0))
            # Aynı aile içindeki tekrar oyları toplanmaz; en güçlü kanıt alınır.
            fam_votes[fam] = max(float(fam_votes.get(fam, 0.0)), w)
            if fam in {"MASTER", "UPPER_MASTER"}: master_support += 1
        if not supporters: continue
        independent = len(fam_votes)
        family_strength = sum(fam_votes.values()) / max(len(families), 1)
        cross = min(independent / 3.0, 1.0)
        master = min(master_support / 2.0, 1.0)
        minority = 1.0 if independent == 1 and master_support == 0 else (0.45 if independent == 2 and master_support == 0 else 0.0)
        score = 58.0 * family_strength + 22.0 * cross + 15.0 * master + 5.0 * minority
        rows.append({
            "Sayı": n, "V18.5.13 Koltuk Puanı": round(score, 3),
            "Bağımsız Aile Desteği": independent, "Aile Gücü": round(family_strength, 3),
            "MASTER Desteği": master_support, "Azınlık Sinyali": round(minority, 2),
            "Aileler": " | ".join(sorted(fam_votes)), "Destekleyenler": " | ".join(supporters),
        })
    table = pd.DataFrame(rows).sort_values(["V18.5.13 Koltuk Puanı", "MASTER Desteği", "Bağımsız Aile Desteği", "Sayı"], ascending=[False, False, False, True]).reset_index(drop=True)
    selected, reasons, band_counts = [], {}, Counter()
    def can_take(n):
        if n in selected or band_counts[ten_band_name(n)] >= 3: return False
        trial = sorted(selected + [n]); run = best = 1
        for a,b in zip(trial, trial[1:]):
    ...
    def take(n, why):
        n=int(n)
        if can_take(n):
            selected.append(n); band_counts[ten_band_name(n)] += 1; reasons[n]=why; return True
        return False

    # 1-2: MASTER koruması.
    for n in table[table["MASTER Desteği"] > 0]["Sayı"].astype(int):
        if len(selected) >= min(2, int(size)): break
        take(n, "MASTER korumalı çekirdek")
    # 3-6: bağımsız aile kanıtı yüksek adaylar.
    for n in table[table["Bağımsız Aile Desteği"] >= 2]["Sayı"].astype(int):
        if len(selected) >= max(0, int(size)-1): break
        take(n, "bağımsız aile konsensüsü")
    # Son koltuk: kontrollü azınlık/keşif.
    if len(selected) < int(size):
        minority_rows = table[(table["Azınlık Sinyali"] > 0) & (~table["Sayı"].isin(selected))]
        for n in minority_rows["Sayı"].astype(int):
            if take(n, "kontrollü azınlık / keşif"): break
    for n in table["Sayı"].astype(int):
        if len(selected) >= int(size): break
        take(n, "global aile-tekilleştirilmiş puan")
    if len(selected) < int(size):
        for n in table["Sayı"].astype(int):
            if n not in selected:
                selected.append(n); reasons[n]="kısıt sonrası tamamlayıcı"
                if len(selected) >= int(size): break
    table["Seçim Nedeni 513"] = table["Sayı"].map(reasons).fillna("")
    table["MASTER 513"] = table["Sayı"].isin(selected)
    chosen = table[table["MASTER 513"]].copy().sort_values("V18.5.13 Koltuk Puanı", ascending=False)
    return sorted(selected[:int(size)]), chosen, table

def pool_coverage_v18512(items, actual_numbers):
    """Sonuç geldikten sonra uzman havuzunda kaç farklı doğru aday bulunduğunu ölçer."""
    pool = set()
    for it in items or []:
        pool.update(int(x) for x in it.get("Kupon", []) if 1 <= int(x) <= 80)
    actual = set(int(x) for x in actual_numbers)
    hits = sorted(pool & actual)
    return {"Havuz Boyutu": len(pool), "Havuz Doğru": len(hits), "Havuzda Tutanlar": " - ".join(map(str, hits))}
...
def live_backtest(df, coupon_size=7, test_count=20, analysis_window=300):
    """
    V18.5.10 faz-kilitli walk-forward test.
    Her hedef çekilişte yalnız geçmiş veri kullanılır; hedef SAAT doğrudan test edilen
    çekilişten alınır. Böylece eski sürümdeki "bir önceki çekiliş saatini hedef saat
    sanma" hatası giderilir. Canlı motorun saat/faz/karakter bağlamı geçmişte hangi
    koşulda çalışıyorsa testte de aynı şekilde yeniden kurulmaya çalışılır.
    """
    if len(df) < 80:
        return pd.DataFrame()

    work = df.sort_values("Cekilis_No").reset_index(drop=True)
    test_count = int(min(test_count, max(1, len(work) - 60)))
    start = len(work) - test_count
    rows = []
    ...
    for i in range(start, len(work)):
        ...
        target_time = str(work.iloc[i].Saat)
        target_phase = period_name(target_time)

        try:
            # Test edilen çekilişin gerçek hedef saatini kullan.
            ...
            coupon = coupons[0]
            actual = set(int(work.iloc[i][c]) for c in NUM_COLS)
            hits = sorted(set(coupon) & actual)

            # V18.5.10: aynı saat/faz ve geçmiş karakter bağlamının kaç örneğe
            # dayandığını ayrıca raporla. Bu sadece geçmiş veriden hesaplanır.
            _, ctx = expert_context_calibration_v1857(
                train,
                target_time=target_time,
                character_window=6,
            )
            ...
            rows.append(
                {
                    "Çekiliş": int(work.iloc[i].Cekilis_No),
                    "Hedef Saat": target_time,
                    "Hedef Faz": target_phase,
                    "Canlı Faz": state.get("Faz", ""),
                    "Benzer Bağlam": int(ctx.get("Benzer", 0)),
                    "Karakter Benzerliği %": round(float(ctx.get("Karakter Benzerliği", 0.0)) * 100.0, 1),
                    "Kupon": " - ".join(map(str, coupon)),
                    "İsabet": len(hits),
                    "Tutan": " - ".join(map(str, hits)),
                }
...
st.title("🎯 Hızlı On Ultimate AI Studio V18.5.12")
...
def expert_context_calibration_v1857(df, target_time=None, character_window=12):
    """V18.5.10: hedef saat + gün fazı + GERÇEK yakın karaktere göre uzman katsayıları.

    Geçmiş bir olay yalnız saat/faz yakın olduğu için seçilmez. O olaydan ÖNCEKİ
    davranış penceresi de bugünkü son davranış penceresine benzemelidir. Böylece
    aynı akşam saatindeki ama tamamen farklı taşıma/blok/bant karakterindeki olaylar
    uzman katsayısını gereksiz yere sürüklemez.
    """
    out = {"Taşıma":1.0,"Dinlenmiş":1.0,"Blok":1.0,"Bölge":1.0,"Eşlikçi":1.0,
           "Senaryo":1.0,"KarşıRejim":1.0}
    if df is None or len(df) < 30:
        return out, {"Faz":"Yetersiz veri","Benzer":0,"Saat":str(target_time or ""),
                     "Karakter Benzerliği":0.0,"Güven":0.0}
    ...
    sets=row_sets(work)
    cw=max(4,int(character_window or 6))

    features=behavior_feature_table(work)
    char_cols=[
        "Taşıma","Yeni","2liBlokYoğunluğu","3lüBlokYoğunluğu","MaksBlok",
        "Bant_1-20","Bant_21-40","Bant_41-60","Bant_61-80",
    ]
    if features.empty or len(features) < cw+4:
        return out,{"Faz":phase,"Benzer":0,"Saat":str(target_time),
                   "Karakter Benzerliği":0.0,"Güven":0.0}

    means=features[char_cols].mean()
    stds=features[char_cols].std(ddof=0).replace(0,1.0)
    current_slice=features.tail(cw)
    current_vec=((current_slice[char_cols].mean()-means)/stds).fillna(0.0).to_numpy(float)

    events=[]
    # i = geçmişte tahmin edilmek istenen hedef çekiliş. Karakter penceresi yalnız i öncesinden gelir.
    for i in range(max(4,cw+1),len(work)):
        ...
        # Faz kilidi + hedef saate makul yakınlık. Aynı geniş fazdaki 3-4 saat uzak olaylar elenir.
        if not same_phase or clock_dist>90:
            continue

        hist_slice=features[features["Index"].astype(int) < i].tail(cw)
        if len(hist_slice) < cw:
            continue
        hist_vec=((hist_slice[char_cols].mean()-means)/stds).fillna(0.0).to_numpy(float)
        distance=float(np.sqrt(np.mean((hist_vec-current_vec)**2)))
        char_sim=1.0/(1.0+distance)
        # Gerçek karakter filtresi. Çok yakın saatlerde biraz tolerans bırakılır.
        if char_sim < (0.34 if clock_dist<=15 else 0.42):
            continue
        ...
        pair_follow=0.0
        if prev:
            near_prev=set(n for n in range(1,81) if any(abs(n-x)<=2 for x in prev))
            pair_follow=len(cur & near_prev)/20.0

        age=max(0,(len(work)-1)-i)
        recency=np.exp(-age/max(18.0,float(cw)*8.0))
        clock_weight=1.0/(1.0+clock_dist/25.0)
        weight=clock_weight*(char_sim**1.65)*(0.40+0.60*recency)
        events.append((weight,carry,block_strength,region,rest,pair_follow,char_sim,clock_dist))

    # Karakter filtresi çok sert kaldıysa yalnız aynı faz/±60 dakika içinden en yakın karakterleri geri al.
    if len(events) < 6:
        fallback=[]
        for i in range(max(4,cw+1),len(work)):
            t=str(work.iloc[i]["Saat"])
            ...
            blocks=consecutive_blocks(cur)
            block_strength=min(1.0,(sum(max(0,len(b)-1) for b in blocks))/5.0)
            prev_bands=Counter(ten_band_name(n) for n in prev); cur_bands=Counter(ten_band_name(n) for n in cur)
            dominant=prev_bands.most_common(1)[0][0] if prev_bands else None
            region=(cur_bands.get(dominant,0)/20.0) if dominant else 0.0
            rested=((cur-prev)&(sets[i-2]|sets[i-3])); rest=len(rested)/20.0
            near_prev=set(n for n in range(1,81) if any(abs(n-x)<=2 for x in prev)) if prev else set()
            pair_follow=len(cur & near_prev)/20.0 if prev else 0.0
            age=max(0,(len(work)-1)-i); recency=np.exp(-age/max(18.0,float(cw)*8.0))
            weight=(1.0/(1.0+clock_dist/25.0))*(char_sim**1.5)*(0.35+0.65*recency)*0.72
            fallback.append((char_sim,weight,carry,block_strength,region,rest,pair_follow,clock_dist))
        fallback=sorted(fallback,key=lambda x:x[0],reverse=True)[:max(0,10-len(events))]
        for cs,w,ca,bs,rg,rs,pf,cd in fallback:
            events.append((w,ca,bs,rg,rs,pf,cs,cd))
    ...
    # Bağlama göre yumuşak ve sınırlı uzman katsayıları; tek örnek aşırı öğrenemez.
    out["Taşıma"]=float(np.clip(0.82+1.05*wav(1),0.78,1.30))
    out["Blok"]=float(np.clip(0.84+0.78*wav(2),0.78,1.30))
    out["Bölge"]=float(np.clip(0.86+1.10*wav(3),0.78,1.26))
    out["Dinlenmiş"]=float(np.clip(0.84+1.10*wav(4),0.78,1.28))
    out["Eşlikçi"]=float(np.clip(0.84+0.72*wav(5),0.80,1.24))

    char_avg=float(wav(6))
    effective=sum(e[0] for e in events)
    confidence=float(np.clip((len(events)/18.0)*0.55 + char_avg*0.45,0.0,1.0))
    # Senaryo/karşı-rejim de artık bağlam güveninden etkilenir.
    out["Senaryo"]=float(np.clip(0.88+0.24*confidence,0.86,1.12))
    out["KarşıRejim"]=float(np.clip(0.92+0.16*(1.0-confidence),0.90,1.08))

    return out,{"Faz":phase,"Benzer":len(events),"Saat":str(target_time),
               "Karakter Benzerliği":char_avg,"Güven":confidence,
               "Etkin Ağırlık":float(effective)}
...
def seven_seat_master_v18567(df, master, items, latest_set, skeleton, regime_summary,
    ...
    # V18.5.10 — Korumalı Uzman Puanı. Konsensüsün güçlü ve gerçekten bağımsız
    # sinyalini, uzman sürüsünün kopyası olmadığı sürece MASTER'ın kolayca atmasını engeller.
    _ctx_conf=float(context_info.get("Güven",0.0) or 0.0)
    _char_sim=float(context_info.get("Karakter Benzerliği",0.0) or 0.0)
    votes["Uzman Koruma Puanı 59"] = (
        0.30*nrm("Oy_UzmanAğırlık 58")
        +0.20*_fam
        +0.18*_ind
        +0.14*nrm("Oy_SenaryoÇeşit")
        +0.10*nrm("Oy_Temel")
        +0.08*nrm("Oy_Eşlikçi")
        -0.16*_corr
    )*100.0
    votes["Uzman Koruma Puanı 59"] *= (0.88 + 0.16*_ctx_conf + 0.08*_char_sim)
    ...
    # V18.5.10 Seat 1-2/3: bağlam güvenine göre KORUMALI UZMAN koltukları.
    # Faz/karakter bağlamı yeterince güvenliyse üç, değilse iki gerçek bağımsız uzman
    # sinyalini korur. Bu, Konsensüs 4/7 iken MASTER'ın 2/7'ye düşmesi gibi aktarım
    # kayıplarını azaltmayı hedefler; ham çoğunluk tek başına yeterli değildir.
    protected_target = 3 if (_ctx_conf>=0.58 and int(context_info.get("Benzer",0))>=10) else 2
    protected = votes.sort_values(
        ["Uzman Koruma Puanı 59","Master567","Bağımsız Motor 567"],
        ascending=[False,False,False]
    )
    for _,r in protected.iterrows():
        if len(selected)>=protected_target: break
        if int(r.get("Uzman Aile Desteği 58",0)) < 2: continue
        if float(r["Bağımsız Motor 567"]) < 2.5: continue
        if float(r["Uzman Koruma Puanı 59"]) < 43: continue
        n=int(r["Sayı"])
        if can_add(n):
            selected.append(n); seat_reason[n]="Korumalı Uzman"
            band_counts[ten_band_name(n)]+=1
            if n in latest_set: carry_used+=1

    # Son koltuk bağımsız keşfe ayrılır; uzman/specialist koltukları onu tüketemez.
    reserve_discovery = 1 if size >= 7 else 0
    specialist_limit = max(0, size-reserve_discovery)

    # Seat 3/4: strongest carry identity, but not forced if weak.
    ...
    for _,r in carr.iterrows():
        if len(selected)>=specialist_limit: break
    ...
    # Kalan normal koltukları toplam güçle tamamla; ayrılmış keşif koltuğunu koru.
    ...
    form["Benzer Saat/Faz/Karakter Olayı"] = int(context_info.get("Benzer", 0))
    form["Karakter Benzerliği %"] = round(float(context_info.get("Karakter Benzerliği",0.0))*100.0,1)
    form["Bağlam Güveni %"] = round(float(context_info.get("Güven",0.0))*100.0,1)
    form["Korumalı Uzman Koltuğu"] = int(protected_target)
...
# ===================================================================
# V18.5.13 + CANLI KARAKTER / OTOMATIK FAZ MOTORU
# Sabit saat dilimi kullanmaz. Faz başlangıcı ve kırılımı çekiliş akışından
# öğrenilir; saat yalnızca açıklayıcı bağlamdır.
# ===================================================================
def phase_state_vector_v18513(row_nums, prev_nums=None):
    cur=set(int(n) for n in row_nums)
    prev=set(int(n) for n in (prev_nums or []))
    carry=len(cur & prev) if prev else 0
    new=len(cur-prev) if prev else 20
    blocks=consecutive_blocks(cur)
    return np.array([
        carry/20.0,
        new/20.0,
        len(blocks)/8.0,
        max([len(b) for b in blocks], default=1)/5.0,
        sum(1<=n<=20 for n in cur)/20.0,
        sum(21<=n<=40 for n in cur)/20.0,
        sum(41<=n<=60 for n in cur)/20.0,
        sum(61<=n<=80 for n in cur)/20.0,
    ], dtype=float)


def phase_break_series_v18513(df, short=3, long=8):
    """Her çekilişte kısa dönem davranışının önceki karakterden uzaklığını ölçer."""
    if df is None or len(df)<long+2:
        return pd.DataFrame(columns=['Index','Cekilis','Saat','Kirilim'])
    sets=row_sets(df)
    vec=[]
    for i,s0 in enumerate(sets):
        prev=sets[i-1] if i else []
        vec.append(phase_state_vector_v18513(s0, prev))
    X=np.vstack(vec)
    rows=[]
    for i in range(long, len(df)):
        recent=X[max(0,i-short+1):i+1].mean(0)
        old=X[max(0,i-long):max(0,i-short+1)]
        if len(old)==0: continue
        prior=old.mean(0)
        # Geçmiş varyansına göre ölçekle; tek bir değişkenin baskınlığını azalt.
        hist=X[:i]
        scale=np.std(hist,axis=0)
        scale=np.where(scale<0.08,0.08,scale)
        dist=float(np.sqrt(np.mean(((recent-prior)/scale)**2)))
        rows.append({'Index':i,'Cekilis':int(df.iloc[i].Cekilis_No),'Saat':str(df.iloc[i].Saat),'Kirilim':dist})
    return pd.DataFrame(rows)


def detect_live_phase_v18513(df):
    """Fazı sabit saatten değil, tarihsel kırılım dağılımından bulur."""
    ser=phase_break_series_v18513(df)
    if ser.empty:
        return {'start':max(0,len(df)-4),'age':min(4,len(df)),'break_score':0.0,'threshold':999.0,'state':'OLUSUYOR'}
    vals=ser['Kirilim'].astype(float).to_numpy()
    hist=vals[:-1] if len(vals)>1 else vals
    med=float(np.median(hist)) if len(hist) else 0.0
    mad=float(np.median(np.abs(hist-med))) if len(hist) else 0.0
    threshold=med+1.35*max(mad,0.08)
    # Son geçmişte threshold üstündeki son gerçek kırılım.
    candidates=ser[ser['Kirilim']>=threshold]
    start=int(candidates.iloc[-1]['Index']) if len(candidates) else max(0,len(df)-min(8,len(df)))
    age=max(1,len(df)-start)
    current=float(ser.iloc[-1]['Kirilim'])
    state='KIRILIM' if current>=threshold else ('YENI' if age<=3 else 'OLGUN' if age<=8 else 'UZUN')
    return {'start':start,'age':age,'break_score':current,'threshold':threshold,'state':state}


def phase_historical_carry_v18513(df, phase_age, lookback=500):
    """Benzer faz yaşlarında sayı bazında bir sonraki ele taşınma oranını öğrenir."""
    work=df.tail(min(lookback,len(df))).reset_index(drop=True)
    sets=row_sets(work)
    cases=np.zeros(80,float); hits=np.zeros(80,float)
    if len(work)<25: return np.full(80,0.25)
    # Geçmiş faz başlangıçlarını yalnız o noktaya kadar olan veriyle yaklaşık belirle.
    ser=phase_break_series_v18513(work)
    if ser.empty: return np.full(80,0.25)
    vals=ser['Kirilim'].to_numpy(float)
    med=float(np.median(vals)); mad=float(np.median(np.abs(vals-med)))
    thr=med+1.35*max(mad,0.08)
    breaks=[0]+ser.loc[ser.Kirilim>=thr,'Index'].astype(int).tolist()
    last_break=0; age_by_i={}
    bset=set(breaks)
    for i in range(len(work)):
        if i in bset:last_break=i
        age_by_i[i]=i-last_break+1
    for i in range(len(work)-1):
        if abs(age_by_i.get(i,99)-phase_age)>1: continue
        for n in sets[i]: cases[n-1]+=1
        for n in sets[i]&sets[i+1]: hits[n-1]+=1
    return (hits+2.5)/(cases+10.0)


def live_character_coupon_v18513(df, target_time=None):
    """Tek kupon: canlı faz + taşıma + dönüş + blok/ardışık + V18 temel puan."""
    if df is None or len(df)<30:
        return [], pd.DataFrame(), {}
    phase=detect_live_phase_v18513(df)
    start=max(0,int(phase['start']))
    phase_df=df.iloc[start:].copy()
    # Çok uzun fazda canlı karakterin son 10 elini ağırlaştır.
    live_df=phase_df.tail(min(10,len(phase_df)))
    latest=set(int(df.iloc[-1][c]) for c in NUM_COLS)

    smart=intelligent_score_table(df, target_time).set_index('Sayı')
    phase_freq=frequency(live_df).set_index('Sayı')['Frekans']/max(len(live_df),1)
    short_freq=frequency(df.tail(min(4,len(df)))).set_index('Sayı')['Frekans']/max(min(4,len(df)),1)
    gapdf=gaps(df).set_index('Sayı')
    cyc=return_cycle_table(df).set_index('Sayı')
    carry=phase_historical_carry_v18513(df, int(phase['age']))

    try:
        blockdf=predicted_block_number_scores(df, lookback=min(450,len(df)))
        blockmap=blockdf.set_index('Sayı')['Blok Geçiş Puanı'].to_dict() if not blockdf.empty else {}
    except Exception:
        blockmap={}

    # Canlı ardışık sınır: son fazda komşuların etkinliği.
    live_sets=row_sets(live_df)
    neighbor=np.zeros(80,float)
    for ss in live_sets:
        for n in range(1,81):
            if (n-1 in ss) or (n+1 in ss): neighbor[n-1]+=1
    neighbor/=max(len(live_sets),1)

    rows=[]
    for n in range(1,81):
        expected=max(0.0,float(cyc.loc[n,'Ort. dönüş aralığı'])-1.0)
        gap=float(gapdf.loc[n,'Dinlenme'])
        return_fit=float(np.exp(-abs(gap-expected)/(expected+1.5)))
        carry_fit=float(carry[n-1]) if n in latest else 0.0
        live=float(phase_freq.get(n,0.0))
        short=float(short_freq.get(n,0.0))
        v18=float(smart.loc[n,'Toplam Puan'])/100.0
        block=float(blockmap.get(n,0.0))/100.0
        neigh=float(neighbor[n-1])
        # Faz yeni ise canlı akış ve taşıma; olgunsa dönüş/blok biraz güçlenir.
        age=int(phase['age'])
        if age<=3:
            weights=(.30,.24,.12,.08,.08,.18) # live,carry,return,block,neighbor,v18
        elif age<=8:
            weights=(.26,.20,.17,.12,.09,.16)
        else:
            weights=(.20,.17,.20,.15,.10,.18)
        score=(weights[0]*live + weights[1]*carry_fit + weights[2]*return_fit +
               weights[3]*block + weights[4]*neigh + weights[5]*v18 + .06*short)
        reasons=[]
        if live>=0.40: reasons.append('CANLI SICAK')
        if carry_fit>=0.30 and n in latest: reasons.append('TAŞIMA')
        if return_fit>=0.72 and n not in latest: reasons.append(f'{int(gap)} EL DİNLENİP DÖNÜŞ')
        if block>=0.55: reasons.append('BLOK')
        if neigh>=0.45: reasons.append('ARDIŞIK/KOMŞU')
        if not reasons: reasons.append('V18 DESTEK')
        rows.append({'Sayı':n,'Puan':score*100,'Canlı Faz %':live*100,'Taşıma %':carry_fit*100,
                     'Dönüş %':return_fit*100,'Blok %':block*100,'Ardışık %':neigh*100,
                     'V18 %':v18*100,'Neden':' + '.join(reasons)})
    tab=pd.DataFrame(rows).sort_values(['Puan','Sayı'],ascending=[False,True]).reset_index(drop=True)

    # Tek kupon boyutu otomatik: skor farkına ve faz olgunluğuna göre 3-7.
    top=tab.head(8)['Puan'].to_numpy(float)
    if len(top)>=5:
        gaps2=top[:-1]-top[1:]
        cut=int(np.argmax(gaps2[:6])+1)
        size=max(3,min(7,cut))
    else:size=3
    # Çok yeni fazda aşırı büyüme yok.
    if int(phase['age'])<=2:size=min(size,4)
    picks=tab.head(size)['Sayı'].astype(int).tolist()
    meta={**phase,'phase_rows':len(phase_df),'coupon_size':size}
    return sorted(picks), tab, meta


def backtest_live_character_v18513(df, test_count=50):
    rows=[]
    start=max(60,len(df)-int(test_count))
    for t in range(start,len(df)):
        train=df.iloc[:t].copy()
        target=df.iloc[t]
        coupon,tab,meta=live_character_coupon_v18513(train, str(target.Saat))
        actual=set(int(target[c]) for c in NUM_COLS)
        hits=sorted(set(coupon)&actual)
        rows.append({
            'Çekiliş':int(target.Cekilis_No),'Tarih':str(target.Tarih),'Saat':str(target.Saat),
            'Faz yaşı':int(meta.get('age',0)),'Faz durumu':meta.get('state',''),
            'Kırılım':round(float(meta.get('break_score',0)),3),'Eşik':round(float(meta.get('threshold',0)),3),
            'Kupon boyu':len(coupon),'Kupon':'-'.join(map(str,coupon)),
            'İsabet':len(hits),'Tutan':'-'.join(map(str,hits)),'Sızıntı':'TEMİZ'
        })
    return pd.DataFrame(rows)


# ============================================================
# V21 — YAŞAYAN ALT MOTORLAR / DİYALEKTİK UYUM
# Her alt motor:
# 1) Kendi sinyalini üretir.
# 2) Kendi güvenini ölçer.
# 3) Son kör performansına göre ağırlık kazanır/kaybeder.
# 4) Aynı motor sürekli başarısızsa kendi etkisini azaltır.
# 5) Tez (devam) + antitez (kırılma) + sentez (ortak karar) mantığı kullanılır.
# ============================================================

def v21_norm_array(values):
...
def v21_percentile(values):
    arr = np.asarray(values, dtype=float)
    if len(arr) <= 1:
        return np.full(len(arr), 50.0)
    order = np.argsort(np.argsort(arr, kind="mergesort"), kind="mergesort")
    return order / (len(arr) - 1) * 100.0


def v21_basic_sets(frame):
    return [
        set(int(row[c]) for c in NUM_COLS)
        for _, row in frame.reset_index(drop=True).iterrows()
    ]


def v21_submotors(frame, target_time=None):
    """
    1..80 için yaşayan alt motor tablosu.
    Saat ana eksen; geçmiş sadece kalibrasyon/destek.
    """
    work = frame.sort_values("Cekilis_No").reset_index(drop=True)
    latest = set(int(work.iloc[-1][c]) for c in NUM_COLS)
    sets = v21_basic_sets(work)

    # 1) SAAT MOTORU
    target_hour = int(str(target_time or work.iloc[-1].Saat)[:2])
    hour_rows = work[work["Saat"].astype(str).str[:2].astype(int) == target_hour]
    hour_count = Counter()
    for _, r in hour_rows.tail(140).iterrows():
        hour_count.update(int(r[c]) for c in NUM_COLS)
    hour_score = np.array(
        [hour_count.get(n, 0) / max(len(hour_rows.tail(140)), 1) for n in range(1,81)],
        dtype=float,
    )

    # 2) SICAK ÇEKİRDEK MOTORU
    last8 = sets[-8:] if len(sets) >= 8 else sets
    core = Counter()
    for s in last8:
        core.update(s)
    core_score = np.array(
        [core.get(n,0)/max(len(last8),1) for n in range(1,81)],
        dtype=float,
    )

    # 3) TAŞIMA MOTORU (tez / devam)
    carry_score = np.zeros(80, dtype=float)
    look = sets[-240:] if len(sets) > 240 else sets
    for n in range(1,81):
        cases = hits = 0
        for i in range(len(look)-1):
            if n in look[i]:
                cases += 1
                hits += int(n in look[i+1])
        carry_score[n-1] = (hits + 1.5) / (cases + 6.0)
        if n not in latest:
            carry_score[n-1] *= 0.35

    # 4) DÖNÜŞ MOTORU (antitez / dinlenip geri gelme)
    return_score = np.zeros(80, dtype=float)
    for n in range(1,81):
        gap = len(sets)
        for g,s in enumerate(reversed(sets)):
            if n in s:
                gap = g
                break
        if 1 <= gap <= 4:
            # basit Bayes yumuşatma ile gap bazlı geri dönüş
            cases = hits = 0
            for i in range(gap, len(look)-1):
                if n in look[i-gap] and all(n not in look[k] for k in range(i-gap+1, i+1)):
                    cases += 1
                    hits += int(n in look[i+1])
            return_score[n-1] = (hits + 1.0) / (cases + 5.0)

    # 5) BLOK / ARDIŞIK MOTORU
    block_score = np.zeros(80, dtype=float)
    for s in last8:
        vals = sorted(s)
        for n in vals:
            neigh = sum((n+d) in s for d in (-2,-1,1,2))
            block_score[n-1] += neigh
            for d in (-2,-1,1,2):
                m = n+d
                if 1 <= m <= 80:
                    block_score[m-1] += 0.25
    block_score = v21_norm_array(block_score)

    # 6) YENİLENME / YERİNE GEÇME MOTORU
    replacement_score = np.zeros(80, dtype=float)
    if len(sets) >= 3:
        prev = sets[-2]
        cur = sets[-1]
        leaving = prev - cur
        entering = cur - prev
        # tarihsel olarak leaving kümelerinden sonra hangi sayılar gelir?
        for i in range(max(1, len(look)-180), len(look)-1):
            src = look[i-1] - look[i]
            if len(src & leaving) >= max(1, min(2, len(leaving))):
                nxt = look[i+1]
                for n in nxt:
                    replacement_score[n-1] += 1.0
        replacement_score = v21_norm_array(replacement_score)

    # 7) BENZER DURUM MOTORU
    similar_score = np.zeros(80, dtype=float)
    if len(sets) >= 12:
        cur_sig = np.array([
            len(sets[-2] & sets[-1]),
            len(sets[-3] & sets[-2]),
            len(set.union(*sets[-3:])),
            len(set.intersection(*sets[-3:])),
        ], dtype=float)

        events = []
        for i in range(3, len(sets)-1):
            sig = np.array([
                len(sets[i-1] & sets[i]),
                len(sets[i-2] & sets[i-1]),
                len(set.union(*sets[i-2:i+1])),
                len(set.intersection(*sets[i-2:i+1])),
            ], dtype=float)
            dist = float(np.linalg.norm(cur_sig - sig))
            w = 1.0 / (1.0 + dist)
            events.append((w, i+1))
        for w, idx in sorted(events, reverse=True)[:20]:
            for n in sets[idx]:
                similar_score[n-1] += w
        similar_score = v21_norm_array(similar_score)

    # 8) ORTA GERÇEKLEŞME BANDI MOTORU
    # En yüksek ham puanı otomatik doğru sayma. Her alt motorun orta-yüksek yüzdeliklerini işaretle.
    motor_arrays = {
        "Saat": hour_score,
        "Çekirdek": core_score,
        "Taşıma": carry_score,
        "Dönüş": return_score,
        "Blok": block_score,
        "Yerine": replacement_score,
        "Benzer": similar_score,
    }
    ...
    for n in range(1,81):
        ...
        band_votes = 0
        high_votes = 0
        for name, arr in motor_arrays.items():
            pct = v21_percentile(arr)[n-1]
            row[name] = float(arr[n-1])
            row[name+" Yüzdelik"] = float(pct)
            # Gerçekleşme bandı: 55–85 yüzdelik varsayılan, kör kalibrasyon bunu sonra ayarlar.
            if 55 <= pct <= 85:
                band_votes += 1
            if pct > 90:
                high_votes += 1
        row["Band Oy"] = band_votes
        row["Aşırı Yüksek Oy"] = high_votes
        rows.append(row)

    out = pd.DataFrame(rows)

    # Diyalektik:
    # tez = devam (saat+çekirdek+taşıma)
    # antitez = kırılma (dönüş+yerine+blok)
    # sentez = benzer durum + ortak band oyları
    out["Tez"] = (
        0.42*v21_norm_array(out["Saat"])
        + 0.33*v21_norm_array(out["Çekirdek"])
        + 0.25*v21_norm_array(out["Taşıma"])
    )
    out["Antitez"] = (
        0.34*v21_norm_array(out["Dönüş"])
        + 0.33*v21_norm_array(out["Yerine"])
        + 0.33*v21_norm_array(out["Blok"])
    )
    out["Sentez"] = (
        0.55*v21_norm_array(out["Benzer"])
        + 0.45*(out["Band Oy"]/7.0)
    )

    # Bir taraf aşırı baskınsa tek başına hükmetmesin.
    out["Diyalektik Denge"] = 1.0 - (out["Tez"] - out["Antitez"]).abs()
    out["Diyalektik Denge"] = out["Diyalektik Denge"].clip(0,1)

    out["V21 Puan"] = (
        0.34*out["Tez"]
        + 0.28*out["Antitez"]
        + 0.22*out["Sentez"]
        + 0.16*out["Diyalektik Denge"]
    ) * 100.0

    # Aşırı yüksek puanların kör şekilde tepeye çıkmasını frenle.
    out["V21 Puan"] -= np.maximum(0, out["Aşırı Yüksek Oy"] - 3) * 2.0
    out["V21 Puan"] += np.maximum(0, out["Band Oy"] - 2) * 1.5

    return out.sort_values(
        ["V21 Puan","Band Oy","Sayı"],
        ascending=[False,False,True]
    ).reset_index(drop=True)


def v21_live_master(frame, target_time=None):
    table = v21_submotors(frame, target_time)
    coupon = sorted(table.head(7)["Sayı"].astype(int).tolist())
    return coupon, table


def v21_blind_test(frame, test_count=25):
    work = frame.sort_values("Cekilis_No").reset_index(drop=True)
    start = max(180, len(work)-int(test_count))
    rows = []

    for i in range(start, len(work)):
        train = work.iloc[:i].copy()
        target = work.iloc[i]
        coupon, table = v21_live_master(train, str(target.Saat))
        actual = set(int(target[c]) for c in NUM_COLS)
        hits = sorted(set(coupon) & actual)
        ...
        rows.append({
            ...
            "İsabet": len(hits),
            "Tutan": "-".join(map(str,hits)),
            "Ort Band Oy": round(float(table.head(7)["Band Oy"].mean()),2),
            "Ort Diyalektik Denge": round(float(table.head(7)["Diyalektik Denge"].mean()),3),
            "Sızıntı": "TEMİZ",
        })
...
# ============================================================
# V22 — ÖĞRENİLEN GERÇEKLEŞME BANDI
#
# V21 düzeltmesi:
# - Sabit 55–85 "orta bant" KALDIRILDI.
# - Her alt motor kendi geçmişinde hangi yüzdelik bantta daha çok gerçek sayı
#   ürettiğini sızıntısız örneklerden öğrenir.
# - En yüksek ham puan otomatik olarak ödüllendirilmez.
# - Diyalektik denge artık ana ödül değildir; yalnız küçük bir kararlılık sinyalidir.
# - Saat yine ana eksendir.
# ============================================================

V22_MOTORS = ["Saat", "Çekirdek", "Taşıma", "Dönüş", "Blok", "Yerine", "Benzer"]


def v22_percentile(values):
    arr = np.asarray(values, dtype=float)
    if len(arr) <= 1:
        return np.full(len(arr), 50.0)
    order = np.argsort(np.argsort(arr, kind="mergesort"), kind="mergesort")
...
def v22_collect_calibration_samples(frame, sample_draws=72, step=2):
    """
    Her hedef çekilişte yalnız hedefin ÖNCESİNİ görür.
    Her alt motor için:
      ham puan -> 0..100 yüzdelik -> gerçek geldi mi?
    """
    work = frame.sort_values("Cekilis_No").reset_index(drop=True)
    start = max(180, len(work) - int(sample_draws))
    rows = []

    for i in range(start, len(work), max(1, int(step))):
        train = work.iloc[:i].copy()
        if len(train) < 180:
            continue

        target = work.iloc[i]
        tab = v21_submotors(train, str(target.Saat)).copy()
        actual = set(int(target[c]) for c in NUM_COLS)

        for motor in V22_MOTORS:
            pct = v22_percentile(tab[motor].to_numpy())
            for j, row in tab.iterrows():
                rows.append({
                    "Motor": motor,
                    "Yüzdelik": float(pct[j]),
                    "Geldi": int(int(row["Sayı"]) in actual),
                    "HedefSaat": str(target.Saat)[:2],
                })

    return pd.DataFrame(rows)


def v22_learn_bands(samples, target_time=None, bin_width=10):
    """
    Bayesian yumuşatma ile gerçekleşme bandını öğrenir.
    Temel olasılık 20/80 = %25'tir.
    Aynı saat örnekleri varsa global örneğe ek ağırlık verir.
    """
    ...
    s["BandStart"] = (
        np.floor(s["Yüzdelik"] / width) * width
    ).clip(0, 100 - width)
    s["BandEnd"] = s["BandStart"] + width

    # Aynı saat, varsa iki kat etkili.
    target_hour = str(target_time)[:2] if target_time is not None else None
    if target_hour is not None:
        s["Ağırlık"] = np.where(
            s["HedefSaat"].astype(str) == target_hour,
            2.0,
            1.0,
        )
    else:
    ...
    rows = []
    # Beta önsel: 5 başarı / 15 başarısız = %25.
    alpha0, beta0 = 5.0, 15.0

    for (motor, lo, hi), g in s.groupby(["Motor", "BandStart", "BandEnd"]):
        w = g["Ağırlık"].to_numpy(float)
        y = g["Geldi"].to_numpy(float)

        succ = float((w * y).sum())
        fail = float((w * (1.0 - y)).sum())
        n_eff = succ + fail

        posterior = (succ + alpha0) / (succ + fail + alpha0 + beta0)
        lift = posterior / 0.25

        # Çok küçük örneğin aşırı lift üretmesini bastır.
        reliability = min(1.0, np.sqrt(max(n_eff, 0.0) / 240.0))
        calibrated_lift = 1.0 + (lift - 1.0) * reliability

        rows.append({
            "Motor": motor,
            "BandStart": float(lo),
            "BandEnd": float(hi),
            "Etkin Örnek": round(n_eff, 1),
            "Gerçekleşme %": round(posterior * 100.0, 2),
            "Lift": round(lift, 3),
            "Güven": round(reliability, 3),
            "Kalibre Lift": round(calibrated_lift, 3),
        })
    ...
    # Her motorun "en iyi orta/uygun gerçekleşme yeri" en yüksek yüzdelik olmak zorunda değil.
    out["Band Gücü"] = (
        (out["Kalibre Lift"] - 1.0)
        * (0.50 + 0.50 * out["Güven"])
    )

    return out.sort_values(
        ["Motor", "Band Gücü", "Gerçekleşme %"],
        ascending=[True, False, False],
    ).reset_index(drop=True)


def v22_band_factor(percentile, motor, learned_bands):
    """
    Canlı sayının ilgili motor yüzdeliği hangi geçmiş bandına düşüyorsa
    o bandın kalibre lift'ini döndürür.
    """
    if learned_bands is None or learned_bands.empty:
        return 1.0

    m = learned_bands[learned_bands["Motor"] == motor]
    if m.empty:
        return 1.0
    ...
    hit = m[
        ...
        & (
            (p < m["BandEnd"])
            | ((m["BandEnd"] >= 100.0) & (p == 100.0))
        )
    ]
    if hit.empty:
        return 1.0

    return float(hit.iloc[0]["Kalibre Lift"])


def v22_motor_skill(learned_bands, motor):
    """
    Motorun son kalibrasyondaki genel becerisi.
    Tek bir uç band yerine ilk 3 bandın ortalama pozitif lift'ini kullanır.
    """
    if learned_bands is None or learned_bands.empty:
        return 1.0

    m = learned_bands[learned_bands["Motor"] == motor].head(3)
    if m.empty:
        return 1.0

    skill = float(m["Kalibre Lift"].mean())
    return float(np.clip(skill, 0.80, 1.25))


def v22_live_master(frame, target_time=None, calibration_draws=72, calibration_step=2):
    work = frame.sort_values("Cekilis_No").reset_index(drop=True)

    raw = v21_submotors(
        work,
        target_time or str(work.iloc[-1].Saat),
    ).copy()

    samples = v22_collect_calibration_samples(
        work,
        sample_draws=min(int(calibration_draws), max(24, len(work) - 180)),
        step=max(1, int(calibration_step)),
    )
    bands = v22_learn_bands(
        samples,
        target_time=target_time or str(work.iloc[-1].Saat),
        bin_width=10,
    )

    # Motor baz ağırlıkları. Saat ana eksen.
    base_weight = {
        "Saat": 0.24,
        "Çekirdek": 0.18,
        "Taşıma": 0.13,
    ...
    total_score = np.zeros(len(raw), dtype=float)
    calibrated_vote = np.zeros(len(raw), dtype=float)
    positive_votes = np.zeros(len(raw), dtype=float)

    for motor in V22_MOTORS:
        vals = raw[motor].to_numpy(float)
        norm = v21_norm_array(vals)
        pct = v22_percentile(vals)

        factors = np.array(
            [
                v22_band_factor(p, motor, bands)
                for p in pct
            ],
            dtype=float,
        )

        skill = v22_motor_skill(bands, motor)
        weight = base_weight[motor] * skill

        # Ham sinyal + öğrenilen gerçekleşme yeri.
        # Faktör 1 = nötr. 1'den yüksek geçmişte olumlu band.
        calibrated = norm * np.clip(factors, 0.70, 1.35)

        total_score += weight * calibrated
        calibrated_vote += np.maximum(0.0, factors - 1.0)
        positive_votes += (factors >= 1.04).astype(float)

        raw[motor + " Yüzdelik"] = pct
        raw[motor + " BandLift"] = factors

    # Diyalektik denge artık yardımcı sinyal; V21'deki gibi yüksek ağırlıklı değil.
    thesis = raw["Tez"].to_numpy(float)
    anti = raw["Antitez"].to_numpy(float)
    dialectic_stability = 1.0 - np.abs(thesis - anti)
    dialectic_stability = np.clip(dialectic_stability, 0.0, 1.0)

    # Konsensüs ancak geçmişte olumlu kalibre olmuş motorlardan gelirse değerlidir.
    consensus = positive_votes / len(V22_MOTORS)
    band_strength = calibrated_vote / len(V22_MOTORS)

    final = (
        0.76 * v21_norm_array(total_score)
        + 0.14 * consensus
        + 0.07 * v21_norm_array(band_strength)
        + 0.03 * dialectic_stability
    ) * 100.0

    # 4+ motor aynı geçmişte başarılı banda denk geliyorsa destek.
    final += np.maximum(0.0, positive_votes - 3.0) * 1.25

    raw["V22 Puan"] = np.round(final, 3)
    raw["Kalibre Motor Oy"] = positive_votes.astype(int)
    raw["Band Gücü"] = np.round(band_strength, 4)
    raw["V22 Diyalektik Yardım"] = np.round(dialectic_stability, 4)

    ranked = raw.sort_values(
        ["V22 Puan", "Kalibre Motor Oy", "Band Gücü", "Sayı"],
        ascending=[False, False, False, True],
    ).reset_index(drop=True)

    coupon = sorted(ranked.head(7)["Sayı"].astype(int).tolist())

    return coupon, ranked, bands


def v22_blind_test(frame, test_count=10, calibration_draws=54):
    """
    Sızıntısız V22 vs V21 karşılaştırması.
    Test ağır olmasın diye kalibrasyon geçmişi örneklenir.
    """
    work = frame.sort_values("Cekilis_No").reset_index(drop=True)
    start = max(220, len(work) - int(test_count))
    rows = []
    ...
    for i in range(start, len(work)):
        ...
        v22_coupon, _, _ = v22_live_master(
            train,
            str(target.Saat),
            calibration_draws=min(
                int(calibration_draws),
                max(30, len(train) - 180),
            ),
            calibration_step=3,
        )
        v21_coupon, _ = v21_live_master(
            train,
            str(target.Saat),
        )

        actual = set(int(target[c]) for c in NUM_COLS)
        h22 = sorted(set(v22_coupon) & actual)
        h21 = sorted(set(v21_coupon) & actual)

        rows.append({
            "Çekiliş": int(target.Cekilis_No),
            "Tarih": str(target.Tarih),
            "Saat": str(target.Saat),
            "V22 Kupon": "-".join(map(str, v22_coupon)),
            "V22 İsabet": len(h22),
            "V22 Tutan": "-".join(map(str, h22)),
            "V21 Kupon": "-".join(map(str, v21_coupon)),
            "V21 İsabet": len(h21),
            "Avantaj": len(h22) - len(h21),
            "Sızıntı": "TEMİZ",
        })
...
PAGES = [
    ...
    "🧠 Yaşayan Motor V17",
    "🫁 Living Engine V18",
    "🧬 Yaşayan Diyalektik MASTER V21",
    "🎯 Öğrenilen Gerçekleşme Bandı V22",
    "🧭 Canlı Karakter Faz V18.5.13",
    "📊 Bant/Saat",
    "🧭 Benzerlik",
    "🧬 Değişim",
    "🌙 Kapanış",
...
if page == "✅ Kontrol":
    st.success("Çalışan sürüm: V22 ÖĞRENİLEN GERÇEKLEŞME BANDI + YAŞAYAN DİYALEKTİK MASTER")
    ...
    if len(df) < 40:
        ...
        st.caption("V18.5.10 test: her geçmiş hedefte gerçek hedef saat kullanılır; yalnız geçmiş veriyle saat/faz/karakter bağlamı yeniden kurulur.")
        if st.button(
            "🧪 V18.5.10 FAZ/KARAKTER WALK-FORWARD TEST",
            key="v150_run_backtest",
        ):
            with st.spinner("V18.5.10 faz/saat/karakter kilitli walk-forward test çalışıyor; mobilde biraz sürebilir..."):
                bt = live_backtest(
                    df,
                    coupon_size=bt_size,
                    test_count=bt_count,
    ...
    if len(df) < 100:
        ...
        st.subheader("4️⃣ V18.5.11 Nefes Puanı")
        st.dataframe(
            master18[
                [
                    "Sayı",
        ...
        st.write("**🧩 V18.5.11 Blok Rejim / Mikro-Konum Kararı**")
        ...
        st.write("**🔁 V18.5.11 Elden Ele Taşıma Kimliği**")
        ...
        st.write("**🌙 V18.5.11 Dinlenmiş Dönüş Motoru**")
        ...
        st.caption("🏆 V18.5.11 OYNA — MASTER 7: V18.5.10 Living + 4 Geçiş mimarisini korur. Yeni ÜST MASTER; uzman içi puan/sıra, kısa dönem + saat/faz + uzun dönem hafıza, Living korumalı çekirdek, çapraz aile köprüsü, sürü freni, uzman şampiyonu ve rezerv adaylarla havuzdaki güçlü sinyalleri 7 koltuğa akıllı biçimde sıkıştırır.")
        ...
        if st.button(
            ...
            # V18.5.10 — Living Engine kendi özgün yapısını aynen korur.
            # Dört Geçiş profili bağımsız ikinci uzman ailesi olarak ayrı üretilir;
            # yalnız en üstte ÜST MASTER tarafından birleştirilir.
            _transition_items510, _transition_hybrid510, _transition_pairs510 = transition_expert_bundle_v18510(
                df,
                target_time=v18_target_time,
                size=v18_size,
                lookback=v18_window,
            )
            # V18.5.11 — her Geçiş uzmanının kendi 80 adaylık puan/sırasını sakla.
            _transition_items511 = enrich_transition_items_v18511(
                _transition_items510,
                _transition_hybrid510,
            )
            _transition_weights511 = {}
            _memory_table511 = pd.DataFrame()
            try:
                _bt_detail510, _bt_summary510 = transition_walkforward_v18510(
                    df, test_count=min(50,max(12,len(df)//20)),
                    analysis_window=v18_window, size=v18_size
                )
                if _bt_summary510 is not None and not _bt_summary510.empty:
                    st.session_state["v18510_transition_bt_summary"] = _bt_summary510
                    st.session_state["v18510_transition_bt_detail"] = _bt_detail510
                _transition_weights511, _memory_table511 = transition_three_memory_weights_v18511(
                    _bt_detail510,
                    target_time=v18_target_time,
                )
                st.session_state["v18511_transition_memory"] = _memory_table511
                st.session_state["v18511_transition_weights"] = _transition_weights511
            except Exception:
                _bt_summary510 = pd.DataFrame()
                _bt_detail510 = pd.DataFrame()

            _upper_coupon511, _upper_explain511, _upper_reserve511, _upper_full511 = upper_meta_master_v18511(
                items,
                _transition_items511,
                size=v18_size,
                transition_weights=_transition_weights511,
                reserve_size=5,
            )
            if _upper_coupon511:
                items.append({
                    "Senaryo":"👑 ÜST MASTER 7 — V18.5.11 Akıllı Sıkıştırma",
                    "Kupon":_upper_coupon511,
                    "Üst Master Açıklama 511":_upper_explain511,
                    "Üst Master Rezerv 511":_upper_reserve511,
                    "Üst Master Tam Havuz 511":_upper_full511,
                    "Geçiş Uzmanları 511":_transition_items511,
                    "Geçiş Ağırlıkları 511":_transition_weights511,
                    "Üç Hafıza 511":_memory_table511,
                })

            # V18.5.12 — tüm mevcut uzmanları tek aday havuzunda yeniden değerlendir.
            # Sonuç sızıntısı yoktur; yalnız geçmişte sonuçlanmış motor hafızası kullanılır.
            _selector_coupon512, _selector_explain512, _selector_full512 = seven_seat_selector_v18512(
                items, memory_df=memory, size=v18_size
            )
            if _selector_coupon512:
                items.append({
                    "Senaryo":"🧠 V18.5.12 — 7 Koltuk Seçici",
                    "Kupon":_selector_coupon512,
                    "7 Koltuk Açıklama 512":_selector_explain512,
                    "7 Koltuk Tam Havuz 512":_selector_full512,
                })

            # V18.5.13 — 5.12 yan yana kalır; yeni seçici aile oylarını tekilleştirir.
            _selector_coupon513, _selector_explain513, _selector_full513 = seven_seat_selector_v18513(
                items, memory_df=memory, size=v18_size
            )
            if _selector_coupon513:
                items.append({
                    "Senaryo":"🧬 V18.5.13 — Bağımsız Aile 7",
                    "Kupon":_selector_coupon513,
                    "7 Koltuk Açıklama 513":_selector_explain513,
                    "7 Koltuk Tam Havuz 513":_selector_full513,
                })
            ...
            if _master_item:
                ...
                if _form:
                    def _fmt_motor_form_value_v18510(v):
                        try:
                            return f"{float(v):.2f}"
                        except (TypeError, ValueError):
                            return str(v)
                    st.caption(
                        "Motor formu: "
                        + " | ".join(
                            f"{k} {_fmt_motor_form_value_v18510(v)}"
                            for k, v in _form.items()
                        )
                    )
                _regime_guard_v18510 = _master_item.get("Rejim Sigorta 567", 0.0)
                try:
                    _regime_guard_text_v18510 = f"{float(_regime_guard_v18510):.2f}"
                except (TypeError, ValueError):
                    _regime_guard_text_v18510 = str(_regime_guard_v18510)
                st.caption(f"Rejim kırılma sigortası: {_regime_guard_text_v18510}")

            _upper_item511 = next((x for x in items if str(x.get("Senaryo","")) .startswith("👑 ÜST MASTER")), None)
            if _upper_item511:
                st.markdown("### 👑 V18.5.11 ÜST MASTER — Havuzdan 7'ye Akıllı Sıkıştırma")
                st.caption("Living Engine ve 4 Geçiş uzmanının kendi formülleri değişmez. Üst katman yalnızca uzman içi puan/sıra + üç hafıza + bağımsız aile desteğini kullanarak 7 koltuğu seçer.")
                _tr_items511=_upper_item511.get("Geçiş Uzmanları 511",[])
                if _tr_items511:
                    for _j,_it in enumerate(_tr_items511,1):
                        _cp = list(_it.get('Kupon',[]))
                        _scores = _it.get('Uzman Puanları 511',{})
                        _ranks = _it.get('Uzman Sıraları 511',{})
                        st.info(f"Geçiş Uzmanı {_j} — {_it.get('Profil','')}: " + " - ".join(map(str,_cp)))
                        _score_rows=[]
                        for _n in _cp:
                            _score_rows.append({
                                "Sayı":int(_n),
                                "Uzman içi puan":round(float(_scores.get(int(_n),0.0)),2),
                                "Uzman sırası":int(_ranks.get(int(_n),99)),
                            })
                        if _score_rows:
                            st.dataframe(pd.DataFrame(_score_rows).sort_values(["Uzman içi puan","Sayı"],ascending=[False,True]),use_container_width=True,hide_index=True)
                _ux=_upper_item511.get("Üst Master Açıklama 511")
                if isinstance(_ux,pd.DataFrame) and not _ux.empty:
                    st.write("**7 koltuğun seçim gerekçesi**")
                    st.dataframe(_ux, use_container_width=True, hide_index=True)
                _reserve=_upper_item511.get("Üst Master Rezerv 511")
                if isinstance(_reserve,pd.DataFrame) and not _reserve.empty:
                    st.write("**🪑 Rezerv adaylar — MASTER dışında kalan en güçlü 5 sayı**")
                    _rc=[c for c in ["Rezerv Sırası","Sayı","V18.5.11 Üst Puan","Living Gücü","Geçiş Gücü","Uzman İçi Puan","Geçiş Profil Desteği","Çapraz Aile"] if c in _reserve.columns]
                    st.dataframe(_reserve[_rc], use_container_width=True, hide_index=True)
                _mem=_upper_item511.get("Üç Hafıza 511")
                if isinstance(_mem,pd.DataFrame) and not _mem.empty:
                    st.write("**🧠 Üç Hafızalı Uzman Güveni — kısa / saat-faz / uzun**")
                    st.dataframe(_mem,use_container_width=True,hide_index=True)

            _selector_item512 = next((x for x in items if str(x.get("Senaryo", "")).startswith("🧠 V18.5.12")), None)
            if _selector_item512:
                st.markdown("### 🧠 V18.5.12 — 7 Koltuk Seçici")
                st.caption("Mevcut Living + MASTER + Geçiş adaylarını tek havuzda toplar; geçmiş senaryo formu, konsensüs, MASTER desteği ve kontrollü azınlık sinyalini kullanır. Hedef sonucu seçim sırasında görmez.")
                _sx = _selector_item512.get("7 Koltuk Açıklama 512")
                if isinstance(_sx, pd.DataFrame) and not _sx.empty:
                    _sc = [c for c in ["Sayı","V18.5.12 Koltuk Puanı","Uzman Desteği","Ağırlıklı Destek","MASTER Desteği","Azınlık Sinyali","Seçim Nedeni 512"] if c in _sx.columns]
                    st.dataframe(_sx[_sc], use_container_width=True, hide_index=True)


            _selector_item513 = next((x for x in items if str(x.get("Senaryo", "")).startswith("🧬 V18.5.13")), None)
            if _selector_item513:
                st.markdown("### 🧬 V18.5.13 — Bağımsız Aile 7")
                st.caption("V18.5.12 yan yana korunur. Aynı kökten gelen kupon oyları tekilleştirilir; MASTER çekirdeği korunur ve bir koltuk kontrollü azınlık/keşif sinyaline ayrılır. Hedef sonucu seçim sırasında kullanılmaz.")
                _sx513 = _selector_item513.get("7 Koltuk Açıklama 513")
                if isinstance(_sx513, pd.DataFrame) and not _sx513.empty:
                    _sc513 = [c for c in ["Sayı","V18.5.13 Koltuk Puanı","Bağımsız Aile Desteği","Aile Gücü","MASTER Desteği","Azınlık Sinyali","Aileler","Seçim Nedeni 513"] if c in _sx513.columns]
                    st.dataframe(_sx513[_sc513], use_container_width=True, hide_index=True)
        ...
        if items and start_draw:
            ...
            if evaluated.empty:
                ...
                try:
                    _actual_row512 = df[(df["Tarih"].astype(str) == str(target_date)) & (df["Saat"].astype(str) == str(target_time))].tail(1)
                    if not _actual_row512.empty:
                        _actual_nums512 = [int(_actual_row512.iloc[0][c]) for c in NUM_COLS]
                        _coverage512 = pool_coverage_v18512(items, _actual_nums512)
                        st.info(
                            f"🧠 V18.5.12 havuz testi: {_coverage512['Havuz Boyutu']} farklı aday içinde "
                            f"{_coverage512['Havuz Doğru']} doğru vardı. Tutanlar: {_coverage512['Havuzda Tutanlar'] or 'Yok'}"
                        )
                except Exception:
                    pass
...
elif page == "🧭 Canlı Karakter Faz V18.5.13":
    st.header("🧭 Canlı Karakter / Otomatik Faz — V18.5.13")
    st.caption("Saat başına zorla sıfırlama yok. Motor, oyun karakterinin ne zaman kırıldığını çekiliş akışından bulur; faz yaşını hesaplar ve tek kupon üretir.")
    if len(df)<60:
        st.warning("En az 60 çekiliş gerekli.")
    else:
        next_no,next_date,next_time=next_draw_defaults(df)
        coupon,detail,meta=live_character_coupon_v18513(df,next_time)
        c1,c2,c3,c4=st.columns(4)
        c1.metric("Aktif faz yaşı",f"{meta.get('age',0)} el")
        c2.metric("Faz durumu",str(meta.get('state','-')))
        c3.metric("Kırılım",f"{meta.get('break_score',0):.2f} / {meta.get('threshold',0):.2f}")
        c4.metric("Kupon boyu",len(coupon))
        st.write(f"**Hedef:** #{next_no} — {next_date} {next_time}")
        st.success("TEK KUPON — "+" - ".join(map(str,coupon)))
        st.dataframe(detail.head(15),use_container_width=True,hide_index=True)

        st.divider()
        st.subheader("🧪 Sızıntısız geçmiş test")
        n=st.select_slider("Test adedi",options=[10,25,50,100,250],value=25,key="live_phase_bt_n")
        if st.button("🚀 CANLI KARAKTER TESTİ",type="primary",use_container_width=True,key="live_phase_bt_btn"):
            with st.spinner(f"Son {n} çekiliş geçmişten geleceğe test ediliyor..."):
                st.session_state['live_phase_bt']=backtest_live_character_v18513(df,n)
        bt=st.session_state.get('live_phase_bt')
        if isinstance(bt,pd.DataFrame) and not bt.empty:
            total_pred=int(bt['Kupon boyu'].sum()); total_hit=int(bt['İsabet'].sum())
            a,b,c,d=st.columns(4)
            a.metric("Test",len(bt))
            b.metric("Sayı doğruluğu",f"%{100*total_hit/max(total_pred,1):.2f}")
            c.metric("Ort. kupon",f"{bt['Kupon boyu'].mean():.2f}")
            d.metric("En yüksek isabet",int(bt['İsabet'].max()))
            st.dataframe(bt,use_container_width=True,hide_index=True)
            st.download_button("⬇️ TEST CSV İNDİR",bt.to_csv(index=False).encode('utf-8-sig'),
                               file_name=f"V18_5_13_CANLI_KARAKTER_{len(bt)}.csv",mime="text/csv",use_container_width=True)


elif page == "🧬 Yaşayan Diyalektik MASTER V21":
    st.header("🧬 Yaşayan Diyalektik MASTER V21")
    st.caption(
        "Alt motorlar ayrı hücreler gibi çalışır: Saat, Sıcak Çekirdek, Taşıma, "
        "Dönüş, Blok/Ardışık, Yerine Geçme ve Benzer Durum. "
        "Tez = devam sinyali, Antitez = kırılma/yenilenme sinyali, "
        "Sentez = ortak gerçekleşme bandı + benzer durum. "
        "Tek bir motorun aşırı yüksek puanı tek başına karar veremez."
    )

    if len(df) < 200:
        st.warning("V21 için 200+ çekiliş önerilir.")
    else:
        next_no, next_date, next_time = next_draw_defaults(df)
        coupon, v21_table = v21_live_master(df, next_time)

        st.write(f"**Hedef:** #{next_no} — {next_date} {next_time}")
        st.success("🏆 V21 TEK 7'Lİ: " + " - ".join(map(str,coupon)))

        show = [
            "Sayı","V21 Puan","Band Oy","Aşırı Yüksek Oy",
            "Tez","Antitez","Sentez","Diyalektik Denge",
            "Saat","Çekirdek","Taşıma","Dönüş","Blok","Yerine","Benzer"
        ]
        show = [c for c in show if c in v21_table.columns]
        st.dataframe(
            v21_table[show].head(25),
            use_container_width=True,
            hide_index=True,
        )

        st.divider()
        st.subheader("🧪 V21 sızıntısız kör test")
        n = st.selectbox(
            "Test adedi",
            [10,25,50],
            index=0,
        ...
        if st.button(
            ...
            with st.spinner("V21 yalnız geçmişi görerek test ediliyor..."):
                r = v21_blind_test(df, n)
            st.session_state["v21_bt_result"] = r

        r = st.session_state.get("v21_bt_result", pd.DataFrame())
        if isinstance(r, pd.DataFrame) and not r.empty:
            a,b,c = st.columns(3)
            a.metric("Test", len(r))
            b.metric("Ort. isabet", f"{r['İsabet'].mean():.2f}")
            c.metric("Maks. isabet", int(r["İsabet"].max()))
            st.dataframe(r, use_container_width=True, hide_index=True)
            st.download_button(
                "⬇️ V21 test CSV",
                r.to_csv(index=False).encode("utf-8-sig"),
                file_name=f"V21_YASAYAN_DIYALEKTIK_TEST_{len(r)}.csv",
                mime="text/csv",
                use_container_width=True,
                key="v21_csv",
            )
...
elif page == "🎯 Öğrenilen Gerçekleşme Bandı V22":
    st.header("🎯 V22 — Öğrenilen Gerçekleşme Bandı")
    st.caption(
        "V21'deki sabit 55–85 gerçekleşme bandı kaldırıldı. "
        "Saat, Çekirdek, Taşıma, Dönüş, Blok, Yerine ve Benzer motorlarının "
        "her biri geçmişte hangi puan yüzdeliklerinde gerçekten sayı getirmişse "
        "o bandı ayrı ayrı öğrenir. En yüksek puan otomatik olarak doğru kabul edilmez."
    )

    if len(df) < 240:
        st.warning("V22 kalibrasyonu için 240+ çekiliş önerilir.")
    else:
        next_no, next_date, next_time = next_draw_defaults(df)

        cal_draws = st.selectbox(
            "Kalibrasyon çekilişi",
            [36, 54, 72, 96],
            index=2,
            key="v22_cal_draws",
        )
        ...
        if st.button(
            "🧠 V22 KALİBRASYONU ÖĞREN + KUPON ÜRET",
            type="primary",
            use_container_width=True,
            key="v22_live_run",
        ):
            with st.spinner(
                "Alt motorların gerçek isabet bantları yalnız geçmiş sonuçlardan öğreniliyor..."
            ):
                c22, t22, b22 = v22_live_master(
                    df,
                    next_time,
                    calibration_draws=cal_draws,
                    calibration_step=2,
                )
            st.session_state["v22_coupon"] = c22
            st.session_state["v22_table"] = t22
            st.session_state["v22_bands"] = b22

        c22 = st.session_state.get("v22_coupon", [])
        t22 = st.session_state.get("v22_table", pd.DataFrame())
        b22 = st.session_state.get("v22_bands", pd.DataFrame())
        ...
        if isinstance(b22, pd.DataFrame) and not b22.empty:
            st.subheader("📍 Alt motorların öğrendiği gerçek gerçekleşme bantları")
            st.dataframe(
                b22.groupby("Motor", group_keys=False).head(3),
                use_container_width=True,
                hide_index=True,
            )

        if isinstance(t22, pd.DataFrame) and not t22.empty:
            st.subheader("🧠 Kalibre edilmiş adaylar")
            cols = [
                "Sayı", "V22 Puan", "Kalibre Motor Oy", "Band Gücü",
                "Saat", "Çekirdek", "Taşıma", "Dönüş", "Blok", "Yerine", "Benzer",
                "Saat Yüzdelik", "Çekirdek Yüzdelik",
                "Taşıma Yüzdelik", "Dönüş Yüzdelik",
            ]
            cols = [c for c in cols if c in t22.columns]
            st.dataframe(
                t22[cols].head(25),
                use_container_width=True,
                hide_index=True,
        ...
        st.divider()
        st.subheader("🧪 V22 sızıntısız karşılaştırmalı test")
        test_n = st.selectbox(
            "Test adedi",
            [10, 25],
            index=0,
        ...
        if st.button(
            ...
            with st.spinner(
                "V22 ve V21 aynı hedeflerde, ileriye bakmadan karşılaştırılıyor..."
            ):
                r22 = v22_blind_test(
                    df,
                    test_count=test_n,
                    calibration_draws=min(cal_draws, 54),
                )
            st.session_state["v22_test_result"] = r22

        r22 = st.session_state.get("v22_test_result", pd.DataFrame())
        if isinstance(r22, pd.DataFrame) and not r22.empty:
            a,b,c,d = st.columns(4)
            a.metric("Test", len(r22))
            b.metric("V22 ortalama", f"{r22['V22 İsabet'].mean():.2f}")
            c.metric("V21 ortalama", f"{r22['V21 İsabet'].mean():.2f}")
            d.metric("Net avantaj", f"{r22['Avantaj'].mean():+.2f}")
            ...
            st.download_button(
                "⬇️ V22 test CSV",
                r22.to_csv(index=False).encode("utf-8-sig"),
                file_name=f"V22_OGRENILEN_GERCEKLESME_BANDI_TEST_{len(r22)}.csv",
                mime="text/csv",
                use_container_width=True,
                key="v22_csv",
            )
    ...
    if not source_numbers:
        ...
        st.divider()
        st.subheader("🧪 V18.5.11 — 4 Geçiş Uzmanı + Üç Hafıza Walk-Forward Test")
        st.caption("Dengeli, Tekrar, Yerine Geçme ve Saat/Sıcaklık motorları hedef sonucu görmeden tek tek ölçülür. V18.5.11 bu sonuçlardan kısa dönem + saat/faz + uzun dönem uzman güvenini yeniden öğrenir.")
        _bt510_count=st.selectbox("Geçiş uzmanı test adedi",[10,25,50],index=0,key="v18510_transition_bt_count")
        if st.button("🧪 4 Geçiş Uzmanını Geçmişte Test Et",key="v18510_transition_bt_run"):
            with st.spinner("Dört Geçiş uzmanı walk-forward test ediliyor..."):
                _d510,_s510=transition_walkforward_v18510(df,test_count=_bt510_count,analysis_window=transition_window,size=transition_size)
            if _s510.empty:
                st.warning("Test sonucu üretilemedi.")
            else:
                st.session_state["v18510_transition_bt_summary"]=_s510
                st.session_state["v18510_transition_bt_detail"]=_d510
                st.dataframe(_s510,use_container_width=True,hide_index=True)
                _next_no511,_next_date511,_next_time511=next_draw_defaults(df)
                _w511,_m511=transition_three_memory_weights_v18511(_d510,target_time=_next_time511)
                st.session_state["v18511_transition_memory"]=_m511
                st.session_state["v18511_transition_weights"]=_w511
                if isinstance(_m511,pd.DataFrame) and not _m511.empty:
                    st.write("**🧠 V18.5.11 öğrenilen uzman güvenleri**")
                    st.dataframe(_m511,use_container_width=True,hide_index=True)
                with st.expander("Walk-forward ayrıntısı"):
                    st.dataframe(_d510.sort_values(["Çekiliş","Profil"],ascending=[False,True]),use_container_width=True,hide_index=True)
...
3 9 22 31 44 58 69""",
    ...
    st.header("⚡ Hızlı Çekiliş Ekle")
    st.success(
        "Sonuç metnini olduğu gibi yapıştır ve tek tuşa bas. "
        "Program çekiliş no + tarih/saat + 20 sayıyı otomatik okur, "
        "kontrol eder, GitHub veri.txt dosyasına kaydeder ve analizi yeniler."
    )

    default_no, default_date, default_time = next_draw_defaults(df)

    raw = st.text_area(
        "Çekiliş sonucunu yapıştır",
        height=330,
        placeholder=f"""Örnek:\nÇekiliş no: {default_no}\n{default_date} - {default_time}\n2\n5\n8\n11\n14\n... toplam 20 sayı\n\nİstersen yalnızca 20 sayıyı da boşlukla yapıştırabilirsin.""",
        key="v18511_quick_draw_text",
    )

    with st.expander("⚙️ Gerekirse çekiliş bilgilerini düzelt", expanded=False):
        ...
        manual_no = field_col1.number_input(
            ...
            step=1,
            key="v18511_quick_draw_no",
        )
        manual_date = field_col2.text_input(
            "Tarih",
            value=default_date,
            placeholder="07.08.2026",
            key="v18511_quick_draw_date",
        )
        manual_time = field_col3.text_input(
            "Saat",
            value=default_time,
            placeholder="20:57",
            key="v18511_quick_draw_time",
        )
        st.caption(
            "Tam çekiliş metninde no/tarih/saat varsa program onları kullanır. "
            "Yalnızca 20 sayı yapıştırırsan bu alanlar kullanılır."
        )

    def _quick_draw_row_v18511(raw_text):
        raw_text = str(raw_text or "").strip()
        if not raw_text:
            return None, "Metin boş."

        # 1) Kullanıcının buraya gönderdiği klasik blok biçimi.
        parsed = parse_draw_block(raw_text)
        if parsed:
            return parsed, None

        # 2) Tek satırlık standart veri biçimi.
        possible_lines = [line.strip() for line in raw_text.splitlines() if line.strip()]
        if len(possible_lines) == 1:
            parsed = parse_standard_line(possible_lines[0])
            if parsed:
                return parsed, None

        # 3) Daha toleranslı metadata okuma: "Çekiliş no", tarih/saat aynı
        # veya farklı satırlarda olabilir. Oyun sayıları metadata temizlendikten
        # sonra tam 20 farklı sayı olmak zorundadır.
        no_match = re.search(r"(?i)Çekiliş\s*no\s*[:#-]?\s*(\d+)", raw_text)
        dt_match = re.search(
            r"(\d{2}[./]\d{2}[./]\d{4})\s*(?:-|–|—)?\s*(\d{2}:\d{2})",
            raw_text,
        )
        nums = extract_exact_twenty_numbers(raw_text)
        if nums:
            if no_match and dt_match:
                return [
            ...
            date_ok = bool(re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", str(manual_date).strip()))
            time_ok = bool(re.fullmatch(r"\d{2}:\d{2}", str(manual_time).strip()))
            if date_ok and time_ok:
                return [
                    int(manual_no),
                    str(manual_date).strip(),
                    str(manual_time).strip(),
                    *nums,
                ], None

        # Teşhis için metadata temizlenmiş sayı adedini göster.
        cleaned = re.sub(r"(?i)Çekiliş\s*no\s*[:#-]?\s*\d+", " ", raw_text)
        cleaned = re.sub(r"\d{2}[./]\d{2}[./]\d{4}", " ", cleaned)
        cleaned = re.sub(r"\d{2}:\d{2}", " ", cleaned)
        found = [
            int(x) for x in re.findall(
                r"(?<!\d)(?:[1-9]|[1-7]\d|80)(?!\d)", cleaned
            )
        ]
        return None, (
            "Çekiliş okunamadı. 20 farklı oyun sayısı bulunmalı. "
            f"Şu an algılanan uygun sayı adedi: {len(found)}."
        )

    preview_row = None
    preview_error = None
    if raw.strip():
        preview_row, preview_error = _quick_draw_row_v18511(raw)

        if preview_row:
            raw_draw_no = int(preview_row[0])
            preview_row[0] = normalize_draw_number(preview_row[0], preview_row[1])

            try:
                datetime.strptime(
                    f"{preview_row[1]} {preview_row[2]}",
                    "%d.%m.%Y %H:%M",
                )
            except ValueError:
                preview_error = "Tarih veya saat geçersiz."
                preview_row = None

            if preview_row:
                nums = list(map(int, preview_row[3:]))
                if len(nums) != 20 or len(set(nums)) != 20 or any(n < 1 or n > 80 for n in nums):
                    preview_error = "Tam 20 farklı sayı olmalı ve tüm sayılar 1–80 arasında olmalı."
                    preview_row = None
                elif int(preview_row[0]) in set(df.Cekilis_No.astype(int)):
                    preview_error = f"Çekiliş #{int(preview_row[0])} zaten veri havuzunda mevcut."
                    preview_row = None
                else:
                    if int(preview_row[0]) != raw_draw_no:
                        st.info(f"Çekiliş numarası otomatik düzeltildi: {raw_draw_no} → {preview_row[0]}")
                    st.success(
                        f"Hazır: #{preview_row[0]} — {preview_row[1]} {preview_row[2]}"
                    )
                    st.write("20 sayı: **" + " - ".join(map(str, preview_row[3:])) + "**")

        if preview_error:
            st.error(preview_error)

    quick_disabled = preview_row is None
    if st.button(
        "⚡ Ekle + Kalıcı Kaydet + Analizi Yenile",
        type="primary",
        use_container_width=True,
        disabled=quick_disabled,
        key="v18511_quick_draw_save",
    ):
        candidate_df = merge_data(
            df, pd.DataFrame([preview_row], columns=COLS)
        )

        if len(candidate_df) != len(df) + 1:
            st.error(
                "Kayıt havuza eklenemedi. Çekiliş numarası veya veri başka bir kayıtla çakışıyor."
            )
        else:
            settings, settings_error = github_settings()
            if settings_error:
                # GitHub bağlantısı yoksa veri kaybolmasın; oturum havuzuna ekle.
                st.session_state.extra_df = merge_data(
                    st.session_state.extra_df,
                    pd.DataFrame([preview_row], columns=COLS),
                )
                st.warning(
                    "GitHub kalıcı kayıt bağlantısı bulunamadı. Çekiliş bu oturumun "
                    "geçici havuzuna eklendi; sayfa yenilenirse kalıcı olmayabilir."
                )
                st.cache_data.clear()
                st.rerun()
            else:
                try:
                    with st.spinner("Çekiliş kaydediliyor ve analiz yenileniyor..."):
                        update_github_file(
                            settings,
                            to_text(candidate_df),
                            f"Hızlı çekiliş eklendi: {int(preview_row[0])}",
                        )
                    # Aynı kayıt GitHub'dan yeniden geleceği için geçici havuzu temizle.
                    st.session_state.extra_df = pd.DataFrame(columns=COLS)
                    st.session_state["v18511_quick_last_saved"] = int(preview_row[0])
                    st.cache_data.clear()
                    st.rerun()
                except Exception as exc:
                    st.error(f"Kalıcı kayıt yapılamadı: {exc}")

    if st.session_state.get("v18511_quick_last_saved"):
        st.caption(
            f"✅ Son hızlı kayıt: #{st.session_state['v18511_quick_last_saved']}"
        )

    with st.expander("🧰 Yedek / eski yöntem", expanded=False):
        st.caption(
            "Sorun yaşarsan doğrulanmış yeni havuzu veri.txt olarak indirebilirsin. "
            "Normal kullanımda buna gerek yok."
        )
        if preview_row:
            backup_df = merge_data(df, pd.DataFrame([preview_row], columns=COLS))
            st.download_button(
                "Yedek veri.txt indir",
                data=to_text(backup_df).encode("utf-8"),
                file_name="veri.txt",
                mime="text/plain",
                key="v18511_quick_backup",
            )
