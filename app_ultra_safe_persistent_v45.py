...
# --------------------------- UI ---------------------------
st.title("🧬 Hızlı On — Yaşam Kimliği MASTER APP V4.5 — KALICI KAYIT FIX + 14 GÜN")
...
# Önceki yarım kalmış geçici yazımları temizle; tamamlanmış .zip dosyalarına dokunma.
for _stale_tmp in _CACHE14.glob("*.zip.tmp"):
    try:
        _stale_tmp.unlink()
    except Exception:
        pass
    ...
    if st.button("▶️ SONRAKİ GÜNÜ HESAPLA",use_container_width=True,key="master14_next_day_persistent",type="primary",disabled=(_progress14>=14)):
        ...
        if len(_idxs)!=217:
            ...
            try:
                ...
                # Geçici dosya final ZIP ile aynı klasörde ve açıkça .zip.tmp adında tutulur.
                _tmp=Path(str(_final_path) + ".tmp")
                _tmp.write_bytes(_dayzip)
                # Yazım gerçekten oluştu mu doğrula; sonra atomik olarak final ZIP'e taşı.
                if (not _tmp.exists()) or _tmp.stat().st_size == 0:
                    raise IOError(f"Geçici MASTER ZIP yazılamadı: {_tmp}")
                _tmp.replace(_final_path)
