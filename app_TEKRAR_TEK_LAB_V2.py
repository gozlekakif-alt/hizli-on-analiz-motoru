...
APP_VERSION = "TEKRAR TEK LAB V2 — Dondurulmuş Yapısal Sinyal"
...
try:
    ...
    st.error(f"Veri hazırlanamadı: {e}")
    st.stop()

st.success(f"Veri hazır: {len(df)} çekiliş")

st.header("1️⃣ Canlı davranış teşhisi")
neighbors=st.slider("Benzer geçmiş blok durumu",30,120,70,5)
live_k=st.slider("Canlı aday sayısı",3,10,7)
pred,table,info=block_predict_v2(eng,len(df),k=live_k,neighbors=neighbors)

a,b,c,d=st.columns(4)
a.metric("Blok sinyali",info["signal"])
b.metric("Kalibre güven",f"%{info['confidence']}")
c.metric("Benzer geçmiş",info["neighbors"])
d.metric("Ort. benzerlik",info["avg"])

st.write("**Son çekiliş blokları:**", info["runs"] if info["runs"] else "Yok")
st.write("**Davranış görülme oranları:**", " | ".join(f"{k}: %{v*100:.1f}" for k,v in info["event_rate"].items()))
st.write("**Dinamik kanal ağırlıkları:**", " | ".join(f"{k}: %{v*100:.1f}" for k,v in info["weights"].items()))
st.info(f"Blok Uzmanı {live_k} aday: " + " - ".join(map(str,pred)))
st.dataframe(table.head(30),use_container_width=True,hide_index=True)

st.header("2️⃣ V2 Sızıntısız Laboratuvar")
tests=st.select_slider("Test adedi",options=[100,250,500,750],value=250)

if st.button("🧪 V2 davranış motorunu test et",type="primary"):
    with st.spinner("DEVAM/BÜYÜME/KAYMA/DOĞUM ayrı ayrı geçmişten öğreniliyor..."):
        bt=backtest_v2(eng,min(tests,len(df)-120),neighbors=neighbors)
    sm=summarize_v2(bt)

    best=sm.iloc[0]
    c1,c2,c3=st.columns(3)
    c1.metric("En iyi koltuk sayısı",int(best["Koltuk"]))
    c2.metric("En iyi net avantaj",f"{best['Net Fark']:+.3f}")
    c3.metric("Top7 ortalama",f"{bt['Top7'].mean():.3f}/7",delta=f"{bt['Top7'].mean()-1.75:+.3f}")

    st.subheader("Kaç koltuk istediğinde gerçekten işe yarıyor?")
    st.dataframe(sm,use_container_width=True,hide_index=True)

    # Güven gerçekten anlamlı mı?
    bt2=bt.copy()
    bt2["Güven Grubu"]=pd.cut(bt2["Güven"],[-1,35,50,65,80,101],labels=["0-35","36-50","51-65","66-80","81+"])
    g=bt2.groupby("Güven Grubu",observed=True).agg(
        Test=("Top7","size"),
        Top7_Ort=("Top7","mean"),
        Güven_Ort=("Güven","mean"),
        Blokİçi_Ort=("Top7 gerçek blok içi","mean"),
    ).reset_index()
    g["Rastgele Top7"]=1.75
    g["Net Top7"]=g["Top7_Ort"]-1.75
    st.subheader("Güven kalibrasyonu")
    st.dataframe(g,use_container_width=True,hide_index=True)

    st.subheader("Çekiliş çekiliş denetim")
    ...
    st.download_button(
        ...
        file_name="blok_motor_lab_v2_test.csv",
        mime="text/csv",
    )



st.header("3️⃣ ESKİ BLOK ↔ YENİ BLOK — Walk-Forward Karşılaştırma")
st.caption(
    "Ana BLOK puanlaması değişmez. Yeni katman yalnız daha önce sonucu görülmüş "
    "çekilişlerin DEVAM/BÜYÜME/KAYMA/DOĞUM/Kanal Desteği verilerinden öğrenir."
)

if st.button("⚖️ 250 çekilişte ESKİ ↔ YENİ karşılaştır", type="primary"):
    with st.spinner("Geçmişten geleceğe karşılaştırma çalışıyor..."):
        cmp = compare_old_new_block(eng, tests=min(250, len(df)-120), neighbors=neighbors)

    valid = cmp[cmp["Eğitim aktif"] == True].copy()
    if valid.empty:
        st.error("Karşılaştırma için yeterli geçmiş oluşmadı.")
    else:
        a,b,c,d = st.columns(4)
        old_mean = valid["ESKİ Top10"].mean()
        new_mean = valid["YENİ Top10"].mean()
        a.metric("ESKİ Top10 ort.", f"{old_mean:.3f}")
        b.metric("YENİ Top10 ort.", f"{new_mean:.3f}", delta=f"{new_mean-old_mean:+.3f}")
        c.metric("ESKİ 5+", int((valid["ESKİ Top10"] >= 5).sum()))
        d.metric("YENİ 5+", int((valid["YENİ Top10"] >= 5).sum()))

        st.write(
            f"**6+** — Eski: {int((valid['ESKİ Top10']>=6).sum())} | "
            f"Yeni: {int((valid['YENİ Top10']>=6).sum())}   •   "
            f"**7+** — Eski: {int((valid['ESKİ Top10']>=7).sum())} | "
            f"Yeni: {int((valid['YENİ Top10']>=7).sum())}"
        )
        selected = evaluate_dynamic_selector(cmp)
        spoken = selected[selected["Seçici"] == "KONUŞ"].copy()

        st.subheader("🎯 Dinamik KONUŞ / PAS seçicisi")
        if spoken.empty:
            st.info("Bu testte seçici konuşma koşulu bulamadı.")
        else:
            s1, s2, s3, s4 = st.columns(4)
            s1.metric("KONUŞ", len(spoken))
            s2.metric("KONUŞ oranı", f"%{100*len(spoken)/max(len(valid),1):.1f}")
            s3.metric("KONUŞ Top10 ort.", f"{spoken['YENİ Top10'].mean():.3f}")
            s4.metric("KONUŞ 5+", int((spoken["YENİ Top10"] >= 5).sum()))
            st.write(
                f"**KONUŞ 6+**: {int((spoken['YENİ Top10']>=6).sum())}  •  "
                f"**KONUŞ 7+**: {int((spoken['YENİ Top10']>=7).sum())}  •  "
                f"**KONUŞ anlarında yeni-eski net fark**: {spoken['Fark'].mean():+.3f}"
            )

        st.dataframe(selected, use_container_width=True, hide_index=True)
        st.download_button(
            "⬇️ ESKİ-YENİ-SEÇİCİ tam CSV indir",
            selected.to_csv(index=False).encode("utf-8-sig"),
            file_name="BLOK_TEK_LAB_ESKI_YENI_SECICI_250.csv",
            mime="text/csv",
        )
        st.download_button(
            "⬇️ ESKİ-YENİ karşılaştırma CSV indir",
            cmp.to_csv(index=False).encode("utf-8-sig"),
            file_name="BLOK_ESKI_YENI_250_WALKFORWARD.csv",
            mime="text/csv",
        )

st.header("4️⃣ SABİT BASELINE — Adli 1–15 Teşhis")
st.caption(
    "Bu bölüm motorun puanını DEĞİŞTİRMEZ. Aynı V2 sıralamasında ilk 15 adayı, "
    "kanal puanlarını ve gerçek sonucu kaydeder. Karşılaştırma için test adedini 250 bırak."
)

if st.button("🔬 250 çekiliş adli teşhis üret", type="secondary"):
    fixed_tests = min(250, len(df) - 120)
    with st.spinner("Mevcut BLOK motoru değiştirilmeden ilk 15 sıra kaydediliyor..."):
        adli_draw, adli_rank = forensic_backtest_v2(
            eng, tests=fixed_tests, neighbors=neighbors
        )

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Sabit test", len(adli_draw))
    c2.metric("Top10 ort.", f"{adli_draw['Top10'].mean():.3f}")
    c3.metric("Top10 rastgele", "2.500")
    c4.metric("Top10 net", f"{adli_draw['Top10'].mean()-2.5:+.3f}")

    st.subheader("Yüksek isabet olayları")
    high = adli_draw[adli_draw["Top10"] >= 5].sort_values(
        ["Top10", "Çekiliş"], ascending=[False, False]
    )
    st.dataframe(high, use_container_width=True, hide_index=True)

    st.download_button(
        "⬇️ 250 çekiliş özet CSV indir",
        adli_draw.to_csv(index=False).encode("utf-8-sig"),
        file_name="BLOK_BASELINE_250_OZET.csv",
        mime="text/csv",
    )
    st.download_button(
        "⬇️ 1–15 sıra adli CSV indir",
        adli_rank.to_csv(index=False).encode("utf-8-sig"),
        file_name="BLOK_BASELINE_250_ADLI_1_15.csv",
        mime="text/csv",
    )

st.warning(
    "Kabul kuralı: Bu uzman final META'ya ancak belirli sinyal/güven koşullarında "
    "rastgele beklentiyi tekrarlanabilir biçimde aşarsa girecek. Gerekirse 0 koltuk alacak."
)



# ===================================================================
# 5) UZMAN BİRLEŞİM MOTORU — dinamik koltuk + dinamik kupon boyu
# ===================================================================

def _rank_matrix_from_orders(orders):
    """4 uzmanın 1-80 sırasını 0-1 güç matrisine çevir."""
    names = list(orders.keys())
    cols = []
    for name in names:
        order = orders[name]
        r = np.zeros(80, dtype=float)
        for pos, n in enumerate(order):
            r[int(n)-1] = 1.0 - (pos / 79.0)
        cols.append(r)
    A = np.column_stack(cols)
    top10_consensus = (A >= (1.0 - 9.0/79.0)).sum(axis=1) / 4.0
    top20_consensus = (A >= (1.0 - 19.0/79.0)).sum(axis=1) / 4.0
    return np.column_stack([
        A,
        top10_consensus,
        top20_consensus,
...
def _expert_orders(engine, t, target_time, neighbors=70):
    """BLOK + GEÇİŞ + DİNLENME + SAAT/FAZ uzman sıraları."""
    _, block_table, block_info = block_predict_v2(
        engine, t, k=10, neighbors=neighbors
    )
    block_order = block_table["Sayı"].astype(int).tolist()

    X = engine.build(t, target_time)
    fmap = {n:i for i,n in enumerate(FEATURE_NAMES)}

    trans_cols = [
        fmap["V18 Geçiş — Dengeli"],
        fmap["V18 Geçiş — Tekrar ağırlıklı"],
        fmap["V18 Geçiş — Yerine geçme ağırlıklı"],
        fmap["V18 Geçiş — Saat ve sıcaklık"],
    ]
    transition = X[:, trans_cols].mean(axis=1)

    rest = X[:, fmap["V18 Uzman — Dinlenmiş Dönüş"]]

    clock = (
        0.75 * X[:, fmap["V18 Uzman — Saat/Faz Genel Güç"]]
        + 0.25 * X[:, fmap["Aynı saat"]]
    )

    # Eksik alt uzmanlar: mevcut sızıntısız FEATURE kanallarından bağımsız sıralama.
    repeat = (
        0.55 * X[:, fmap["Son çekilişte"]]
        + 0.30 * X[:, fmap["Devam serisi"]]
        - 0.25 * X[:, fmap["Tekrar karşı sinyali"]]
        + 0.20 * X[:, fmap["V18 Geçiş — Tekrar ağırlıklı"]]
    )

    # Birliktelik için mevcut motorun pair_strength bilgisini taşıyan genel/saat kanalı
    # ile blok-komşuluk ve geçiş desteğini bir araya getiriyoruz.
    pair = (
        0.45 * X[:, fmap["V18 Uzman — Saat/Faz Genel Güç"]]
        + 0.30 * X[:, fmap["Son 5 blok komşuluğu"]]
        + 0.25 * X[:, fmap["Geçiş olasılığı"]]
    )

    band = (
        0.60 * X[:, fmap["10'luk bölge basıncı"]]
        + 0.40 * X[:, fmap["Son 10 bölge eğimi"]]
    )

    neighbor = (
        0.35 * X[:, fmap["Komşu ±1"]]
        + 0.25 * X[:, fmap["Komşu ±2"]]
        + 0.40 * X[:, fmap["Son 5 blok komşuluğu"]]
    )

    orders = {
        "BLOK": block_order,
        "GEÇİŞ": (np.argsort(-transition) + 1).astype(int).tolist(),
        "DİNLENME": (np.argsort(-rest) + 1).astype(int).tolist(),
        "SAAT/FAZ": (np.argsort(-clock) + 1).astype(int).tolist(),
        "TEKRAR": (np.argsort(-repeat) + 1).astype(int).tolist(),
        "BİRLİKTELİK": (np.argsort(-pair) + 1).astype(int).tolist(),
        "BANT": (np.argsort(-band) + 1).astype(int).tolist(),
        "KOMŞU": (np.argsort(-neighbor) + 1).astype(int).tolist(),
    }
    return orders, block_info


def _expert_recent_edges(expert_rows, upto_i, window=80, k=10):
    """
    Her uzmanın yalnız geçmişteki Top-k performansından net avantajını ölçer.
    Rastgele Top-k beklentisi k*0.25'tir. Negatif uzman 0 ağırlık alabilir.
    """
    names = list(expert_rows[0]["orders"].keys())
    lo = max(0, upto_i-window)
    hist = expert_rows[lo:upto_i]
    edges = {}
    for e in names:
        vals = []
        for r in hist:
            top = set(r["orders"][e][:k])
            hit = len(top & r["actual"])
            vals.append(hit - 0.25*k)
        edges[e] = float(np.mean(vals)) if vals else 0.0
    return edges

def _expert_weights_from_edges(edges):
    """
    Negatif uzmanı bastır, pozitif uzmanı büyüt.
    Tamamen sıfıra düşen durumda eşit ağırlığa geri dön.
    """
    names = list(edges.keys())
    raw = np.array([max(0.0, float(edges[n])) for n in names], dtype=float)
    if raw.sum() <= 1e-12:
        raw[:] = 1.0
    # Çok küçük farkların sistemi ele geçirmesini önle
    raw = np.sqrt(raw + 1e-6)
    raw /= raw.sum()
    return {n: float(w) for n, w in zip(names, raw)}

def _weighted_rank_matrix(orders, weights):
    names = list(orders.keys())
    cols = []
    for name in names:
        order = orders[name]
        ...
        for pos, n in enumerate(order):
            r[int(n)-1] = 1.0 - (pos / 79.0)
        cols.append(r * float(weights.get(name, 0.0)))
    A = np.column_stack(cols)

    # Uzman ağırlıklı konsensus
    top10 = np.zeros(80, dtype=float)
    top20 = np.zeros(80, dtype=float)
    denom = max(sum(weights.values()), 1e-9)
    for j, name in enumerate(names):
        w = float(weights.get(name, 0.0))
        raw = A[:,j] / max(w,1e-9) if w > 0 else np.zeros(80)
        top10 += w * (raw >= (1.0 - 9.0/79.0))
        top20 += w * (raw >= (1.0 - 19.0/79.0))
    top10 /= denom
    top20 /= denom

    weighted_sum = A.sum(axis=1)
    weighted_max = A.max(axis=1)
    weighted_std = A.std(axis=1)
    return np.column_stack([A, top10, top20, weighted_sum, weighted_max, weighted_std])

def _selector_features_from_row(row):
    """
    KONUŞ/PAS için sonuçtan bağımsız mevcut durum özellikleri.
    """
    vals = [
        float(row["Kupon boyu"]),
        float(row.get("BLOK koltuk",0)),
        float(row.get("GEÇİŞ koltuk",0)),
        float(row.get("DİNLENME koltuk",0)),
        float(row.get("SAAT/FAZ koltuk",0)),
        float(row.get("TEKRAR koltuk",0)),
        float(row.get("BİRLİKTELİK koltuk",0)),
        float(row.get("BANT koltuk",0)),
        float(row.get("KOMŞU koltuk",0)),
    ]
    return vals

def _walkforward_speak_probability(rows, min_hist=120, window=220):
    """
    Yalnız daha önce sonucu görülmüş OOS satırlardan mevcut kuponun
    pozitif net üretme olasılığını tahmin eder.
    """
    if len(rows) < min_hist:
        return np.nan
    hist = rows[-window:]
    X = np.array([_selector_features_from_row(r) for r in hist], dtype=float)
    y = np.array([1 if float(r["Net"]) > 0 else 0 for r in hist], dtype=int)
    if len(np.unique(y)) < 2:
        return np.nan
    model = make_pipeline(
        StandardScaler(),
        LogisticRegression(max_iter=400, C=0.20)
    )
    model.fit(X, y)
    # current row will be supplied externally after seat allocation
    return model

def unified_walkforward(engine, data_df, history_start=220, meta_window=180,
                        k_history=120, min_meta_history=80, neighbors=70):
    """
    Sızıntısız birleşim:
    - Her hedef çekilişte 4 uzman yalnız geçmişi görür.
    - META sıralayıcı yalnız daha önce sonucu belli çekilişlerden eğitilir.
    - Kupon boyu 3..10 arasından geçmiş Top-k net avantajına göre seçilir.
    - Rejim kapısı yalnız geçmiş performansın iki ardışık 40'lık penceresine bakar.
    """
    expert_rows = []
    start = max(220, int(history_start))

    # 1) Uzman özelliklerini kronolojik çıkar.
    for t in range(start, len(engine.A)):
        tt = data_df.iloc[t]["DT"].strftime("%H:%M")
        orders, binfo = _expert_orders(engine, t, tt, neighbors=neighbors)
        # İlk kayıt ham uzman sıralarını taşır; ağırlık yalnız geçmiş performanstan
        # meta tahmin anında hesaplanacaktır.
        F = _rank_matrix_from_orders(orders)
        actual = set((np.where(engine.A[t] == 1)[0] + 1).tolist())
        y = np.array([1 if n in actual else 0 for n in range(1,81)], dtype=int)
        expert_rows.append({
            "t": t,
            "draw": int(engine.draw_nos[t]),
            "orders": orders,
            "F": F,
            "y": y,
            "actual": actual,
    ...
    # 2) Birleşik META walk-forward tahminleri.
    meta = []
    for i in range(min_meta_history, len(expert_rows)):
        lo = max(0, i-meta_window)
        train_feats = []
        train_y = []
        for j in range(lo, i):
            ed = _expert_recent_edges(expert_rows, j, window=80, k=10)
            wt = _expert_weights_from_edges(ed)
            train_feats.append(_weighted_rank_matrix(expert_rows[j]["orders"], wt))
            train_y.append(expert_rows[j]["y"])
        Xtr = np.vstack(train_feats)
        ytr = np.hstack(train_y)

        model = make_pipeline(
            StandardScaler(),
            LogisticRegression(max_iter=350, C=0.08)
        )
        model.fit(Xtr, ytr)

        cur = expert_rows[i]
        cur_edges = _expert_recent_edges(expert_rows, i, window=80, k=10)
        cur_weights = _expert_weights_from_edges(cur_edges)
        Fcur = _weighted_rank_matrix(cur["orders"], cur_weights)
        p = model.predict_proba(Fcur)[:,1]
        order = np.argsort(-p)
        hits = {}
        for k in range(3,11):
            nums = set((order[:k] + 1).astype(int).tolist())
            hits[k] = len(nums & cur["actual"])
        ...
        meta.append({
            ...
            "orders": cur["orders"],
            "actual": cur["actual"],
            "expert_edges": cur_edges,
            "expert_weights": cur_weights,
            "block_info": cur["block_info"],
        })

    # 3) Kupon boyunu geçmişteki marjinal net avantaja göre dinamik seç.
    rows = []
    ks = list(range(3,11))
    for i, cur in enumerate(meta):
        if i < 60:
            continue
        lo = max(0, i-k_history)
        past = meta[lo:i]

        means = []
        for k in ks:
            vals = [r["hits"][k] - 0.25*k for r in past]
            means.append(float(np.mean(vals)))
        best_pos = int(np.argmax(means))
        k = ks[best_pos]

        # Rejim kapısı: yalnız geçmiş OOS performansı.
        prior_rows = rows[-80:]
        recent40 = float(np.mean([r["Net"] for r in prior_rows[-40:]])) if len(prior_rows) >= 40 else np.nan
        prev40 = float(np.mean([r["Net"] for r in prior_rows[-80:-40]])) if len(prior_rows) >= 80 else np.nan
        # KONUŞ kararı biraz aşağıda, koltuk dağılımı belli olduktan sonra
        # yalnız geçmiş OOS satırlardan eğitilen ayrı seçici tarafından verilir.
        speak = False

        nums = (cur["order"][:k] + 1).astype(int).tolist()
        hit = int(cur["hits"][k])
        net = float(hit - 0.25*k)

        # Seçilen sayıların hangi uzman tarafından taşındığını belirle.
        expert_names = list(cur["orders"].keys())
        rank_strength = {}
        for e in expert_names:
            order_e = cur["orders"][e]
            posmap = {int(n):pos for pos,n in enumerate(order_e)}
            rank_strength[e] = {
                n: 1.0 - posmap.get(n,79)/79.0 for n in nums
            }

        attribution = {e:0 for e in expert_names}
        number_owner = {}
        for n in nums:
            owner = max(expert_names, key=lambda e: rank_strength[e][n])
            attribution[owner] += 1
            number_owner[n] = owner
        ...
        row_dict = {
            ...
            "Net": round(net,4),
            "KONUŞ/PAS": "PAS",
            "Son40 net": round(recent40,4) if np.isfinite(recent40) else np.nan,
            "Önceki40 net": round(prev40,4) if np.isfinite(prev40) else np.nan,
            "BLOK koltuk": attribution["BLOK"],
            "GEÇİŞ koltuk": attribution["GEÇİŞ"],
            "DİNLENME koltuk": attribution["DİNLENME"],
            "SAAT/FAZ koltuk": attribution["SAAT/FAZ"],
            "TEKRAR koltuk": attribution["TEKRAR"],
            "BİRLİKTELİK koltuk": attribution["BİRLİKTELİK"],
            "BANT koltuk": attribution["BANT"],
            "KOMŞU koltuk": attribution["KOMŞU"],
            "Kupon": " - ".join(map(str, nums)),
            "Tutan": " - ".join(map(str, sorted(set(nums)&cur["actual"]))) or "Yok",
        }

        # Uzman son-performans ağırlıklarını da rapora yaz.
        for e, w in cur.get("expert_weights", {}).items():
            row_dict[f"{e} ağırlık"] = round(float(w), 6)
        for e, ed in cur.get("expert_edges", {}).items():
            row_dict[f"{e} son80 net"] = round(float(ed), 6)

        selector_model = _walkforward_speak_probability(rows, min_hist=120, window=220)
        speak_p = np.nan
        if selector_model is not None and not isinstance(selector_model, float):
            xcur = np.array([_selector_features_from_row(row_dict)], dtype=float)
            speak_p = float(selector_model.predict_proba(xcur)[0,1])

        # 0.65 keşif eşiği: laboratuvar aşamasında. Canlı ileri testte sabit kalacak.
        speak = bool(np.isfinite(speak_p) and speak_p >= 0.65)
        row_dict["KONUŞ olasılığı"] = round(speak_p, 6) if np.isfinite(speak_p) else np.nan
        row_dict["KONUŞ/PAS"] = "KONUŞ" if speak else "PAS"
        rows.append(row_dict)

    return pd.DataFrame(rows), expert_rows, meta

def unified_next_coupon(engine, data_df, expert_rows, meta_rows, wf_rows, neighbors=70):
    """Son bilinen çekilişten SONRAKİ hedef için dinamik kupon."""
    t = len(engine.A)
    target_dt = next_draw_dt(data_df.iloc[-1]["DT"])
    target_time = target_dt.strftime("%H:%M")
    orders, binfo = _expert_orders(engine, t, target_time, neighbors=neighbors)

    # Güncel uzman ağırlıkları yalnız geçmiş uzman başarısından.
    cur_edges = _expert_recent_edges(expert_rows, len(expert_rows), window=80, k=10)
    cur_weights = _expert_weights_from_edges(cur_edges)
    Fcur = _weighted_rank_matrix(orders, cur_weights)

    # Son 180 geçmiş uzman-hedef örneğiyle kalibre edilmiş eğitim.
    hist_start = max(0, len(expert_rows)-180)
    Xparts, yparts = [], []
    for j in range(hist_start, len(expert_rows)):
        ed = _expert_recent_edges(expert_rows, j, window=80, k=10)
        wt = _expert_weights_from_edges(ed)
        Xparts.append(_weighted_rank_matrix(expert_rows[j]["orders"], wt))
        yparts.append(expert_rows[j]["y"])
    Xtr = np.vstack(Xparts)
    ytr = np.hstack(yparts)
    model = make_pipeline(
        StandardScaler(),
        LogisticRegression(max_iter=300, C=0.08)
    )
    model.fit(Xtr, ytr)
    p = model.predict_proba(Fcur)[:,1]
    order = np.argsort(-p)

    # Kupon boyu: son 120 OOS meta tahmininde en iyi net Top-k.
    ks = list(range(3,11))
    recent_meta = meta_rows[-120:]
    means = []
    for k in ks:
        means.append(float(np.mean([r["hits"][k]-0.25*k for r in recent_meta])))
    k = ks[int(np.argmax(means))]
    nums = (order[:k]+1).astype(int).tolist()

    recent40 = float(wf_rows.tail(40)["Net"].mean()) if len(wf_rows) >= 40 else np.nan
    prev40 = float(wf_rows.iloc[-80:-40]["Net"].mean()) if len(wf_rows) >= 80 else np.nan
    # Güncel KONUŞ/PAS: geçmiş OOS koltuk dağılımlarından pozitif net olasılığı.
    speak = False

    # Dinamik koltuk dağılımı.
    expert_names = list(orders.keys())
    attribution = {e:0 for e in expert_names}
    ownership = {}
    posmaps = {e:{int(n):pos for pos,n in enumerate(orders[e])} for e in expert_names}
    for n in nums:
        owner = max(
            expert_names,
            key=lambda e: 1.0 - posmaps[e].get(n,79)/79.0
        )
        attribution[owner] += 1
        ownership[n] = owner
    ...
    current_row = {
        "Kupon boyu": k,
        "BLOK koltuk": attribution.get("BLOK",0),
        "GEÇİŞ koltuk": attribution.get("GEÇİŞ",0),
        "DİNLENME koltuk": attribution.get("DİNLENME",0),
        "SAAT/FAZ koltuk": attribution.get("SAAT/FAZ",0),
        "TEKRAR koltuk": attribution.get("TEKRAR",0),
        "BİRLİKTELİK koltuk": attribution.get("BİRLİKTELİK",0),
        "BANT koltuk": attribution.get("BANT",0),
        "KOMŞU koltuk": attribution.get("KOMŞU",0),
    }
    selector_model = _walkforward_speak_probability(
        wf_rows.to_dict("records"), min_hist=120, window=220
    )
    speak_p = np.nan
    if selector_model is not None and not isinstance(selector_model, float):
        speak_p = float(selector_model.predict_proba(
            np.array([_selector_features_from_row(current_row)], dtype=float)
        )[0,1])
    speak = bool(np.isfinite(speak_p) and speak_p >= 0.65)

    return {
        "target_draw": int(data_df.iloc[-1]["Cekilis_No"]) + 1,
...
st.header("5️⃣ UZMAN BİRLEŞİM V6.5 — Kalibre Uzman / Dinamik Kupon / Seçici")
st.caption(
    "BLOK + GEÇİŞ + DİNLENME + SAAT/FAZ + TEKRAR + BİRLİKTELİK + BANT + KOMŞU tek kupon üretir. "
    "Her uzman 0-N koltuk alabilir; kupon boyu 3-10 arasında motor tarafından seçilir. "
    "KONUŞ/PAS kararı yalnız geçmiş walk-forward performansından gelir; gelecek sonuç kullanılmaz."
)

if st.button("🧠 Birleşim motorunu 500 elde test et ve güncel kuponu hazırla", type="primary"):
    with st.spinner("4 uzman + META + dinamik koltuk walk-forward testi çalışıyor..."):
        uwf, uexpert, umeta = unified_walkforward(
            eng, df, history_start=max(220, len(df)-700),
            meta_window=180, k_history=120, min_meta_history=80,
            neighbors=neighbors
        )

    if uwf.empty:
        st.error("Birleşim testi için yeterli geçmiş yok.")
    else:
        last250 = uwf.tail(min(250, len(uwf))).copy()
        spoken = last250[last250["KONUŞ/PAS"]=="KONUŞ"].copy()

        a,b,c,d = st.columns(4)
        a.metric("Son 250 ort. kupon boyu", f"{last250['Kupon boyu'].mean():.2f}")
        a2 = float(last250["İsabet"].mean())
        b.metric("Son 250 ort. isabet", f"{a2:.3f}")
        c.metric("KONUŞ olayı", len(spoken))
        if len(spoken):
            d.metric("KONUŞ ort. net", f"{spoken['Net'].mean():+.3f}")
        else:
            d.metric("KONUŞ ort. net", "—")

        st.write(
            f"**Son 250 maksimum isabet:** {int(last250['İsabet'].max())}  •  "
            f"**5+ olay:** {int((last250['İsabet']>=5).sum())}  •  "
            f"**7+ olay:** {int((last250['İsabet']>=7).sum())}  •  "
            f"**10/10 olay:** {int(((last250['İsabet']==10)&(last250['Kupon boyu']==10)).sum())}"
        )

        st.subheader("Walk-forward sonuçları")
        st.dataframe(last250.sort_values("Çekiliş", ascending=False),
                     use_container_width=True, hide_index=True)

        current = unified_next_coupon(
            eng, df, uexpert, umeta, uwf, neighbors=neighbors
        )
        st.subheader("🎯 Güncel birleşik karar")
        if current["speak"]:
            st.success(
                f"KONUŞ — #{current['target_draw']} / {current['target_time']} • "
                f"{current['k']} sayılık kupon"
            )
            st.markdown("### " + " - ".join(map(str,current["nums"])))
        else:
            st.warning(
                f"PAS — #{current['target_draw']} / {current['target_time']} • "
                "motorun son dönem avantajı konuşma eşiğinde değil."
            )

        seats = current["seats"]
        if np.isfinite(current.get("speak_p", np.nan)):
            st.write(f"**KONUŞ olasılığı:** %{100*current['speak_p']:.1f}")

        st.write(
            "**Koltuk dağılımı:** "
            f"BLOK {seats['BLOK']} + GEÇİŞ {seats['GEÇİŞ']} + "
            f"DİNLENME {seats['DİNLENME']} + SAAT/FAZ {seats['SAAT/FAZ']} + "
            f"TEKRAR {seats['TEKRAR']} + BİRLİKTELİK {seats['BİRLİKTELİK']} + "
            f"BANT {seats['BANT']} + KOMŞU {seats['KOMŞU']}"
        )

        st.download_button(
            "⬇️ Birleşim walk-forward CSV indir",
            uwf.to_csv(index=False).encode("utf-8-sig"),
            file_name="UZMAN_BIRLESIM_WALKFORWARD.csv",
            mime="text/csv",
        )
...
# ===================================================================
# 7) TEKRAR TEK LAB — yüksek skor değil, doğrulanmış sayı sinyali
# ===================================================================

def _repeat_features(engine, t):
    """
    Yalnız geçmiş A[:t] ile son çekilişte bulunan 20 sayı için
    TEKRAR'a özel ölçümler üretir.
    """
    if t < 40:
        return pd.DataFrame()
    ...
    # Geçmiş ardışık geçişlerde genel tekrar tabanı
    overlaps = []
    for j in range(1, t):
        if engine.draw_nos[j] == engine.draw_nos[j-1] + 1:
            overlaps.append(int(np.sum(A[j] * A[j-1])))
    global_repeat_per_number = (float(np.mean(overlaps[-200:])) / 20.0) if overlaps else 0.25

    for idx in prev_idx:
        n = int(idx + 1)

        # Uzun dönem kişisel tekrar
        cases = hits = 0
        ...
        p_long = (hits + 5*global_repeat_per_number) / max(cases + 5, 1)
        ...
        # Son 5 / 20 görünüm oranı
        f5 = float(A[max(0,t-5):t, idx].mean())
        f20 = float(A[max(0,t-20):t, idx].mean())

        # Son çekilişin diğer 19 sayısıyla birlikte-gelme lift desteği
        ...
        # Kaynak->aynı sayı taşıma geçiş desteği
        trans_same = 0.0
        if t >= 2:
            TT = engine.cumT[t-1].astype(float)
            SS = engine.cumS[t-1].astype(float)
            if SS[idx] > 0:
                trans_same = float(TT[idx, idx] / SS[idx])

        # Aynı saat diliminde tekrar eğilimi
        ...
        rows.append({
            ...
            "f20": f20,
            "pair_lift": pair_lift,
            "trans_same": trans_same,
            "cases": cases,
            "recent_cases": recent_cases,
            "hour_cases": hour_cases,
            "global_repeat": global_repeat_per_number,
        })
    return pd.DataFrame(rows)


def repeat_signal_score_table(engine, t):
    """
    TEKRAR uzmanı için tek sayı skoru.
    Yüksek skor tek başına karar değildir; kalibrasyon için kullanılır.
    """
    d = _repeat_features(engine, t)
    if d.empty:
        return d
    ...
    d["long_n"] = mm(d["p_long"])
    d["hour_n"] = mm(d["p_hour"])
    d["pair_n"] = mm(d["pair_lift"])
    d["trans_n"] = mm(d["trans_same"])

    # Uzun seri cezalı; 1-2 seri nötr/iyi
    d["streak_factor"] = d["streak"].apply(lambda s: 1.0 if s <= 2 else max(0.55, 1.0 - 0.14*(s-2)))
    ...
    d["TEKRAR skor"] = np.clip(raw, 0, 1)
    d = d.sort_values(["TEKRAR skor","p_recent","p_long"], ascending=False).reset_index(drop=True)
    d["Sıra"] = np.arange(1, len(d)+1)
    return d


def repeat_walkforward_lab(engine, start_t=140):
    """
    Her hedefte yalnız geçmişi görür.
    Tüm son-çekiliş sayıları aday olarak kaydedilir; sonra skor kalibrasyonu yapılır.
    """
    rows = []
    for t in range(max(140, int(start_t)), len(engine.A)):
        if engine.draw_nos[t] != engine.draw_nos[t-1] + 1:
            continue
        tab = repeat_signal_score_table(engine, t)
        actual = set((np.where(engine.A[t] == 1)[0] + 1).tolist())
        for _, r in tab.iterrows():
            ...
            rows.append({
                ...
                "Sayı": n,
                "Sıra": int(r["Sıra"]),
                "TEKRAR skor": float(r["TEKRAR skor"]),
                "p_recent": float(r["p_recent"]),
                "p_long": float(r["p_long"]),
                "p_hour": float(r["p_hour"]),
                "pair_lift": float(r["pair_lift"]),
                "trans_same": float(r["trans_same"]),
                "streak": int(r["streak"]),
                "cases": int(r["cases"]),
                "recent_cases": int(r["recent_cases"]),
                "hour_cases": int(r["hour_cases"]),
                "Doğru": int(n in actual),
            })
    return pd.DataFrame(rows)


def repeat_calibration_report(wf):
    """
    Skor dilimlerini ve rank katmanlarını zaman parçalarıyla ölçer.
    Amaç: yüksek skor gerçekten doğru mu?
    """
    if wf.empty:
    ...
    d = wf.copy()
    # Kronolojik üç bölüm
    qs = np.quantile(d["Index"].unique(), [0, 1/3, 2/3, 1])
    def era(x):
        if x <= qs[1]: return "ERKEN"
        if x <= qs[2]: return "ORTA"
        return "SON"
    d["Dönem"] = d["Index"].map(era)

    # Skor decile
    try:
        d["Skor dilimi"] = pd.qcut(d["TEKRAR skor"], 10, duplicates="drop")
    except Exception:
        d["Skor dilimi"] = pd.cut(d["TEKRAR skor"], bins=10)

    score_summary = (
        d.groupby("Skor dilimi", observed=True)
         .agg(Aday=("Doğru","size"), Doğru=("Doğru","sum"),
              Doğruluk=("Doğru","mean"),
              Ort_Skor=("TEKRAR skor","mean"))
         .reset_index()
         .sort_values("Ort_Skor", ascending=False)
    )
    score_summary["Net_vs_%25"] = score_summary["Doğruluk"] - 0.25
    ...
    for k in [1,2,3,4,5,6]:
        ...
        row = {
            "TopK": k,
            "Aday": len(sub),
            "Doğruluk": sub["Doğru"].mean(),
            "Net_vs_%25": sub["Doğru"].mean()-0.25
        }
        for er in ["ERKEN","ORTA","SON"]:
            s = sub[sub["Dönem"]==er]
            row[er] = s["Doğru"].mean() if len(s) else np.nan
        rank_rows.append(row)

    return score_summary, pd.DataFrame(rank_rows)


def repeat_strict_selector_from_history(wf, target_index, current_tab):
    """
    Dinamik 0/1/2/3... çıkış.
    Yalnız geçmiş walk-forward adaylarından öğrenir.
    Bir rank/score bölgesi en az 60 geçmiş adayda >= %36 doğruluk
    ve son iki yarıda >= %32 doğruluk gösteriyorsa geçer.
    """
    hist = wf[wf["Index"] < target_index].copy()
    if len(hist) < 500:
        return [], pd.DataFrame()
    ...
    for _, r in current_tab.iterrows():
        rank = int(r["Sıra"])
        score = float(r["TEKRAR skor"])

        # Benzer rank + yakın skor bandı
        h = hist[(hist["Sıra"] == rank) & (np.abs(hist["TEKRAR skor"] - score) <= 0.08)].copy()
        if len(h) < 60:
            # biraz genişlet
            h = hist[(hist["Sıra"] == rank) & (np.abs(hist["TEKRAR skor"] - score) <= 0.12)].copy()
        ...
        if len(h) >= 60:
            ...
            mid = len(h)//2
            a1 = float(h.iloc[:mid]["Doğru"].mean()) if mid else np.nan
            a2 = float(h.iloc[mid:]["Doğru"].mean()) if len(h)-mid else np.nan
            acc = float(h["Doğru"].mean())
            ok = bool(acc >= 0.36 and a1 >= 0.32 and a2 >= 0.32)

        if ok:
            selected.append(int(r["Sayı"]))

        details.append({
            "Sayı": int(r["Sayı"]),
            "Sıra": rank,
            "TEKRAR skor": round(score,4),
            "Benzer geçmiş aday": int(len(h)),
            "Geçmiş doğruluk": round(acc,4) if np.isfinite(acc) else np.nan,
            "İlk yarı": round(a1,4) if np.isfinite(a1) else np.nan,
            "Son yarı": round(a2,4) if np.isfinite(a2) else np.nan,
            "KONUŞ": bool(ok),
        })

    return sorted(set(selected)), pd.DataFrame(details)


def repeat_strict_walkforward(engine, start_t=140):
    """
    Önce tüm aday walk-forward'ı oluşturur.
    Sonra her çekilişte yalnız daha eski satırlardan seçim yapar.
    """
    wf = repeat_walkforward_lab(engine, start_t=start_t)
    if wf.empty:
        return pd.DataFrame(), wf

    rows = []
    for t in sorted(wf["Index"].unique()):
        curtab = repeat_signal_score_table(engine, int(t))
        picks, detail = repeat_strict_selector_from_history(wf, int(t), curtab)
        actual = set((np.where(engine.A[int(t)] == 1)[0] + 1).tolist())
        hit = len(set(picks) & actual)
        rows.append({
            "Index": int(t),
            "Çekiliş": int(engine.draw_nos[int(t)]),
            "Kupon boyu": len(picks),
            "İsabet": hit,
            "Tam isabet": bool(len(picks)>0 and hit==len(picks)),
            "Karar": "KONUŞ" if picks else "PAS",
            "Tahmin": " - ".join(map(str,picks)) if picks else "",
            "Tutan": " - ".join(map(str, sorted(set(picks)&actual))) if picks else "",
        })
    return pd.DataFrame(rows), wf


st.header("7️⃣ TEKRAR TEK LAB — Sadece doğrulanmış tekrar adayları")
st.caption(
    "Yüksek TEKRAR skoru tek başına yeterli değildir. "
    "Her sayı geçmiş walk-forward'da kendi rank+skor bölgesinin gerçek doğruluğuyla kalibre edilir. "
    "Kanıt yoksa 0 sayı / PAS."
)

if st.button("🔁 TEKRAR motorunu tek başına test et", type="primary"):
    with st.spinner("TEKRAR walk-forward kalibrasyonu çalışıyor..."):
        strict_df, repeat_wf = repeat_strict_walkforward(eng, start_t=140)
        score_sum, rank_sum = repeat_calibration_report(repeat_wf)

    st.subheader("TEKRAR skor kalibrasyonu")
    st.dataframe(score_sum, use_container_width=True, hide_index=True)

    st.subheader("Top-K gerçek doğruluğu")
    st.dataframe(rank_sum, use_container_width=True, hide_index=True)

    active = strict_df[strict_df["Kupon boyu"] > 0].copy()
    full = active[active["Tam isabet"] == True].copy()

    c1,c2,c3,c4 = st.columns(4)
    c1.metric("KONUŞ", len(active))
    c2.metric("Toplam verilen sayı", int(active["Kupon boyu"].sum()) if len(active) else 0)
    c3.metric(
        "Sayı doğruluğu",
        f"%{100*active['İsabet'].sum()/max(active['Kupon boyu'].sum(),1):.1f}"
    )
    c4.metric("Tam kolon", len(full))

    if len(full):
        st.subheader("Tam isabetli TEKRAR olayları")
        st.dataframe(full.sort_values("Çekiliş", ascending=False),
                     use_container_width=True, hide_index=True)

    st.subheader("Son 250 TEKRAR kararı")
    st.dataframe(strict_df.tail(250).sort_values("Çekiliş", ascending=False),
                 use_container_width=True, hide_index=True)

    st.download_button(
        "⬇️ TEKRAR TEK LAB CSV indir",
        strict_df.to_csv(index=False).encode("utf-8-sig"),
        file_name="TEKRAR_TEK_LAB_WALKFORWARD.csv",
        mime="text/csv",
    )

    st.download_button(
        "⬇️ TEKRAR aday kalibrasyon CSV indir",
        repeat_wf.to_csv(index=False).encode("utf-8-sig"),
        file_name="TEKRAR_ADAY_KALIBRASYON.csv",
        mime="text/csv",
    )
...
# ===================================================================
# 8) TEKRAR TEK LAB V2 — DONDURULMUŞ YAPISAL SİNYAL
# ===================================================================
...
def repeat_v2_candidates(engine, t):
    """
    V1 aday kalibrasyonundan çıkarılan ve artık DONDURULAN yapısal kural.
    Yüksek TEKRAR skorunu karar olarak KULLANMAZ.

    Aday:
      - sayı son çekilişte bulunuyor,
      - streak >= 3,
      - trans_same 0.247–0.269 aralığında.
    Koşul yoksa PAS.
    """
    tab = _repeat_features(engine, t)
    if tab.empty:
        return [], pd.DataFrame()
    ...
    sel = sorted(tab.loc[tab["V2_sinyal"], "Sayı"].astype(int).unique().tolist())
    show_cols = ["Sayı","streak","trans_same","p_recent","p_long","p_hour",
                 "pair_lift","cases","recent_cases","hour_cases","V2_sinyal"]
    return sel, tab[show_cols].sort_values(["V2_sinyal","streak","trans_same"],
                                           ascending=[False,False,False])

def repeat_v2_walkforward(engine, start_t=140):
    rows=[]
    for t in range(max(140,int(start_t)), len(engine.A)):
        if engine.draw_nos[t] != engine.draw_nos[t-1] + 1:
            continue
        picks,_ = repeat_v2_candidates(engine,t)
        actual=set((np.where(engine.A[t]==1)[0]+1).tolist())
...
st.header("8️⃣ TEKRAR V2 — Dondurulmuş Yapısal Sinyal")
st.caption(
    "V1'de yüksek TEKRAR skorunun rastgele beklentiyi aşmadığı görüldü. "
    "V2 bu skoru karar olarak kullanmaz. Kural: streak ≥ 3 ve "
    "trans_same 0.247–0.269. Eşikler yeni kör veri gelene kadar değiştirilmez."
)

if st.button("🧪 TEKRAR V2 dondurulmuş walk-forward testi", type="primary"):
    with st.spinner("TEKRAR V2 yapısal sinyali test ediliyor..."):
        v2=repeat_v2_walkforward(eng,140)

    active=v2[v2["Kupon boyu"]>0].copy()
    full=active[active["Tam isabet"]==True].copy()
    ...
    c3.metric("Sayı doğruluğu",f"%{100*total_hit/max(total_n,1):.2f}")
    c4.metric("Tam kolon",len(full))

    st.subheader("KONUŞ olayları")
    st.dataframe(active.sort_values("Çekiliş",ascending=False),
                 use_container_width=True,hide_index=True)

    st.download_button(
        "⬇️ TEKRAR V2 walk-forward CSV indir",
        v2.to_csv(index=False).encode("utf-8-sig"),
        file_name="TEKRAR_V2_DONDURULMUS_WALKFORWARD.csv",
        mime="text/csv",
    )

try:
    _v2_now,_v2_detail=repeat_v2_candidates(eng,len(eng.A))
    st.subheader("🎯 Güncel TEKRAR V2 kararı")
    if _v2_now:
        st.success("KONUŞ — "+" - ".join(map(str,_v2_now)))
    else:
        st.warning("PAS — dondurulmuş V2 koşulunu geçen sayı yok.")
    if not _v2_detail.empty:
        st.dataframe(_v2_detail,use_container_width=True,hide_index=True)
except Exception as _e:
    st.info(f"Güncel TEKRAR V2 karar üretilemedi: {_e}")
