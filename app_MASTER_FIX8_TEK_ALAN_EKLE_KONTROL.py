...
QUICK_COUPON_FILE = Path("hizli_kuponlar.csv")
...
def eval_snapshot(snapshot,actual):
    ...
    for mc in snapshot.get("manual_coupons",[]) or []:
        arr=list(map(int,mc.get("numbers",[]) or []))
        if not arr: continue
        hit=sorted(set(arr)&actual); miss=sorted(set(arr)-actual)
        label=str(mc.get("label","HIZLI KUPON"))
        rows.append({"Kupon/Havuz":f"⚡ {label}","Boyut":len(arr),"Doğru":len(hit),
                     "Kaçta Kaç":f"{len(hit)}/{len(arr)}",
                     "Tuttu":"-".join(map(str,hit)),"Kaçtı":"-".join(map(str,miss)),
                     "Seçim":"-".join(map(str,arr))})
...
def _coupon_numbers_from_text(text):
    nums=[]
    for x in re.findall(r"(?<!\d)(?:[1-9]|[1-7]\d|80)(?!\d)",str(text or "")):
        n=int(x)
        if n not in nums: nums.append(n)
    return nums


def _result_numbers_from_text(text):
    raw=str(text or "")
    m=re.search(r"\d{1,2}\.\d{1,2}\.\d{4}\s*[-–— ]\s*\d{1,2}:\d{2}",raw)
    if m:
        raw=raw[m.end():]
    else:
        raw="\n".join(ln for ln in raw.splitlines() if not re.search(r"çekiliş\s*no",ln,re.I))
    return sorted(set(int(x) for x in re.findall(r"(?<!\d)(?:[1-9]|[1-7]\d|80)(?!\d)",raw)))


def _parse_full_draw_blob(text, fallback_draw=None, fallback_date=None, fallback_time=None):
    """Kullanıcının resmi kopyala-yapıştır formatını tek alanda çözer.
    Beklenen örnek:
      Çekiliş no: 50179
      20.08.2026 - 10:12
      7
    ...
    date=fallback_date
    tm=fallback_time
    md=re.search(r"çekiliş\s*no\s*[:：]?\s*(\d+)", raw, re.I)
    if md:
        draw=int(md.group(1))
    mdt=re.search(r"(\d{1,2}\.\d{1,2}\.\d{4})\s*[-–—]\s*(\d{1,2}:\d{2})", raw)
    if mdt:
        date=mdt.group(1)
        tm=mdt.group(2)
    nums=_result_numbers_from_text(raw)
    return {"draw_no": int(draw) if draw is not None else None,
            "date": str(date) if date is not None else None,
            "time": str(tm) if tm is not None else None,
            "numbers": nums}


def _manual_coupon_df():
    parts=[]
    mem=st.session_state.get("manual_coupon_rows",[]) or []
    if mem:
        try: parts.append(pd.DataFrame(mem))
        except Exception: pass
    if QUICK_COUPON_FILE.exists():
        try: parts.append(pd.read_csv(QUICK_COUPON_FILE))
        except Exception: pass
    # Streamlit Cloud yeniden başlasa bile hızlı kuponlar kaybolmasın.
    # GitHub yalnız oturumda ilk gerektiğinde okunur.
    gh_cached=st.session_state.get("manual_coupon_github_df",None)
    if gh_cached is None:
        try:
            import io
            path=str(st.secrets.get("GITHUB_QUICK_COUPON_PATH","hizli_kuponlar.csv"))
            txt,_=github_read_path(path)
            gh_cached=pd.read_csv(io.StringIO(txt))
        except Exception:
            gh_cached=pd.DataFrame()
        st.session_state["manual_coupon_github_df"]=gh_cached
    if isinstance(gh_cached,pd.DataFrame) and not gh_cached.empty:
        parts.append(gh_cached)
    if not parts:
        return pd.DataFrame(columns=["saved_at","target_draw","label","numbers"])
    out=pd.concat(parts,ignore_index=True)
    for c in ["saved_at","target_draw","label","numbers"]:
        if c not in out.columns: out[c]=""
    out["target_draw"]=pd.to_numeric(out["target_draw"],errors="coerce").fillna(0).astype(int)
    out=out.drop_duplicates(["target_draw","label","numbers"],keep="last")
    return out


def _manual_coupons_for_draw(draw_no):
    dfm=_manual_coupon_df()
    if dfm.empty: return []
    hit=dfm[dfm["target_draw"].astype(int)==int(draw_no)]
    out=[]
    for _,r in hit.iterrows():
        nums=_coupon_numbers_from_text(str(r.get("numbers","")))
        if nums:
            out.append({"label":str(r.get("label","HIZLI KUPON")),"numbers":nums})
    return out


def _persist_manual_coupon(draw_no,label,nums):
    row={"saved_at":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
         "target_draw":int(draw_no),"label":str(label).strip() or "HIZLI KUPON",
         "numbers":"-".join(map(str,nums))}
    mem=st.session_state.get("manual_coupon_rows",[]) or []
    mem.append(row); st.session_state["manual_coupon_rows"]=mem[-500:]
    old=_manual_coupon_df()
    new=pd.concat([old,pd.DataFrame([row])],ignore_index=True)
    new=new.drop_duplicates(["target_draw","label","numbers"],keep="last")
    new.to_csv(QUICK_COUPON_FILE,index=False,encoding="utf-8-sig")
    try:
        path=str(st.secrets.get("GITHUB_QUICK_COUPON_PATH","hizli_kuponlar.csv"))
        github_write_path(path,new.to_csv(index=False),"Hızlı kupon kaydı")
        st.session_state["manual_coupon_github_df"]=new.copy()
        return "Kupon kaydedildi: yerel + GitHub"
    except Exception:
        return "Kupon kaydedildi: yerel/oturum"


def _attach_manual_coupons(snapshot):
    if not isinstance(snapshot,dict): return snapshot
    snap=dict(snapshot)
    draw=int(snap.get("target_draw",0) or 0)
    snap["manual_coupons"]=_manual_coupons_for_draw(draw)
    return snap
...
def find_snapshot_for_draw(draw_no):
    ...
    if cur and int(cur.get("target_draw",-1))==int(draw_no):
        return _attach_manual_coupons(cur)
    ...
    try:
        ...
        if hit:
            return _attach_manual_coupons(hit)
    except Exception:
        pass
    ...
    if not saved.empty and "target_draw" in saved.columns and "snapshot_json" in saved.columns:
        try:
            ...
            if not hit.empty:
                return _attach_manual_coupons(json.loads(hit.iloc[-1]["snapshot_json"]))
        except Exception:
            pass
    manual=_manual_coupons_for_draw(int(draw_no))
    if manual:
        return {"target_draw":int(draw_no),"target_date":"","target_time":"",
                "manual_coupons":manual,"manual_only":True,"expert_top5":{},"top_rows":[]}
    return None
...
def auto_learn_from_snapshot(snapshot, actual):
    ...
    if bool(snapshot.get("manual_only",False)):
        return False, "Yalnız manuel kupon kaydı var: kaçta-kaç hesaplandı, merkez beyin bu kayıttan öğrenmedi.", None
...
# ============================================================
# TELEFON İÇİN SABİT HIZLI PANEL — TEK ALAN
# ÜST: resmi çekiliş metnini olduğu gibi yapıştır
# ORTA: hangi çekilişe ekleneceği + o hedefin verilmiş kuponları
# ALT: ekle + kaçta kaç sonucu
# ============================================================
with st.expander("⚡ HIZLI ÇEKİLİŞ EKLE + KUPON KONTROL", expanded=True):
    st.caption("Çekiliş sonucunu telefondan kopyaladığın biçimde TEK ALANA yapıştır. Çekiliş no, tarih, saat ve 20 sayı otomatik okunur. Sonucu eklemeden önce o çekilişe ait verilmiş kuponlar aşağıda görünür.")

    # 1) TEK BOŞ ALAN
    draw_blob=st.text_area(
        "Çekiliş sonucunu buraya yapıştır",
        height=260,
        placeholder="Çekiliş no: 50179\n20.08.2026 - 10:12\n7\n9\n10\n13\n...",
        key="top_full_draw_blob"
    )
    parsed=_parse_full_draw_blob(draw_blob, target_draw, target_date, target_time)

    # 2) HANGİ ÇEKİLİŞE EKLENECEĞİ
    st.markdown("#### Hangi çekilişe eklenecek?")
    cno,cdate,ctime=st.columns([1,1.4,1])
    add_draw=cno.number_input("Çekiliş no",min_value=1,step=1,value=int(parsed["draw_no"] or target_draw),key="top_full_draw_no")
    add_date=cdate.text_input("Tarih",value=str(parsed["date"] or target_date),key="top_full_draw_date")
    _tm=str(parsed["time"] or target_time)
    if _tm in SLOTS:
        add_time=ctime.selectbox("Saat",SLOTS,index=SLOTS.index(_tm),key="top_full_draw_time")
    else:
        add_time=ctime.text_input("Saat",value=_tm,key="top_full_draw_time_text")
    add_nums=parsed["numbers"]
    if draw_blob:
        if len(add_nums)==20:
            st.success(f"✅ 20/20 sayı okundu · #{int(add_draw)} · {add_date} {add_time}")
        else:
            st.warning(f"Okunan sonuç: {len(add_nums)}/20 · Tam 20 sayı gerekli.")

    # 3) O HEDEF İÇİN VERİLMİŞ KUPONLAR
    st.markdown(f"#### 🎟️ #{int(add_draw)} için verilmiş kuponlar")
    preview_snap=find_snapshot_for_draw(int(add_draw))
    preview_rows=[]
    if preview_snap is not None:
        _labels=[
            ("core4","4 ÇEKİRDEK"),("alt4","4 ALTERNATİF"),("core5","5 ÇEKİRDEK"),("alt5","5 ALTERNATİF"),
            ("live5","CANLI5"),("v7_final4","V7 Final4"),("v7_final5","V7 Final5"),
            ("v9_final4","V9 Final4"),("v9_final5","V9 Final5"),("v10_final4","V10 Final4"),("v10_final5","V10 Final5"),
            ("independent5","BAĞIMSIZ5"),("ardisik9","ARDIŞIK UZMAN 9")
        ]
        for _k,_lab in _labels:
            _arr=list(map(int,preview_snap.get(_k,[]) or []))
            if _arr:
                preview_rows.append({"Kupon":_lab,"Boyut":len(_arr),"Sayılar":" - ".join(map(str,_arr))})
        for _mc in preview_snap.get("manual_coupons",[]) or []:
            _arr=list(map(int,_mc.get("numbers",[]) or []))
            if _arr:
                preview_rows.append({"Kupon":"⚡ "+str(_mc.get("label","HIZLI KUPON")),"Boyut":len(_arr),"Sayılar":" - ".join(map(str,_arr))})
    # Snapshot dışında kaydedilmiş manuel hızlı kuponları da göster
    for _mc in _manual_coupons_for_draw(int(add_draw)):
        _arr=list(map(int,_mc.get("numbers",[]) or []))
        _row={"Kupon":"⚡ "+str(_mc.get("label","HIZLI KUPON")),"Boyut":len(_arr),"Sayılar":" - ".join(map(str,_arr))}
        if _row not in preview_rows:
            preview_rows.append(_row)
    if preview_rows:
        st.dataframe(pd.DataFrame(preview_rows),use_container_width=True,hide_index=True)
    else:
        st.info("Bu çekiliş için sonuçtan önce kaydedilmiş/verilmiş kupon bulunamadı.")

    # İSTEĞE BAĞLI: elde oynanan başka kolon varsa aynı hedefe ekle
    with st.expander("➕ İstersen bu çekilişe elle kupon ekle", expanded=False):
        mq_name=st.text_input("Kupon adı",value="HIZLI KUPON",key="top_manual_name_fix8")
        mq_text=st.text_input("Kupon sayıları",placeholder="39 51 7 38 28",key="top_manual_nums_fix8")
        mq_nums=_coupon_numbers_from_text(mq_text)
        if mq_text:
            st.caption(f"Okunan: {len(mq_nums)} sayı · {' - '.join(map(str,mq_nums))}")
        if st.button("➕ BU ÇEKİLİŞE KUPONU EKLE",use_container_width=True,key="top_manual_save_fix8"):
            if not (2 <= len(mq_nums) <= 20):
                st.error("Kupon için 2–20 benzersiz sayı gir.")
            else:
                _msg=_persist_manual_coupon(int(add_draw),mq_name,mq_nums)
                _led=st.session_state.get("snapshot_ledger",{}) or {}
                _kk=str(int(add_draw))
                if _kk in _led:
                    _led[_kk]=_attach_manual_coupons(_led[_kk]); st.session_state["snapshot_ledger"]=_led
                st.success(_msg)
                st.rerun()

    _tok,_repo,_,_=github_config()
    save_gh=st.checkbox("veri.txt'ye kalıcı kaydet",value=bool(_tok and _repo),key="top_full_save_gh")

    if st.button("✅ ÇEKİLİŞİ EKLE + VERİLEN KUPONLARI KONTROL ET",type="primary",use_container_width=True,key="top_full_add_result"):
        if len(add_nums)!=20:
            st.error("Tam 20 benzersiz sonuç sayısı gerekli.")
        else:
            _row=pd.DataFrame([{"draw_no":int(add_draw),"date":str(add_date),"time":str(add_time),"numbers":add_nums}])
            _row["_dt"]=pd.to_datetime(_row["date"]+" "+_row["time"],format="%d.%m.%Y %H:%M",errors="coerce")
            if _row["_dt"].isna().any():
                st.error("Tarih/saat okunamadı. Örnek: 20.08.2026 - 10:12")
            else:
                _snap=find_snapshot_for_draw(int(add_draw))
                if _snap is not None:
                    _snap=_attach_manual_coupons(_snap)
                    _summ,_,_=eval_snapshot(_snap,add_nums)
                    st.session_state["last_added_hit_summary"]=_summ
                    _learned,_learn_msg,_=auto_learn_from_snapshot(_snap,add_nums)
                    st.session_state["last_auto_learn_message"]=_learn_msg
                else:
                    # Snapshot yoksa manuel kuponlar yine kontrol edilsin
                    _manual=_manual_coupons_for_draw(int(add_draw))
                    if _manual:
                        _fake={"target_draw":int(add_draw),"manual_coupons":_manual}
                        _summ,_,_=eval_snapshot(_fake,add_nums)
                        st.session_state["last_added_hit_summary"]=_summ
                    else:
                        st.session_state["last_added_hit_summary"]=None
                    st.session_state["last_auto_learn_message"]="Snapshot yok: merkez beyin öğrenmedi (leakage engeli)."
                st.session_state["last_added_hit_meta"]=(int(add_draw),str(add_date),str(add_time))
                _old=st.session_state.get("master_extra_df",pd.DataFrame())
                st.session_state["master_extra_df"]=merge_dataframes(_old,_row)
                _local=persist_local_draws([{"draw_no":int(add_draw),"date":str(add_date),"time":str(add_time),"numbers":add_nums}])
                _msg="Oturuma eklendi · "+_local
                if save_gh:
                    try:
                        _msg += " · "+add_draw_to_github(int(add_draw),str(add_date),str(add_time),add_nums)
                    except Exception as e:
                        _msg += f" · GitHub yazılamadı: {e}"
                st.session_state["last_add_message"]=_msg
                # metin alanını temizlemiyoruz; kullanıcı sonucu görsün.
                st.rerun()

    # 4) SON EKLENEN ÇEKİLİŞİN KAÇTA KAÇ RAPORU
    _last_top=st.session_state.get("last_added_hit_meta")
    if _last_top:
        st.success(st.session_state.get("last_add_message","Çekiliş eklendi."))
        _alm_top=st.session_state.get("last_auto_learn_message")
        if _alm_top:
            st.info("🧠 "+_alm_top)
        _summ_top=st.session_state.get("last_added_hit_summary")
        if isinstance(_summ_top,pd.DataFrame) and not _summ_top.empty:
            st.markdown(f"### 🎯 #{_last_top[0]} — VERİLEN KUPONLAR KAÇTA KAÇ TUTTU?")
            st.dataframe(_summ_top,use_container_width=True,hide_index=True)
        else:
            st.warning("Bu çekiliş için sonuçtan önce kaydedilmiş kupon/snapshot bulunamadı.")
