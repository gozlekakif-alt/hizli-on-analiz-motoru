...
def train_model(df,A,t,lookback=180):
...
def walkforward(df,A,n=750):
    ...
    for pos,t in enumerate(valid):
        if model is None or trained_at is None or t-trained_at>=100:
            model=train_model(df,A,t,lookback=180)
            trained_at=t
...
# UI
# ============================================================
st.title("🧠 GÜN × SAAT × FAZ AKTİF ÇEKİRDEK — HAFİF LAB")
st.info(
...
with tabs[0]:
    ...
    test_n = st.select_slider("Test adedi", options=[25,50,100,250], value=50)
    st.caption(
