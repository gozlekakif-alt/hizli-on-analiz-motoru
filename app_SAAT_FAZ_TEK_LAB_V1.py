...
APP_VERSION = "SAAT/FAZ TEK LAB V1 — Walk-forward zaman kalibrasyonu"
...
# ===================================================================
# 11) SAAT/FAZ TEK LAB V1 — walk-forward saat davranışı kalibrasyonu
# ===================================================================

def _saatfaz_features(engine,t):
    """
    Hedef çekilişin saat/dakika slotunu ve yakın saat fazını yalnız geçmişten ölçer.
    Her sayı için aynı slot, aynı saat ve yakın faz davranışlarını üretir.
    """
    if t < 120:
        return pd.DataFrame()
    ...
    # yalnız geçmiş indeksler
    past=np.arange(t)
    same_hour=np.array([i for i in past if engine.dts[i].hour==hh],dtype=int)
    same_slot=np.array([i for i in past if engine.dts[i].strftime("%H:%M")==slot],dtype=int)

    # Yakın faz: aynı dakika veya +/-5 dakika; saat sınırlarını datetime farkı yerine dakika-of-day ile ölç.
    target_mod=hh*60+mm
    near=[]
    for i in past:
        mod=engine.dts[i].hour*60+engine.dts[i].minute
        d=abs(mod-target_mod)
        d=min(d,1440-d)
        if d<=10:
            near.append(i)
    ...
    # Son 300 genel baz
    H=A[max(0,t-300):t]
    base=H.mean(axis=0) if len(H) else np.full(80,.25)

    for idx in range(80):
        def shrink(indices,prior,k):
            if len(indices)==0:return float(prior)
            return float((A[indices,idx].sum()+k*prior)/(len(indices)+k))

        p_slot=shrink(same_slot,base[idx],8)
        p_hour=shrink(same_hour,base[idx],12)
        p_near=shrink(near,base[idx],10)

        # Son 120 içinde aynı saat/fazın yakın dönem davranışı
        recent_cut=max(0,t-120)
        rs=same_slot[same_slot>=recent_cut]
        rh=same_hour[same_hour>=recent_cut]
        rn=near[near>=recent_cut]
        p_slot_recent=shrink(rs,p_slot,5)
        p_hour_recent=shrink(rh,p_hour,7)
        p_near_recent=shrink(rn,p_near,6)

        # slot/hours lift
        lift_slot=p_slot/max(float(base[idx]),1e-6)
        lift_hour=p_hour/max(float(base[idx]),1e-6)
        lift_near=p_near/max(float(base[idx]),1e-6)

        rows.append({
            "Sayı":idx+1,
            "p_slot":p_slot,"p_hour":p_hour,"p_near":p_near,
            "p_slot_recent":p_slot_recent,
            "p_hour_recent":p_hour_recent,
            "p_near_recent":p_near_recent,
            "lift_slot":lift_slot,"lift_hour":lift_hour,"lift_near":lift_near,
            "slot_cases":len(same_slot),"hour_cases":len(same_hour),"near_cases":len(near),
            "slot_recent_cases":len(rs),"hour_recent_cases":len(rh),"near_recent_cases":len(rn),
            "base":float(base[idx]),
        })
    d=pd.DataFrame(rows)

    def mm(s):
        a=np.asarray(s,dtype=float); lo=float(a.min()); hi=float(a.max())
        if hi<=lo:return np.full(len(a),.5)
        return (a-lo)/(hi-lo)

    d["slot_n"]=mm(d["p_slot_recent"])
    d["hour_n"]=mm(d["p_hour_recent"])
    d["near_n"]=mm(d["p_near_recent"])
    d["lift_n"]=mm((d["lift_slot"]+d["lift_hour"]+d["lift_near"])/3)

    # Yalnız sıralama; karar değildir.
    d["SAATFAZ skor"]=np.clip(
        .34*d["slot_n"]+.24*d["hour_n"]+.24*d["near_n"]+.18*d["lift_n"],0,1)
    d=d.sort_values(["SAATFAZ skor","p_slot_recent","p_near_recent"],
                    ascending=False).reset_index(drop=True)
    d["Sıra"]=np.arange(1,len(d)+1)
    return d


def saatfaz_walkforward_lab(engine,start_t=160):
    rows=[]
    for t in range(max(160,int(start_t)),len(engine.A)):
        if engine.draw_nos[t] != engine.draw_nos[t-1]+1:
            continue
        tab=_saatfaz_features(engine,t)
        actual=set((np.where(engine.A[t]==1)[0]+1).tolist())
        for _,r in tab.iterrows():
            n=int(r["Sayı"])
            rows.append({
                "Index":t,"Çekiliş":int(engine.draw_nos[t]),"Sayı":n,
                "Sıra":int(r["Sıra"]),"SAATFAZ skor":float(r["SAATFAZ skor"]),
                "p_slot":float(r["p_slot"]),"p_hour":float(r["p_hour"]),"p_near":float(r["p_near"]),
                "p_slot_recent":float(r["p_slot_recent"]),
                "p_hour_recent":float(r["p_hour_recent"]),
                "p_near_recent":float(r["p_near_recent"]),
                "lift_slot":float(r["lift_slot"]),"lift_hour":float(r["lift_hour"]),
                "lift_near":float(r["lift_near"]),
                "slot_cases":int(r["slot_cases"]),"hour_cases":int(r["hour_cases"]),
                "near_cases":int(r["near_cases"]),
                "base":float(r["base"]),"Doğru":int(n in actual),
            })
    return pd.DataFrame(rows)


def saatfaz_calibration_report(wf):
    if wf.empty:return pd.DataFrame(),pd.DataFrame()
    d=wf.copy()
    ids=np.array(sorted(d["Index"].unique()))
    q1,q2=ids[len(ids)//3],ids[(2*len(ids))//3]
    d["Dönem"]=np.where(d["Index"]<=q1,"ERKEN",np.where(d["Index"]<=q2,"ORTA","SON"))
    try:d["Skor dilimi"]=pd.qcut(d["SAATFAZ skor"],10,duplicates="drop")
    except:d["Skor dilimi"]=pd.cut(d["SAATFAZ skor"],10)
    score=(d.groupby("Skor dilimi",observed=True)
             .agg(Aday=("Doğru","size"),Doğru=("Doğru","sum"),
                  Doğruluk=("Doğru","mean"),Ort_Skor=("SAATFAZ skor","mean"))
             .reset_index().sort_values("Ort_Skor",ascending=False))
    score["Net_vs_%25"]=score["Doğruluk"]-.25
    rr=[]
    for k in [1,2,3,4,5,6,8,10]:
        s=d[d["Sıra"]<=k]
        row={"TopK":k,"Aday":len(s),"Doğruluk":s["Doğru"].mean(),
             "Net_vs_%25":s["Doğru"].mean()-.25}
        for e in ["ERKEN","ORTA","SON"]:
            z=s[s["Dönem"]==e]; row[e]=z["Doğru"].mean() if len(z) else np.nan
        rr.append(row)
    return score,pd.DataFrame(rr)


def saatfaz_strict_selector_from_history(wf,target_index,current_tab):
    hist=wf[wf["Index"]<target_index].copy()
    if len(hist)<1600:return [],pd.DataFrame()
    selected=[];details=[]
    for _,r in current_tab.iterrows():
        rank=int(r["Sıra"]); sc=float(r["SAATFAZ skor"])
        ls=float(r["lift_slot"]); lh=float(r["lift_hour"]); ln=float(r["lift_near"])

        h=hist[
            (hist["Sıra"]==rank)&
            (np.abs(hist["SAATFAZ skor"]-sc)<=.07)&
            (np.abs(hist["lift_slot"]-ls)<=.35)&
            (np.abs(hist["lift_near"]-ln)<=.30)
        ].copy()
        if len(h)<70:
            h=hist[
                (hist["Sıra"]==rank)&
                (np.abs(hist["SAATFAZ skor"]-sc)<=.11)&
                (np.abs(hist["lift_slot"]-ls)<=.55)&
                (np.abs(hist["lift_near"]-ln)<=.48)
            ].copy()

        ok=False;acc=a1=a2=np.nan
        if len(h)>=70:
            h=h.sort_values("Index");mid=len(h)//2
            a1=float(h.iloc[:mid]["Doğru"].mean());a2=float(h.iloc[mid:]["Doğru"].mean())
            acc=float(h["Doğru"].mean())
            ok=bool(acc>=.35 and a1>=.31 and a2>=.31)

        if ok:selected.append(int(r["Sayı"]))
        details.append({
            "Sayı":int(r["Sayı"]),"Sıra":rank,"SAATFAZ skor":round(sc,4),
            "lift_slot":round(ls,3),"lift_hour":round(lh,3),"lift_near":round(ln,3),
            "Benzer geçmiş":len(h),
...
def saatfaz_strict_walkforward(engine,start_t=160):
    wf=saatfaz_walkforward_lab(engine,start_t)
    if wf.empty:return pd.DataFrame(),wf
    rows=[]
    for t in sorted(wf["Index"].unique()):
        tab=_saatfaz_features(engine,int(t))
        picks,_=saatfaz_strict_selector_from_history(wf,int(t),tab)
        actual=set((np.where(engine.A[int(t)]==1)[0]+1).tolist())
        hits=sorted(set(picks)&actual)
        rows.append({
            "Index":int(t),"Çekiliş":int(engine.draw_nos[int(t)]),
            "Kupon boyu":len(picks),"İsabet":len(hits),
...
st.header("1️⃣1️⃣ SAAT/FAZ TEK LAB V1")
st.caption(
    "Aynı saat veya slotta sık çıkmış olmak tek başına yeterli değildir. "
    "Slot, saat ve yakın faz davranışı yalnız geçmiş walk-forward sonuçlarıyla doğrulanır. "
    "Kanıt yoksa PAS."
)
if st.button("🕒 SAAT/FAZ motorunu tek başına test et",type="primary"):
    with st.spinner("SAAT/FAZ walk-forward kalibrasyonu çalışıyor..."):
        strict_df,sf_wf=saatfaz_strict_walkforward(eng,160)
        score_sum,rank_sum=saatfaz_calibration_report(sf_wf)
    st.subheader("SAAT/FAZ skor kalibrasyonu")
    ...
    if len(full):
        st.dataframe(full.sort_values("Çekiliş",ascending=False),
                     use_container_width=True,hide_index=True)
    st.subheader("Son 250 SAAT/FAZ kararı")
    st.dataframe(strict_df.tail(250).sort_values("Çekiliş",ascending=False),
                 use_container_width=True,hide_index=True)
    st.download_button("⬇️ SAAT/FAZ TEK LAB CSV indir",
        strict_df.to_csv(index=False).encode("utf-8-sig"),
        file_name="SAAT_FAZ_TEK_LAB_WALKFORWARD.csv",mime="text/csv")
    st.download_button("⬇️ SAAT/FAZ aday kalibrasyon CSV indir",
        sf_wf.to_csv(index=False).encode("utf-8-sig"),
        file_name="SAAT_FAZ_ADAY_KALIBRASYON.csv",mime="text/csv")
