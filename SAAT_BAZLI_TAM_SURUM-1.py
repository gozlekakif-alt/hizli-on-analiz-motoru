...
def detect_active_phase(df, lookback=36):
    sets = row_sets(df)
    if len(sets) < 5:
        return max(0, len(sets)-2), len(sets)
    ...
    scale = P.std(axis=0)
    scale[scale < 0.5] = 1.0
    breaks = [max(0, start-1)]

    for j in range(3, len(P)):
        recent = P[max(0,j-2):j].mean(axis=0)
        d = float(np.sqrt(np.mean(((P[j]-recent)/scale)**2)))
        if d >= 1.05:
            breaks.append(idxs[j])
...
def live_hour_character_score(df, target_time):
    ...
    # BUGÜN / AKTİF FAZ SICAK ÇEKİRDEĞİ
    ...
    # KISA CANLI SAAT
    last4 = sets[-4:] if len(sets)>=4 else sets
    short_counter=Counter()
    for s in last4:
        short_counter.update(s)
    short_hot=np.array([short_counter.get(n,0)/max(len(last4),1) for n in range(1,81)],float)
    ...
    for n in range(1,81):
        ...
        if phase_hot[n-1] >= 0.50: reason.append("faz sıcak")
...
with tab_test:
    ...
    if st.session_state.bt_total:
        ...
        if isinstance(res,pd.DataFrame) and not res.empty:
            ...
            x2.metric("Sayı doğruluğu",f"%{100*hit/max(pred,1):.2f}")
            x3.metric("Ort. isabet",f"{res['İsabet'].mean():.2f}")
            x4.metric("Maks. isabet",int(res["İsabet"].max()))
