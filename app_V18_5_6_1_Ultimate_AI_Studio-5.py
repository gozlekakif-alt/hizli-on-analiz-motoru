...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.5.6.1",
    page_icon="🎯",
    layout="wide",
)
...
st.title("🎯 Hızlı On Ultimate AI Studio V18.5.6.1")
...
def consensus_coupon_v1856(items, master, latest_set, skeleton, size=7):
    ...
    for _, row in carriers.iterrows():
        ...
        if identity >= 52 or support >= 2 or scen >= 2:
            selected.append(n)
...
if page == "✅ Kontrol":
    ...
    if len(df) < 100:
        ...
        st.subheader("4️⃣ V18.5.6.1 Nefes Puanı")
        st.dataframe(
            master18[
                [
                    "Sayı",
        ...
        st.caption("V18.5.6.1: Birleşik Güç için adaptif taşıma kotası 0–7; yüksek taşıma rejiminde 6–7 gerçek taşıma adayına izin verir.")
        q1, q2 = st.columns(2)
        with q1:
                ...
                if not settings_error:
                    try:
                        ...
                        if len(combined) > before_count:
                            ...
                            st.success(
                                "🧠 V18.5.6.1 sonucu otomatik öğrendi ve GitHub motor hafızasına yazdı."
                            )
                            st.info(
