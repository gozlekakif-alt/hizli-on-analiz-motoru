...
st.set_page_config(
    page_title="Hızlı On V13 Taşıyıcı Kimliği / Sönüş / Dönüş Motoru",
    page_icon="🌙",
    layout="wide",
)

st.title("🌙 Hızlı On V13 — Taşıyıcı Kimliği / Sönüş / Dönüş Motoru")
st.caption(
...
with tabs[0]:
    if target_slot is None:
        ...
        try:
            ...
            d.metric("Beklenen gerçek taşıma", f"{tickets.get('_expected_carry',0):.1f}/20")

            st.caption(
                f"7A: {tickets.get('_k7a','-')} taşıma adayı · "
                f"7B: {tickets.get('_k7b','-')} taşıma adayı · "
                f"10'lu: {tickets.get('_k10','-')} taşıma adayı. "
                "Bu değerler benzer geçmiş olaylardan öğrenilir."
            )
...
st.caption(
    "V13 önce kaynak çekilişteki 20 sayıyı DEVAM / SÖNÜŞ diye ayırır; sonra dinlenip dönüş adaylarını ayrı öğrenir. 2 adet 7'li + 1 adet 10'lu üretir. Yüksek skor tek başına seçim sebebi değildir. "
