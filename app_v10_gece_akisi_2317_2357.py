...
st.set_page_config(
    page_title="Hızlı On V10 Gece Akış Motoru",
    page_icon="🌙",
    layout="wide",
)

st.title("🌙 Hızlı On V10 — Gece Akış / Karakter / Kupon Motoru")
st.caption(
...
def make_tickets(tab, gc, target_slot):
    carry = tab[tab["Kaynakta"]].sort_values(
        ["Kanıt","Destek"], ascending=False
    )
    ret = tab[~tab["Kaynakta"]].sort_values(
        ["Kanıt","Destek"], ascending=False
    )

    # İlk üçten gelen gece karakteri bütün gece için ana rejim.
    ...
    for size in [6,7,8,9,10]:
        cseats = int(np.clip(round(base_carry*size/10), 1, min(4,size-2)))
        rseats = size - cseats

        chosen = pd.concat([
            carry.head(cseats),
            ret.head(rseats)
        ]).sort_values(["Kanıt","Destek"],ascending=False)

        tickets[size] = chosen["Sayı"].astype(int).tolist()
...
st.caption(
    "V10'da yüksek ham skor tek başına seçim sebebi değildir. "
