...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.5.13",
    page_icon="🎯",
    layout="wide",
)
...
# ============================================================
# V18.5.13 — İKİ ESKİ ARAMA MOTORU KANIT LAB
# 1) Benzer Durum Arama: similar_state_next_scores
# 2) Blok Basınç Arama: block_pressure_engine_v184
# Amaç: İki motoru ayrı ayrı, sonra birlikte 7'li olarak walk-forward test etmek.
# ============================================================
...
def v18513_similar_search_scores(df):
    ...
    out = out.rename(columns={"Benzer Durum Puanı":"Benzer Arama"})
    return out


def v18513_block_pressure_number_scores(df, target_time=None):
    """
    Blok Basınç Arama'nın bölge tahminini, V18'in blok-geçiş sayı sinyaliyle
    aynı 1..80 aday tablosuna taşır. Ana sinyal blok basıncı/bölgedir.
    """
    ...
    out = out.merge(
        transition[["Sayı","Blok Geçiş Puanı"]],
        on="Sayı",
        how="left",
    ).fillna(0.0)

    # Basınç motoru ana; blok-geçiş eşit puanlı bölge içinde ayırıcı.
    out["Blok Arama"] = (
        0.68 * v18513_norm(out["Bölge Basıncı"])
        + 0.32 * v18513_norm(out["Blok Geçiş Puanı"])
    ) * 100.0
...
def v18513_two_search_coupons(df, target_time=None, size=7):
    sim = v18513_similar_search_scores(df)
    block = v18513_block_pressure_number_scores(df, target_time)

    table = sim.merge(
        block[["Sayı","Blok Arama","Bölge Basıncı","Blok Geçiş Puanı"]],
        on="Sayı",
        how="outer",
    ).fillna(0.0)

    table["Benzer_n"] = v18513_norm(table["Benzer Arama"])
    table["Blok_n"] = v18513_norm(table["Blok Arama"])

    # İki motorun ortak desteğine bonus.
    table["Ortak"] = np.minimum(table["Benzer_n"], table["Blok_n"])
    table["İki Motor Puanı"] = (
        0.46 * table["Benzer_n"]
        + 0.46 * table["Blok_n"]
        + 0.08 * table["Ortak"]
    ) * 100.0
    ...
    sim_coupon = sorted(
        table.sort_values(["Benzer Arama","Sayı"], ascending=[False,True])
             .head(int(size))["Sayı"].astype(int).tolist()
    )
    block_coupon = sorted(
        table.sort_values(["Blok Arama","Sayı"], ascending=[False,True])
             .head(int(size))["Sayı"].astype(int).tolist()
    )
    combo_coupon = sorted(
        table.sort_values(["İki Motor Puanı","Sayı"], ascending=[False,True])
             .head(int(size))["Sayı"].astype(int).tolist()
    )

    return sim_coupon, block_coupon, combo_coupon, table.sort_values(
        ["İki Motor Puanı","Sayı"], ascending=[False,True]
    ).reset_index(drop=True)


def v18513_two_search_backtest(df, test_count=50, size=7):
    work = df.sort_values("Cekilis_No").reset_index(drop=True)
    ...
    for i in range(start, len(work)):
        ...
        sim_c, block_c, combo_c, _ = v18513_two_search_coupons(
            train,
            target_time=str(target.Saat),
            size=size,
        )
        ...
        sim_h = sorted(set(sim_c) & actual)
        block_h = sorted(set(block_c) & actual)
        combo_h = sorted(set(combo_c) & actual)
        ...
        rows.append({
            ...
            "Benzer Kupon": "-".join(map(str,sim_c)),
            "Benzer İsabet": len(sim_h),
            "Benzer Tutan": "-".join(map(str,sim_h)),

            "Blok Kupon": "-".join(map(str,block_c)),
            "Blok İsabet": len(block_h),
            "Blok Tutan": "-".join(map(str,block_h)),

            "İki Motor Kupon": "-".join(map(str,combo_c)),
            "İki Motor İsabet": len(combo_h),
            "İki Motor Tutan": "-".join(map(str,combo_h)),
...
elif page == "🧪 V18.5.13 İki Arama Motoru LAB":
    st.header("🧪 V18.5.13 — İki Eski Arama Motoru Kanıt LAB")
    st.caption(
        "Motor 1: Benzer Durum Arama. "
        "Motor 2: Blok Basınç Arama. "
        "İkisi ayrı ayrı ve ortak 7'li olarak, hedef sonucu görmeden test edilir."
    )

    if len(df) < 150:
        st.warning("Bu test için 150+ çekiliş önerilir.")
    else:
        next_no, next_date, next_time = next_draw_defaults(df)

        sim_c, block_c, combo_c, lab_table = v18513_two_search_coupons(
            df,
        ...
        c1,c2,c3 = st.columns(3)
        with c1:
            st.success("🔎 BENZER 7: " + " - ".join(map(str,sim_c)))
        with c2:
            st.success("🧱 BLOK 7: " + " - ".join(map(str,block_c)))
        with c3:
            st.success("🧠 ORTAK 7: " + " - ".join(map(str,combo_c)))

        st.dataframe(
            lab_table[
                ["Sayı","Benzer Arama","Blok Arama","Bölge Basıncı",
                 "Blok Geçiş Puanı","Ortak","İki Motor Puanı"]
            ].head(30),
            use_container_width=True,
            hide_index=True,
        )
        ...
        st.divider()
        st.subheader("Walk-forward sızıntısız test")
        test_n = st.selectbox(
            "Test adedi",
            [10,25,50,100],
            index=2,
            key="v18513_two_test_n",
        )
        ...
        if st.button(
            ...
            type="primary",
            use_container_width=True,
            key="v18513_two_test_run",
        ):
            with st.spinner(
                f"{test_n} hedefte Benzer, Blok ve Ortak motor ayrı ayrı test ediliyor..."
            ):
                rr = v18513_two_search_backtest(
                    df,
                    test_count=test_n,
                    size=7,
                )
            st.session_state["v18513_two_result"] = rr

        rr = st.session_state.get(
            "v18513_two_result",
            pd.DataFrame(),
        )
        ...
        if isinstance(rr,pd.DataFrame) and not rr.empty:
            ...
            b.metric("Benzer ort.",f"{rr['Benzer İsabet'].mean():.2f}")
            c.metric("Blok ort.",f"{rr['Blok İsabet'].mean():.2f}")
            d.metric("Ortak ort.",f"{rr['İki Motor İsabet'].mean():.2f}")
            ...
            st.download_button(
                "⬇️ 18.5.13 İki Motor Test CSV",
                rr.to_csv(index=False).encode("utf-8-sig"),
                file_name=f"V18513_IKI_ARAMA_MOTORU_TEST_{len(rr)}.csv",
                mime="text/csv",
                use_container_width=True,
                key="v18513_two_csv",
            )
