...
st.set_page_config(
    page_title="Hızlı On Ultimate V23 — Revize Alt Motor Master",
    page_icon="🎯",
    layout="wide",
)
...
# ============================================================
# V23 — V21 ALT MOTOR REVİZYONU / SEÇİCİ KONSENSÜS
# V21 omurga korunur. Her alt motor artık:
# - kendi sinyal kalitesini (ayrışma) ölçer,
# - saat/aktif faz bağlamında güven üretir,
# - zayıfsa oy kullanmaz,
# - aşırı tepe puanı tek başına aday yapamaz.
# Amaç garanti değil; sızıntısız testte seçiciliği artırmaktır.
# ============================================================

def v23_entropy_confidence(values):
    x = np.asarray(values, dtype=float)
    x = x - np.nanmin(x)
    if np.nanmax(x) <= 0:
        return 0.0
    x = x / (np.nansum(x) + 1e-12)
    x = x[x > 0]
    h = -float(np.sum(x * np.log(x + 1e-12)))
    hmax = np.log(max(len(values), 2))
    return float(np.clip(1.0 - h / hmax, 0.0, 1.0))


def v23_robust_z(values):
    x = np.asarray(values, dtype=float)
...
def v23_revised_submotors(frame, target_time=None):
    base = v21_submotors(frame, target_time).copy()
    motors = ["Saat","Çekirdek","Taşıma","Dönüş","Blok","Yerine","Benzer"]

    # Her motor kendi canlı ayrışmasına göre güven üretir.
    conf = {}
    for m in motors:
        arr = base[m].to_numpy(float)
        z = v23_robust_z(arr)
        pct = v21_percentile(arr)

        # Orta-yüksek bölgeyi koru; en uç tepeyi otomatik ödüllendirme.
        shape = np.exp(-((pct - 72.0) / 23.0) ** 2)
        positive = 1.0 / (1.0 + np.exp(-z))
        base[m + " Revize"] = 0.58 * positive + 0.42 * shape
        conf[m] = max(0.15, v23_entropy_confidence(arr))

    # Saat ana motor; çekirdek ikinci.
    weights = {
        "Saat":0.25, "Çekirdek":0.19, "Taşıma":0.13,
        "Dönüş":0.11, "Blok":0.09, "Yerine":0.09, "Benzer":0.14
    }
    # Canlı güven ile ağırlığı yumuşak adapte et.
    adj = {m: weights[m] * (0.70 + 0.60*conf[m]) for m in motors}
    sw = sum(adj.values())
    adj = {m:v/sw for m,v in adj.items()}

    score = np.zeros(len(base), dtype=float)
    votes = np.zeros(len(base), dtype=int)
    strong = np.zeros(len(base), dtype=int)

    for m in motors:
        r = base[m + " Revize"].to_numpy(float)
        score += adj[m] * r
        votes += (r >= 0.56).astype(int)
        strong += (r >= 0.68).astype(int)

    # Tez/antitez artık sayı puanlamak için değil, rejim kırılması kontrolü için.
    thesis = base["Tez"].to_numpy(float)
    anti = base["Antitez"].to_numpy(float)
    regime_balance = 1.0 - np.abs(thesis - anti)
    regime_balance = np.clip(regime_balance, 0, 1)

    # Seçici konsensüs: en az 3 motor desteklemiyorsa ceza.
    score += 0.055 * np.clip((votes - 2)/5.0, 0, 1)
    score += 0.035 * np.clip((strong - 1)/4.0, 0, 1)
    score += 0.015 * regime_balance
    score -= 0.055 * (votes < 3)

    base["V23 Puan"] = np.round(score*100, 3)
    base["Motor Oy"] = votes
    base["Güçlü Oy"] = strong
    base["Rejim Dengesi"] = np.round(regime_balance, 3)

    ranked = base.sort_values(
        ["V23 Puan","Motor Oy","Güçlü Oy","Sayı"],
        ascending=[False,False,False,True]
    ).reset_index(drop=True)

    return ranked, adj, conf


def v23_live_master(frame, target_time=None):
    ranked, weights, conf = v23_revised_submotors(frame, target_time)

    # Önce 3+ motorun ortak adayları; yetmezse sıradaki en iyi adaylarla 7'ye tamamla.
    pool = ranked[ranked["Motor Oy"] >= 3].copy()
    chosen = pool.head(7)["Sayı"].astype(int).tolist()
    if len(chosen) < 7:
        for n in ranked["Sayı"].astype(int):
            if n not in chosen:
                chosen.append(n)
    ...
    return sorted(chosen), ranked, weights, conf


def v23_blind_test(frame, test_count=25):
    work = frame.sort_values("Cekilis_No").reset_index(drop=True)
    start = max(180, len(work)-int(test_count))
    rows = []
    ...
    for i in range(start, len(work)):
        train = work.iloc[:i].copy()
        target = work.iloc[i]
        c23, _, _, _ = v23_live_master(train, str(target.Saat))
        c21, _ = v21_live_master(train, str(target.Saat))
        actual = set(int(target[c]) for c in NUM_COLS)
        h23 = sorted(set(c23) & actual)
        ...
        rows.append({
            ...
            "Tarih":str(target.Tarih),
            "Saat":str(target.Saat),
            "V23 Kupon":"-".join(map(str,c23)),
            "V23 İsabet":len(h23),
            "V23 Tutan":"-".join(map(str,h23)),
            "V21 İsabet":len(h21),
            "Avantaj":len(h23)-len(h21),
            "Sızıntı":"TEMİZ",
        })
    return pd.DataFrame(rows)
...
if page == "✅ Kontrol":
    st.success("Çalışan sürüm: V23 REVİZE ALT MOTOR + YAŞAYAN DİYALEKTİK MASTER")
...
elif page == "🧠 Revize Alt Motor MASTER V23":
    st.header("🧠 V23 — Revize Alt Motor MASTER")
    st.caption(
        "V21 omurgası korunur. Saat, Çekirdek, Taşıma, Dönüş, Blok, "
        "Yerine Geçme ve Benzer Durum motorları ayrı ayrı yeniden ölçeklenir. "
        "Tek motorun aşırı yüksek puanı yetmez; 3+ motor ortak sinyal arar."
    )

    if len(df) < 200:
        st.warning("V23 için 200+ çekiliş önerilir.")
    else:
        next_no, next_date, next_time = next_draw_defaults(df)
        c23, t23, w23, cf23 = v23_live_master(df, next_time)

        st.write(f"**Hedef:** #{next_no} — {next_date} {next_time}")
        st.success("🏆 V23 TEK 7'Lİ: " + " - ".join(map(str,c23)))

        mdf = pd.DataFrame([
            {"Motor":m, "Canlı ağırlık %":round(w23[m]*100,2),
             "Sinyal güveni":round(cf23[m],3)}
            for m in w23
        ]).sort_values("Canlı ağırlık %", ascending=False)
        st.dataframe(mdf, use_container_width=True, hide_index=True)

        cols = [
            "Sayı","V23 Puan","Motor Oy","Güçlü Oy","Rejim Dengesi",
            "Saat Revize","Çekirdek Revize","Taşıma Revize","Dönüş Revize",
            "Blok Revize","Yerine Revize","Benzer Revize"
        ]
        cols = [c for c in cols if c in t23.columns]
        st.dataframe(t23[cols].head(25), use_container_width=True, hide_index=True)

        st.divider()
        st.subheader("🧪 V23 vs V21 sızıntısız kör test")
        tn = st.selectbox("Test adedi",[10,25,50],index=1,key="v23_test_n")

        if st.button("🚀 V23 KÖR TESTİ",type="primary",
                     use_container_width=True,key="v23_test_run"):
            with st.spinner("V23 ve V21 aynı hedeflerde karşılaştırılıyor..."):
                rr = v23_blind_test(df, tn)
            st.session_state["v23_result"] = rr

        rr = st.session_state.get("v23_result", pd.DataFrame())
        if isinstance(rr,pd.DataFrame) and not rr.empty:
            a,b,c,d = st.columns(4)
            a.metric("Test",len(rr))
            b.metric("V23 ort.",f"{rr['V23 İsabet'].mean():.2f}")
            c.metric("V21 ort.",f"{rr['V21 İsabet'].mean():.2f}")
            d.metric("Net avantaj",f"{rr['Avantaj'].mean():+.2f}")
            st.dataframe(rr,use_container_width=True,hide_index=True)
            st.download_button(
                "⬇️ V23 test CSV",
                rr.to_csv(index=False).encode("utf-8-sig"),
                file_name=f"V23_REVIZE_ALT_MOTOR_TEST_{len(rr)}.csv",
                mime="text/csv",
                use_container_width=True,
                key="v23_csv",
            )
