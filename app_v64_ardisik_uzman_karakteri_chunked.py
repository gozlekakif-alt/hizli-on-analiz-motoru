...
# --------------------------- UI ---------------------------
st.title("🧬 Hızlı On — Yaşam Kimliği MASTER APP V6.4 — ARDIŞIK UZMAN KARAKTERİ PARÇALI/KALICI")
...
# =====================================================================
# V6.3 UZMAN-01C — ARDIŞIK UZMAN KARAKTERİ
# Amaç: genel oyun karakterini zorla kullanmak yerine, ardışık kanalın
# kendi mikro karakterini üretmek ve aynı/benzer uzman karakter içinde
# hareket zamanlamasını gerçek walk-forward ile sınamak.
# Kupon üretimi YOK.
# =====================================================================

_V63_DIR = Path(".hizli_on_v63_ardisik_character")
_V63_DIR.mkdir(parents=True, exist_ok=True)

def _v63_count_band(v):
    v=int(v)
    if v<=2: return "B0_2"
    if v<=4: return "B3_4"
    return "B5P"
...
def _v63_motion_family(row):
    dc=int(row["block_count_delta_21"])
    dl=int(row["max_len_delta_21"])
...
def _v63_density_family(row):
    # sadece ardışık uzmanına ait yoğunluk: blok sayısı + max blok uzunluğu
    bc=int(row["H1_block_count"])
    ml=int(row["H1_max_len"])
    if bc>=5 or ml>=4: return "HIGH"
    if bc>=3 or ml>=3: return "MID"
    return "LOW"

def _v63_osc_family(row):
    # 800 burada ana karakter değil, yardımcı bağlam.
    o=str(row.get("osc800","NA"))
    if "CROSS" in o: return "CROSS"
    if "RISING" in o: return "RISING"
    if "FALLING" in o: return "FALLING"
    return "NA"

def _v63_build_expert_character(chain, pair):
    st=chain.copy()
    st["motion_family"]=st.apply(_v63_motion_family, axis=1)
    st["density_family"]=st.apply(_v63_density_family, axis=1)
    st["block_count_band"]=st.H1_block_count.map(_v63_count_band)
    st["max_len_band"]=st.H1_max_len.map(_v63_len_band)
    st["count_delta_band"]=st.block_count_delta_21.map(_v63_delta_band)
    st["len_delta_band"]=st.max_len_delta_21.map(_v63_delta_band)
    st["expert_character"]=(
        st.motion_family.astype(str)+"|"+
        st.density_family.astype(str)+"|"+
        st.count_delta_band.astype(str)+"|"+
        st.len_delta_band.astype(str)
    )
    st["target_block_delta"]=st.H_block_count-st.H1_block_count
    st["target_maxlen_delta"]=st.H_max_len-st.H1_max_len
    st["target_motion"]=np.select(
        [st.target_maxlen_delta>=1,
         st.target_maxlen_delta<=-1,
         st.target_block_delta>=1,
         st.target_block_delta<=-1],
    ...
    q=pair.copy()
    q["motion_family"]=q.apply(_v63_motion_family, axis=1)
    q["density_family"]=q.apply(_v63_density_family, axis=1)
    q["block_count_band"]=q.H1_block_count.map(_v63_count_band)
    q["max_len_band"]=q.H1_max_len.map(_v63_len_band)
    q["count_delta_band"]=q.block_count_delta_21.map(_v63_delta_band)
    q["len_delta_band"]=q.max_len_delta_21.map(_v63_delta_band)
    q["osc_family"]=q.apply(_v63_osc_family,axis=1)
    q["expert_character"]=(
...
def _v63_character_summary(st):
    x=(st.groupby(["expert_character","target_motion"],dropna=False)
       .size().rename("events").reset_index())
    if x.empty:return x
    den=x.groupby("expert_character").events.transform("sum")
    x["rate"]=x.events/den
    return x.sort_values(["expert_character","rate"],ascending=[True,False])

def _v63_hier_pool(train, row, min_events=18):
    # Ardışık uzman kendi karakterini hiyerarşik şekilde eşleştirir.
    # Exact yetersizse yalnız ardışık mikro bileşenler gevşetilir.
    levels=[
    ...
    for name,fs in levels:
        ...
        if len(g)>=min_events:
            return g,name
    return train.iloc[0:0].copy(),"NO_HISTORY"

def _v63_state_walkforward(st, warmup=900, block=120):
    draws=sorted(st.draw_no.unique())
    pred_rows=[]
    audit=[]
    for pos in range(warmup,len(draws),block):
        td=draws[pos:min(pos+block,len(draws))]
        if not td: continue
        train=st[st.draw_no<td[0]]
        test=st[st.draw_no.isin(td)]

        for _,r in test.iterrows():
            pool,level=_v63_hier_pool(train,r,min_events=18)
            if pool.empty:
                pred="SUS"
                conf=0.0
            else:
                ...
                if conf<0.42:
                    pred="SUS"
            ok=int(pred!="SUS" and pred==str(r.target_motion))
            pred_rows.append({
                "test_from":td[0],"test_to":td[-1],
                "draw_no":r.draw_no,"date":r.date,"time":r.time,
        ...
        audit.append({
            "test_from":td[0],"test_to":td[-1],
            "test_draws":len(td),
            "history_draws":train.draw_no.nunique()
        })
    return pd.DataFrame(pred_rows),pd.DataFrame(audit)

def _v63_q2_walkforward(q, warmup=500, block=120):
    q=q[q.Q2_GATE_FROZEN==1].copy()
    if q.empty:return pd.DataFrame()
    draws=sorted(q.draw_no.unique())
    rows=[]
    # Q2 hedefi nadir olduğu için eşleşme daha kaba ama sadece ardışık mikro karakter.
    for pos in range(min(warmup,max(1,len(draws)//3)),len(draws),block):
        td=draws[pos:min(pos+block,len(draws))]
        if not td:continue
        tr=q[q.draw_no<td[0]]
        te=q[q.draw_no.isin(td)]
        for _,r in te.iterrows():
            ...
            pool=None;level="NO_HISTORY"
            for nm,fs in levels:
                g=tr.copy()
                for f in fs:g=g[g[f].astype(str)==str(r[f])]
                if len(g)>=5:
                    pool=g;level=nm;break
            if pool is None or len(pool)==0:
                rows.append({"draw_no":r.draw_no,"date":r.date,"time":r.time,
                             "match_level":"NO_HISTORY","history_signals":0,
                             "history_rate":np.nan,"actual_hit":int(r.Q2_HIT)})
                continue
            rows.append({
                "draw_no":r.draw_no,"date":r.date,"time":r.time,
                "match_level":level,"history_signals":len(pool),
                "history_hits":int(pool.Q2_HIT.sum()),
                "history_rate":float(pool.Q2_HIT.mean()),
                "actual_hit":int(r.Q2_HIT)
            })
    return pd.DataFrame(rows)

def _v63_reports(pred, q2pred):
    out={}
    if not pred.empty:
        active=pred[pred.predicted_motion!="SUS"].copy()
        sum1=(pred.groupby("match_level")
              .agg(tests=("draw_no","count"),
                   active=("predicted_motion",lambda z:int((z!="SUS").sum())),
                   correct=("correct","sum"))
              .reset_index())
        sum1["accuracy_active"]=sum1.correct/sum1.active.replace(0,np.nan)
        out["01_STATE_MATCH_LEVEL.csv"]=sum1

        conf=active.copy()
        if not conf.empty:
            conf["confidence_band"]=pd.cut(conf.confidence,
                bins=[0,0.42,0.50,0.60,0.70,0.80,1.01],
                labels=["42-50","50-60","60-70","70-80","80-100"],
                include_lowest=False)
            s2=(conf.groupby("confidence_band",observed=True)
                .agg(tests=("draw_no","count"),correct=("correct","sum"))
                .reset_index())
            s2["accuracy"]=s2.correct/s2.tests
            out["02_CONFIDENCE_ACCURACY.csv"]=s2

        bychar=(active.groupby(["expert_character","predicted_motion"],dropna=False)
                .agg(tests=("draw_no","count"),correct=("correct","sum"),
                     avg_conf=("confidence","mean")).reset_index())
        bychar["accuracy"]=bychar.correct/bychar.tests
        out["03_CHARACTER_OOS_ACCURACY.csv"]=bychar
    ...
    if not q2pred.empty:
        ...
        out["04_Q2_CHARACTER_OOS.csv"]=q2s
    return out

def _v63_make_zip():
    days,(block61,pair61,num61,chain61)=_v62_load_v61_days()
    st,q=_v63_build_expert_character(chain61,pair61)
    summary=_v63_character_summary(st)
    pred,audit=_v63_state_walkforward(st,warmup=900,block=120)
    q2pred=_v63_q2_walkforward(q)
    reports=_v63_reports(pred,q2pred)

    bio=io.BytesIO()
    with zipfile.ZipFile(bio,"w",zipfile.ZIP_DEFLATED) as z:
        z.writestr("00_MANIFEST.txt",
            "V6.3 ARDISIK UZMAN KARAKTERI\n"
            "Genel oyun karakteri yerine ardışık kanalın kendi mikro karakteri kullanılır.\n"
            "Uzman karakter: motion + density + block-count delta + max-length delta.\n"
            "Eşleşme hiyerarşik: EXACT -> MOTION_DENSITY_DELTA -> MOTION_DENSITY -> MOTION_ONLY -> SUS.\n"
            "Sayi kimligi öğrenilmez. Hedef sonuc PRE-H kurala sizmaz. Kupon yok.\n")
        for fn,t in {
            "01_EXPERT_STATE_TIMELINE.csv":st,
            "02_EXPERT_CHARACTER_MAP.csv":summary,
            "03_STATE_TRUE_WALK_FORWARD.csv":pred,
            "04_WF_AUDIT.csv":audit,
            "05_Q2_EXPERT_CHARACTER_EVENTS.csv":q[q.Q2_GATE_FROZEN==1],
            "06_Q2_EXPERT_CHARACTER_WF.csv":q2pred,
            **reports
        }.items():
            z.writestr(fn,t.to_csv(index=False).encode("utf-8-sig"))
    return bio.getvalue()
...
st.divider()
st.header("🧬 V6.3 UZMAN-01C — ARDIŞIK UZMAN KARAKTERİ")
st.caption("Genel oyun karakterini zorlamaz. Ardışık kanal kendi mikro karakterini kurar; aynı/benzer uzman karakterde hareket zamanlamasını gerçek walk-forward ile test eder.")

if st.button("▶️ V6.3 ARDIŞIK UZMAN KARAKTERİNİ ÇALIŞTIR",
             use_container_width=True,type="primary",key="v63_run",
             disabled=(_V63_DIR/"RESULTS.zip").exists()):
    ss=st.status("Ardışık uzman karakteri çalışıyor…",expanded=True)
    try:
        bb=_v63_make_zip()
        (_V63_DIR/"RESULTS.zip").write_bytes(bb)
        ss.update(label="✅ V6.3 Ardışık Uzman Karakteri tamamlandı",state="complete",expanded=False)
        st.rerun()
    except Exception as e:
        ss.update(label="❌ V6.3 hata verdi",state="error",expanded=True)
        st.exception(e)

if (_V63_DIR/"RESULTS.zip").exists():
    st.download_button("⬇️ V6.3 ARDIŞIK UZMAN KARAKTERİ — TEK ZIP",
        (_V63_DIR/"RESULTS.zip").read_bytes(),
        "HIZLI_ON_V6_3_ARDISIK_UZMAN_KARAKTERI_RESULTS.zip",
        "application/zip",use_container_width=True,key="v63_download")
# =====================================================================
# /V6.3
# =====================================================================
...
# =====================================================================
# V6.4 UZMAN-01C — ARDIŞIK UZMAN KARAKTERİ / PARÇALI-KALICI
# V6.3 araştırma mantığı korunur. Tek fark: ağır hesap tek seferde yapılmaz.
# 1) uzman karakter tabanı bir kez hazırlanır ve kalıcı kaydedilir
# 2) walk-forward blokları tek tek çalışır ve kalıcı kaydedilir
# 3) Q2 uzman-karakter testi ayrı ve kalıcı çalışır
# 4) hepsi tamamlanınca tek sonuç ZIP'i birleşir
# =====================================================================

_V64_DIR = Path(".hizli_on_v64_ardisik_character_chunked")
_V64_DIR.mkdir(parents=True, exist_ok=True)
_V64_WF_DIR = _V64_DIR / "wf"
_V64_WF_DIR.mkdir(parents=True, exist_ok=True)

def _v64_base_path():
    return _V64_DIR / "BASE.pkl"

def _v64_q2_path():
    return _V64_DIR / "Q2.pkl"

def _v64_wf_path(k):
    return _V64_WF_DIR / f"wf_{int(k):02d}.pkl"

def _v64_final_path():
    return _V64_DIR / "RESULTS.zip"

def _v64_build_base():
    days,(block61,pair61,num61,chain61)=_v62_load_v61_days()
    st,q=_v63_build_expert_character(chain61,pair61)
    summary=_v63_character_summary(st)
    obj={"state":st,"pair":q,"summary":summary}
    pd.to_pickle(obj,_v64_base_path())
    return obj

def _v64_load_base():
    if not _v64_base_path().exists():
        raise RuntimeError("V6.4 tabanı hazır değil.")
    return pd.read_pickle(_v64_base_path())

def _v64_wf_plan(st,warmup=900,block=120):
    draws=sorted(st.draw_no.unique())
    plan=[];k=1
    for pos in range(warmup,len(draws),block):
        td=draws[pos:min(pos+block,len(draws))]
        if td:
            plan.append({"block":k,"from":int(td[0]),"to":int(td[-1]),"n_draws":len(td)})
            k+=1
    return plan

def _v64_run_one_wf(st, info):
    train=st[st.draw_no<info["from"]]
    test=st[(st.draw_no>=info["from"])&(st.draw_no<=info["to"])]
    rows=[]
    ...
    for _,r in test.iterrows():
        ...
        if pool.empty:
            ...
            vc=pool.target_motion.value_counts()
            pred=str(vc.index[0]);conf=float(vc.iloc[0]/vc.sum())
            if conf<0.42:
                pred="SUS"
        rows.append({
            "wf_block":info["block"],"test_from":info["from"],"test_to":info["to"],
            "draw_no":r.draw_no,"date":r.date,"time":r.time,
            "expert_character":r.expert_character,
            "match_level":level,
            "history_events":len(pool),
    ...
    res=pd.DataFrame(rows)
    pd.to_pickle(res,_v64_wf_path(info["block"]))
    return res

def _v64_run_q2(q):
    q2=_v63_q2_walkforward(q)
    pd.to_pickle(q2,_v64_q2_path())
    return q2

def _v64_merge_wf(plan):
    arr=[]
    for info in plan:
        p=_v64_wf_path(info["block"])
        if not p.exists():
            raise RuntimeError(f"WF blok eksik: {info['block']}")
        arr.append(pd.read_pickle(p))
    return pd.concat(arr,ignore_index=True) if arr else pd.DataFrame()

def _v64_make_final():
    obj=_v64_load_base()
    st=obj["state"];q=obj["pair"];summary=obj["summary"]
    plan=_v64_wf_plan(st)
    pred=_v64_merge_wf(plan)
    q2pred=pd.read_pickle(_v64_q2_path()) if _v64_q2_path().exists() else pd.DataFrame()
    reports=_v63_reports(pred,q2pred)

    bio=io.BytesIO()
    with zipfile.ZipFile(bio,"w",zipfile.ZIP_DEFLATED) as z:
        z.writestr("00_MANIFEST.txt",
            "V6.4 ARDISIK UZMAN KARAKTERI — PARCALI/KALICI\n"
            "V6.3 karakter-first ardışık uzman mantığı korunmuştur.\n"
            "Ağır hesaplar tek seferde çalıştırılmaz; taban, WF blokları ve Q2 ayrı kalıcıdır.\n"
            "Sayi kimligi öğrenilmez. PRE-H -> POST-H ayrımı korunur. Kupon yok.\n")
        z.writestr("01_EXPERT_STATE_TIMELINE.csv",st.to_csv(index=False).encode("utf-8-sig"))
        z.writestr("02_EXPERT_CHARACTER_MAP.csv",summary.to_csv(index=False).encode("utf-8-sig"))
        z.writestr("03_STATE_TRUE_WALK_FORWARD.csv",pred.to_csv(index=False).encode("utf-8-sig"))
        z.writestr("04_Q2_EXPERT_CHARACTER_EVENTS.csv",q[q.Q2_GATE_FROZEN==1].to_csv(index=False).encode("utf-8-sig"))
        z.writestr("05_Q2_EXPERT_CHARACTER_WF.csv",q2pred.to_csv(index=False).encode("utf-8-sig"))
        for fn,t in reports.items():
            z.writestr("06_REPORTS/"+fn,t.to_csv(index=False).encode("utf-8-sig"))
    _v64_final_path().write_bytes(bio.getvalue())
    return bio.getvalue()

st.divider()
st.header("🧬 V6.4 UZMAN-01C — ARDIŞIK UZMAN KARAKTERİ / PARÇALI-KALICI")
st.caption("V6.3 mantığı aynı; artık ağır hesap tek düğmede bitmeye zorlanmıyor.")

if not _v64_base_path().exists():
    if st.button("1️⃣ UZMAN KARAKTER TABANINI HAZIRLA",use_container_width=True,type="primary",key="v64_base"):
        ss=st.status("Uzman karakter tabanı hazırlanıyor…",expanded=True)
        try:
            obj=_v64_build_base()
            ss.write(f"Durum satırı: {len(obj['state']):,}")
            ss.write(f"Pair satırı: {len(obj['pair']):,}")
            ss.update(label="✅ Uzman karakter tabanı kalıcı kaydedildi",state="complete",expanded=False)
            st.rerun()
        except Exception as e:
...
_obj64=_v64_load_base()
_st64=_obj64["state"]
_q64=_obj64["pair"]
_plan64=_v64_wf_plan(_st64)
_done64={i["block"]:_v64_wf_path(i["block"]) for i in _plan64 if _v64_wf_path(i["block"]).exists()}

st.success("✅ Uzman karakter tabanı hazır.")
st.progress(len(_done64)/len(_plan64) if _plan64 else 1.0,
            text=f"V6.4 walk-forward: {len(_done64)}/{len(_plan64)} blok")

if len(_done64)<len(_plan64):
    if st.button("2️⃣ SONRAKİ WALK-FORWARD BLOĞUNU TEST ET",use_container_width=True,type="primary",key="v64_wf"):
        info=next(i for i in _plan64 if i["block"] not in _done64)
        ss=st.status(f"WF blok {info['block']}/{len(_plan64)} çalışıyor…",expanded=True)
        try:
            rr=_v64_run_one_wf(_st64,info)
            active=rr[rr.predicted_motion!="SUS"]
            if len(active):
                acc=float(active.correct.mean())
                ss.write(f"Aktif tahmin: {len(active)} | Doğru: {int(active.correct.sum())} | Accuracy: {acc:.3f}")
            else:
                ss.write("Bu blokta uzman SUS kaldı.")
            ss.update(label=f"✅ WF blok {info['block']} kaydedildi",state="complete",expanded=False)
            st.rerun()
        except Exception as e:
...
if len(_done64)==len(_plan64):
    ...
    if not _v64_q2_path().exists():
        if st.button("3️⃣ Q2 UZMAN-KARAKTER TESTİNİ ÇALIŞTIR",use_container_width=True,type="primary",key="v64_q2"):
            ss=st.status("Q2 uzman-karakter testi çalışıyor…",expanded=True)
            try:
                q2r=_v64_run_q2(_q64)
                ss.write(f"Q2 WF satırı: {len(q2r):,}")
                ss.update(label="✅ Q2 testi kaydedildi",state="complete",expanded=False)
                st.rerun()
            except Exception as e:
                ss.update(label="❌ Q2 testi hata verdi",state="error",expanded=True)
                st.exception(e)

if len(_done64)==len(_plan64) and _v64_q2_path().exists():
    if not _v64_final_path().exists():
        if st.button("4️⃣ TEK SONUÇ ZIP'İNİ OLUŞTUR",use_container_width=True,type="primary",key="v64_final"):
            ss=st.status("Sonuç ZIP'i birleştiriliyor…",expanded=True)
            try:
                _v64_make_final()
                ss.update(label="✅ V6.4 final ZIP hazır",state="complete",expanded=False)
                st.rerun()
            except Exception as e:
                ss.update(label="❌ Final ZIP oluşturulamadı",state="error",expanded=True)
                st.exception(e)

if _v64_final_path().exists():
    st.download_button("⬇️ V6.4 ARDIŞIK UZMAN KARAKTERİ — TEK ZIP",
        _v64_final_path().read_bytes(),
        "HIZLI_ON_V6_4_ARDISIK_UZMAN_KARAKTERI_RESULTS.zip",
        "application/zip",use_container_width=True,key="v64_download")
# =====================================================================
# /V6.4
# =====================================================================
