...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.5.6",
    page_icon="🎯",
    layout="wide",
)
...
st.title("🎯 Hızlı On Ultimate AI Studio V18.5.6")
...
def consensus_coupon_v1856(items, master, latest_set, skeleton, size=7):
    """
    Birleşik Güç / Konsensüs kuponu.

    Amaç: farklı senaryolarda bulunan güçlü adayları tek kolonda toplamak.
    Kör 'en çok tekrar eden sayı' seçimi yapmaz; kupon desteği + aday kalitesi +
    ortak çekirdek güveni + gerçek taşıma kimliği birlikte kullanılır.
    """
    ...
    work["Konsensüs 56"] = (
        0.47*nrm(score_col)
        + 0.20*nrm("Kupon Desteği 56")
        + 0.14*nrm("Taşıma Kimlik")
        + 0.10*nrm("Ortak Çekirdek Güveni")
        + 0.09*nrm("Blok Mikro 56")
    ) * 100.0

    # Beklenen taşıma miktarını KORU; yalnız kimlik seçiminde yeni motoru kullan.
    carry_target = int(round(float(skeleton.get("Taşıma", max(1,int(size)//2)))))
    carry_target = max(1, min(int(size)-1, carry_target))

    latest_set = set(int(n) for n in latest_set)
    carriers = work[work["Sayı"].astype(int).isin(latest_set)].sort_values(
        ["Taşıma Kimlik","Konsensüs 56"], ascending=[False,False]
    )
    newcomers = work[~work["Sayı"].astype(int).isin(latest_set)].sort_values(
        ["Konsensüs 56",score_col], ascending=[False,False]
    )

    selected = []
    # 7'li kolonda beklenen taşımanın tamamını kör zorlamak yerine en fazla size-2,
    # ama iskelet tahminini mümkün olduğunca koru.
    effective_carry = min(carry_target, max(1, int(size)-2))
    for n in carriers["Sayı"].astype(int):
        if len(selected) >= effective_carry:
            break
        selected.append(n)
...
if page == "✅ Kontrol":
    ...
    if len(df) < 100:
        ...
        st.subheader("4️⃣ V18.5.6 Nefes Puanı")
        st.dataframe(
            master18[
                [
                    "Sayı",
        ...
        st.write("**🧩 V18.5.6 Blok Rejim / Mikro-Konum Kararı**")
                ...
                if not settings_error:
                    try:
                        ...
                        if len(combined) > before_count:
                            ...
                            st.success(
                                "🧠 V18.5.6 sonucu otomatik öğrendi ve GitHub motor hafızasına yazdı."
                            )
                            st.info(
