...
APP_VERSION = "META V6.4 — 8 Uzman Dinamik Birleşim LAB"
...
def unified_walkforward(engine, data_df, history_start=220, meta_window=180,
    ...
    for i in range(min_meta_history, len(expert_rows)):
        ...
        Xtr = np.vstack([r["F"] for r in expert_rows[lo:i]])
        ytr = np.hstack([r["y"] for r in expert_rows[lo:i]])
    ...
    for i, cur in enumerate(meta):
        ...
        speak = bool(
            np.isfinite(recent40) and np.isfinite(prev40)
            and recent40 > 0.20
            and prev40 > -0.15
        )
        ...
        rows.append({
            ...
            "İsabet": hit,
            "Net": round(net,4),
            "KONUŞ/PAS": "KONUŞ" if speak else "PAS",
...
def unified_next_coupon(engine, data_df, expert_rows, meta_rows, wf_rows, neighbors=70):
    ...
    Fcur = _rank_matrix_from_orders(orders)

    # Son 180 geçmiş uzman-hedef örneğiyle eğit.
    hist = expert_rows[-180:]
    Xtr = np.vstack([r["F"] for r in hist])
    ytr = np.hstack([r["y"] for r in hist])
    model = make_pipeline(
        StandardScaler(),
        LogisticRegression(max_iter=300, C=0.08)
    )
...
st.header("5️⃣ UZMAN BİRLEŞİM MOTORU — Dinamik Koltuk / Dinamik Kupon")
st.caption(
