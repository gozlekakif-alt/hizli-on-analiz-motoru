...
APP_VERSION = "GEÇİŞ TEK LAB V1 — Walk-forward sayı kalibrasyonu"
...
# ===================================================================
# 9) GEÇİŞ TEK LAB V1 — yalnız doğrulanmış geçiş adayları
# ===================================================================

def _gecis_features(engine, t):
    """
    Yalnız geçmiş A[:t] ile kaynak -> sonraki sayı geçiş davranışını ölçer.
    Son çekilişteki 20 kaynak sayının her birinden 1..80 aday sayılara geçiş desteği üretir.
    """
    if t < 80:
        return pd.DataFrame()

    A = engine.A
    last_idx = np.where(A[t-1] == 1)[0]
    base = engine.window_rate(t, min(200, t))

    # Son 250 ardışık geçiş
    look = max(0, t - 250)
    end_transition = max(t - 1, 0)
    TT = engine.cumT[end_transition] - engine.cumT[look]
    SS = engine.cumS[end_transition] - engine.cumS[look]
    ...
    for n in range(80):
        ...
        for s in last_idx:
            ...
            if ev <= 0:
                continue
            src_events += ev
            p = (int(TT[s, n]) + 4.0 * base[n]) / (ev + 4.0)
            probs.append(p)
            lifts.append(p / max(base[n], 1e-6))
            if int(TT[s, n]) > 0:
                cover += 1

        p_mean = float(np.mean(probs)) if probs else float(base[n])
        lift_mean = float(np.mean(lifts)) if lifts else 1.0
        coverage = cover / max(len(last_idx), 1)

        # kısa/uzun frekans farkı
        f10 = float(engine.window_rate(t, 10)[n])
        f50 = float(engine.window_rate(t, 50)[n])
        trend = f10 - f50

        # aynı saat desteği
        target_time = engine.dts[t].strftime("%H:%M") if t < len(engine.dts) else engine.dts[t-1].strftime("%H:%M")
        inds = [i for i in engine.slot_idxs[target_time] if i < t]
        same_slot = float((engine.A[inds, n].sum() + 8*base[n]) / (len(inds)+8)) if inds else float(base[n])
    ...
    d["p_n"] = mm(d["p_mean"])
    d["lift_n"] = mm(np.clip(d["lift_mean"], 0, 5))
    d["coverage_n"] = mm(d["coverage"])
    d["trend_n"] = mm(d["trend"])
    d["slot_n"] = mm(d["same_slot"])

    # Skor sadece sıralama için; karar sonradan kalibrasyonla verilir.
    d["GEÇİŞ skor"] = np.clip(
        0.36*d["p_n"]
        + 0.24*d["lift_n"]
        + 0.18*d["coverage_n"]
        + 0.12*d["trend_n"]
    ...
    d = d.sort_values(
        ["GEÇİŞ skor","p_mean","lift_mean","coverage"],
        ascending=False
    ).reset_index(drop=True)
...
def gecis_walkforward_lab(engine, start_t=140):
    """
    Tüm adayları kaydeder. Her hedefte sadece geçmiş veri kullanılır.
    """
    rows = []
    for t in range(max(140, int(start_t)), len(engine.A)):
        if engine.draw_nos[t] != engine.draw_nos[t-1] + 1:
            continue
        tab = _gecis_features(engine, t)
        actual = set((np.where(engine.A[t] == 1)[0] + 1).tolist())
        for _, r in tab.iterrows():
            ...
            rows.append({
                ...
                "Sayı": n,
                "Sıra": int(r["Sıra"]),
                "GEÇİŞ skor": float(r["GEÇİŞ skor"]),
                "p_mean": float(r["p_mean"]),
                "lift_mean": float(r["lift_mean"]),
                "coverage": float(r["coverage"]),
                "trend": float(r["trend"]),
                "same_slot": float(r["same_slot"]),
                "base": float(r["base"]),
...
def gecis_calibration_report(wf):
    if wf.empty:
        return pd.DataFrame(), pd.DataFrame()

    d = wf.copy()
    idxs = np.array(sorted(d["Index"].unique()))
    q1 = idxs[len(idxs)//3]
    q2 = idxs[(2*len(idxs))//3]
    ...
    try:
        d["Skor dilimi"] = pd.qcut(d["GEÇİŞ skor"], 10, duplicates="drop")
    except Exception:
        d["Skor dilimi"] = pd.cut(d["GEÇİŞ skor"], 10)
...
def gecis_strict_selector_from_history(wf, target_index, current_tab):
    """
    0/1/2/3... sayı çıkışı.
    Yalnız geçmiş walk-forward adaylarından karar verir.

    Aynı rank + yakın skor + benzer coverage/lift bölgesinde:
      - en az 80 geçmiş aday
      - genel doğruluk >= %34
      - ilk yarı >= %31
      - son yarı >= %31
    koşulları sağlanırsa sayı verir.
    """
    ...
    for _, r in current_tab.iterrows():
        rank = int(r["Sıra"])
        score = float(r["GEÇİŞ skor"])
        cov = float(r["coverage"])
        lift = float(r["lift_mean"])

        h = hist[
            (hist["Sıra"] == rank)
            & (np.abs(hist["GEÇİŞ skor"] - score) <= 0.07)
            & (np.abs(hist["coverage"] - cov) <= 0.10)
            & (np.abs(hist["lift_mean"] - lift) <= 0.35)
        ].copy()

        if len(h) < 80:
            h = hist[
                (hist["Sıra"] == rank)
                & (np.abs(hist["GEÇİŞ skor"] - score) <= 0.11)
                & (np.abs(hist["coverage"] - cov) <= 0.16)
                & (np.abs(hist["lift_mean"] - lift) <= 0.55)
            ].copy()
        ...
        if len(h) >= 80:
            ...
            ok = bool(acc >= 0.34 and a1 >= 0.31 and a2 >= 0.31)
...
def gecis_strict_walkforward(engine, start_t=140):
    wf = gecis_walkforward_lab(engine, start_t=start_t)
    if wf.empty:
        return pd.DataFrame(), wf

    rows = []
    for t in sorted(wf["Index"].unique()):
        curtab = _gecis_features(engine, int(t))
        picks, detail = gecis_strict_selector_from_history(wf, int(t), curtab)
...
st.header("9️⃣ GEÇİŞ TEK LAB V1 — Sadece doğrulanmış geçiş adayları")
st.caption(
    "Yüksek GEÇİŞ skoru tek başına karar değildir. "
    "Her sayı geçmiş walk-forward'da aynı rank + skor + coverage + lift bölgesinin "
    "gerçek doğruluğuyla kalibre edilir. Kanıt yoksa PAS."
)

if st.button("🔀 GEÇİŞ motorunu tek başına test et", type="primary"):
    with st.spinner("GEÇİŞ walk-forward kalibrasyonu çalışıyor..."):
        strict_df, gecis_wf = gecis_strict_walkforward(eng, start_t=140)
        score_sum, rank_sum = gecis_calibration_report(gecis_wf)

    st.subheader("GEÇİŞ skor kalibrasyonu")
    ...
    if len(full):
        st.subheader("Tam isabetli GEÇİŞ olayları")
        st.dataframe(
            full.sort_values("Çekiliş", ascending=False),
            use_container_width=True, hide_index=True
        )

    st.subheader("Son 250 GEÇİŞ kararı")
    st.dataframe(
        strict_df.tail(250).sort_values("Çekiliş", ascending=False),
        use_container_width=True, hide_index=True
    )

    st.download_button(
        "⬇️ GEÇİŞ TEK LAB CSV indir",
        strict_df.to_csv(index=False).encode("utf-8-sig"),
        file_name="GECIS_TEK_LAB_WALKFORWARD.csv",
        mime="text/csv",
    )
    st.download_button(
        "⬇️ GEÇİŞ aday kalibrasyon CSV indir",
        gecis_wf.to_csv(index=False).encode("utf-8-sig"),
        file_name="GECIS_ADAY_KALIBRASYON.csv",
        mime="text/csv",
    )
