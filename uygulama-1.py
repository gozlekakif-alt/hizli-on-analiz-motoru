...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.5.9",
    page_icon="🎯",
    layout="wide",
)
...
def persistent_save_panel(df_to_save, key_prefix):
    """V18.5.9 — çekiliş/veri havuzu kalıcı kaydı PIN istemeden tek dokunuşla yapılır."""
...
def live_backtest(df, coupon_size=7, test_count=20, analysis_window=300):
    """
    V18.5.9 faz-kilitli walk-forward test.
    ...
    for i in range(start, len(work)):
        ...
        try:
            ...
            # V18.5.9: aynı saat/faz ve geçmiş karakter bağlamının kaç örneğe
...
st.title("🎯 Hızlı On Ultimate AI Studio V18.5.9")
...
def expert_context_calibration_v1857(df, target_time=None, character_window=12):
    """V18.5.9: hedef saat + gün fazı + GERÇEK yakın karaktere göre uzman katsayıları.
...
def seven_seat_master_v18567(df, master, items, latest_set, skeleton, regime_summary,
    ...
    # V18.5.9 — Korumalı Uzman Puanı. Konsensüsün güçlü ve gerçekten bağımsız
    ...
    # V18.5.9 Seat 1-2/3: bağlam güvenine göre KORUMALI UZMAN koltukları.
...
if page == "✅ Kontrol":
    ...
    if len(df) < 40:
        ...
        st.caption("V18.5.9 test: her geçmiş hedefte gerçek hedef saat kullanılır; yalnız geçmiş veriyle saat/faz/karakter bağlamı yeniden kurulur.")
        if st.button(
            "🧪 V18.5.9 FAZ/KARAKTER WALK-FORWARD TEST",
            key="v150_run_backtest",
        ):
            with st.spinner("V18.5.9 faz/saat/karakter kilitli walk-forward test çalışıyor; mobilde biraz sürebilir..."):
                bt = live_backtest(
                    df,
                    coupon_size=bt_size,
                    test_count=bt_count,
    ...
    if len(df) < 100:
        ...
        st.subheader("4️⃣ V18.5.9 Nefes Puanı")
        st.dataframe(
            master18[
                [
                    "Sayı",
        ...
        st.write("**🧩 V18.5.9 Blok Rejim / Mikro-Konum Kararı**")
        ...
        st.write("**🔁 V18.5.9 Elden Ele Taşıma Kimliği**")
        ...
        st.write("**🌙 V18.5.9 Dinlenmiş Dönüş Motoru**")
        ...
        st.caption("🏆 V18.5.9 OYNA — MASTER 7: faz-kilitli walk-forward doğrulama + gerçek karakter benzerliği + korumalı uzman koltukları + bağlama göre dinamik uzman ağırlığı + korelasyon/sürü freni + bağımsız keşif koltuğu + taşıma + dinlenmiş dönüş + blok + bölge/mikro + eşlikçi sinyallerini tek nihai kolonda birleştirir.")
