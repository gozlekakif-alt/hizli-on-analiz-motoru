...
def make_tickets(tab,expected_carry):
    carry=tab[tab["Kaynakta"]].sort_values(["Final","GüçlüKanal"],ascending=False)
    ret=tab[~tab["Kaynakta"]].sort_values(["Final","GüçlüKanal"],ascending=False)

    # 7A: taşıma + yol ağırlıklı
    k7a=int(np.clip(round(expected_carry*7/20),2,5))
    a=list(carry.head(k7a)["Sayı"].astype(int))
    a+=list(ret.head(7-len(a))["Sayı"].astype(int))

    # 7B: dönüş + sıcak/küme/pattern ağırlıklı
    pattern=tab.copy()
    pattern["P2"]=(
        0.28*pattern["Final"]+
        0.18*pattern["DönüşKanıt"]+
        0.16*pattern["SıcakKanıt"]+
        0.14*pattern["KümeKanıt"]+
        0.10*pattern["Step2Kanıt"]+
        0.08*pattern["ArdışıkKanıt"]+
        0.06*pattern["BantKanıt"]
    )
    b=list(pattern.sort_values(["P2","GüçlüKanal"],ascending=False).head(7)["Sayı"].astype(int))

    # 10'lu: consensus; iki uzman ortakları korunur
    scoreA={int(r.Sayı):i for i,r in carry.reset_index(drop=True).iterrows()}
    rankA={n:1/(1+i) for n,i in scoreA.items()}
    rankB={int(r.Sayı):1/(1+i) for i,r in pattern.sort_values("P2",ascending=False).reset_index(drop=True).iterrows()}
    z=tab.copy()
    z["Consensus"]=z.apply(
        lambda r: 0.58*r["Final"] + 0.22*rankA.get(int(r["Sayı"]),0) + 0.20*rankB.get(int(r["Sayı"]),0),
        axis=1
    )
    t10=list(z.sort_values(["Consensus","GüçlüKanal"],ascending=False).head(10)["Sayı"].astype(int))
...
st.caption(
    "V15'in ana ilkesi: yüksek skor tek başına yeterli değildir. "
