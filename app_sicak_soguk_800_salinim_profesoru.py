...
def streamlit_main():
    ...
    f=st.file_uploader("14 günlük ana TXT",type=["txt"])
    c1,c2=st.columns(2)
    ...
    if f and st.button("TAM ANALİZİ ÇALIŞTIR",type="primary"):
        raw=f.getvalue()
        tmp=Path("uploaded_master.txt"); tmp.write_bytes(raw)
        out=Path("HIZLI_ON_SICAK_SOGUK_800_SALINIM_TAM_CIKTI.zip")
        try:
            df,outputs=run(tmp,out,int(window),int(min_history))
