...
APP_VERSION = "META Tez–Antitez V4 — V18 Dört Uzman Füzyonu"
...
class MetaFeatureEngine:
    ...
    def build(self, t, target_time):
        ...
        # V18 Geçiş/Lift uzmanı: koşullu geçiş + lift + bağımsız kaynak desteği.
        transition_coverage = np.zeros(80, dtype=float)
        for n in range(80):
            transition_coverage[n] = sum(
                1 for s in src if int(SS[s]) > 0 and int(TT[s, n]) > 0
            )
        trans_lift = trans / np.maximum(base, 1e-6)
        v18_transition = (
            0.50 * self._minmax01(trans)
            + 0.30 * self._minmax01(trans_lift)
            + 0.20 * self._minmax01(transition_coverage)
        )
        ...
        v18_clock_general = self._v18_clock_general_expert(
            f[10], f[25], f[50], f[100],
            cycle_fit, repeat_rate, pair_strength,
            same_slot, same_hour, v18_block
        )
        ...
        return np.column_stack([
            ...
            v18_transition, v18_rest, v18_block, v18_clock_general,
        ]).astype(float)
...
st.title("🧠 Hızlı On — META Tez–Antitez V4 + V18 Dört Uzman")
st.caption(
...
st.info(
    "🧬 V18 Dört Uzman Füzyonu aktif: Geçiş/Lift + Dinlenmiş Dönüş + "
