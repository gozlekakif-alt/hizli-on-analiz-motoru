...
st.set_page_config(
    page_title="Hızlı On Ultimate Analiz Motoru V10.1",
    page_icon="🎯",
    layout="wide",
)
...
def github_settings():
    """Streamlit Secrets içinden GitHub kayıt ayarlarını oku.

    Hem [github] tablo biçimini hem de eski düz GITHUB_* anahtarlarını
    destekler. Böylece mobil arayüzde hangi biçim kullanılırsa kullanılsın
    kalıcı kayıt bağlantısı çalışır.
    """
    try:
        nested = st.secrets.get("github", {})
        token = str(
            nested.get("token", st.secrets.get("GITHUB_TOKEN", ""))
        ).strip()
        owner = str(
            nested.get("owner", st.secrets.get("GITHUB_OWNER", "gozlekakif-alt"))
        ).strip()
        repo = str(
            nested.get("repo", st.secrets.get("GITHUB_REPO", "hizli-on-analiz-motoru"))
        ).strip()
        branch = str(
            nested.get("branch", st.secrets.get("GITHUB_BRANCH", "main"))
        ).strip()
        path = str(
            nested.get("data_path", st.secrets.get("GITHUB_DATA_PATH", "veri.txt"))
        ).strip()
        admin_pin = str(
            nested.get("admin_pin", st.secrets.get("ADMIN_PIN", ""))
        ).strip()

        if not token:
            raise ValueError("GitHub token eksik")
        if not admin_pin:
            raise ValueError("Kalıcı kayıt PIN'i eksik")
        ...
        return None, (
            "Kalıcı GitHub kayıt kapalı. Streamlit > Settings > Secrets "
            "bölümüne [github] token, owner, repo, branch, data_path ve "
            "admin_pin bilgilerini ekle."
        )
...
st.title("🎯 Hızlı On Ultimate Analiz Motoru V10.1")
...
with tabs[0]:
    st.success("Çalışan sürüm: V10.1 — Ana dosya: app.py")
    ...
    if not st.session_state.extra_df.empty:
        st.subheader("💾 Yüklenen çekilişleri kalıcı kaydet")
        st.write(
            f"Birleşik havuz **{len(df)} çekiliş**. Kaydetme tamamlanınca "
            "GitHub'daki veri.txt güncellenir."
        )
        persistent_save_panel(df, "control_bulk")
