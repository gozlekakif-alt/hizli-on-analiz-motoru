...
APP_VERSION = "BLOK V3 — SIKI SAYI ÇIKIŞ LAB"
...
# ===================================================================
# 5) BLOK V3 — SIKI SAYI ÇIKIŞI
# ===================================================================

def _personal_repeat_rate(engine, t, n, lookback=200):
    """
    Hedef t için yalnız geçmiş ardışık geçişlerde n sayısının
    bir sonraki çekilişe tekrar etme oranı. Gelecek kullanılmaz.
    """
    idx = int(n) - 1
    cases = 0
    hits = 0
    j0 = max(0, t - int(lookback))
    for j in range(j0, t - 1):
        if engine.draw_nos[j + 1] != engine.draw_nos[j] + 1:
            continue
        if engine.A[j, idx]:
            cases += 1
            hits += int(engine.A[j + 1, idx])
    # 10 örneklik %25 prior ile shrink
    rate = (hits + 2.5) / (cases + 10.0) if cases >= 0 else 0.25
    return float(rate), int(cases), int(hits)

def block_v3_strict_candidates(engine, t):
    """
    Dondurulmuş BLOK V3 sıkı sinyali:
    1) Son çekilişte TAM 3 uzunluklu ardışık blok olacak.
    2) Son çekilişte toplam run_count == 4 olacak.
    3) Aday yalnız 3'lünün ORTA sayısıdır.
    4) SIKI modda kişisel tekrar oranı <= 0.24 ise sayı çıkar.
       Koşul yoksa 0 sayı.
    """
    if t < 120:
        return [], pd.DataFrame()

    prev_nums = (np.where(engine.A[t - 1] == 1)[0] + 1).tolist()
    bf = _bf(prev_nums)
    ...
    for run in bf["runs"]:
        if len(run) != 3:
            continue
        n = int(run[1])  # yalnız orta sayı
        rr, cases, hits = _personal_repeat_rate(engine, t, n, lookback=200)

        base_signal = (bf["run_count"] == 4)
        strict_signal = bool(base_signal and rr <= 0.24)

        rows.append({
            "Sayı": n,
            "Blok": "-".join(map(str, run)),
            "Toplam blok sayısı": int(bf["run_count"]),
            "Kişisel tekrar oranı": round(rr, 4),
            "Tekrar örneği": cases,
            "Tekrar isabeti": hits,
            "Temel sinyal": bool(base_signal),
            "SIKI sinyal": strict_signal,
        })
...
def block_v3_walkforward(engine, start_t=120):
    rows = []
    for t in range(max(120, int(start_t)), len(engine.A)):
        picks, detail = block_v3_strict_candidates(engine, t)
        actual = set((np.where(engine.A[t] == 1)[0] + 1).tolist())
        ...
        rows.append({
            ...
            "Kupon boyu": k,
            "İsabet": hit,
            "Tam isabet": bool(k > 0 and hit == k),
            "Karar": "KONUŞ" if k > 0 else "PAS",
...
st.header("5️⃣ BLOK V3 — SIKI SAYI ÇIKIŞI")
st.caption(
    "Bu bölüm yüksek Güven puanını karar olarak kullanmaz. "
    "Yalnız doğrulanmış yapısal BLOK sinyalinde 0/1/2... sayı çıkarır; "
    "koşul yoksa PAS verir."
)

if st.button("🎯 BLOK V3 sıkı walk-forward testini çalıştır", type="primary"):
    with st.spinner("BLOK V3 sıkı sinyal geçmişten geleceğe test ediliyor..."):
        v3 = block_v3_walkforward(eng, start_t=120)

    active = v3[v3["Kupon boyu"] > 0].copy()
    full = active[active["Tam isabet"] == True].copy()

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("KONUŞ olayı", len(active))
    ...
    st.subheader("Tam isabetli BLOK V3 olayları")
    if len(full):
        st.dataframe(
        ...
        st.info("Sıkı kuralda tam kolon oluşmadı.")

    st.subheader("Son 250 karar")
    st.dataframe(
        v3.tail(250).sort_values("Çekiliş", ascending=False),
        use_container_width=True, hide_index=True
    )

    st.download_button(
        "⬇️ BLOK V3 sıkı walk-forward CSV indir",
        v3.to_csv(index=False).encode("utf-8-sig"),
        file_name="BLOK_V3_SIKI_WALKFORWARD.csv",
        mime="text/csv",
    )

# Güncel hedef için otomatik sıkı karar
try:
    _v3_now, _v3_detail = block_v3_strict_candidates(eng, len(eng.A))
    st.subheader("🎯 Güncel BLOK V3 kararı")
    if _v3_now:
        st.success(
            "KONUŞ — BLOK yalnız şu sayılarda sıkı sinyal verdi: "
            + " - ".join(map(str, _v3_now))
        )
    else:
        st.warning("PAS — BLOK şu anda yeterince sıkı sayı sinyali vermiyor.")
    if not _v3_detail.empty:
        st.dataframe(_v3_detail, use_container_width=True, hide_index=True)
except Exception as _e:
    st.info(f"Güncel V3 karar üretilemedi: {_e}")
