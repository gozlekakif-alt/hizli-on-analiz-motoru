...
st.set_page_config(page_title="Hızlı On — ORTAK BEYİN V8", page_icon="🧠", layout="wide")
st.title("🧠 Hızlı On — ORTAK BEYİN V8")
st.caption("Geçmiş karakter + dinamik H kaynakları + aile/takım + ritim/uyku + son10 sıcak hafıza + NEGATIVE60 veto + dondurulmuş ileri test")
...
def _json_safe(obj):
    """SQLite JSON alanına yazmadan önce numpy/pandas tiplerini saf Python tiplerine çevir."""
...
def save_coupon_pack(target_draw,c4a,c4b,c5,diag):
    con=sqlite3.connect(DB)
    vals=(int(target_draw),int(df.iloc[-1]["draw"]),
          ",".join(map(str,c4a)),",".join(map(str,c4b)),",".join(map(str,c5)))
    exists=con.execute("SELECT id FROM coupon_sets WHERE target_draw=? AND history_last_draw=? AND coupon4_a=? AND coupon4_b=? AND coupon5=? LIMIT 1",vals).fetchone()
    if not exists:
        con.execute("""INSERT INTO coupon_sets(
            target_draw,history_last_draw,coupon4_a,coupon4_b,coupon5,diagnostics
        ) VALUES(?,?,?,?,?,?)""", vals+(json.dumps(_json_safe(diag),ensure_ascii=False),))
    con.commit(); con.close()
...
def freeze_prediction(target_draw,target_time,final5,final10,diag):
    ...
    con.execute("""INSERT INTO frozen_predictions
        ...
        (int(target_draw),str(target_time),int(df.iloc[-1]["draw"]),
         ...
         json.dumps(_json_safe(diag),ensure_ascii=False)))
    con.commit(); con.close()
...
def real20_pool(t, tab, diag):
    ...
    # --------------------------------------------------------
    # DİNAMİK KARAKTER HAKEMİ
    # Sabit H1/SHORT/MID/LONG/DEEP koltuk kotası YOK.
    # Her kaynak o eldeki PRE-H arzı, rejimi, aktif motoru,
    # aile yoğunluğu ve son10 canlılığı ile kendi ağırlığını kazanır.
    # --------------------------------------------------------
    srcs=["H1","SHORT","MID","LONG","DEEP"]
    regime_w={"HIGH":1.35,"MID":1.00,"LOW":0.72}
    strength={}
    detail={}

    for src in srcs:
        q=z[z["Kaynak"]==src].copy()
        supply=int(diag["supply"].get(src, len(q)))
        rw=regime_w.get(str(diag["regime"].get(src,"MID")),1.0)
        active=int((q["Sinyal"]!="ABSTAIN").sum())
        fam=int((q["Aile_skor"]>0).sum())
        hotn=int((q["Son10"]>=3).sum())
        top=float(q["REAL20_skor"].nlargest(min(5,len(q))).mean()) if len(q) else 0.0

        # kaynak içi sinyal kalitesi; salt arz büyük diye kaynak kazanmasın
        sig_density=active/max(1,len(q))
        fam_density=fam/max(1,len(q))
        hot_density=hotn/max(1,len(q))
        score=max(0.05, rw*(1.0 + 1.8*sig_density + 0.55*fam_density
                            + 0.35*hot_density + 0.035*max(0,top)))
        strength[src]=score
        detail[src]={"arz":supply,"rejim":diag["regime"].get(src,"MID"),
                     "aktif_sinyal":active,"aile_adayı":fam,"son10_sıcak":hotn,
                     "karakter_gücü":round(score,3)}

    # Kaynaklar 20 koltuk için yarışır. Minimum zorunlu koltuk yok.
    total=sum(strength.values())
    raw={k:20*strength[k]/total for k in srcs}
    seats={k:int(raw[k]) for k in srcs}
    left=20-sum(seats.values())
    order=sorted(srcs,key=lambda k:(raw[k]-seats[k],strength[k]),reverse=True)
    for k in order[:left]:
        seats[k]+=1

    # Kaynakta yeterli aday yoksa boş koltukları tekrar rekabete aç.
    overflow=0
    for k in srcs:
        avail=int((z["Kaynak"]==k).sum())
        if seats[k]>avail:
            overflow += seats[k]-avail
            seats[k]=avail
    while overflow>0:
        candidates=[k for k in srcs if seats[k] < int((z["Kaynak"]==k).sum())]
        if not candidates: break
        k=max(candidates,key=lambda x:strength[x])
        seats[k]+=1
        overflow-=1

    chosen=[]
    for src in sorted(srcs,key=lambda k:strength[k],reverse=True):
        q=z[z["Kaynak"]==src].sort_values(
            ["REAL20_skor","Aile_skor","Son10"],ascending=False)
        for n in q.head(seats[src])["Sayı"].astype(int):
            if n not in chosen: chosen.append(n)

    # Sadece teknik eksik kalırsa karakter sıralamasından tamamla.
    if len(chosen)<20:
        q=z[~z["Sayı"].isin(chosen)].sort_values(
            ["REAL20_skor","Aile_skor","Son10"],ascending=False)
        chosen += q.head(20-len(chosen))["Sayı"].astype(int).tolist()
    chosen=chosen[:20]

    pool=z[z["Sayı"].isin(chosen)].copy().sort_values("REAL20_skor",ascending=False)
    character_order=sorted(srcs,key=lambda k:strength[k],reverse=True)
    character={
        "sıralama":character_order,
        "güç":{k:round(strength[k],3) for k in character_order},
        "koltuk":{k:seats[k] for k in character_order},
        "detay":detail
    }
    return chosen,pool,character
...
def traced_coupon_pack(pool):
    ...
    for _, r in pool.iterrows():
        ...
        if src not in used_src:
            five.append(int(r["Sayı"])); used_src.add(src)
        if len(five)>=5: break
    for n in ranked:
...
# ============================================================
# V8 ORTAK BEYİN — KUPON HAKEMİ
# Tekil skor değil: takım uyumu + yaşam karakteri + sıcak hafıza + çeşitlilik.
# ============================================================
def _norm_series(x):
    x=pd.Series(x,dtype=float)
    mn,mx=float(x.min()),float(x.max())
    if mx<=mn: return pd.Series([0.5]*len(x),index=x.index)
    return (x-mn)/(mx-mn)

def recent_pair_co(t,a,b,w=24):
    lo=max(0,t-w)
    return sum(a in sets[i] and b in sets[i] for i in range(lo,t))

def recent_triple_co(t,tri,w=48):
    lo=max(0,t-w)
    ss=set(tri)
    return sum(ss.issubset(sets[i]) for i in range(lo,t))

def rhythm_bonus(t,n):
    # Korunan R1/R2/R3/R4 yaşam motiflerinin PRE-H karşılığı.
    hist=[i for i in range(max(0,t-40),t) if n in sets[i]]
    if len(hist)<3: return 0.0,[]
    gaps=[hist[i]-hist[i-1] for i in range(1,len(hist))]
    a=age_at(t,n); tags=[]; b=0.0
    # R3 [1,4,1] ve hedef age4; R4 [1,2,1] ve age7-9.
    if len(gaps)>=3 and gaps[-3:]==[1,4,1] and a==4:
        b+=1.25; tags.append('R3')
    if len(gaps)>=3 and gaps[-3:]==[1,2,1] and 7<=a<=9:
        b+=1.35; tags.append('R4')
    # R1/R2 benzeri sabit aralıklı titreşim için küçük destek; tek başına üretici değil.
    if len(gaps)>=3 and len(set(gaps[-3:]))==1:
        b+=0.55; tags.append('RİTİM')
    return b,tags

def neighbor_bonus(n, chosen):
    if not chosen: return 0.0
    # komşu/ardışık/birer atlamalı aileyi ödüllendir ama kolonun tamamını aynı kümeye yığma
    ds=[abs(n-x) for x in chosen]
    return 0.45*sum(d==1 for d in ds)+0.22*sum(d in (2,3) for d in ds)

def same_last_bonus(n, chosen):
    return 0.20*sum((n%10)==(x%10) for x in chosen)

def team_score(t, combo, score_map, source_map):
    combo=tuple(map(int,combo))
    base=sum(score_map.get(n,0.0) for n in combo)
    pair=sum(min(recent_pair_co(t,a,b,24),6)*0.30 + min(pair_history_score(t,a,b,48),12)*0.055
             for a,b in combinations(combo,2))
    tri=0.0
    if len(combo)>=3:
        tri=sum(min(recent_triple_co(t,x,48),3)*0.35 for x in combinations(combo,3))
    neigh=sum(0.18 for a,b in combinations(combo,2) if abs(a-b) in (1,2,3))
    last=sum(0.10 for a,b in combinations(combo,2) if a%10==b%10)
    diversity=0.28*len({source_map.get(n,'?') for n in combo})
    # aşırı tek aile/kaynak yığılmasını frenle
    counts=Counter(source_map.get(n,'?') for n in combo)
    concentration=sum(max(0,c-3)*0.35 for c in counts.values())
    return base+pair+tri+neigh+last+diversity-concentration

def build_v8_selector(t,pool,tab,diag):
    z=pool.copy().reset_index(drop=True)
    hot=hot_memory_table(t,10)[['Sayı','Sıcak_skor','Son10','Son3']]
    z=z.drop(columns=[c for c in ['Sıcak_skor','Son10','Son3'] if c in z.columns],errors='ignore').merge(hot,on='Sayı',how='left').fillna(0)
    z['N_real']=_norm_series(z['REAL20_skor'])
    z['N_family']=_norm_series(z['Aile_skor'])
    z['N_hot']=_norm_series(z['Sıcak_skor'])
    rbs=[]; rtags=[]
    for n in z['Sayı'].astype(int):
        b,tags=rhythm_bonus(t,n); rbs.append(b); rtags.append(tags)
    z['Ritim_bonus']=rbs
    z['Ritim']=rtags
    z['V8_tekil']=0.52*z['N_real']+0.23*z['N_family']+0.17*z['N_hot']+0.08*np.clip(z['Ritim_bonus'],0,1.5)/1.5

    # NEGATIVE60 veto: REAL20'ye zor giren, hiçbir aktif sinyali/aile/sıcak desteği olmayan aday düşürülür.
    z['Veto']=z.apply(lambda r: bool(r['Sinyal']=='ABSTAIN' and float(r['Aile_skor'])<=0 and int(r['Son10'])<=1),axis=1)
    eligible=z[~z['Veto']].copy()
    if len(eligible)<12: eligible=z.copy()
    ranked=eligible.sort_values(['V8_tekil','REAL20_skor','Aile_skor'],ascending=False)['Sayı'].astype(int).tolist()
    score_map={int(r.Sayı):float(r.V8_tekil) for r in z.itertuples()}
    source_map={int(r.Sayı):str(r.Kaynak) for r in z.itertuples()}

    # Kombinasyon uzayı 20C5=15504; telefon/Cloud için rahat ve deterministik.
    universe=ranked[:20]
    combos4=list(combinations(universe,4))
    scored4=sorted(((team_score(t,c,score_map,source_map),c) for c in combos4),reverse=True)
    c4a=list(scored4[0][1]) if scored4 else universe[:4]

    # B kolonunu A'nın kopyası yapma: en fazla 1 ortak sayı; yoksa en düşük ortaklı en güçlü.
    alternatives=[x for x in scored4 if len(set(x[1])&set(c4a))<=1]
    if not alternatives:
        minov=min(len(set(c)&set(c4a)) for _,c in scored4)
        alternatives=[x for x in scored4 if len(set(x[1])&set(c4a))==minov]
    c4b=list(alternatives[0][1]) if alternatives else universe[4:8]

    combos5=list(combinations(universe,5))
    scored5=sorted(((team_score(t,c,score_map,source_map),c) for c in combos5),reverse=True)
    c5=list(scored5[0][1]) if scored5 else universe[:5]

    # 4L-A1 özel motoru: yalnız mevcut yaklaşık tetikleyiciler aynı anda görünürse A kolonuna öncelik.
    supply=diag.get('supply',{})
    grid_a=(supply.get('SHORT',0)>=28 and supply.get('MID',99)<=18 and supply.get('LONG',99)<14)
    pair2c=diag.get('h1_mode')=='PAIR2C'
    deep=bool(diag.get('deep_exists'))
    h5='H5' in set(diag.get('mid_types',[]))
    sniper_on=bool(grid_a and pair2c and deep and h5)
    if sniper_on:
        h1=z[(z['Kaynak']=='H1') & z['Sinyal'].str.contains('PAIR2C',na=False)].sort_values(['V8_tekil','Aile_skor'],ascending=False)['Sayı'].astype(int).tolist()
        short=z[z['Kaynak']=='SHORT'].sort_values(['V8_tekil','Aile_skor'],ascending=False)['Sayı'].astype(int).tolist()
        sn=[]
        for n in h1[:3]+short[:3]:
            if n not in sn: sn.append(n)
            if len(sn)==4: break
        if len(sn)==4: c4a=sn
    ...
    for r in z.itertuples():
        tags=[]
        if str(r.Sinyal)!='ABSTAIN': tags.extend(str(r.Sinyal).split(' + '))
        if float(r.Aile_skor)>0: tags.append('AİLE/CO-LIFE')
        if int(r.Son10)>=3: tags.append('SON10 SICAK')
        tags.extend(list(r.Ritim))
        tags.append(str(r.Kaynak))
        reason[int(r.Sayı)]=' + '.join(dict.fromkeys(tags))
    diag2=dict(diag)
    diag2['v8']={'sniper_on':sniper_on,'4A_team_score':round(team_score(t,c4a,score_map,source_map),3),
                 '4B_team_score':round(team_score(t,c4b,score_map,source_map),3),
                 '5_team_score':round(team_score(t,c5,score_map,source_map),3)}
    return c4a,c4b,c5,z.sort_values('V8_tekil',ascending=False),reason,diag2
...
real20,real20_df,character20=real20_pool(t,tab,diag)
coupon4a,coupon4b,coupon5,v8_rank,reason_map,diag=build_v8_selector(t,real20_df,tab,diag)
...
st.caption("Koltuklar sabit değildir. Her elde PRE-H karakteri, aktif motorlar ve aile yapısı kaynakların payını yeniden belirler; son 10 sıcak hafıza canlı doğrulayıcıdır.")
