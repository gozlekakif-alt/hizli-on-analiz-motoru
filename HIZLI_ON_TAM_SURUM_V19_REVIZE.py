...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V19 — Saat Bazlı Canlı Karakter",
    page_icon="🎯",
    layout="wide",
)
...
# ============================================================
# V19 — SAAT BAZLI CANLI KARAKTER / SICAK ÇEKİRDEK MASTER
# Ana eksen: aynı saat + yakın slot.
# Tüm geçmiş yalnız destek katmanıdır.
# ============================================================

def _v19_minute_of_day(t):
...
def _v19_behavior(prev_set, cur_set):
    blocks = _v19_blocks(cur_set)
    return np.array(
        [
            len(prev_set & cur_set),
            len(cur_set - prev_set),
            len(blocks),
            sum(len(b) == 2 for b in blocks),
            sum(len(b) >= 3 for b in blocks),
            max([len(b) for b in blocks], default=1),
            sum(1 <= n <= 20 for n in cur_set),
            sum(21 <= n <= 40 for n in cur_set),
            sum(41 <= n <= 60 for n in cur_set),
            sum(61 <= n <= 80 for n in cur_set),
        ],
        dtype=float,
    )


def v19_detect_active_phase(frame, lookback=48):
    sets = _v19_sets(frame)
    if len(sets) < 8:
    ...
    for i in range(start, len(sets)):
        profiles.append(_v19_behavior(sets[i - 1], sets[i]))
        idxs.append(i)
    ...
    # Tek eldeki sıçrama fazı bozmasın; kalıcı değişim ara.
    for j in range(6, len(p)):
        before = p[j - 6:j - 2].mean(axis=0)
        after = p[j - 2:j + 1].mean(axis=0)
        distance = float(np.sqrt(np.mean(((after - before) / scale) ** 2)))
        candidate = idxs[j - 2]
        if distance >= 1.45 and candidate - breaks[-1] >= 4:
            breaks.append(candidate)

    phase_start = breaks[-1]
    # Çok eski bir fazı sonsuza kadar taşımamak için aktif pencereyi 18 elde sınırla.
    if len(sets) - phase_start > 18:
        phase_start = len(sets) - 18

    return phase_start, len(sets) - phase_start


def v19_phase_state(frame, phase_start):
    sets = _v19_sets(frame)
    psets = sets[phase_start:]
    if len(psets) < 2:
        return "Yeni / Belirsiz Faz"

    carries, block_strength = [], []
    for i in range(1, len(psets)):
        carries.append(len(psets[i - 1] & psets[i]))
        block_strength.append(
            sum(max(0, len(b) - 1) for b in _v19_blocks(psets[i]))
        )

    avg_carry = float(np.mean(carries)) if carries else 0.0
    avg_block = float(np.mean(block_strength)) if block_strength else 0.0
    ...
    return "Karışık / Dengeli Faz"


def _v19_indices_same_hour(frame, target_time, lookback=420):
    work = frame.tail(min(int(lookback), len(frame))).reset_index(drop=True)
    target_hour = int(str(target_time)[:2])
    return [
        i for i, row in work.iterrows()
        if int(str(row.Saat)[:2]) == target_hour
    ], work


def _v19_indices_same_slot(frame, target_time, tolerance=7, lookback=420):
    work = frame.tail(min(int(lookback), len(frame))).reset_index(drop=True)
    tm = _v19_minute_of_day(target_time)
    idxs = []
    for i, row in work.iterrows():
        d = abs(_v19_minute_of_day(row.Saat) - tm)
        d = min(d, 1440 - d)
        if d <= int(tolerance):
            idxs.append(i)
    return idxs, work
...
def _v19_frequency_rate(work, idxs):
    counter = Counter()
    for i in idxs:
        counter.update(int(work.iloc[i][c]) for c in NUM_COLS)
    denom = max(len(idxs), 1)
    return np.array(
        [counter.get(n, 0) / denom for n in range(1, 81)],
        dtype=float,
    )


def _v19_transition_rate(frame, number, lookback=300):
    sets = _v19_sets(frame.tail(min(int(lookback), len(frame))))
    cases = hits = 0
    for i in range(len(sets) - 1):
        if number in sets[i]:
            cases += 1
            hits += int(number in sets[i + 1])
    return (hits + 1.5) / (cases + 6.0)


def _v19_return_rate(frame, number, gap_target, lookback=300):
    sets = _v19_sets(frame.tail(min(int(lookback), len(frame))))
    cases = hits = 0
    for i in range(gap_target, len(sets) - 1):
        if (
            number in sets[i - gap_target]
            and all(
                number not in sets[k]
                for k in range(i - gap_target + 1, i + 1)
            )
        ):
            cases += 1
            hits += int(number in sets[i + 1])
...
def _v19_block_scores(frame, window=8):
    scores = np.zeros(80, dtype=float)
    sets = _v19_sets(frame.tail(min(int(window), len(frame))))
    for draw in sets:
        for block in _v19_blocks(draw):
            strength = max(1, len(block) - 1)
            for n in block:
                scores[n - 1] += strength
            for n in range(max(1, block[0] - 1), min(80, block[-1] + 1) + 1):
                scores[n - 1] += 0.30
    if scores.max() > 0:
        scores /= scores.max()
    return scores
...
def _v19_similar_hour_character(frame, target_time, state_window=5, search_window=360):
    if len(frame) < state_window + 35:
        return np.zeros(80, dtype=float)

    work = frame.tail(min(int(search_window), len(frame))).reset_index(drop=True)
    sets = _v19_sets(work)

    feats = [
        _v19_behavior(sets[i - 1], sets[i])
        for i in range(1, len(sets))
    ]
    f = np.vstack(feats)
    ...
    for end in range(state_window, len(f) - 1):
        ...
        dist = float(
            np.sqrt(np.mean((((hist - mu) / sd) - curz) ** 2))
        )
        next_idx = end + 1
        hour = int(str(work.iloc[next_idx].Saat)[:2])
        hour_bonus = 1.35 if hour == target_hour else 0.65
        weight = hour_bonus / (0.20 + dist)
        matches.append((weight, next_idx))

    matches = sorted(matches, reverse=True)[:24]
    score = np.zeros(80, dtype=float)
    total = 0.0
    for w, idx in matches:
        total += w
        score += w * np.array(
            [1.0 if n in sets[idx] else 0.0 for n in range(1, 81)]
        )
...
def v19_live_score_table(frame, target_time):
    work = frame.sort_values("Cekilis_No").reset_index(drop=True)
    phase_start, phase_age = v19_detect_active_phase(work)
    phase_name = v19_phase_state(work, phase_start)

    sets = _v19_sets(work)
    latest_set = sets[-1]
    phase_sets = sets[phase_start:]

    hour_idx, hour_work = _v19_indices_same_hour(work, target_time, 420)
    slot_idx, slot_work = _v19_indices_same_slot(work, target_time, 7, 420)

    hour_rate = _v19_frequency_rate(hour_work, hour_idx)
    slot_rate = _v19_frequency_rate(slot_work, slot_idx)

    phase_counter = Counter()
    for draw in phase_sets:
        phase_counter.update(draw)
    phase_hot = np.array(
        [phase_counter.get(n, 0) / max(len(phase_sets), 1) for n in range(1, 81)],
        dtype=float,
    )
    ...
    for draw in last8:
        core_counter.update(draw)
    core_hot = np.array(
        [core_counter.get(n, 0) / max(len(last8), 1) for n in range(1, 81)],
        dtype=float,
    )
    ...
    persistence = np.array(
        [
            0.65 * (sum(n in s for s in last3) / max(len(last3), 1))
            + 0.35 * core_hot[n - 1]
            for n in range(1, 81)
        ],
        dtype=float,
    ...
    for draw in last8[-5:]:
        short_counter.update(draw)
    short_hot = np.array(
        [
            short_counter.get(n, 0) / max(min(5, len(last8)), 1)
            for n in range(1, 81)
        ],
        dtype=float,
    )
    ...
    carry = np.array(
        [_v19_transition_rate(work, n) for n in range(1, 81)],
        dtype=float,
    )
    ...
    for n in range(1, 81):
        gap = len(sets)
        for i, draw in enumerate(reversed(sets)):
            if n in draw:
                gap = i
                break
        gaps[n] = gap
    ...
    returns = np.array(
        [
            _v19_return_rate(work, n, gaps[n])
            if 1 <= gaps[n] <= 4 else 0.0
            for n in range(1, 81)
        ],
        dtype=float,
    )

    block = _v19_block_scores(work, 8)
    similar = _v19_similar_hour_character(work, target_time)

    recent_idx = list(range(max(0, len(work) - 120), len(work)))
    history = _v19_frequency_rate(work, recent_idx)
    last_bonus = np.array(
        [1.0 if n in latest_set else 0.0 for n in range(1, 81)],
        dtype=float,
    )

    # Saat ana eksen. Geçmiş toplam yalnız %1.
    weights = {
        "hour": 0.30,
        "slot": 0.14,
        "phase": 0.16,
    ...
    elif phase_name == "Blok / Ardışık Fazı":
        weights["block"] += 0.06
        weights["core"] += 0.03
        weights["phase"] += 0.02
        weights["hour"] -= 0.02
    ...
    elif phase_name == "Yenilenme Fazı":
        weights["return"] += 0.05
        weights["core"] += 0.03
    ...
    score = (
        ...
        + weights["persist"] * persistence
        + weights["short"] * short_hot
        + weights["carry"] * (
            carry * last_bonus
            + 0.35 * carry * (1.0 - last_bonus)
        )
        + weights["return"] * returns
        + weights["block"] * block
    ...
    for n in range(1, 81):
        ...
        if hour_rate[n - 1] >= np.quantile(hour_rate, 0.70):
            reasons.append("saat güçlü")
        if slot_rate[n - 1] >= np.quantile(slot_rate, 0.70):
            reasons.append("aynı slot")
        if phase_hot[n - 1] >= 0.42:
            reasons.append("faz sıcak")
        if core_hot[n - 1] >= 0.50:
            reasons.append("sıcak çekirdek")
        if persistence[n - 1] >= 0.60:
            reasons.append("çekirdek sürekliliği")
        if n in latest_set and carry[n - 1] >= np.quantile(carry, 0.65):
            reasons.append("taşıma")
        if 1 <= gaps[n] <= 4 and returns[n - 1] >= 0.24:
            reasons.append(f"{gaps[n]} el dönüş")
        if block[n - 1] >= 0.35:
            reasons.append("blok / ardışık")
        if similar[n - 1] >= np.quantile(similar, 0.70):
            reasons.append("benzer saat karakteri")

        rows.append(
            {
                "Sayı": n,
                "V19 Puan": round(score[n - 1] * 100.0, 3),
                "Saat %": round(hour_rate[n - 1] * 100.0, 2),
                "Slot %": round(slot_rate[n - 1] * 100.0, 2),
                "Faz sıcak %": round(phase_hot[n - 1] * 100.0, 2),
                "Çekirdek %": round(core_hot[n - 1] * 100.0, 2),
                "Süreklilik %": round(persistence[n - 1] * 100.0, 2),
                "Taşıma %": round(carry[n - 1] * 100.0, 2),
                "Dinlenme": gaps[n],
                "Dönüş %": round(returns[n - 1] * 100.0, 2),
                "Blok %": round(block[n - 1] * 100.0, 2),
                "Benzer %": round(similar[n - 1] * 100.0, 2),
                "Neden": ", ".join(reasons) if reasons else "destek",
            }
        )
    ...
    # Esas oyun 7'li tek kupondur.
    coupon = sorted(table.head(7)["Sayı"].astype(int).tolist())

    meta = {
        "phase_start": int(phase_start),
        "phase_age": int(phase_age),
        "phase": phase_name,
        "hour_samples": len(hour_idx),
        "slot_samples": len(slot_idx),
        "weights": weights,
    }
    return coupon, table, meta


def v19_blind_test(frame, test_count=10):
    work = frame.sort_values("Cekilis_No").reset_index(drop=True)
    rows = []
    start = max(120, len(work) - int(test_count))

    for i in range(start, len(work)):
        train = work.iloc[:i].copy()
        target = work.iloc[i]
        coupon, _, meta = v19_live_score_table(train, str(target.Saat))
        actual = set(int(target[c]) for c in NUM_COLS)
        hits = sorted(set(coupon) & actual)

        hour_idx, hour_work = _v19_indices_same_hour(
            train, str(target.Saat), 420
        )
        base_rate = _v19_frequency_rate(hour_work, hour_idx)
        baseline = sorted(
            (np.argsort(base_rate)[::-1][:7] + 1).tolist()
        )
...
if page == "✅ Kontrol":
    st.success("Çalışan sürüm: V19 SAAT BAZLI MASTER — Tam Sürüm Revizyon")
...
elif page == "🧠 Saat Bazlı MASTER V19":
    st.header("🧠 Saat Bazlı MASTER V19 — Canlı Karakter / Sıcak Çekirdek")
    st.caption(
        "Ana eksen aynı saat + yakın zaman slotudur. "
        "Aktif faz sıcak çekirdeği, çekirdek sürekliliği, taşıma, 1–4 el dönüş, "
        "blok/ardışık ve benzer saat karakteri birlikte çalışır. "
        "Tüm geçmiş yalnız destek katmanıdır."
    )

    if len(df) < 120:
        st.warning("V19 için en az 120 çekiliş önerilir.")
    else:
        next_no, next_date, next_time = next_draw_defaults(df)
        coupon, score_table, meta = v19_live_score_table(
            df, next_time
        )

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Aktif faz", meta["phase"])
        c2.metric("Faz yaşı", f"{meta['phase_age']} el")
        c3.metric("Aynı saat örneği", meta["hour_samples"])
        c4.metric("Yakın slot örneği", meta["slot_samples"])

        phase_row = df.sort_values("Cekilis_No").reset_index(drop=True).iloc[
            meta["phase_start"]
        ]
        st.write(
            f"**Hedef:** #{next_no} — {next_date} {next_time}"
        )
        st.write(
            f"**Aktif faz başlangıcı:** "
            f"#{int(phase_row.Cekilis_No)} — "
            f"{phase_row.Tarih} {phase_row.Saat}"
        )
        st.success(
            "🏆 TEK MASTER 7: "
            + " - ".join(map(str, coupon))
        ...
        st.subheader("🔥 Sıcak çekirdek / saat puan tablosu")
        st.dataframe(
            score_table.head(25),
            use_container_width=True,
            hide_index=True,
        ...
        with st.expander("⚖️ V19 canlı ağırlıkları", expanded=False):
            weight_df = pd.DataFrame(
                [
                    {"Motor": key, "Ağırlık %": round(value * 100.0, 2)}
                    for key, value in meta["weights"].items()
                ]
        ...
        st.divider()
        st.subheader("🧪 Sızıntısız kör test")
        test_n = st.selectbox(
            "Test adedi",
            [10, 25, 50, 100],
            index=0,
        ...
        if st.button(
            "🚀 V19 KÖR TESTİ ÇALIŞTIR",
            type="primary",
            use_container_width=True,
            key="v19_test_run",
        ):
            with st.spinner(
                f"Son {test_n} çekiliş ileriye bakmadan test ediliyor..."
            ):
                result_v19 = v19_blind_test(df, test_n)

            st.session_state["v19_last_test"] = result_v19

        result_v19 = st.session_state.get(
            "v19_last_test", pd.DataFrame()
        )

        if isinstance(result_v19, pd.DataFrame) and not result_v19.empty:
            x1, x2, x3, x4 = st.columns(4)
            x1.metric("Test", len(result_v19))
            x2.metric(
                "V19 ortalama",
                f"{result_v19['İsabet'].mean():.2f}",
            )
            x3.metric(
                "Saat baz çizgisi",
                f"{result_v19['Baz İsabet'].mean():.2f}",
            )
            x4.metric(
                "Net avantaj",
                f"{result_v19['Avantaj'].mean():+.2f}",
            )
            ...
            st.download_button(
                "⬇️ V19 kör test CSV indir",
                data=result_v19.to_csv(
                    index=False
                ).encode("utf-8-sig"),
                file_name=f"V19_SAAT_BAZLI_KOR_TEST_{len(result_v19)}.csv",
                mime="text/csv",
                use_container_width=True,
                key="v19_test_csv",
            )
...
3 9 22 31 44 58 69""",
    ...
    st.header("✅ Kupon Sonucu Kontrol + Havuza Kaydet")

    default_no, default_date, default_time = next_draw_defaults(df)

    rc1, rc2, rc3 = st.columns(3)
    result_no = rc1.number_input(
        "Çekiliş no",
        min_value=1,
    ...
    result_date = rc2.text_input(
        "Tarih",
        value=default_date,
        key="v19_result_date",
    )
    result_time = rc3.text_input(
        "Saat",
        value=default_time,
        key="v19_result_time",
    )
    ...
    coupon_text = st.text_area(
        "Kupon sayıları",
        placeholder="7 11 18 24 39 52 71",
        key="v19_result_coupon",
    )
    result_text = st.text_area(
        "Gerçek çekiliş sonucu — tam 20 sayı",
        placeholder="1 7 11 14 18 ... toplam 20 farklı sayı",
        key="v19_result_draw",
        height=180,
    )

    result_numbers = extract_exact_twenty_numbers(result_text)
    coupon_vals = [
        int(x)
        for x in re.findall(
            r"(?<!\d)(?:[1-9]|[1-7]\d|80)(?!\d)",
    ...
    coupon_vals = sorted(set(coupon_vals))

    if result_text.strip() and result_numbers is None:
        st.error(
            "Sonuçta tam 20 farklı sayı bulunmalı."
        )

    if result_numbers:
        hits = sorted(set(coupon_vals) & set(result_numbers))
        if coupon_vals:
            st.metric(
                "İsabet",
                f"{len(hits)} / {len(coupon_vals)}",
            )
            st.write(
                "Tutan sayılar:",
        ...
        st.success(
            "Sonuç geçerli: 20 farklı sayı okundu."
        )

        result_row = [
            int(result_no),
            str(result_date).strip(),
            str(result_time).strip(),
            *sorted(result_numbers),
        ]

        if st.button(
            "✅ Sonucu canlı havuza ekle",
            use_container_width=True,
            key="v19_result_add_session",
        ):
            candidate = merge_data(
                df,
                pd.DataFrame([result_row], columns=COLS),
            )
            ...
            st.success(
                f"#{int(result_no)} bu oturumun havuzuna eklendi."
            )
            st.cache_data.clear()
            st.rerun()

        if st.button(
            "☁️ Sonucu veri.txt ana havuzuna KALICI kaydet",
            type="primary",
            use_container_width=True,
            key="v19_result_save_github",
        ):
            candidate = merge_data(
                df,
                pd.DataFrame([result_row], columns=COLS),
            ...
            if settings_error:
                st.error(
                    "GitHub kalıcı kayıt ayarları hazır değil. "
                    "Streamlit Secrets içindeki [github] ayarlarını kontrol et."
                )
            else:
                try:
                    with st.spinner(
                        "Sonuç veri.txt ana havuzuna yazılıyor..."
                    ):
                        update_github_file(
                            settings,
                            to_text(candidate),
                            f"V19 sonuç kaydı: {int(result_no)}",
                        )
                    st.session_state.extra_df = pd.DataFrame(columns=COLS)
                    st.cache_data.clear()
                    st.success(
                        f"#{int(result_no)} GitHub veri.txt ana havuzuna kaydedildi."
                    )
                    st.rerun()
                except Exception as exc:
    ...
    st.divider()
    st.header("📚 Çoklu Çekiliş Ekle")
    st.caption(
        "Birden fazla standart satırı veya birden fazla 'Çekiliş no:' bloğunu "
        "tek seferde yapıştırabilirsin."
    )

    multi_raw = st.text_area(
        "Çoklu sonuç metni",
        height=320,
        key="v19_multi_draw_text",
        placeholder=(
            "47776;09.08.2026;08:52;1,2,3,...\n"
            "47777;09.08.2026;08:57;...\n\n"
            "veya Çekiliş no: bloklarını art arda yapıştır."
        ),
    )

    multi_df = pd.DataFrame(columns=COLS)
    if multi_raw.strip():
        multi_df = dataframe_from_text(multi_raw)

        if multi_df.empty:
            st.error("Okunabilir çekiliş bulunamadı.")
        else:
            merged_preview = merge_data(df, multi_df)
            net_added = len(merged_preview) - len(df)
            st.success(
                f"{len(multi_df)} çekiliş okundu. "
                f"Havuza net eklenecek kayıt: {net_added}"
            )
            st.dataframe(
                multi_df[
                    ["Cekilis_No", "Tarih", "Saat"]
                ].tail(30),
                use_container_width=True,
                hide_index=True,
            )
    ...
    if not multi_df.empty:
        if st.button(
            "➕ Çoklu veriyi bu oturumun havuzuna ekle",
            use_container_width=True,
            key="v19_multi_add_session",
        ):
            st.session_state.extra_df = merge_data(
                st.session_state.extra_df,
                multi_df,
        ...
        if st.button(
            "☁️ Çoklu veriyi veri.txt ana havuzuna KALICI kaydet",
            type="primary",
            use_container_width=True,
            key="v19_multi_save_github",
        ):
            candidate = merge_data(df, multi_df)
            settings, settings_error = github_settings()
            ...
            if settings_error:
                ...
                try:
                    with st.spinner(
                        "Çoklu çekilişler ana havuza kaydediliyor..."
                    ):
                        update_github_file(
                            settings,
                            to_text(candidate),
                            (
                                "V19 çoklu çekiliş kaydı: "
                                f"{int(multi_df.Cekilis_No.min())}-"
                                f"{int(multi_df.Cekilis_No.max())}"
                            ),
                        )
                    st.session_state.extra_df = pd.DataFrame(columns=COLS)
                    st.cache_data.clear()
                    st.success(
                        "Çoklu çekilişler GitHub veri.txt ana havuzuna kaydedildi."
                    )
                    st.rerun()
                except Exception as exc:
                    st.error(f"Kalıcı kayıt başarısız: {exc}")
...
st.caption(
    "V19; mevcut tam sürümün analiz, arşiv, dışa aktarım ve önceki motorlarını korur. Yeni ana karar motoru saat bazlı canlı karakterdir. "
