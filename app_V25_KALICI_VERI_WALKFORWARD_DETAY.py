...
def load_data():
    ...
    if up is not None:
        ...
        st.session_state["v25_uploaded_name"]=up.name
        st.session_state["v25_uploaded_text"]=raw_text
        return parsed,f"Yüklenen: {up.name}"
    st.session_state.pop("v25_uploaded_name",None)
    st.session_state.pop("v25_uploaded_text",None)
    try:
        txt=github_read()
        df=parse_any(txt)
...
def dataframe_to_veri_txt(df):
    """Tek kanonik satır biçimi: çekiliş;tarih;saat;20 sayı."""
    if df is None or df.empty:
        return ""
    z=df.copy()
    if "_dt" not in z.columns:
        z["_dt"]=pd.to_datetime(z["date"]+" "+z["time"],format="%d.%m.%Y %H:%M",errors="coerce")
    z=(z.dropna(subset=["_dt"]).sort_values(["_dt","draw_no"])
         .drop_duplicates(["date","time"],keep="last").reset_index(drop=True))
    lines=[]
    for _,r in z.iterrows():
        nums=sorted(set(map(int,r["numbers"])))
        if len(nums)!=20:
            continue
        lines.append(f"{int(r['draw_no'])};{r['date']};{r['time']};"+",".join(map(str,nums)))
    return "\n".join(lines)+( "\n" if lines else "" )


def persist_uploaded_dataset(upload_df):
    """Yüklenen toplu veriyi mevcut veri.txt ile birleştirir; mümkünse GitHub'a kalıcı yazar."""
    if upload_df is None or upload_df.empty:
        raise ValueError("Yüklenen dosyada geçerli çekiliş bulunamadı.")
    existing=pd.DataFrame()
    try:
        existing=parse_any(github_read())
    except Exception:
        if DATA_FILE.exists():
            try: existing=parse_any(DATA_FILE.read_text(encoding="utf-8",errors="ignore"))
            except Exception: existing=pd.DataFrame()
    merged=pd.concat([existing,upload_df],ignore_index=True) if not existing.empty else upload_df.copy()
    if "_dt" not in merged.columns:
        merged["_dt"]=pd.to_datetime(merged["date"]+" "+merged["time"],format="%d.%m.%Y %H:%M",errors="coerce")
    merged=(merged.dropna(subset=["_dt"]).sort_values(["_dt","draw_no"])
            .drop_duplicates(["date","time"],keep="last")
            .drop_duplicates(["draw_no"],keep="last").reset_index(drop=True))
    txt=dataframe_to_veri_txt(merged)
    DATA_FILE.write_text(txt,encoding="utf-8")
    where="Yerel oturum"
    try:
        _,_,_,data_path=github_config()
        github_write_path(data_path,txt,"V25 toplu veri.txt kalıcı güncelleme")
        where="GitHub veri.txt + yerel"
    except Exception:
        pass
    return merged,where
...
def _join_nums(values):
    return "-".join(map(str,sorted(set(map(int,values)))))


def _surgery_reason_for_number(n,p,actual):
    n=int(n); surg=p["surgery"]
    base=set(map(int,surg["base5"])); exp=set(map(int,surg["experimental5"])); live=set(map(int,surg["live5"]))
    entry=surg.get("entry"); exit_n=surg.get("exit")
    if n in exp:
        if n==entry and n not in base:
            return "Cerrahi giriş adayıydı ve üçlü kilitle 5'liye girdi"
        if n in base:
            return "Base5 seçildi; cerrahi sonrasında da korundu"
        return "Cerrahi5 içinde"
    if n in base and n==exit_n:
        return "Base5'teydi fakat cerrahi çıkış adayı olarak çıkardı"
    if n in base:
        return "Base5'teydi; deneysel listede beklenmedik şekilde yok"
    if entry is not None and n==int(entry):
        if not surg.get("triple_lock"):
            return "Cerrahi giriş adayıydı fakat üçlü kilit açılmadı"
        return "Cerrahi giriş adayıydı fakat nihai 5'e girmedi"
    rank_rows=p["tab"].index[p["tab"]["Sayı"].astype(int)==n].tolist()
    rank=(rank_rows[0]+1) if rank_rows else 999
    if rank<=16:
        return "Havuz16'da; 5 koltuk sınırı / çeşitlilik seçicisi nedeniyle Base5'e inmedi; cerrahi yalnız tek dış giriş adayı denedi"
    return "Havuz16 dışında"


def blind_test(df,ntest=40,min_train=120):
    """Sıralı öğrenen kör test + hedef ve sayı bazlı teşhis. Sonuç, tahminden sonra açılır."""
    rows=[]; number_rows=[]; simbrain=_default_brain(); start=max(min_train,len(df)-ntest)
    for i in range(start,len(df)):
        train=df.iloc[:i].reset_index(drop=True)
        if len(train)<min_train: continue
        target=df.iloc[i]
        ...
        actual=set(map(int,target["numbers"])); snap=make_snapshot(p,int(target["draw_no"]),target["date"],target["time"])
        pool12=p["tab"].head(12)["Sayı"].astype(int).tolist()
        pool16=p["tab"].head(16)["Sayı"].astype(int).tolist()
        core5=list(map(int,p["core5"])); exp5=list(map(int,p["surgery"]["experimental5"])); live5=list(map(int,p["surgery"]["live5"]))
        pool16_hits=sorted(set(pool16)&actual); core5_hits=sorted(set(core5)&actual); exp5_hits=sorted(set(exp5)&actual); live5_hits=sorted(set(live5)&actual)
        missed_from_pool=sorted(set(pool16_hits)-set(exp5))
        missed_reasons=[]
        for n in missed_from_pool:
            missed_reasons.append(f"{n}: {_surgery_reason_for_number(n,p,actual)}")
        rows.append({
            "Çekiliş":int(target["draw_no"]),"Tarih":target["date"],"Saat":target["time"],"Gece":p["char"]["label"],
            "Gerçek20":_join_nums(actual),
            "4 Çekirdek":len(set(p["core4"])&actual),"4 Alternatif":len(set(p["alt4"])&actual),
            "5 Çekirdek":len(core5_hits),"5 Alternatif":len(set(p["alt5"])&actual),
            "Canlı5":len(live5_hits),"Havuz12":len(set(pool12)&actual),"Havuz16":len(pool16_hits),
            "Cerrahi5":len(exp5_hits),"CerrahiAçıldı":int(p["surgery"]["triple_lock"]),"CerrahiOnaylı":int(p["surgery"]["brain_approved"]),
            "Havuz16 Sayıları":_join_nums(pool16),"Havuz16 Doğruları":_join_nums(pool16_hits),
            "Base5 Sayıları":_join_nums(core5),"Base5 Doğruları":_join_nums(core5_hits),
            "Cerrahi5 Sayıları":_join_nums(exp5),"Cerrahi5 Doğruları":_join_nums(exp5_hits),
            "Canlı5 Sayıları":_join_nums(live5),"Canlı5 Doğruları":_join_nums(live5_hits),
            "Cerrahi Çıkış":p["surgery"].get("exit"),"Cerrahi Giriş":p["surgery"].get("entry"),
            "Havuz Doğru Ama Cerrahi5 Dışı":_join_nums(missed_from_pool),
            "Neden Cerrahi5'e Girmedi": " | ".join(missed_reasons)
        })
        # Top16'nın her sayısını tek tek kaydet: doğru 8'in neden seçilmediğini burada görebiliriz.
        top16=p["tab"].head(16).copy().reset_index(drop=True)
        for rank,r in top16.iterrows():
            n=int(r["Sayı"])
            number_rows.append({
                "Çekiliş":int(target["draw_no"]),"Tarih":target["date"],"Saat":target["time"],"Sıra":rank+1,"Sayı":n,
                "Gerçekte Çıktı":int(n in actual),"Base5":int(n in core5),"Cerrahi5":int(n in exp5),"Canlı5":int(n in live5),
                "Cerrahi Durumu":_surgery_reason_for_number(n,p,actual),
                "Ana Puan":float(r["Ana Puan"]),"Ana Yüzdelik":float(r["Ana Yüzdelik"]),
                "Uzman Oy":int(r["Uzman Oy"]),"Güçlü Oy":int(r["Güçlü Oy"]),"Gap":int(r["Gap"]),
                "Yaşam İzi":str(r["Yaşam İzi"]),"Kaynaklar":str(r["Kaynaklar"]),
                "Cerrahi Çıkış":p["surgery"].get("exit"),"Cerrahi Giriş":p["surgery"].get("entry"),
                "Üçlü Kilit":int(p["surgery"]["triple_lock"]),"Merkez Onayı":int(p["surgery"]["brain_approved"])
            })
        simbrain,_,_=update_brain_from_result(simbrain,snap,actual)
    return pd.DataFrame(rows),simbrain,pd.DataFrame(number_rows)
...
# Toplu yükleme önce geçicidir; kullanıcı açıkça isterse mevcut veri.txt ile birleştirip kalıcılaştırılır.
if source.startswith("Yüklenen:"):
    st.sidebar.success(f"Geçici dosya okundu: {len(df)} çekiliş")
    if st.sidebar.button("💾 TXT'YE KALICI KAYDET",type="primary",key="persist_uploaded_v25"):
        try:
            merged,where=persist_uploaded_dataset(df)
            st.sidebar.success(f"Kalıcı kayıt tamam: {len(merged)} çekiliş · {where}")
            st.session_state["v25_persist_message"]=f"{len(merged)} çekiliş kalıcı veri.txt'ye birleştirildi ({where})."
        except Exception as e:
            st.sidebar.error(f"Kalıcı kayıt başarısız: {e}")
if st.session_state.get("v25_persist_message"):
    st.sidebar.caption("✅ "+st.session_state["v25_persist_message"])
...
with tabs[4]:
    ...
    if st.button("▶️ Kör testi çalıştır",type="primary"):
        with st.spinner("Kör walk-forward + sıralı öğrenme + cerrahi teşhis çalışıyor..."):
            bt,simbrain,btd=blind_test(df,ntest=ntest,min_train=min(120,max(50,len(df)//3)))
            ma=mechanism_blind_audit(df,ntest=min(ntest,100),min_train=min(120,max(50,len(df)//3)))
        st.session_state["v25_bt"]=bt; st.session_state["v25_btd"]=btd; st.session_state["v25_ma"]=ma; st.session_state["v25_simbrain"]=simbrain
    bt=st.session_state.get("v25_bt")
    btd=st.session_state.get("v25_btd")
    if isinstance(bt,pd.DataFrame) and not bt.empty:
        first=bt.iloc[0]; last=bt.iloc[-1]
        st.info(f"Test aralığı: **{first['Tarih']} {first['Saat']} → {last['Tarih']} {last['Saat']}** · {len(bt)} hedef · {bt['Tarih'].nunique()} takvim günü")
        ...
        max16=int(bt["Havuz16"].max())
        st.markdown(f"### 🎯 Havuz16 maksimum {max16} doğru yakaladığı çekilişler")
        maxrows=bt[bt["Havuz16"]==max16].copy()
        diagcols=["Çekiliş","Tarih","Saat","Havuz16","Havuz16 Doğruları","5 Çekirdek","Base5 Doğruları","Cerrahi5","Cerrahi5 Doğruları","Havuz Doğru Ama Cerrahi5 Dışı","Neden Cerrahi5'e Girmedi"]
        st.dataframe(maxrows[[c for c in diagcols if c in maxrows.columns]],use_container_width=True,hide_index=True)

        st.markdown("### 📋 Çekiliş bazlı walk-forward detay")
        st.dataframe(bt.sort_values("Çekiliş",ascending=False),use_container_width=True,hide_index=True)
        cdl1,cdl2,cdl3=st.columns(3)
        cdl1.download_button("⬇️ Özet CSV",summary.to_csv(index=False).encode("utf-8-sig"),file_name="v25_walk_forward_ozet.csv",mime="text/csv")
        cdl2.download_button("⬇️ DETAY walk-forward CSV",bt.to_csv(index=False).encode("utf-8-sig"),file_name="v25_walk_forward_detay.csv",mime="text/csv")
        if isinstance(btd,pd.DataFrame) and not btd.empty:
            cdl3.download_button("⬇️ SAYI/CERRAHİ teşhis CSV",btd.to_csv(index=False).encode("utf-8-sig"),file_name="v25_walk_forward_sayi_cerrahi_detay.csv",mime="text/csv")
            st.markdown("### 🩺 Havuz16 doğru olup Cerrahi5'e girmeyen sayılar")
            lost=btd[(btd["Gerçekte Çıktı"]==1)&(btd["Cerrahi5"]==0)].copy()
            st.dataframe(lost.sort_values(["Çekiliş","Sıra"],ascending=[False,True]),use_container_width=True,hide_index=True)
