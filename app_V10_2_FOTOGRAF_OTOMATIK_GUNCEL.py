...
st.markdown("### ➕ Yeni çekiliş sonucu")

def _ocr_result_photo(uploaded, fallback_draw, fallback_date, fallback_time):
    """Fotoğraftan çekiliş başlığı + 20 sayıyı OCR ile okumayı dener."""
    if uploaded is None:
        return None, ""
    try:
        ...
        # Telefon ekranı / fiş fotoğrafı için OCR ön işleme.
        w,h=img.size
        if w < 1400:
            scale=max(2, int(1400/max(w,1)))
            img=img.resize((w*scale,h*scale))
        img=ImageOps.autocontrast(img)
        img=ImageEnhance.Contrast(img).enhance(1.8)
        img=img.filter(ImageFilter.SHARPEN)
        text=pytesseract.image_to_string(img,config="--psm 6")
        parsed=parse_result_blob(text)
        if parsed:
            return parsed,text
        # OCR başlığı kaçırsa bile 20 adet 1..80 sayısını yakalamayı dene.
        vals=[int(x) for x in re.findall(r"(?<!\d)(?:[1-9]|[1-7]\d|80)(?!\d)",text)]
        # OCR metnindeki tarih/saat vb. yüzünden fazlalık olabilir: tekrarları korumadan
        # yalnız geçerli sayıları sırayla al; tam 20 ise fallback başlığı kullan.
        uniq=[]
        for n in vals:
            if n not in uniq: uniq.append(n)
        if len(uniq)==20:
            return {"draw_no":int(fallback_draw),"date":str(fallback_date),"time":str(fallback_time),"numbers":sorted(uniq)},text
        return None,text
    except Exception as e:
        return None,f"OCR_ERROR: {e}"

photo=st.file_uploader("📷 Sonuç fotoğrafını yükle",type=["png","jpg","jpeg","webp"],key=f"v10_result_photo_{target_draw}")
photo_parsed=None; ocr_text=""
if photo is not None:
    photo_parsed,ocr_text=_ocr_result_photo(photo,target_draw,target_date,target_time)
    if photo_parsed is not None:
        st.success(f"Fotoğraf okundu: #{photo_parsed['draw_no']} · {photo_parsed['date']} {photo_parsed['time']} · 20 sayı")
        st.caption("Okunan sayılar: "+" - ".join(map(str,photo_parsed["numbers"])))
    else:
        st.warning("Fotoğraftan 20 sayı otomatik okunamadı. Aşağıdaki metin kutusuna sonucu yapıştırabilir veya daha net/kırpılmış fotoğraf yükleyebilirsin.")
        if str(ocr_text).startswith("OCR_ERROR:"):
            st.error("Fotoğraf OCR bileşeni çalışmıyor: "+str(ocr_text).replace("OCR_ERROR:","",1).strip())

st.caption("İstersen fotoğraf yerine metin de yapıştırabilirsin.")
...
text_parsed=_simple_parse_result(result_blob,target_draw,target_date,target_time) if result_blob.strip() else None
parsed=photo_parsed or text_parsed
if result_blob.strip() and text_parsed is None and photo_parsed is None:
...
# Eski session kartını yeni veriyle karıştırma: yalnız veri setindeki son gerçekleşen çekilişle aynıysa göster.
try:
    _latest_done_draw=int(df.sort_values("_dt").iloc[-1]["draw_no"])
except Exception:
    _latest_done_draw=None
if isinstance(last_eval,dict) and (_latest_done_draw is None or int(last_eval.get("draw",-1))==_latest_done_draw):
