...
# Önce bir önceki eklenen çekilişin isabetini göster.
last_eval=st.session_state.get("v10_simple_last_eval")
if isinstance(last_eval,dict):
    st.markdown("### ✅ Son eklenen çekiliş — kupon sonucu")
    st.success(f"#{last_eval['draw']} · {last_eval['date']} {last_eval['time']} · kayıt: {last_eval.get('persist','')}")
    cards=st.columns(3)
    for i,item in enumerate(last_eval.get("cards",[])):
        cards[i].metric(item["name"],item["score"],delta=("Tuttu: "+item["hits"] if item["hits"] else "Tuttu: —"))
...
q1.success("**V10 ANA 5**\\n\\n"+" - ".join(map(str,p.get("v10final5",[]))))
q2.info("**BAĞIMSIZ 5 — V9.5**\\n\\n"+" - ".join(map(str,p.get("independent5",p.get("v9final5",[])))))
q3.warning("**3. KUPON — V7**\\n\\n"+" - ".join(map(str,p.get("v7final5",[]))))
...
with st.expander("⚙️ Teknik durum",expanded=False):
    ...
    st.write("Aktif motorlar: V10 Ana · V9.5 Bağımsız · V7 üçüncü kupon")
