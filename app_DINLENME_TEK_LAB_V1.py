...
APP_VERSION = "DİNLENME TEK LAB V1 — Walk-forward dönüş kalibrasyonu"
...
# ===================================================================
# 10) DİNLENME TEK LAB V1 — walk-forward sayı kalibrasyonu
# ===================================================================

def _dinlenme_features(engine, t):
    """
    Her sayı için yalnız t'den önceki çekilişlerden dinlenme/dönüş özellikleri.
    ...
    for idx in range(80):
        ...
        # Güncel gap: son görülmeden beri kaç çekiliş geçti?
        seen = np.where(A[:t, idx] == 1)[0]
        gap = (t - 1 - int(seen[-1])) if len(seen) else t

        # Geçmişte bu sayının görülme aralıkları
        if len(seen) >= 2:
            gaps_hist = np.diff(seen)
            med_gap = float(np.median(gaps_hist))
            mean_gap = float(np.mean(gaps_hist))
            q75 = float(np.quantile(gaps_hist, .75))
            q90 = float(np.quantile(gaps_hist, .90))
        else:
            med_gap = mean_gap = q75 = q90 = 3.0

        # Gap'in kendi tarihine göre büyüklüğü
        gap_ratio = gap / max(mean_gap, 1e-6)
        overdue75 = gap / max(q75, 1e-6)
        overdue90 = gap / max(q90, 1e-6)

        # Benzer geçmiş gap olaylarında bir sonraki çekilişte dönüş oranı
        cases = hits = 0
        ...
        for j in range(start, t):
            past_seen = np.where(A[:j, idx] == 1)[0]
            pgap = (j - 1 - int(past_seen[-1])) if len(past_seen) else j
            if lo <= pgap <= hi:
                cases += 1
                hits += int(A[j, idx])
                if j >= t - 140:
        ...
        p_gap = (hits + 6*base) / max(cases + 6, 1)
        p_gap_recent = (recent_hits + 5*p_gap) / max(recent_cases + 5, 1)

        # Kısa/uzun frekans; dinlenmiş sayı için aşırı sıcaklık cezası sonra kullanılabilir
        f10 = float(A[max(0,t-10):t, idx].mean())
        f50 = float(A[max(0,t-50):t, idx].mean())
    ...
    d["gap_n"] = mm(np.clip(d["gap_ratio"],0,4))
    d["pgap_n"] = mm(d["p_gap"])
    d["recent_n"] = mm(d["p_gap_recent"])
    d["over_n"] = mm(np.clip(d["overdue75"],0,4))

    # Sadece sıralama skoru; karar değildir.
    d["DİNLENME skor"] = np.clip(
        .28*d["gap_n"] + .30*d["pgap_n"] + .30*d["recent_n"] + .12*d["over_n"],
        0,1
    )
    d=d.sort_values(["DİNLENME skor","p_gap_recent","gap_ratio"],ascending=False).reset_index(drop=True)
    d["Sıra"]=np.arange(1,len(d)+1)
    return d


def dinlenme_walkforward_lab(engine,start_t=140):
    rows=[]
    for t in range(max(140,int(start_t)),len(engine.A)):
        if engine.draw_nos[t] != engine.draw_nos[t-1]+1:
            continue
        tab=_dinlenme_features(engine,t)
        actual=set((np.where(engine.A[t]==1)[0]+1).tolist())
        for _,r in tab.iterrows():
            n=int(r["Sayı"])
            rows.append({
                "Index":t,"Çekiliş":int(engine.draw_nos[t]),"Sayı":n,
                "Sıra":int(r["Sıra"]),"DİNLENME skor":float(r["DİNLENME skor"]),
                "gap":int(r["gap"]),"gap_ratio":float(r["gap_ratio"]),
                "overdue75":float(r["overdue75"]),"overdue90":float(r["overdue90"]),
                "p_gap":float(r["p_gap"]),"p_gap_recent":float(r["p_gap_recent"]),
                "cases":int(r["cases"]),"recent_cases":int(r["recent_cases"]),
                "base":float(r["base"]),"f10":float(r["f10"]),"f50":float(r["f50"]),
                "Doğru":int(n in actual),
            })
    return pd.DataFrame(rows)


def dinlenme_calibration_report(wf):
    ...
    try:
        d["Skor dilimi"]=pd.qcut(d["DİNLENME skor"],10,duplicates="drop")
    except Exception:
        d["Skor dilimi"]=pd.cut(d["DİNLENME skor"],10)

    score=(d.groupby("Skor dilimi",observed=True)
             .agg(Aday=("Doğru","size"),Doğru=("Doğru","sum"),
                  Doğruluk=("Doğru","mean"),Ort_Skor=("DİNLENME skor","mean"))
    ...
    for k in [1,2,3,4,5,6,8,10]:
        ...
        for e in ["ERKEN","ORTA","SON"]:
            z=s[s["Dönem"]==e]
            row[e]=z["Doğru"].mean() if len(z) else np.nan
        rr.append(row)
    return score,pd.DataFrame(rr)


def dinlenme_strict_selector_from_history(wf,target_index,current_tab):
    """
    V1 keşif seçicisi. Aynı gap bandı + yakın p_gap_recent + rank bölgesinin
    geçmiş gerçek doğruluğunu ister. Kanıt yoksa sayı vermez.
    """
    hist=wf[wf["Index"]<target_index].copy()
    if len(hist)<1200: return [],pd.DataFrame()

    selected=[]; details=[]
    for _,r in current_tab.iterrows():
        rank=int(r["Sıra"]); gap=int(r["gap"])
        pr=float(r["p_gap_recent"]); gr=float(r["gap_ratio"])

        # Dinlenme uzmanında yalnız gerçekten dinlenmiş adayları değerlendir.
        if gap < 3:
            continue

        h=hist[
            (hist["Sıra"]==rank) &
            (np.abs(hist["gap"]-gap)<=1) &
            (np.abs(hist["p_gap_recent"]-pr)<=.055) &
            (np.abs(hist["gap_ratio"]-gr)<=.45)
        ].copy()
        if len(h)<60:
            h=hist[
                (hist["Sıra"]==rank) &
                (np.abs(hist["gap"]-gap)<=2) &
                (np.abs(hist["p_gap_recent"]-pr)<=.08) &
                (np.abs(hist["gap_ratio"]-gr)<=.70)
            ].copy()

        ok=False; acc=a1=a2=np.nan
        if len(h)>=60:
            h=h.sort_values("Index"); mid=len(h)//2
            a1=float(h.iloc[:mid]["Doğru"].mean())
            a2=float(h.iloc[mid:]["Doğru"].mean())
            acc=float(h["Doğru"].mean())
            ok=bool(acc>=.36 and a1>=.32 and a2>=.32)

        if ok: selected.append(int(r["Sayı"]))
        details.append({
            "Sayı":int(r["Sayı"]),"Sıra":rank,"gap":gap,
            "gap_ratio":round(gr,3),"p_gap_recent":round(pr,4),
            "Benzer geçmiş":len(h),
...
def dinlenme_strict_walkforward(engine,start_t=140):
    wf=dinlenme_walkforward_lab(engine,start_t)
    if wf.empty:return pd.DataFrame(),wf
    rows=[]
    for t in sorted(wf["Index"].unique()):
        tab=_dinlenme_features(engine,int(t))
        picks,_=dinlenme_strict_selector_from_history(wf,int(t),tab)
...
st.header("🔟 DİNLENME TEK LAB V1 — Sadece doğrulanmış dönüş adayları")
st.caption(
    "Yüksek dinlenme/gap değeri tek başına karar değildir. "
    "Her sayı kendi geçmiş benzer gap durumlarında gerçekten dönmüş mü diye walk-forward test edilir. "
    "Kanıt yoksa PAS."
)

if st.button("💤 DİNLENME motorunu tek başına test et",type="primary"):
    with st.spinner("DİNLENME walk-forward kalibrasyonu çalışıyor..."):
        strict_df,din_wf=dinlenme_strict_walkforward(eng,140)
        score_sum,rank_sum=dinlenme_calibration_report(din_wf)

    st.subheader("DİNLENME skor kalibrasyonu")
    ...
    if len(full):
        st.subheader("Tam isabetli DİNLENME olayları")
        st.dataframe(full.sort_values("Çekiliş",ascending=False),
                     use_container_width=True,hide_index=True)

    st.subheader("Son 250 DİNLENME kararı")
    st.dataframe(strict_df.tail(250).sort_values("Çekiliş",ascending=False),
                 use_container_width=True,hide_index=True)

    st.download_button(
        "⬇️ DİNLENME TEK LAB CSV indir",
        strict_df.to_csv(index=False).encode("utf-8-sig"),
        file_name="DINLENME_TEK_LAB_WALKFORWARD.csv",mime="text/csv")
    st.download_button(
        "⬇️ DİNLENME aday kalibrasyon CSV indir",
        din_wf.to_csv(index=False).encode("utf-8-sig"),
        file_name="DINLENME_ADAY_KALIBRASYON.csv",mime="text/csv")
