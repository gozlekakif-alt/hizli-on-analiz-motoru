...
def final_ticket_v5(train, target_slot, tab, regime_model, carry_pool, return_pool, narrow_pool, size=10):
    """
    V6 TEK FINAL:
    Sayının yalnız tekrar sayısına değil, yaşam döngüsüne bakar:
    DOĞUM → DEVAM → UZAMIŞ/SÖNME ve DİNLENME → DÖNÜŞ.
    """
    t = _final_feature_row(tab, regime_model, carry_pool, return_pool, narrow_pool)
    life = lifecycle_score_table(train, target_slot)
    t = t.merge(life, on="Sayı", how="left")

    # Yaşam döngüsü ana kanal: "2 kez çıktı" otomatik artı değildir.
    # Örn. DEVAM-2 ancak geçmişte aynı faz sürmüşse yükselir;
    # UZAMIŞ-ALEV ise sönme riski yüzünden aşağı çekilebilir.
    t["YaşamRank"] = pct_rank(t["Yaşam Olasılığı"].fillna(BASE))
    t["LifeSupportRank"] = pct_rank(t["Yaşam Destek"].fillna(0))
    t["SönmeCeza"] = 0.0
    hot = t["Kaynakta"].astype(bool)
    t.loc[hot, "SönmeCeza"] = (
        t.loc[hot, "Sönme Riski"].fillna(1-BASE) * 0.12
    )

    # V5'in salt skor uzlaşmasını koruyoruz ama ağırlığın merkezini yaşam fazına çekiyoruz.
    t["FinalHamV6"] = (
        0.17*t["KanıtRank"] +
        0.13*t["KarsiRank"] +
        0.12*t["UyumRank"] +
        ...
        0.05*t["RejimUzlasi"] +
        0.23*t["YaşamRank"] +
        0.05*t["LifeSupportRank"]
        - t["SönmeCeza"]
    )

    # Eski başarı/başarısızlık öğrenimi yardımcıdır; yaşam motorunu ezemez.
    rel = _learn_final_reliability(train, target_slot)
    if rel:
        w = rel["weights"]
        learned = sum(w.get(c,0.0) * t[c] for c in w if c in t.columns)
        denom = max(1.0, sum(abs(x) for x in w.values()))
        t["Öğrenilmiş"] = learned / denom
        t["FinalPuan"] = 0.84*t["FinalHamV6"] + 0.16*pct_rank(t["Öğrenilmiş"])
    else:
        t["Öğrenilmiş"] = 0.0
        t["FinalPuan"] = t["FinalHamV6"]

    # Taşıma kotası artık beklenen taşıma + yaşam canlılığı ile ayarlanır.
    expected = float(regime_model["expected_carry"])
    base_carry_n = int(round(size * expected / 20.0))
    carry_alive = t[t["Kaynakta"]]["Yaşam Olasılığı"]
    carry_lift = float(carry_alive.mean()-BASE) if len(carry_alive) else 0.0
    if carry_lift > 0.035:
        base_carry_n += 1
    elif carry_lift < -0.035:
        base_carry_n -= 1
    carry_n = max(1, min(size-1, base_carry_n))
    return_n = size - carry_n

    carry = t[t["Kaynakta"]].sort_values(
        ["FinalPuan","Yaşam Olasılığı","Kanıt"], ascending=False
    )
    ret = t[~t["Kaynakta"]].sort_values(
        ["FinalPuan","Yaşam Olasılığı","Kanıt"], ascending=False
    )

    selected = carry.head(carry_n)["Sayı"].astype(int).tolist()
    selected += ret.head(return_n)["Sayı"].astype(int).tolist()

    # Çok güçlü yaşam adayı kota yüzünden dışarıda kalmasın.
    chosen = list(dict.fromkeys(selected))
    selected_set = set(chosen)
    rescue = t[
        (t["YaşamRank"] >= .90) &
        (t["KanıtRank"] >= .60)
    ].sort_values(["FinalPuan","Yaşam Olasılığı"], ascending=False)

    for n in rescue["Sayı"].astype(int):
        if n in selected_set:
            continue
        # En zayıf seçiliyi ancak anlamlı fark varsa değiştir.
        current = t[t["Sayı"].isin(selected_set)].sort_values("FinalPuan")
        if current.empty:
            break
        weak = int(current.iloc[0]["Sayı"])
        cand_score = float(t.loc[t["Sayı"]==n, "FinalPuan"].iloc[0])
        weak_score = float(current.iloc[0]["FinalPuan"])
        if cand_score > weak_score + 0.025:
            selected_set.remove(weak)
            selected_set.add(n)

    final = (
        t[t["Sayı"].isin(selected_set)]
        .sort_values(["FinalPuan","Yaşam Olasılığı","Kanıt"], ascending=False)
        .head(size)["Sayı"].astype(int).tolist()
    )

    reasons = {}
    for n in final:
        r = t[t["Sayı"]==n].iloc[0]
        tags = [str(r["Yaşam Durumu"])]
        if r["Momentum"] != "DENGELİ":
            tags.append(str(r["Momentum"]))
        if n in set(carry_pool): tags.append("Taşıma")
        if n in set(return_pool): tags.append("Dönüş")
        if n in set(narrow_pool): tags.append("Dar16")
        if r["YaşamRank"] >= .80: tags.append("Yaşam+")
        if r["KanıtRank"] >= .75: tags.append("Kanıt")
        reasons[n] = "/".join(tags[:5])
...
st.title("🧬 Sayı Laboratuvarı V6 — Yaşam Döngüsü + Rejim Motoru")
st.caption(
    "Sayıyı yalnız kaç kez çıktığıyla değil; ne zaman DOĞDU, kaç el DEVAM etti, "
    "ne zaman SÖNMEYE başladı ve hangi dinlenme fazında DÖNDÜ diye inceler."
)
...
with tabs[0]:
    ...
    if pred:
        ...
        rel = pred.get("final_reliability")
        if rel:
            st.info(
                f"FINAL motoru {rel['events']} geçmiş aynı-nota olayı üzerinde "
                "sızıntısız başarı/başarısızlık ağırlıkları öğrendi."
            )

        st.markdown("### 🫀 FINAL Yaşam Ayrışımı")
        ft = pred.get("final_table")
        if ft is not None:
            show = ft[ft["Sayı"].isin(pred["final"])].copy()
            show = show.sort_values(["FinalPuan","Yaşam Olasılığı"], ascending=False)
            for c in ["Yaşam Olasılığı","Sönme Riski","FinalPuan"]:
                if c in show.columns:
                    show[c] = show[c].map(lambda x: round(float(x),3))
            life_cols = [
                "Sayı","Yaşam Durumu","Faz","Yaş","Önceki Faz","Momentum",
                "AynıNota6","Yaşam Olasılığı","Sönme Riski","Yaşam Destek","FinalPuan"
            ]
            st.dataframe(show[[c for c in life_cols if c in show.columns]],
                         use_container_width=True, hide_index=True)
            st.caption(
                "Önemli: DEVAM-2 veya DEVAM-3 otomatik yüksek puan değildir. "
                "Aynı yaşam fazının geçmişte sürme/sönme oranı düşükse sayı cezalandırılır."
            )
