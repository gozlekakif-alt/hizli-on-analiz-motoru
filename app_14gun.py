...
# --------------------------- UI ---------------------------
st.title("🧬 Hızlı On — Yaşam Kimliği MASTER APP V4.1 — TAM GÜN 80 YAŞAM + 20/60 + NETLEŞME")
...
_v_target_zip=make_research_zip(df,i,snap,labeled,real20,neg60,rival20,ts,_v_report)
st.download_button(
    "🎯 BU HEDEFİN TAM ARAŞTIRMA ZIP'İNİ İNDİR",
    _v_target_zip,
...
st.divider()
st.subheader("🗓️ 14 GÜNLÜK TÜM ANALİZLERİ TEK PAKET")
st.caption("25.08.2026–07.09.2026 arasındaki 14 tam günün günlük MASTER araştırma ZIP'lerini tek ana ZIP içinde toplar. İşlem ağırdır; hesaplama sürerken sayfadan çıkma.")

if st.button("📦 14 GÜNLÜK MASTER PAKETİNİ HAZIRLA", use_container_width=True, key="prepare_14_day_master", type="primary"):
    st.session_state.pop("master_14_day_zip", None)
    _status14 = st.status("14 günlük MASTER paket hazırlanıyor…", expanded=True)
    try:
        st.write("14 gün sırayla işleniyor…")
        _zip14 = make_14_day_research_zip(df)
        st.session_state["master_14_day_zip"] = _zip14
        _status14.update(label="✅ 14 günlük MASTER paket hazır", state="complete", expanded=False)
        st.success("14 günlük paket hazır. Aşağıdaki indirme düğmesine bas.")
    except Exception as _e:
        st.session_state.pop("master_14_day_zip", None)
        _status14.update(label="❌ 14 günlük paket hazırlanamadı", state="error", expanded=True)
        st.exception(_e)

if st.session_state.get("master_14_day_zip"):
    st.download_button(
        "⬇️ 25.08–07.09 14 GÜNLÜK MASTER ZIP’İ İNDİR",
        st.session_state["master_14_day_zip"],
        "HIZLI_ON_25_08_2026_07_09_2026_14_GUN_MASTER.zip",
        "application/zip",
        use_container_width=True,
        key="download_14_day_master"
    )
    st.success("Bana göndermen gereken 14 günlük ana analiz paketi budur.")

st.divider()
st.caption("MASTER V4.1 araştırma kuralı: Veri hızlı güncellenir → PRE-H 80 fotoğrafı → gerçek 20'nin tüm yaşam karakterleri → aynı kimlikteki rakipler → dışarıdaki 60'ın fark haritası → sayı özel yaşam izi → karakter salınımı → kör doğrulama. Kupon üretimi bu araştırma sürümünde bilinçli olarak yoktur.")
