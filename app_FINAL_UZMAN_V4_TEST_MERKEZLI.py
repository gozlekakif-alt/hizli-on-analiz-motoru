...
st.set_page_config(
    page_title="Hızlı On — FINAL Uzman V4",
    page_icon="🎯",
    layout="wide",
)
...
# Akış kilidi:
# Bir tahmin gerçek sonuçla kontrol edildiğinde, o çekiliş ana havuza
# kaydedilene kadar aynı havuzdan yeni tahmin üretilemez.
if "prediction_locked" not in st.session_state:
    st.session_state["prediction_locked"]=False
if "locked_target_no" not in st.session_state:
    st.session_state["locked_target_no"]=None
if "last_saved_no" not in st.session_state:
    st.session_state["last_saved_no"]=int(df.iloc[-1]["Cekilis_No"]) if len(df) else None

st.title("🎯 HIZLI ON — FINAL UZMAN V4 — TEST MERKEZLİ")
st.info(
    "V4 çalışma kuralı: Önce geçmiş walk-forward testi. "
    "Canlı tahmin ayrı sekmededir. Sonuç kontrol edilip ana havuza kaydedilmeden "
    "bir sonraki tahmine geçilmez."
)
st.caption(
...
with tabs[0]:
    ...
    locked=bool(st.session_state.get("prediction_locked",False))
    if locked:
        st.error(
            f"🔒 AKIŞ KİLİTLİ — #{st.session_state.get('locked_target_no')} sonucu kontrol edildi "
            "ama henüz ana havuza kaydedilmedi. Önce 'Sonucu Kontrol Et → Kaydet' sekmesinden kaydet."
        )

    if st.button(
        "🧠 ÖĞREN + GÜNCEL KARARI ÜRET",
        type="primary",
        use_container_width=True,
        disabled=locked
    ):
        # Aynı havuz damgası için mevcut tahmini tekrar hesaplamak yerine açıkça yenile.
    ...
    if snap:
        ...
        st.info(
            f"📌 VERİ DAMGASI: Bu tahmin yalnız **#{snap['after_no']}** çekilişine kadar olan ana havuzla üretildi. "
            f"Hedef **#{snap['target_no']}**."
        )
        a,b=st.columns(2)
        with a:
...
with tabs[1]:
    ...
    if st.button("🔎 SONUCU KONTROL ET",type="primary",use_container_width=True):
        ...
        if nums is None:
            ...
            # Tahmin varsa yanlış hedef numarasıyla kontrol yapılmasını engelle.
            if snap and int(draw_no)!=int(snap["target_no"]):
                st.error(
                    f"Bu açık tahmin #{snap['target_no']} için. Girdiğin sonuç #{int(draw_no)}. "
                    "Doğru hedef çekilişi gir."
                )
            else:
                st.session_state["pending_verified"]=result
                st.session_state["prediction_locked"]=True
                st.session_state["locked_target_no"]=int(draw_no)
    ...
    if pending:
        ...
        st.error(
            f"🔒 #{int(pending['row_df'].iloc[0]['Cekilis_No'])} kontrol edildi ama HENÜZ ÖĞRENİLMEDİ. "
            "Ana havuza kaydetmeden yeni tahmin üretilemez."
        )

        if st.button("✅ KONTROL EDİLEN ÇEKİLİŞİ ANA HAVUZA KAYDET",use_container_width=True):
            pno=int(pending["row_df"].iloc[0]["Cekilis_No"])
            existing=set(df["Cekilis_No"].astype(int).tolist())
            if pno in existing:
                st.warning(f"#{pno} zaten ana havuzda. Mükerrer kayıt yapılmadı.")
                ...
                ok,msg=persist_pool(newdf,message=f"Kontrol sonrası çekiliş #{pno} eklendi")
                st.session_state["pending_verified"]=None
                st.session_state["current_prediction"]=None
                st.session_state["prediction_locked"]=False
                st.session_state["locked_target_no"]=None
                st.session_state["last_saved_no"]=pno
...
st.divider()
st.caption(
    "FINAL V4 ilkesi: Elenen GEÇİŞ / DİNLENME / KOMŞU / BİRLİKTELİK / BANT bağımsız sayı üretmez. "
