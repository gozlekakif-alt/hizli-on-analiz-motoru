    ...
    def bursts(posdict, min_hits=3, max_span=12):
        ...
        for fam,pos in posdict.items():
            ...
            for r in range(len(pos)):
                while pos[r]-pos[l] >= max_span: ltrim=True; l+=1
                cnt=r-l+1
...
".join(master_head+[st.session_state.texts[d] for d in days])
    ...
    st.markdown("---")
    st.header("🎯 Canlı Hızlı On Kupon Üretim Motoru")
    st.write("Sistem kümülatif aile bağlarını ve en son elin taşıma verilerini harmanlayarak nokta atışı 4'lü kuponlar üretti:")

    last_day_draws = df[df.day == days[-1]].sort_values("draw_id")
    last_draw = list(last_day_draws.iloc[-1]["nums"])
    
    full_analysis = day_analysis(df)
    
    top_quads = full_analysis["quad"].most_common(5)
    top_trios = full_analysis["tri"].most_common(5)
    top_pairs = full_analysis["pair"].most_common(5)

    kolon_1 = sorted(list(top_quads[0][0]))

    best_last_pair = None
    max_pair_count = -1
    for comb in combinations(last_draw, 2):
        if full_analysis["pair"][comb] > max_pair_count:
            max_pair_count = full_analysis["pair"][comb]
            best_last_pair = comb

    kolon_2 = list(best_last_pair)
    for trio_data in top_trios:
        for num in trio_data[0]:
            if num not in kolon_2 and len(kolon_2) < 4:
                kolon_2.append(num)
    kolon_2 = sorted(kolon_2)

    kolon_3 = list(top_trios[0][0])
    for quad_data in top_quads:
        for num in quad_data[0]:
            if num not in kolon_3 and len(kolon_3) < 4:
                kolon_3.append(num)
    kolon_3 = sorted(kolon_3)

    k1, k2, k3 = st.columns(3)
    with k1:
        st.info(f"**KOLON 1 (Kümülatif Örüntü):**\n\n ## `{kolon_1[0]} - {kolon_1[1]} - {kolon_1[2]} - {kolon_1[3]}`")
    with k2:
        st.info(f"**KOLON 2 (Taşıma Zinciri):**\n\n ## `{kolon_2[0]} - {kolon_2[1]} - {kolon_2[2]} - {kolon_2[3]}`")
    with k3:
        st.info(f"**KOLON 3 (Burst Tetikleyici):**\n\n ## `{kolon_3[0]} - {kolon_3[1]} - {kolon_3[2]} - {kolon_3[3]}`")
