...
APP_VERSION = "KOMŞU TEK LAB V1 — Walk-forward komşuluk kalibrasyonu"
...
# ===================================================================
# 12) KOMŞU TEK LAB V1 — walk-forward komşuluk kalibrasyonu
# ===================================================================

def _komsu_features(engine,t):
    """
    Son çekilişteki sayıların ±1 / ±2 komşularını ve son 5 çekilişteki
    blok-komşuluk yoğunluğunu yalnız geçmişten ölçer.
    """
    if t < 120:
        return pd.DataFrame()
    ...
    for idx in range(80):
        ...
        last_n1=sum(last[j] for j in n1)
        last_n2=sum(last[j] for j in n2)

        # Son 5 ve 20 çekilişte komşuluk aktivitesi
        near5=0.0
        near20=0.0
        for j in n1+n2:
            near5 += float(h5[:,j].mean()) if len(h5) else 0.0
            near20 += float(h20[:,j].mean()) if len(h20) else 0.0
        denom=max(len(n1)+len(n2),1)
        near5/=denom
        near20/=denom

        # Adayın geçmişte, komşularından biri önceki çekilişte varken sonraki çekilişte gelme oranı
        cases=hits=recent_cases=recent_hits=0
        for j in range(max(1,t-350),t):
            prev=A[j-1]
            cond=sum(prev[k] for k in n1+n2)>0
            if cond:
                cases+=1
                hits+=int(A[j,idx])
                if j>=t-140:
        ...
        p_hist=(hits+6*base)/max(cases+6,1)
        p_recent=(recent_hits+5*p_hist)/max(recent_cases+5,1)
    ...
    d["KOMŞU skor"]=np.clip(
        .34*d["recent_n"]+.22*d["hist_n"]+.18*d["near5_n"]+
        .12*d["near20_n"]+.14*d["last_n"],0,1
    )
    d=d.sort_values(["KOMŞU skor","p_recent","last_n1","last_n2"],
                    ascending=False).reset_index(drop=True)
    d["Sıra"]=np.arange(1,len(d)+1)
    return d


def komsu_walkforward_lab(engine,start_t=140):
    rows=[]
    for t in range(max(140,int(start_t)),len(engine.A)):
        if engine.draw_nos[t] != engine.draw_nos[t-1]+1:
            continue
        tab=_komsu_features(engine,t)
        actual=set((np.where(engine.A[t]==1)[0]+1).tolist())
        for _,r in tab.iterrows():
            n=int(r["Sayı"])
            rows.append({
                "Index":t,"Çekiliş":int(engine.draw_nos[t]),"Sayı":n,
                "Sıra":int(r["Sıra"]),"KOMŞU skor":float(r["KOMŞU skor"]),
                "last_n1":float(r["last_n1"]),"last_n2":float(r["last_n2"]),
                "near5":float(r["near5"]),"near20":float(r["near20"]),
                "p_hist":float(r["p_hist"]),"p_recent":float(r["p_recent"]),
...
def komsu_calibration_report(wf):
    ...
    try:d["Skor dilimi"]=pd.qcut(d["KOMŞU skor"],10,duplicates="drop")
    except:d["Skor dilimi"]=pd.cut(d["KOMŞU skor"],10)

    score=(d.groupby("Skor dilimi",observed=True)
             .agg(Aday=("Doğru","size"),Doğru=("Doğru","sum"),
                  Doğruluk=("Doğru","mean"),Ort_Skor=("KOMŞU skor","mean"))
...
def komsu_strict_selector_from_history(wf,target_index,current_tab):
    hist=wf[wf["Index"]<target_index].copy()
    if len(hist)<1400:return [],pd.DataFrame()

    selected=[];details=[]
    for _,r in current_tab.iterrows():
        rank=int(r["Sıra"]); sc=float(r["KOMŞU skor"])
        n1=float(r["last_n1"]); n2=float(r["last_n2"]); pr=float(r["p_recent"])

        h=hist[
            (hist["Sıra"]==rank)&
            (np.abs(hist["KOMŞU skor"]-sc)<=.07)&
            (hist["last_n1"]==n1)&
            (np.abs(hist["last_n2"]-n2)<=1)&
            (np.abs(hist["p_recent"]-pr)<=.06)
        ].copy()

        if len(h)<70:
            h=hist[
                (hist["Sıra"]==rank)&
                (np.abs(hist["KOMŞU skor"]-sc)<=.11)&
                (np.abs(hist["last_n1"]-n1)<=1)&
                (np.abs(hist["last_n2"]-n2)<=1)&
                (np.abs(hist["p_recent"]-pr)<=.09)
            ].copy()
        ...
        details.append({
            "Sayı":int(r["Sayı"]),"Sıra":rank,"KOMŞU skor":round(sc,4),
            "last_n1":n1,"last_n2":n2,"p_recent":round(pr,4),
            "Benzer geçmiş":len(h),
...
def komsu_strict_walkforward(engine,start_t=140):
    wf=komsu_walkforward_lab(engine,start_t)
    if wf.empty:return pd.DataFrame(),wf
    rows=[]
    for t in sorted(wf["Index"].unique()):
        tab=_komsu_features(engine,int(t))
        picks,_=komsu_strict_selector_from_history(wf,int(t),tab)
...
st.header("1️⃣2️⃣ KOMŞU TEK LAB V1")
st.caption(
    "±1 / ±2 komşuluk yüksek diye sayı otomatik seçilmez. "
    "Komşu koşulunun geçmişte gerçekten sonraki çekilişte isabet üretip üretmediği "
    "walk-forward kalibre edilir. Kanıt yoksa PAS."
)

if st.button("🧲 KOMŞU motorunu tek başına test et",type="primary"):
    with st.spinner("KOMŞU walk-forward kalibrasyonu çalışıyor..."):
        strict_df,kom_wf=komsu_strict_walkforward(eng,140)
        score_sum,rank_sum=komsu_calibration_report(kom_wf)

    st.subheader("KOMŞU skor kalibrasyonu")
    ...
    if len(full):
        st.subheader("Tam isabetli KOMŞU olayları")
        st.dataframe(full.sort_values("Çekiliş",ascending=False),
                     use_container_width=True,hide_index=True)

    st.subheader("Son 250 KOMŞU kararı")
    st.dataframe(strict_df.tail(250).sort_values("Çekiliş",ascending=False),
                 use_container_width=True,hide_index=True)

    st.download_button(
        "⬇️ KOMŞU TEK LAB CSV indir",
        strict_df.to_csv(index=False).encode("utf-8-sig"),
        file_name="KOMSU_TEK_LAB_WALKFORWARD.csv",mime="text/csv")
    st.download_button(
        "⬇️ KOMŞU aday kalibrasyon CSV indir",
        kom_wf.to_csv(index=False).encode("utf-8-sig"),
        file_name="KOMSU_ADAY_KALIBRASYON.csv",mime="text/csv")
