...
st.set_page_config(
    page_title="Hızlı On V15 — Gece Karakteri / Akış / Sinyal Motoru",
    page_icon="🌙",
    layout="wide",
)

st.title("🌙 Hızlı On V15 — Gece Karakteri / Akış / Sinyal Motoru")
st.caption(
...
def make_tickets(tab,expected_carry):
    """V16: ham Final sırası değil, çoklu bağımsız kanıt + rol uzmanlığı seçer."""
    z=tab.copy()
    channels=["TaşımaKanıt","DönüşKanıt","SıcakKanıt","KümeKanıt","Step2Kanıt","ArdışıkKanıt","BantKanıt","YolKanıt"]

    # Her kanalın kendi dağılımına göre göreli kanıt: sabit yüksek skor kovalamaz.
    for c in channels:
        z[c+"_R"]=z[c].rank(pct=True,method="average")

    # Bir adayın kaç BAĞIMSIZ kanalda gerçekten üst grupta olduğunu say.
    rcols=[c+"_R" for c in channels]
    z["KanıtSayısı"]=z[rcols].ge(0.68).sum(axis=1)
    z["ÇokluKanıt"]=z[rcols].apply(lambda r: float(np.mean(sorted(r,reverse=True)[:3])),axis=1)

    # Rol puanları. Final yalnız küçük tie-break; seçim motorunun patronu değil.
    z["CarryRole"]=(
        0.30*z["TaşımaKanıt_R"]+0.25*z["YolKanıt_R"]+0.15*z["SıcakKanıt_R"]+
        0.10*z["KümeKanıt_R"]+0.10*z["ArdışıkKanıt_R"]+0.10*z["BantKanıt_R"]
    )
    z["ReturnRole"]=(
        0.30*z["DönüşKanıt_R"]+0.22*z["YolKanıt_R"]+0.14*z["SıcakKanıt_R"]+
        0.12*z["KümeKanıt_R"]+0.08*z["Step2Kanıt_R"]+0.07*z["ArdışıkKanıt_R"]+0.07*z["BantKanıt_R"]
    )
    z["PatternRole"]=(
        0.20*z["SıcakKanıt_R"]+0.18*z["KümeKanıt_R"]+0.17*z["Step2Kanıt_R"]+
        0.17*z["ArdışıkKanıt_R"]+0.14*z["BantKanıt_R"]+0.14*z["YolKanıt_R"]
    )
    z["Gate"]=(z["KanıtSayısı"]>=2).astype(int)

    def pick(df,n,score,used=None,max_overlap=None):
        used=set(used or [])
        q=df.sort_values(["Gate","KanıtSayısı","ÇokluKanıt",score,"Final"],ascending=False)
        out=[]
        for _,r in q.iterrows():
            x=int(r["Sayı"])
            # ortaklık yasak değil; sadece kanıt kapısından geçmeyen kopyayı engelle
            if max_overlap is not None and x in used and len(set(out)&used)>=max_overlap and int(r["KanıtSayısı"])<3:
                continue
            if x not in out: out.append(x)
            if len(out)==n: break
        return out

    carry=z[z["Kaynakta"]].copy(); ret=z[~z["Kaynakta"]].copy()
    k7a=int(np.clip(round(expected_carry*7/20),1,5))
    a=pick(carry,k7a,"CarryRole")
    a += [x for x in pick(ret,7,"ReturnRole") if x not in a][:7-len(a)]

    # 7B ayrı uzman: dönüş/pattern. A ile ortaklık ancak 3+ bağımsız kanıt varsa rahat bırakılır.
    b=pick(z,7,"PatternRole",used=a,max_overlap=3)

    # 10'lu: iki uzmanı körlemesine birleştirmez; çoklu kanıt + rol maksimumu.
    z["ConsensusRole"]=z[["CarryRole","ReturnRole","PatternRole"]].max(axis=1)
    z["ConsensusRole"]=0.55*z["ConsensusRole"]+0.35*z["ÇokluKanıt"]+0.10*(z["KanıtSayısı"].clip(0,4)/4)
    t10=pick(z,10,"ConsensusRole")
    return {"7A":a[:7],"7B":b[:7],"10":t10[:10]}
    ...
    if target_slot is None:
        ...
        try:
            ...
            st.caption(
                ...
                "Taşıma ve dönüş sayıları sabit kota değildir; benzer geçmiş gecelerin gerçek davranışından öğrenilir."
            )
