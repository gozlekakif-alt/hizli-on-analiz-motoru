...
# --------------------------- UI ---------------------------
st.title("🧬 Hızlı On — Yaşam Kimliği MASTER APP V5.5 — CHARACTER-FIRST CHUNKED TRUE WALK-FORWARD")
...
st.divider()
st.header("🧱 V5.5 CHARACTER-FIRST — PARÇALI/KALICI WALK-FORWARD")
st.caption("V5.4 mantığı aynı. Fark: her tıklamada yalnız 1 blok hesaplanır ve diske kalıcı kaydedilir. 18/18 olunca tek ZIP oluşur.")

if "v55_matrix" not in st.session_state:
    with st.spinner("V5.5 için 80→20 PRE-H matrisi hazırlanıyor..."):
        st.session_state["v55_matrix"]=_v52_build_matrix(df)

_v55_matrix=st.session_state["v55_matrix"]
...
st.progress(_v55_n/len(_v55_plan) if _v55_plan else 0.0,
            text=f"V5.5 blok ilerleme: {_v55_n}/{len(_v55_plan)}")
...
if len(_v55_done)==len(_v55_plan) and len(_v55_plan)>0:
    ...
    st.success("✅ V5.5 18/18 tamamlandı. Tüm bloklar tek out-of-sample sonuç ZIP'inde birleştirildi.")
    st.download_button(
        "⬇️ V5.5 CHARACTER-FIRST PARÇALI WALK-FORWARD — TEK ZIP",
        st.session_state["v55_final_zip"],
        "HIZLI_ON_V5_5_CHARACTER_FIRST_CHUNKED_WF_RESULTS.zip",
