...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS coupons_v2(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        target_draw_id INTEGER NOT NULL,
        created_after_draw_id INTEGER,
        coupon_size INTEGER DEFAULT 4,
        coupon_no INTEGER,
        kind TEXT,
        numbers TEXT NOT NULL,
        architecture TEXT,
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS pools_v2(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        target_draw_id INTEGER UNIQUE,
        created_after_draw_id INTEGER,
        pool20 TEXT,
        detail_csv TEXT)""")
    c.commit(); c.close()
...
def seed(force=False):
    ...
    c=con()
    if force:
        c.execute("DELETE FROM draws"); c.execute("DELETE FROM coupons_v2"); c.execute("DELETE FROM pools_v2")
...
def make_coupons(pool,size,count=6):
    ...
    # deterministic greedy: maximize score + source diversity + pair separation across coupons_v2
...
def save_prediction(df):
    ...
    c=con()
    c.execute("""INSERT OR REPLACE INTO pools_v2(target_draw_id,created_after_draw_id,pool20,detail_csv)
    ...
    for size in (4,5):
        for i,cp in enumerate(make_coupons(pool,size,6),1):
            c.execute("""INSERT OR IGNORE INTO coupons_v2(target_draw_id,created_after_draw_id,coupon_size,coupon_no,numbers,architecture)
...
def check_and_roll():
    ...
    rows=c.execute("""SELECT c.id,c.target_draw_id,c.numbers,c.coupon_size,d.numbers
                      FROM coupons_v2 c JOIN draws d ON d.draw_id=c.target_draw_id
                      WHERE c.checked=0""").fetchall()
    checked=[]
    for cid,target,cp,size,real in rows:
        a=set(nums(cp)); b=set(nums(real)); hit=sorted(a&b)
        c.execute("UPDATE coupons_v2 SET checked=1,hit_count=?,hits=?,matched_numbers=? WHERE id=?",
                  (len(hit),len(hit),",".join(map(str,hit)),cid))
...
# Save current six 4-number coupons_v2 for exact target, without duplicates.
c=con()
for i,cp in enumerate(ana4,1):
    c.execute("""INSERT INTO coupons_v2(target_draw_id,coupon_size,kind,numbers,checked,hits,hit_count)
                 SELECT ?,4,?,?,0,0,0 WHERE NOT EXISTS(
                 SELECT 1 FROM coupons_v2 WHERE target_draw_id=? AND kind=? AND numbers=?)""",
...
res=pd.read_sql_query("""SELECT target_draw_id AS Hedef, kind AS Kolon, numbers AS Sayilar,
                                hits AS Isabet
                         FROM coupons_v2 WHERE checked=1
