...
def parse_text(txt,source='seed'):
    """İki formatı da okur:
    1) draw_id;DD.MM.YYYY HH:MM;n1,...,n20
    2) Milli Piyango kopyala/yapıştır: Çekiliş no + tarih-saat + 20 satır sayı + Detaylar linki.
    Girdi ters sırada olsa bile kronolojik döner.
    """
    rows=[]
    # Satır-bazlı arşiv formatı
    ...
    # Doğal kopyala/yapıştır formatı. Markdown ### ve Detaylar satırları sorun değildir.
    clean=[re.sub(r'^\s*#+\s*','',x).strip() for x in txt.splitlines()]
    i=0
    while i < len(clean):
        line=clean[i]
        if re.fullmatch(r'Çekiliş no\s*:',line,re.I):
            j=i+1
            while j<len(clean) and not clean[j]: j+=1
            if j>=len(clean) or not re.fullmatch(r'\d+',clean[j]): i+=1; continue
            draw_id=int(clean[j]); j+=1
            while j<len(clean) and not clean[j]: j+=1
            if j>=len(clean): break
            dm=re.fullmatch(r'(\d{2}\.\d{2}\.\d{4})\s*-?\s*(\d{2}:\d{2})',clean[j])
            if not dm: i+=1; continue
            dt=datetime.strptime(dm.group(1)+' '+dm.group(2),'%d.%m.%Y %H:%M'); j+=1
            nums=[]
            while j<len(clean) and len(nums)<20:
                q=clean[j]
                if re.fullmatch(r'\d{1,2}',q): nums.append(int(q)); j+=1; continue
                if not q: j+=1; continue
                break
            if len(nums)==20 and len(set(nums))==20 and min(nums)>=1 and max(nums)<=80:
...
def process_hour_block(txt):
    ...
    if not rows: raise ValueError('Bu metinden geçerli çekiliş okunamadı.')
    results=[]
...
with tabs[3]:
    st.subheader('15. Gün — saatlik toplu giriş')
    st.caption('Sen 12 çekilişi tek seferde yapıştırırsın. APP tarih/saat sırasına dizer ve içeride EL1→EL2→... tek tek walk-forward işler. Detaylar linklerini yok sayar.')
    ...
    if st.button('SAATLİK BLOĞU İŞLE → çekiliş çekiliş ilerle',type='primary'):
        try:
            rr=process_hour_block(bulk)
            st.success(f'{len(rr)} çekiliş okundu. Kronolojik olarak tek tek işlendi.')
