...
def parse_user_blocks(text):
    """
    Kullanıcının doğal formatını kabul eder:
    Çekiliş no: 54251
    08.09.2026 - 00:02
    3
    7
    ...
    Aynı metin içinde arka arkaya birden fazla çekilişi de ayırır.
    """
    text = str(text).replace("\r\n", "\n").replace("\r", "\n")
    # Next "Çekiliş no" starts the next block.
    pat = re.compile(
        r"(?is)(?:#+\s*)?Çekiliş\s*no\s*:\s*(\d+)\s*\n"
        r"\s*(\d{2}\.\d{2}\.\d{4})\s*-\s*(\d{2}:\d{2})\s*\n"
        r"(.*?)(?=(?:#+\s*)?Çekiliş\s*no\s*:|\Z)"
    )
    out=[]; errors=[]
    for m in pat.finditer(text):
        did=int(m.group(1))
        dt=f"{m.group(2)} {m.group(3)}"
        ns=[int(x) for x in re.findall(r"(?m)^\s*(\d{1,2})\s*$", m.group(4))]
        if valid(ns):
            out.append((did,dt,ns))
        else:
            errors.append((did,len(ns)))
    return out, errors

def import_user_blocks(text):
    blocks, errors = parse_user_blocks(text)
    inserted=duplicate=0
    c=con()
    for did,dt,ns in blocks:
        cur=c.execute("INSERT OR IGNORE INTO draws VALUES(?,?,?)",
                      (did,dt,",".join(map(str,ns))))
        if cur.rowcount: inserted += 1
        else: duplicate += 1
    c.commit(); c.close()
    return inserted, duplicate, errors
...
def seed(force=False):
    """Load and verify the locked 14-day seed. Never silently run with an empty/partial base."""
    ...
    if len(rows) != 3038:
        return (False, len(rows), f"Ana dosya 3038 çekiliş olmalı; okunan={len(rows)}, bozuk={bad[:10]}")
    dates=pd.to_datetime([r[1] for r in rows],dayfirst=True,errors="coerce")
    if dates.isna().any() or len(set(d.date() for d in dates)) != 14:
        return (False, len(rows), "Ana dosyanın 14 günlük tarih kilidi doğrulanamadı.")
    counts=pd.Series([d.date() for d in dates]).value_counts()
    if not (counts==217).all():
        return (False, len(rows), f"Gün başına 217 kilidi bozuk: {counts.to_dict()}")
    c=con()
    if force:
        c.execute("DELETE FROM draws")
        c.execute("DELETE FROM coupons")
        c.execute("DELETE FROM pools")
    # Insert seed without touching later live draws unless force=True.
    ...
    present=c.execute(f"SELECT COUNT(*) FROM draws WHERE draw_id BETWEEN ? AND ?",
                      (min(seed_ids),max(seed_ids))).fetchone()[0]
    c.close()
    if present < 3038:
        return (False,present,f"Ana hafıza eksik: {present}/3038")
...
if memory_ok:
    ...
    st.error("⛔ " + memory_msg)
    st.warning("Ana 14 günlük hafıza doğrulanmadan kupon motoru güvenli kabul edilmez.")
    if st.button("14 günlük ana hafızayı sıfırdan yeniden yükle"):
        ok,n,msg=seed(force=True)
        if ok: st.rerun()
        else: st.error(msg)
...
with tab1:
    if not memory_ok:
        st.error("Ana hafıza kilidi geçmediği için kupon üretimi durduruldu.")
    elif df.empty:
        st.warning("Veri yok.")
    else:
        last=df.iloc[-1]
        ...
        with st.expander("🔎 Hafıza teşhisi"):
            st.write("Toplam veritabanı çekilişi:", len(df))
            st.write("İlk kayıt:", int(df.iloc[0].draw_id), df.iloc[0].draw_time)
            st.write("Son kayıt:", int(df.iloc[-1].draw_id), df.iloc[-1].draw_time)
            seed_df=df[df["draw_id"]<=54199] if not df.empty else df
            st.write("14 günlük kilit:", memory_msg)
        ...
        st.divider()
        st.subheader("🧠 İki Numara Üreticisi – Aynı APP")
        ana4=make_coupons(pool,4,3)
        km=kullanici_numara_ureticisi(df)
        left,right=st.columns(2)
        with left:
            st.markdown("**ANA BEYİN — 3×4'lü**")
            for i,cp in enumerate(ana4,1):
                st.success(f"ANA-{i}: " + " • ".join(f"{n:02d}" for n in cp))
            st.caption("PRE-H yaşam + kaynak bütçesi + R1–R4 + H1 takım desteği + +6 + NEG/PROTECT.")
        with right:
            st.markdown("**SENİN GÖNDERDİĞİN ÜRETİCİ — 3×4'lü**")
            st.write("Rejim:",km["Rejim"]," | H1 takım:",km["Takim"] if km["Takim"] else "Sinyal yok")
            if km["Kolonlar"]:
                for i,cp in enumerate(km["Kolonlar"],1):
                    st.info(f"KULLANICI-{i}: " + " • ".join(f"{n:02d}" for n in cp))
            else:
                st.warning("Bu elde yeterli H1 takım sinyali yok; bu uzman susuyor.")
            st.caption("V33 H1 takım + SHORT/MID rejimi + gönderdiğin cooling filtresi. Ana motoru veto etmez.")
        ...
        day=same_day_history(df)
        st.info(f"Aktif gün içi PRE-H hafızası: {len(day)} çekiliş. Araştırma kuralı gereği H-yaşı takvim günü başında sıfırlanır. Yeni günün ilk sonucu eklendiyse sonraki hedefte son 20 sayı H1, diğer 60 sayı YENİ görünmesi normaldir; bu, 14 günlük verinin kayıp olduğu anlamına gelmez.")


with tab2:
    st.subheader("Tek veya peş peşe çekilişleri eski gönderdiğin biçimde yapıştır")
    st.caption("Format değiştirmene gerek yok. 'Çekiliş no:', tarih-saat ve alt alta 20 sayıyı doğrudan kabul eder.")
    sample = """Çekiliş no: 54251
08.09.2026 - 00:02
3
7
10
...
74
77"""
    tx=st.text_area("Çekiliş metni", value="", height=420, placeholder=sample)
    if st.button("Yapıştırılan çekilişleri ekle → sonuçları kontrol et → yeni kupon üret",type="primary"):
        if not memory_ok:
            st.error("Önce 14 günlük ana hafıza kilidini düzelt.")
            st.stop()
        ins,dup,errs=import_user_blocks(tx)
        if ins:
            checked=check_and_roll()
            st.success(f"{ins} çekiliş eklendi. {dup} tekrar kayıt atlandı. Bekleyen kuponlar kontrol edildi ve son çekilişin ardından yeni hedef üretildi.")
            if checked:
                st.dataframe(pd.DataFrame(checked,columns=["Hedef","Boyut","Kupon","İsabet","Tutanlar"]),use_container_width=True)
            if errs:
                st.warning("20 sayı olarak okunamayan çekilişler: " + ", ".join(f"#{d} ({n} sayı)" for d,n in errs))
            st.rerun()
        else:
            if errs:
                st.error("Çekiliş bulundu fakat 20 sayı okunamadı: " + ", ".join(f"#{d} ({n} sayı)" for d,n in errs))
            else:
                st.error("Çekiliş okunamadı. 'Çekiliş no: ...' + tarih-saat + alt alta 20 sayı biçimini olduğu gibi yapıştır.")
...
with tab3:
    ...
    if st.button("Toplu ekle ve tüm bekleyen kuponları kontrol et"):
        ...
        if re.search(r"(?i)Çekiliş\s*no\s*:", text):
            ins,sk,errs=import_user_blocks(text)
            bad=[f"#{d} ({n} sayı)" for d,n in errs]
        else:
            ins,sk,bad=bulk_import(text)
        checked=check_and_roll()
        st.success(f"{ins} çekiliş eklendi, {sk} tekrar/bozuk kayıt atlandı. {len(checked)} kupon sonucu kontrol edildi.")
        if bad: st.warning(f"Kontrol edilmesi gereken kayıtlar: {bad[:20]}")
        st.rerun()
...
with tab5:
    if not df.empty:
        ...
        st.dataframe(pool[["Havuz_Sirasi","Sayi","H_Yasi","Kaynak","Ritim","H1_Takim","V33_Destek","V33_Takim","NEG_Oyu","Protect","Skor","Neden"]],
