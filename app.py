...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.5",
    page_icon="🎯",
    layout="wide",
)
...
st.title("🎯 Hızlı On Ultimate AI Studio V18.5")
...
def decorate_master_v184(df, master18, block_birth, region_df, lookback=600):
    ...
    out["V18.5 Nefes Puanı"] -= 0.10 * out["Blok Takip Cezası"]
...
def generate_v184_coupons(master184, latest_set, skeleton, block_forecast, block_birth, size=7, count=5):
    ...
    temp = master184.copy()
    temp["V18 Nefes Puanı"] = temp["V18.5 Nefes Puanı"]
...
def decorate_master_v185(master184, location_df):
    out = master184.copy()
    ...
    out["V18.5 Nefes Puanı"] = (
        0.86 * normalized_series(out["V18.4 Nefes Puanı"])
        + 0.14 * normalized_series(out["Blok Konum"])
    ) * 100.0
    out["V18.5 Nefes Puanı"] = out["V18.5 Nefes Puanı"].clip(0,100).round(2)
    return out.sort_values(["V18.5 Nefes Puanı","Sayı"], ascending=[False,True])
...
if page == "✅ Kontrol":
    st.success("Çalışan sürüm: V18.5 — Ana dosya: app.py")
    ...
    if len(df) < 100:
        ...
        st.subheader("3️⃣-B Blok Sinyal + Konum Motoru V18.5")
        ...
        st.subheader("4️⃣ V18.5 Nefes Puanı")
        st.dataframe(
            master18[
                [
                    "Sayı",
        ...
        st.info(
            ...
            "V18.5 ayrıca blok basıncı → uzunluk → konum → alternatif konum → büyüme/kırılma sırasını izler; ortak çekirdek aşırı yığılmasını frenler."
        )
