...
def finalize_day(vh,gun):
    with conn() as c:
        pdf=pd.read_sql_query('SELECT parca,a,b,birlikte FROM cift_parca WHERE veri_hash=? AND gun=?',(vh,gun),c)
        if pdf.empty: return
        ...
        af=pd.read_sql_query('SELECT parca,boyut,aile,tekrar FROM aile_parca WHERE veri_hash=? AND gun=?',(vh,gun),c)
        if not af.empty:
...
def gun_raporu(vh,gun):
    with conn() as c:
        pairs=pd.read_sql_query('SELECT a AS sayı_1,b AS sayı_2,birlikte AS birlikte_çıkış,aktif_parca AS aktif_parça,ilk_parca AS ilk_parça,son_parca AS son_parça FROM gun_cift WHERE veri_hash=? AND gun=? ORDER BY aktif_parca DESC,birlikte DESC LIMIT 100',(vh,gun),c)
        ret=pd.read_sql_query('SELECT a AS sayı_1,b AS sayı_2,onceki_parca AS dağıldığı_parça,donus_parca AS döndüğü_parça,uyku_parca AS uyku_parçası FROM donus_olay WHERE veri_hash=? AND gun=? ORDER BY uyku_parca DESC LIMIT 100',(vh,gun),c)
        fam=pd.read_sql_query('SELECT parca AS parça,boyut AS aile_büyüklüğü,aile,tekrar FROM aile_parca WHERE veri_hash=? AND gun=? ORDER BY tekrar DESC,boyut DESC LIMIT 300',(vh,gun),c)
        trans=pd.read_sql_query('SELECT parca1 AS önceki_parça,parca2 AS sonraki_parça,cekirdek AS ortak_çekirdek,onceki_aile AS önceki_aile,sonraki_aile AS sonraki_aile,ortak AS ortak_üye FROM aile_gecis WHERE veri_hash=? AND gun=? ORDER BY ortak DESC LIMIT 200',(vh,gun),c)
...
with T[0]:
    ...
    with conn() as cc:
        pr=pd.read_sql_query('SELECT gun AS gün,COUNT(*) AS tamamlanan_parça,MAX(ts) AS son_işlem FROM ilerleme WHERE veri_hash=? GROUP BY gun ORDER BY gun',(vh,),cc)
    st.dataframe(pr,use_container_width=True,hide_index=True)

with T[1]:
    gun=st.selectbox('Gün',sorted(df.gun.unique()),key='g1')
    with conn() as cc:
        q=pd.read_sql_query('SELECT a AS sayı_1,b AS sayı_2,birlikte AS birlikte_çıkış,aktif_parca AS aktif_parça,ilk_parca AS ilk_parça,son_parca AS son_parça FROM gun_cift WHERE veri_hash=? AND gun=? ORDER BY aktif_parca DESC,birlikte DESC',(vh,gun),cc)
        f=pd.read_sql_query('SELECT parca AS parça,boyut AS aile_büyüklüğü,aile,tekrar FROM aile_parca WHERE veri_hash=? AND gun=? ORDER BY tekrar DESC,boyut DESC LIMIT 1000',(vh,gun),cc)
...
with T[2]:
    ...
    with conn() as cc:
        q=pd.read_sql_query('SELECT a AS sayı_1,b AS sayı_2,onceki_parca AS son_aktif_parça,donus_parca AS geri_dönüş_parçası,uyku_parca AS uyku_süresi_parça FROM donus_olay WHERE veri_hash=? AND gun=? ORDER BY uyku_parca DESC',(vh,gun),cc)
...
with T[3]:
    ...
    with conn() as cc:
        q=pd.read_sql_query('SELECT parca1 AS önceki_parça,parca2 AS sonraki_parça,cekirdek AS ortak_çekirdek,onceki_aile AS önceki_aile,sonraki_aile AS sonraki_aile,ortak AS ortak_üye FROM aile_gecis WHERE veri_hash=? AND gun=? ORDER BY ortak DESC LIMIT 2000',(vh,gun),cc)
    st.dataframe(q,use_container_width=True,hide_index=True)
...
with T[4]:
    ...
    with conn() as cc:
        q=pd.read_sql_query('''SELECT gun AS gün, CASE WHEN a=? THEN b ELSE a END AS partner, birlikte AS birlikte_çıkış,
          aktif_parca AS aktif_parça FROM gun_cift WHERE veri_hash=? AND (a=? OR b=?) ORDER BY gun,aktif_parca DESC,birlikte DESC''',(sayi,vh,sayi,sayi),cc)
    st.dataframe(q,use_container_width=True,hide_index=True)
    if not q.empty:
