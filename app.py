...
@st.cache_data(show_spinner=True)
def build_all_triggers(ev, env, min_base=30, min_rule=15):
    """All 1-80 x observed masks. Features are strictly pre-draw.
    Returns base summary and the best exploratory second-trigger rule for every eligible number-mask.
    """
    z=ev.merge(env,on='target_draw_id',how='left').copy()
    for h in range(1,9): z[f'Diger79_H{h}']=z[f'ALL_H{h}']-z[f'H{h}']
    for k in range(0,9): z[f'Diger79_HC{k}']=z[f'ALL_HC{k}']-(z.H_count==k).astype(int)
    z['Diger79_AyniMaske']=[int(d.get(m,0))-1 for d,m in zip(z.MASK_COUNTS,z.pre_mask)]
    feats=[f'Diger79_H{h}' for h in range(1,9)]+[f'Diger79_HC{k}' for k in range(0,9)]+['Diger79_AyniMaske']
    base=z.groupby(['num','pre_mask']).REAL.agg(Aday='size',REAL='sum').reset_index()
    base['NEGATIVE']=base.Aday-base.REAL
    base=add_stats(base)
    rules=[]
    for (num,mask),g in z.groupby(['num','pre_mask'],sort=False):
        if len(g)<min_base: continue
        need=max(min_rule,int(np.ceil(len(g)*.15)))
        for f in feats:
            vals=np.sort(g[f].dropna().unique())
            if len(vals)>15:
                cuts=np.unique(np.quantile(vals,np.linspace(.1,.9,9),method='nearest'))
            else: cuts=vals
            for cut in cuts:
                for op in ('>=','<='):
                    x=g[g[f]>=cut] if op=='>=' else g[g[f]<=cut]
                    if len(x)>=need and len(x)<len(g):
                        rules.append([num,mask,f,op,float(cut),len(g),int(g.REAL.sum()),len(x),int(x.REAL.sum())])
    if not rules:
        return base,pd.DataFrame(),pd.DataFrame()
    r=pd.DataFrame(rules,columns=['Sayı','Ana_Maske','Özellik','Koşul','Eşik','Temel_Aday','Temel_REAL','Aday','REAL'])
    r=add_stats(r) # BH across the complete all-number/all-mask exploratory rule family
    r['Temel_Başarı']=r.Temel_REAL/r.Temel_Aday
    r['Temel_Lift']=r.Temel_Başarı/BASE
    r['Ek_Kazanç']=r['Başarı']-r['Temel_Başarı']
    # best positive rule per number-mask: q first, then success/lift, then sample size
    best=(r.sort_values(['Sayı','Ana_Maske','q_BH','Başarı','Aday'],ascending=[True,True,True,False,False])
            .groupby(['Sayı','Ana_Maske'],as_index=False).first())
    return base,r,best
...
if st.button('H1→H8 TETİKLEYİCİ V3 ANALİZİNİ ÇALIŞTIR',type='primary'):
...
if 'R' in st.session_state:
 ...
 if 'TRIG_ALL' not in st.session_state:
  st.session_state['TRIG_ALL']=build_all_triggers(ev,env)
 trig_base,trig_rules,trig_best=st.session_state['TRIG_ALL']
 ...
 with tabs[6]:
  ...
  st.subheader('TÜM 1–80 × MASKELER — temel ve en iyi ikinci tetikleyici')
  st.caption('Aday>=30 sayı-maskelerde ikinci tetikleyici aranır. q_BH tüm sayı×maske×eşik keşif ailesi üzerinde birlikte hesaplanır; bu tablo keşiftir, holdout değildir.')
  st.dataframe(trig_best.sort_values(['q_BH','Ek_Kazanç'],ascending=[True,False]).head(500),use_container_width=True,height=420)
  st.download_button('TÜM SAYI-MASKE TEMEL CSV',trig_base.to_csv(index=False).encode('utf-8-sig'),'H_TETIK_TUM_TEMEL_V3_1.csv','text/csv')
  st.download_button('TÜM İKİNCİ TETİKLEYİCİ KURALLARI CSV',trig_rules.to_csv(index=False).encode('utf-8-sig'),'H_TETIK_TUM_KURALLAR_V3_1.csv','text/csv')
  st.download_button('TÜM EN İYİ TETİKLEYİCİLER CSV',trig_best.to_csv(index=False).encode('utf-8-sig'),'H_TETIK_TUM_EN_IYI_V3_1.csv','text/csv')
  st.divider()
 ...
 with tabs[11]:
  ...
  sections=[('H1-H8',single),('H KOMBINASYON',combo),('TAM MASKELER',paths),('MASKE GECIS',mt),('1-80 H PARMAK IZI',fingerprint),('SAYI x H',nh),('SAYI x MASKE',nm),('TUM SAYI-MASKE TEMEL REAL-NEGATIVE',trig_base),('TUM IKINCI TETIKLEYICI KURALLARI',trig_rules),('TUM SAYI-MASKE EN IYI IKINCI TETIKLEYICI',trig_best),('UYKU HAZARD',hz),('SAYI UYKU HAZARD',nhz),('DONUS YOLLARI',rhythm),('SAYI DONUS YOLLARI',nrhythm),('GUN TUTARLILIK',daily)]
  for title,d in sections: lines += [f'=== {title} ===',d.to_csv(index=False),'']
  master='\n'.join(lines)
  st.download_button('H1-H8 TETIKLEYICI MASTER V3 TXT',master.encode('utf-8'),'H1_H8_TETIKLEYICI_MASTER_V3_1.txt','text/plain')
