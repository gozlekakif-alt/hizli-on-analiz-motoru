...
def github_config():
    """Streamlit Secrets'tan GitHub ayarlarını güvenli biçimde oku."""
    token = ""
...
def github_write_file(token, repo, branch, path, content, message):
    import urllib.request
    import urllib.parse
    _current, sha = github_read_file(token, repo, branch, path)
...
def parse_draw_block(text):
    ...
    draw_time = m_dt.group(2)

    if draw_time not in SLOTS:
        raise ValueError(f"Geçersiz saat: {draw_time}")

    nums = [int(x) for x in re.findall(r"\b\d{1,2}\b", raw[m_dt.end():])]
    nums = [n for n in nums if 1 <= n <= 80]

    if len(nums) != 20:
        raise ValueError(f"20 sayı bekleniyor; {len(nums)} sayı bulundu.")
    if len(set(nums)) != 20:
        raise ValueError("20 sayının tamamı farklı olmalı.")

    return draw_no, draw_date, draw_time, sorted(nums)


def fast_df_with_session(base_df):
    extra = st.session_state.get("fast_rows", [])
    if not extra:
        return base_df.copy()
...
def next_target_from_result(draw_date, draw_time):
    ns = next_slot(draw_time)
    if ns:
        return draw_date, ns
    return fmt(to_date(draw_date) + timedelta(days=1)), "23:02"


def cosmos_from_existing_engine(ranked, coupons):
    rank_pos = {c["n"]: i for i, c in enumerate(ranked)}
    votes = Counter()
    for cs in coupons.values():
        for c in cs:
            votes[c["n"]] += 1

    candidates = sorted(
        {c["n"] for cs in coupons.values() for c in cs},
        key=lambda n: (-votes[n], rank_pos.get(n, 999), n),
    )

    selected = [n for n in candidates if votes[n] >= 2]
    for c in ranked:
        n = c["n"]
...
st.title("⚡ Sayı Laboratuvarı v2.6 — Hızlı COSMOS")
...
tab_fast, tab_live, tab_hours, tab_numbers, tab_roads, tab_research, tab_history = st.tabs(["⚡ Hızlı 5dk","🔒 Live Blind","🎼 Saat Karakterleri","🧬 80 Sayı Profili","🛣️ 64 Yol Sözlüğü","🔬 Research","📚 Kilit Geçmişi"])


with tab_fast:
    st.subheader("⚡ Hızlı 5dk — Tek Yapıştır / Tek Tuş")
    st.caption(
        "Sonuç gelir gelmez tamamını yapıştır. Sonuç RAM'e eklenir ve GitHub beklenmeden "
        "bir sonraki çekiliş için tek COSMOS kuponu hesaplanır."
    )
    ...
    if st.button("⚡ SONUCU İŞLE + SONRAKİ KUPONU ÜRET", type="primary", use_container_width=True):
        try:
            draw_no, draw_date, draw_time, nums = parse_draw_block(pasted)

            fast_rows = st.session_state.get("fast_rows", [])
            fast_rows = [
                r for r in fast_rows
                if not (r["date"] == draw_date and r["time"] == draw_time)
            ]
            fast_rows.append({
                "draw_no": draw_no,
                "date": draw_date,
            ...
            st.session_state["fast_rows"] = fast_rows

            work_df = fast_df_with_session(df)
            target_date, target_time = next_target_from_result(draw_date, draw_time)

            st.success(f"✅ #{draw_no} — {draw_date} {draw_time} işlendi.")

            if target_time == "23:02":
                st.warning(
                    f"Yeni gün başladı. Önce {target_date} 23:02 gerçek sonucu gelsin; "
                    "onu yapıştırınca 23:07 kuponu üretilir."
                )
            else:
                source_time = draw_time
                target_d = to_date(target_date)
                live_days = complete_days(work_df)
                hist_days = [d for d in live_days if d < target_d]

                if len(hist_days) < 12:
                    raise ValueError("Yeterli geçmiş tam gün yok.")

                train_n_eff = min(20, max(10, len(hist_days) - 5))
                ...
                model = learn_hour_model(
                    work_df, source_time, target_time, train_days, calibration_days
                )
                ranked = score_live(model, target_d)
                abc = make_coupons(ranked)
                cosmos, votes = cosmos_from_existing_engine(ranked, abc)

                st.session_state["fast_last"] = {
                    "target_date": target_date,
                    "target_time": target_time,
                    "cosmos": cosmos,
                }

                st.markdown(f"## 🌌 COSMOS — {target_date} {target_time}")
                st.code("  ".join(f"{n:02d}" for n in cosmos))
                st.caption(
                    "A/B/C ortak oyları + ana saat-akoru sıralaması. "
                    "Kalıcı GitHub kaydı bu hesabı bekletmez."
                )

                st.write("A:", " ".join(f"{c['n']:02d}" for c in abc["A"]))
                st.write("B:", " ".join(f"{c['n']:02d}" for c in abc["B"]))
                st.write("C:", " ".join(f"{c['n']:02d}" for c in abc["C"]))

                vote_view = pd.DataFrame([
                    {"Sayı": n, "Motor oyu": int(votes.get(n, 0))}
                    for n in cosmos
                ])
                st.dataframe(vote_view, use_container_width=True, hide_index=True)

        except Exception as exc:
            st.error(f"Hızlı işlem başarısız: {exc}")

    last = st.session_state.get("fast_last")
    if last:
        st.divider()
        st.subheader(f"Son COSMOS: {last['target_date']} {last['target_time']}")
        st.code("  ".join(f"{n:02d}" for n in last["cosmos"]))

    st.info(
        "Hızlı modda yeni sonuçlar oturum RAM'inde zincire eklenir. "
        "Böylece 5 dakikalık döngüde veri.txt/GitHub yüklemesini beklemezsin."
    )
                    ...
                    if st.button("💾 Sonucu kalıcı veri.txt'ye ekle", type="primary"):
                        try:
                            ...
                            if token:
                                ...
                                st.warning("GitHub otomatik kayıt kapalı: Streamlit Secrets'a GITHUB_TOKEN eklenmemiş. Sağdaki 'Güncel veri.txt indir' düğmesiyle kayıt kaybolmadan dosyayı alabilirsin.")
                        except Exception as e:
