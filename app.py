...
st.set_page_config(
    page_title="Hızlı On Ultimate Analiz Motoru V11.2",
    page_icon="🎯",
    layout="wide",
)
...
def extract_exact_twenty_numbers(text: str):
    """
    Yalnızca 20 sayının satır, boşluk, virgül veya tire ile
    yapıştırıldığı girişleri kabul eder. Tarih/çekiliş başlığı varsa
    bu yardımcı kullanılmaz.
    """
    raw = str(text).strip()
    if not raw:
        return None

    # Başlık veya tarih varsa metadata ayrıştırıcıları kullanılsın.
    if re.search(r"Çekiliş\s*no|Tarih|Saat|\d{2}[./]\d{2}[./]\d{4}|\d{2}:\d{2}", raw, re.I):
        return None

    tokens = [
        int(x)
        for x in re.findall(r"(?<!\d)(?:[1-9]|[1-7]\d|80)(?!\d)", raw)
    ]
    if len(tokens) != 20 or len(set(tokens)) != 20:
        return None
    if any(n < 1 or n > 80 for n in tokens):
        return None
...
st.title("🎯 Hızlı On Ultimate Analiz Motoru V11.2")
...
if page == "✅ Kontrol":
    st.success("Çalışan sürüm: V11.2 — Ana dosya: app.py")
...
st.caption(
    "V11.2 tam çekiliş metnini veya yalnızca 20 sayıyı kabul eder; veri onarımı ve öğrenme özelliklerini korur. "
