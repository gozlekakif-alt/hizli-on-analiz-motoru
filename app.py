...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.5.7",
    page_icon="🎯",
    layout="wide",
)
...
st.title("🎯 Hızlı On Ultimate AI Studio V18.5.7")
...
def engine_votes_v18567(master, items, latest_set, skeleton, block_loc_info=None,
    ...
    for col,thr in [
        ("Oy_Senaryo",0.45),("Oy_Taşıma",55),("Oy_Dinlenmiş",58),("Oy_Blok",55),
...
def expert_context_calibration_v1857(df, target_time=None, character_window=12):
    """V18.5.7: saat + gün fazı + yakın karaktere göre uzman motor katsayılarını kalibre eder."""
    ...
    for i in range(3,len(work)):
        ...
        weight=1.0/(1.0+clock_dist/30.0)
...
def seven_seat_master_v18567(df, master, items, latest_set, skeleton, regime_summary,
    ...
    form=recent_engine_form_v18567(df,lookback=12)
    ...
    context_form, context_info = expert_context_calibration_v1857(
        df, target_time=target_time, character_window=12
    )
    ...
    votes["Master567"] = (
        0.14*nrm("Oy_Temel")
        +0.17*form["Senaryo"]*nrm("Oy_Senaryo")
        +0.08*form["Senaryo"]*nrm("Oy_SenaryoÇeşit")
        +0.10*form["Taşıma"]*nrm("Oy_Taşıma")
        +0.10*form["Dinlenmiş"]*nrm("Oy_Dinlenmiş")
        +0.14*form["Blok"]*nrm("Oy_Blok")
        +0.08*form["Bölge"]*nrm("Oy_Bölge")
        +0.08*form["Eşlikçi"]*nrm("Oy_Eşlikçi")
        +0.07*form["Blok"]*nrm("Oy_BlokPaket")
        +0.04*break_guard*nrm("Oy_Blok")
    )*100.0
    votes["Master567"] += 3.0*votes["Bağımsız Motor 567"]
    votes["Master567"] -= votes["Sürü Freni 57"]

    # V18.5.7: gerçek bağımsızlık bonusu. Senaryo kalabalığı tek başına yetmez.
    _ind = normalized_series(votes["Bağımsız Motor 567"])
    _scn = normalized_series(votes["Oy_Senaryo"])
    votes["Bağımsızlık Bonusu 57"] = 6.0*_ind*(1.0-0.45*_scn)
    votes["Master567"] += votes["Bağımsızlık Bonusu 57"]

    # hard safeguard: if 4+ independent engines support a number, don't let ranking bury it.
    votes.loc[votes["Bağımsız Motor 567"]>=4,"Master567"] += 8.0
    votes.loc[votes["Bağımsız Motor 567"]>=5,"Master567"] += 6.0
    ...
    multi=float(regime_summary.get("Çoklu Küçük %",0.0)) if regime_summary else 0.0
    longp=float(regime_summary.get("Uzun Blok %",0.0)) if regime_summary else 0.0
    max_run=3 if multi>=65 and longp<35 else 4
    max_band=3
    ...
    # Seat 7+: best remaining by total evidence.
    for _,r in ranked.iterrows():
    ...
    if break_guard>=0.45 and runlen(selected)<2:
        for _,r in blk.iterrows():
            ...
            if any(abs(n-x)==1 for x in selected):
                ...
                weak=min(selected,key=lambda x: float(votes.loc[votes["Sayı"].astype(int)==x,"Master567"].iloc[0]))
...
if page == "✅ Kontrol":
    ...
    if len(df) < 100:
        ...
        st.subheader("4️⃣ V18.5.7 Nefes Puanı")
        st.dataframe(
            master18[
                [
                    "Sayı",
        ...
        st.write("**🧩 V18.5.7 Blok Rejim / Mikro-Konum Kararı**")
        ...
        st.write("**🔁 V18.5.7 Elden Ele Taşıma Kimliği**")
        ...
        st.write("**🌙 V18.5.7 Dinlenmiş Dönüş Motoru**")
        ...
        st.caption("🏆 V18.5.7 OYNA — MASTER 7: Motor formu + çapraz motor onayı + taşıma + dinlenmiş dönüş + blok + bölge/mikro + eşlikçi + rejim kırılma sigortasını 7 koltuklu tek nihai kolonda birleştirir.")
            ...
            if _master_item:
                ...
                if isinstance(_exp, pd.DataFrame) and not _exp.empty:
                    _cols = [c for c in [
                        ...
                        "Oy_Taşıma","Oy_Dinlenmiş","Oy_Blok","Oy_Bölge","Oy_Eşlikçi","Oy_BlokPaket"
