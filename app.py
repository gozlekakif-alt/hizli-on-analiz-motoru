...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.4",
    page_icon="🎯",
    layout="wide",
)
...
st.title("🎯 Hızlı On Ultimate AI Studio V18.4")
...
def generate_v184_coupons(master184, latest_set, skeleton, block_forecast, block_birth, size=7, count=5):
    ...
    score_idx = temp.set_index("Sayı")["V18.4 Nefes Puanı"].to_dict()
...
if page == "✅ Kontrol":
    st.success("Çalışan sürüm: V18.4 — Ana dosya: app.py")
    ...
    st.caption(
        "V17/V18 özellikleri korunur. V18.4; blok basıncı, 2/3/4/5'li blok doğumu, "
    ...
    if len(df) < 100:
        ...
        st.subheader("3️⃣-B Blok Sinyal Motoru V18.4")
        ...
        if not block_region184.empty:
            st.write("**Blok doğum bölgesi tahmini**")
        ...
        st.subheader("4️⃣ V18.4 Nefes Puanı")
        st.dataframe(
            master18[
                [
                    "Sayı",
        ...
        st.info(
            ...
            "V18.4 ayrıca blok basıncı → uzunluk → bölge → büyüme/kırılma sırasını izler."
        )
