...
# ============================================================
# HIZLI ON ARAŞTIRMA LABORATUVARI V2
        ...
        while i < len(lines) and not DRAW_HEADER_RE.search(lines[i]):
            ...
            if mi:
                ...
                if 1 <= v <= 80:
                    nums.append(v)
            i += 1
        # take first 20 distinct valid numbers
        uniq = []
        for n in nums:
            if n not in uniq:
                uniq.append(n)
            if len(uniq) == 20:
                break
        if dt is not None and not pd.isna(dt) and len(uniq) == 20:
            out.append(Draw(draw_no, dt, uniq, source))
    return out
...
def txt_summary(dfs, meta):
    lines = [
        "HIZLI ON ARASTIRMA LABORATUVARI V2",
...
# -------------------------- UI --------------------------
st.set_page_config(page_title="Hızlı On Araştırma Laboratuvarı V2", layout="wide")
st.title("Hızlı On — Araştırma Laboratuvarı V2")
...
if day_count == 14:
    st.success("14 günlük veri otomatik algılandı.")
else:
    st.warning(f"Repo verisinde {day_count} gün algılandı. 14 gün bekliyorsan Dosya Denetimi sekmesini kontrol et.")
...
with tabs[12]:
    ...
    st.download_button(
        ...
        data=z,
        file_name="HIZLI_ON_ARASTIRMA_KUTUKLERI_V2.zip",
