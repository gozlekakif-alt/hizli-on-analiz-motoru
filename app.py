...
APP_VERSION = "META V6.2 — Yaşam Döngüsü + Faz Kırılım Motoru"
...
def allocate_lifecycle_master20(engine, t, target_time, ranking, size=20):
    ...
    role = []
    phase_w = 0.04 + 0.12 * min(ctx["phase_break_score"] / 100.0, 1.0)
    for i in range(80):
        if i in ctx["last_set_idx"]:
            remaining_w = 1.0 - phase_w
            final[i] = (
                remaining_w * (
                    0.44 * base_n[i]
...
st.title("🧠 Hızlı On — META V6.2 Yaşam Döngüsü + Faz Kırılım Motoru")
st.caption(
...
with st.expander("🌙 Dinlenmiş dönüş + geçiş + birliktelik ayrıntısı"):
    cols_show = [
        ...
        "Birliktelik Desteği","Faz Desteği","MASTER-20","Koltuk"
    ]
    st.dataframe(
...
with st.expander("🧬 META beyninde kullanılan analizler"):
    st.write(
        ...
        "MASTER-20 artık doğrudan ilk 20 META puanı değildir; dinamik koltuk planı ve faz kırılım bilgisiyle oluşturulur."
    )
