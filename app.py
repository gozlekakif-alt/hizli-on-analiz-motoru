...
st.set_page_config(
    page_title="Hızlı On V13.3 Tek Motor / Canlı Kupon",
    page_icon="🌙",
    layout="wide",
)

st.title("🌙 Hızlı On V13.3 — Tek Motor / Canlı Kupon")
st.caption(
...
def _validate_v13_candidate_schema(tab):
    required = {
        "Sayı","Kaynakta","TamYol","Son4Yol","SeriKanıt","GapKanıt",
        "Sıcaklık","TrendKanıt","BlokKanıt","Destek",
        "Taşı/Dön Skor","BeklenenTaşıma"
    }
    missing = sorted(required - set(tab.columns))
    if missing:
        raise ValueError(
            "V13 aday tablosunda eksik alan var: " + ", ".join(missing)
        )
...
def make_tickets(tab, gc, target_slot, df=None):
    ...
    """
    _validate_v13_candidate_schema(tab)
...
def build_last_playable_coupon(df):
    """
    En son tamamlanmış hedef için, hedef sonucu modele sızmadan kuponu yeniden kur.
    Bu sadece geçmiş kuponu göstermeye yarar; canlı oynanacak yeni kupon değildir.
    """
    last_date, last_target = latest_completed_target(df)
    if not last_date or not last_target:
        return None
    tab, gc, tickets = replay_completed_target(df, last_date, last_target)
    actual = day_map(df).get(last_date,{}).get(last_target)
    return {
        "date": last_date,
        "target": last_target,
        "gc": gc,
...
def render_coupon_block(title, tickets, target_slot):
    st.markdown(f"## {title}")
    st.markdown(f"### 🎯 {target_slot} — 7'Lİ A · TAŞIYICI")
...
with tabs[0]:
    if target_slot is not None:
        try:
            ...
            st.session_state["last_visible_coupon"] = {
                "date": target_date,
                "target": target_slot,
                "tickets": tickets,
                "live": True,
            ...
            st.success(f"🎯 CANLI OYNANACAK HEDEF: {target_date} {target_slot}")
            ...
            render_coupon_block("🎟️ ŞİMDİ OYNANACAK KUPONLAR", tickets, target_slot)

        except Exception as e:
            st.error(f"Canlı kupon üretilemedi: {e}")
            st.session_state.pop("v10_prediction", None)

    else:
        st.warning(target_error)

        # Önce oturumda son oluşturulan kuponu göster.
        cached = st.session_state.get("last_visible_coupon")
        shown = False
        if cached:
            st.info(
                f"📌 Son üretilen kupon: {cached['date']} {cached['target']} "
                "(bu hedef tamamlandı; yeni oyun için kullanma)."
            )
            render_coupon_block(
                "🧾 SON ÜRETİLEN KUPONLAR",
                cached["tickets"],
                cached["target"]
            )
            shown = True

        # Oturum cache yoksa veri.txt'den sızıntısız yeniden kur.
        if not shown:
            try:
                last = build_last_playable_coupon(df)
                if last:
                    st.info(
                        f"📌 Son tamamlanmış hedef: {last['date']} {last['target']}. "
                        "Aşağıdaki kupon geçmiş kupondur; yeni canlı kupon değildir."
                    )
                    render_coupon_block(
                        "🧾 SON ÜRETİLEN KUPONLAR",
                        last["tickets"],
                    ...
                    if last["actual"] is not None:
                        st.markdown("### 📊 Bu kuponların gerçek karnesi")
                        st.dataframe(
                            evaluate_tickets(last["tickets"], last["actual"]),
                            use_container_width=True,
                            hide_index=True
                        )
                    shown = True
            except Exception as e:
                st.error(f"Son kupon yeniden kurulamadı: {e}")

        if not shown:
            st.info(
                "Yeni canlı kupon için yeni gecenin 23:02, 23:07 ve 23:12 sonuçlarını ekle. "
                "Üçü tamamlanınca burada 7A + 7B + 10'lu kupon otomatik çıkar."
            )
...
st.caption(
    "V13.3 yalnız tek V13 taşıyıcı-kimliği motorunu kullanır. Canlı hedef varsa 7A + 7B + 10'lu kuponu doğrudan gösterir; gece tamamlanınca son kuponları silmez. 2 adet 7'li + 1 adet 10'lu üretir. Yüksek skor tek başına seçim sebebi değildir. "
