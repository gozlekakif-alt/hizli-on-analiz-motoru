...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.5.6.2 HF2",
    page_icon="🎯",
    layout="wide",
)
...
if page == "✅ Kontrol":
    ...
    if len(df) < 100:
        ...
        st.write("**🔁 V18.5.6.2 HF2 Elden Ele Taşıma Kimliği**")
        # HF2: Bu ana akışın gerçek veri çerçevesi `df`.
        # Son gerçek çekilişten panel için güvenli taşıma kümesi oluştur.
        _latest_set562 = set()
        if df is not None and not df.empty:
            _last_row562 = df.iloc[-1]
            _latest_set562 = set(
                int(_last_row562[c]) for c in num_cols(df)
                if c in _last_row562.index and pd.notna(_last_row562[c])
            )
