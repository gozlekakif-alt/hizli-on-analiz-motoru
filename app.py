...
st.set_page_config(page_title="Hızlı On — Tek Sayı + Aile & Örüntü Kadavrası", layout="wide")
st.title("🧬 Hızlı On — 1'li / 2'li / 3'lü / 4'lü Aile & Sonraki-El Örüntü Motoru")
...
def analyze_day(daydf):
    ...
    return {
        "draws":len(daydf), "single":single, "pair":pair, "tri":tri, "quad":quad,
...
def result_to_text(day, r):
    ...
    L += ["", "[TEK SAYI - 1/80 FREKANS]"]
    for k,v in sorted(r["single"].items(), key=lambda x:(-x[1],x[0])): L.append(f"{k}|N={v}|ORAN={pct(v,r['draws']):.2f}%")
...
def aggregate(all_day_results):
    S=Counter(); P=Counter(); T=Counter(); Q=Counter(); LD=Counter(); C=Counter()
    ...
    return S,P,T,Q,LD,C,tr,ranks(p2,p2b,30),ranks(t2,t2b,15),rg
...
if next_day:
    ...
    S,P,T,Q,LD,C,TR,P2,T2,RG=aggregate(st.session_state.day_texts)
    L=[
        "HIZLI ON 14 GUN 1LI-2LI-3LU-4LU AILE & ORUNTU MASTER V1.2",
        f"CEKILIS={len(df)}|GUN={len(days)}|BAS={df.draw_id.min()}|SON={df.draw_id.max()}",
        "NOT=Gun gecisleri analizlere dahil edilmez; her gun kendi icinde kronolojiktir.",
        "BASELINE_TEK_SAYI=25%","","================ 14 GUN GENEL ================",
        "[TEK SAYI - 1/80 FREKANS]"
    ]
    for k,v in sorted(S.items(), key=lambda x:(-x[1],x[0])):
        L.append(f"{k}|N={v}|ORAN={pct(v,len(df)):.3f}%")
    L += ["","[TOP 300 IKILI AILE]"]
    ...
    L += ["","================ GUNLUK KADAVRALAR ================"]
    for d in days: L += ["",st.session_state.day_texts[d]]
    master="\n".join(L)

    st.success("✅ 14/14 TAMAMLANDI — TEK MASTER HAZIR")
    st.download_button("⬇️ 14 GÜN TEK MASTER TXT İNDİR",
        master.encode("utf-8"),
        "HIZLI_ON_14_GUN_AILE_ORUNTU_MASTER_V1_2.txt",
        "text/plain", type="primary", use_container_width=True)
    st.caption(f"MASTER: {len(master):,} karakter • bütün 14 gün tek dosyada")
