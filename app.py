...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS pools(id INTEGER PRIMARY KEY AUTOINCREMENT,target_draw_id INTEGER,numbers TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP)""")
    exists=c.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='coupons'").fetchone()
    if not exists:
        c.execute("""CREATE TABLE coupons(id INTEGER PRIMARY KEY AUTOINCREMENT,target_draw_id INTEGER NOT NULL,kind TEXT NOT NULL,numbers TEXT NOT NULL,checked INTEGER DEFAULT 0,hits INTEGER DEFAULT 0)""")
    else:
        info=c.execute("PRAGMA table_info(coupons)").fetchall()
        cols=[r[1] for r in info]
        canonical=["id","target_draw_id","kind","numbers","checked","hits"]
        if cols != canonical:
            c.execute("DROP TABLE IF EXISTS coupons_new")
            c.execute("""CREATE TABLE coupons_new(id INTEGER PRIMARY KEY AUTOINCREMENT,target_draw_id INTEGER NOT NULL,kind TEXT NOT NULL,numbers TEXT NOT NULL,checked INTEGER DEFAULT 0,hits INTEGER DEFAULT 0)""")
            k="kind" if "kind" in cols else ("coupon_type" if "coupon_type" in cols else "'ESKI'")
            h="hits" if "hits" in cols else ("hit_count" if "hit_count" in cols else "0")
            ch="checked" if "checked" in cols else "0"
            tg="target_draw_id" if "target_draw_id" in cols else "0"
            nu="numbers" if "numbers" in cols else "''"
            c.execute(f"""INSERT INTO coupons_new(target_draw_id,kind,numbers,checked,hits)
                          SELECT COALESCE({tg},0),COALESCE({k},'ESKI'),COALESCE({nu},''),COALESCE({ch},0),COALESCE({h},0) FROM coupons""")
            c.execute("DROP TABLE coupons")
            c.execute("ALTER TABLE coupons_new RENAME TO coupons")
    c.execute("CREATE UNIQUE INDEX IF NOT EXISTS uq_coupon ON coupons(target_draw_id,kind,numbers)")
    c.commit(); c.close()
...
# Save current six 4-number coupons for exact target, without duplicates.
c=con()
for i,cp in enumerate(ana4,1):
    c.execute("""INSERT OR IGNORE INTO coupons(target_draw_id,kind,numbers,checked,hits)
                 VALUES(?,?,?,0,0)""",(target,f"ANA-{i}",",".join(map(str,cp))))
for i,cp in enumerate(user4,1):
    c.execute("""INSERT OR IGNORE INTO coupons(target_draw_id,kind,numbers,checked,hits)
                 VALUES(?,?,?,0,0)""",(target,f"KOD-{i}",",".join(map(str,cp))))
c.commit(); c.close()
