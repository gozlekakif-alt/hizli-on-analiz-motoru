...
def github_config():
    """Yeni düz Secrets ile eski V18 [github] Secrets biçimini birlikte destekler."""
    try:
        ...
        if gh:
            ...
            rname=str(gh.get("repo","")).strip()
            repo=(rname if "/" in rname else (f"{owner}/{rname}" if owner and rname else rname))
...
with st.expander("⚙️ Teknik durum",expanded=False):
    ...
    st.write("GitHub kalıcı kayıt:", "✅ bağlı" if (_ght and _ghr) else "⛔ bağlı değil / Secrets okunamadı")
    if _ghr:
