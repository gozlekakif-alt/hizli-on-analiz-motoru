...
def github_config():
    try:
        ...
        repo = st.secrets.get("GITHUB_REPO", "gozlekakif-alt/hizli-on-analiz-motoru")
        branch = st.secrets.get("GITHUB_BRANCH", "main")
        path = st.secrets.get("GITHUB_DATA_PATH", "veri.txt")
        return token, repo, branch, path
    except Exception:
        return "", "gozlekakif-alt/hizli-on-analiz-motoru", "main", "veri.txt"
...
def github_write_file(token, repo, branch, path, new_text, message):
    if not token:
        raise RuntimeError("GITHUB_TOKEN tanımlı değil.")
    _, sha = github_read_file(token, repo, branch, path)
    url = f"https://api.github.com/repos/{repo}/contents/{path}"
    headers = {"Accept":"application/vnd.github+json","Authorization":f"Bearer {token}"}
    payload = {"message":message,"content":base64.b64encode(new_text.encode()).decode(),"sha":sha,"branch":branch}
...
def persist_result_line(line, commit_message):
    """
    Sonuç satırını kalıcı veri.txt'ye ekler.
    GitHub token varsa doğrudan repoya yazar; yoksa False döner.
    """
    token, repo, branch, gh_path = github_config()
    if not token:
        return False, "GITHUB_TOKEN tanımlı değil."
    current_text, _sha = github_read_file(token, repo, branch, gh_path)
    updated = append_result_to_text(current_text, line)
    github_write_file(token, repo, branch, gh_path, updated, commit_message)
    return True, "Kalıcı veri.txt güncellendi."
...
st.title("🎼 Sayı Laboratuvarı v2.2 — Orkestra Motoru")
...
with tab_live:
    ...
    if start_nums_text:
        ...
        if start_nums is not None and start_draw_no.strip():
            ...
            with csave:
                if st.button("💾 23:02'yi kalıcı veri.txt'ye ekle", type="primary", key="save_2302"):
                    try:
                        ok, msg = persist_result_line(
                            start_line,
                            f"Add draw {start_draw_no.strip()} {start_date} 23:02"
                        )
                        if ok:
                            st.success("23:02 kalıcı kaydedildi. Şimdi 23:07 hedefi güncel veriyle hazırlanabilir.")
                            st.cache_data.clear()
                            st.rerun()
                        else:
                            st.error(msg)
                            st.info("Token yoksa sağdaki düğmeyle güncel veri.txt'yi indirip GitHub'a elle yükleyebilirsin.")
                    except Exception as e:
                        st.error(f"23:02 kaydı başarısız: {e}")
