...
APP_VERSION = "META Tez–Antitez V2"
...
def backtest(df, Xs, ys, ts, test_draws=250, refit_every=20):
...
st.title("🧠 Hızlı On — META Tez–Antitez V2")
st.caption(
...
if st.button("🔬 Walk-forward testi çalıştır", type="primary"):
    with st.spinner("Her çekilişte yalnız o ana kadarki geçmiş kullanılıyor..."):
        bt = backtest(df, Xs, ys, ts, test_draws=test_draws)
    if bt.empty:
