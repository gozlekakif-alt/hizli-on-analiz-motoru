...
st.title("🎯 Hızlı On V9.1 — 23:17 Sızıntısız Ritm / Taşıma / Dönüş Motoru")
st.caption(
    "İlk 3 çekilişten oyunun karakterini okur; gün içi sıcaklaşma, taşıma, dinlenip dönüş, "
    "ardışık blok ve 6x6 yol ritmini birlikte değerlendirir. Hedef: yalnız sonucu henüz bilinmeyen 23:17."
)
...
def score_candidates(df, allow_completed_target=False):
    dm=day_map(df); dates=ordered_dates(df)
    if not dates:
        raise ValueError("Veri yok.")

    # CANLI HEDEF SEÇİMİ:
    # En son günün 23:17 sonucu zaten varsa o gün için yeniden kupon üretme.
    # 23:17 yalnız 23:02/07/12 mevcut ve 23:17 henüz yoksa canlı hedef olabilir.
    d = dates[-1]
    latest = dm.get(d, {})

    if TARGET in latest and not allow_completed_target:
        raise ValueError(
            f"{d} 23:17 sonucu veri.txt içinde zaten kayıtlı. "
            "Bu gün için canlı kupon üretmiyorum. Yeni gecenin 23:02, 23:07 ve 23:12 "
            "sonuçlarını ekleyince hedef otomatik 23:17 olacaktır."
        )

    if not all(s in latest for s in INPUTS):
        miss=[s for s in INPUTS if s not in latest]
        raise ValueError(
            "Canlı 23:17 kuponu için en son gecede şu çekilişler eksik: " + ", ".join(miss)
        )

    a,b,c=[latest[s] for s in INPUTS]
...
def historical_day_prediction(df, target_date):
    """
    Tamamlanmış bir günü geriye dönük test ederken hedef 23:17 satırını modelden keser.
    Böylece o günün gerçek 23:17 sonucu tahmin üretimine sızmaz.
    """
    dts = pd.to_datetime(df["date"]+" "+df["time"], format="%d.%m.%Y %H:%M")
    cutoff = pd.to_datetime(target_date+" 23:17", format="%d.%m.%Y %H:%M")
    train = df[dts < cutoff].reset_index(drop=True)
    tab,gc,d = score_candidates(train, allow_completed_target=False)
    tickets = make_tickets(tab,gc)
    return tab,gc,d,tickets
...
with tabs[0]:
    try:
        ...
        st.success(f"🎯 CANLI HEDEF: {target_date} 23:17 — hedef sonucu veri.txt içinde henüz yok.")
        ...
        # Eski/stale tahmini de temizle ki alttaki gerçek sonuç yanlış kuponla test edilmesin.
        st.session_state.pop("v9_pred", None)
        st.warning(str(e))
        st.info(
            "Program yalnız sonucu bilinmeyen 23:17 için canlı kupon üretir. "
            "23:17 kayıtlıysa yeni gecenin 23:02 → 23:07 → 23:12 sonuçlarını bekler."
        )
...
if st.button("🏁 TEST ET + KALICI KAYDET",type="primary",use_container_width=True):
    try:
        ...
        if pred and r["date"]==pred["date"] and r["time"]==pred["time"]:
            ...
            # Hedef kupon yoksa "test ettim" deme; yalnız kayıt yapılabilir.
            st.session_state["v9_eval"]=pd.DataFrame([{
                "Kupon":"-",
                "İsabet":"-",
                "Tutanlar":"Bu gerçek sonuç için önceden üretilmiş canlı hedef kuponu yok.",
                "Kupon":"-"
            }])
        persist_result(r)
