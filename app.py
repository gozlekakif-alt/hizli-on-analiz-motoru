...
def _v9_dynamic_conductor(df, tab, pool, v7_base, target_time, size=5):
    ...
    if len(zin) and len(zout):
        ...
        # İç adayın mekanizma zayıflığı artık binary değil, dereceli.
        # İkinci/üçüncü bağımsız aile desteği zayıfsa ve/veya negatif kanıt varsa zayıflık yükselir.
        weak_second=float(fs[wi,1]); weak_third=float(fs[wi,2])
        weak_mechanism_loss = (
            0.55*np.clip((0.36-weak_second)/0.20,0,1) +
            0.25*np.clip((0.24-weak_third)/0.16,0,1) +
            0.20*np.clip(weak_neg/0.30,0,1)
        )
        ...
        v9_swap_meta.update({
            ...
            "weak_mechanism_loss": float(weak_mechanism_loss),
        ...
        if strong_independent and independent_edge and (weak_mechanism_loss>=0.48) and clean_challenger and phase_ok and band_ok:
            ...
            for k,v in [
                ...
                ("ic_aday_zayifligi",weak_mechanism_loss>=0.48),
...
with tabs[4]:
    ...
    if isinstance(bt,pd.DataFrame) and not bt.empty:
        ...
        if "V9.1 Swap Uygulandı" in bt.columns:
            st.markdown("#### ⚖️ V9.4 Relatif Dinamik / İtiraz Karnesi")
