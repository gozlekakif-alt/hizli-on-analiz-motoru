...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V17.1",
    page_icon="🎯",
    layout="wide",
)
...
def next_draw_defaults(df):
    """
    Son kayda göre bir sonraki gerçek çekiliş numarası ve zamanını üretir.

    Günlük akış:
    - 07:02 ile 01:02 arasında 5 dakikada bir çekiliş.
    - 01:02 sonrasında 01:07 yoktur.
    - 01:02'den sonraki çekiliş bir sonraki gün 07:02'dir.
    """
    ...
    try:
        ...
        # Gece kapanışı: 01:02 -> ertesi gün 07:02
        if last_dt.hour == 1 and last_dt.minute == 2:
            next_dt = (last_dt + timedelta(days=1)).replace(
                hour=7,
                minute=2,
                second=0,
                microsecond=0,
            ...
            # Güvenlik: 01:07-06:59 arasında otomatik saat üretme.
            if (
                (next_dt.hour == 1 and next_dt.minute > 2)
                or (2 <= next_dt.hour < 7)
            ):
                next_dt = (last_dt + timedelta(days=1)).replace(
                    hour=7,
...
st.title("🎯 Hızlı On Ultimate AI Studio V17.1")
...
if page == "✅ Kontrol":
    st.success("Çalışan sürüm: V17.1 — Ana dosya: app.py")
...
st.caption(
    "V17.1 blok doğumu, blok kayması, komşuluk/küme tamamlama, kalıcı motor hafızası ve 01:02→07:02 gece zaman geçişi düzeltmesini içerir. "
