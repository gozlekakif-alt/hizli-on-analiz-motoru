...
st.set_page_config(
    page_title="Hızlı On V19.1 — Gece Karakteri + Yolculuk + Test Lab",
    page_icon="🌙",
    layout="wide",
)

st.title("🌙 Hızlı On V19.1 — Gece Karakteri + Yolculuk + Test Lab")
st.caption(
...
def cutoff_df_before_target(df, date_s, target_slot):
    """
    Tarih/hedef için sonucu modele sızdırmadan yalnız hedef öncesi veriyi bırakır.
    Aynı gün hedef saatinden önceki çekilişler kalır, hedef ve sonrası çıkarılır.
    """
    cutoff = pd.to_datetime(
        f"{date_s} {target_slot}",
        format="%d.%m.%Y %H:%M",
        errors="coerce",
    )
    dts = pd.to_datetime(
    ...
    return df[dts < cutoff].reset_index(drop=True)


def historical_test(df, date_s, target_slot):
    """
    Seçilen geçmiş hedefi gerçek walk-forward mantığıyla test eder.
    Hedef sonucu tahminde kullanılmaz.
    """
    dm_full = day_map(df)
    if date_s not in dm_full or target_slot not in dm_full[date_s]:
        raise ValueError("Seçilen tarih/hedef sonucu veri.txt içinde yok.")

    train = cutoff_df_before_target(df, date_s, target_slot)
    tab, gc, neighbors, regime_conf = score_live_target(
        train, date_s, target_slot
    )
    tickets = build_tickets(tab, regime_conf)
    actual = dm_full[date_s][target_slot]
    result = evaluate_tickets(tickets, actual)
...
with tabs[1]:
    st.subheader("🧪 Geçmiş Walk-Forward Testi")
    st.caption(
        "Bir tarihi ve hedef saatini seç. Program o hedef sonucu hiç görmeden, "
        "yalnız o ana kadarki verilerle kuponu yeniden üretir ve gerçek sonuçla karşılaştırır."
    )

    dm_test = day_map(df)
    test_dates = [
        d for d in complete_night_dates(df)
        if any(s in dm_test.get(d,{}) for s in TARGETS)
    ]

    if not test_dates:
        st.info("Test edilecek tam gece bulunamadı.")
    else:
        c1,c2 = st.columns(2)
        selected_date = c1.selectbox(
            "Test tarihi",
            options=list(reversed(test_dates)),
            index=0,
        )
        selected_target = c2.selectbox(
            "Hedef saat",
            options=TARGETS,
            index=len(TARGETS)-1,
        )

        if st.button("▶️ BU HEDEFİ TEST ET", use_container_width=True):
            try:
                test = historical_test(df, selected_date, selected_target)
                st.session_state["historical_test"] = {
                    "date": selected_date,
                    "target": selected_target,
                    "tickets": test["tickets"],
                    "actual": sorted(test["actual"]),
                    "result": test["result"],
                    "gc": test["gc"],
                    "regime_conf": test["regime_conf"],
                    "neighbors": test["neighbors"],
                }
            except Exception as e:
                st.error(f"Test yapılamadı: {e}")

        hist = st.session_state.get("historical_test")
        if hist:
            st.success(f"🎯 TEST: {hist['date']} {hist['target']}")
            a,b,c = st.columns(3)
            a.metric("Gece karakteri", hist["gc"]["carry_regime"])
            b.metric("Pattern", hist["gc"]["pattern_regime"])
            c.metric("Uzman güveni", hist["regime_conf"])

            st.markdown("**Gerçek sonuç:**")
            st.code("  ".join(f"{n:02d}" for n in hist["actual"]))

            st.markdown("### 7'Lİ A")
            st.code("  ".join(f"{n:02d}" for n in hist["tickets"]["7A"]))

            st.markdown("### 7'Lİ B")
            st.code("  ".join(f"{n:02d}" for n in hist["tickets"]["7B"]))

            st.markdown("### 10'LU")
            st.code("  ".join(f"{n:02d}" for n in hist["tickets"]["10"]))
            ...
            st.caption(
                "Benzer geçmiş geceler: " + ", ".join(hist["neighbors"])
            )
