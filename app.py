...
for i,day in enumerate(DAYS):
    cols[i%7].write(('✅ ' if day in done else '⬜ ')+f'{i+1}. gün\n{day}')
...
if done:
    ...
    a.metric('Çekiliş',r['draws'])
    b.metric('2li aile',len(r['families'][2]))
    c.metric('3lü aile',len(r['families'][3]))
    d.metric('Tekrarlayan 5li',len(r['families'][5]))
    ...
    with tabs[1]:
        k=st.radio('Boyut',[2,3,4,5],horizontal=True,key='srk')
        st.dataframe(r['families'][k].head(500),width='stretch',hide_index=True)
        if k in (4,5): st.caption('4/5 yaşam tablosu: ikinci gelişi olan (tekrarlayan) aileler.')
    with tabs[2]:
        k=st.radio('Boyut',[2,3,4,5],horizontal=True,key='ck')
        st.dataframe(r['consecutive'][k],width='stretch',hide_index=True)
    with tabs[3]:
        step=st.radio('Adım',[2,3],horizontal=True)
        k=st.radio('Boyut',[2,3,4,5],horizontal=True,key='sk')
        st.dataframe(r['step2' if step==2 else 'step3'][k],width='stretch',hide_index=True)
...
st.divider()
st.subheader('🧬 14/14 Master — güvenli kurtarma')

def validate_zip(path):
    try:
        with zipfile.ZipFile(path, 'r') as z:
            bad=z.testzip()
            if bad: raise ValueError(f'CRC hatası: {bad}')
            return z.namelist()
    except Exception as exc:
        raise ValueError(f'Arşiv doğrulanamadı: {exc}') from exc

if len(done)==14:
    st.info('Günlük .pkl kayıtları korunur. Master, tek tek diske yazılıp doğrulanır; eski Master ancak yeni dosya tamamen sağlam olursa değiştirilir.')
    if st.button('🛠️ 14 GÜNÜ KURTAR / MASTER YENİDEN OLUŞTUR', type='primary', width='stretch'):
        out=STORE/'HIZLI_ON_14_GUN_AILE_KADAVRA_MASTER_KURTARILMIS.zip'
        tmp=STORE/'master_kurtarma_gecici.zip'
        try:
            with zipfile.ZipFile(tmp,'w',compression=zipfile.ZIP_STORED,allowZip64=True) as master:
                for idx,day in enumerate(DAYS,1):
                    rr=load_day_result(day)
                    day_path=STORE/f'{day}_GUN_KURTARMA.zip'
                    day_tmp=STORE/f'{day}_GUN_KURTARMA.tmp'
                    day_tmp.write_bytes(result_zip(rr))
                    del rr
                    gc.collect()
                    validate_zip(day_tmp)
                    os.replace(day_tmp,day_path)
                    # ZIP_STORED: günlük ZIP'leri ikinci kez sıkıştırma; bellek kullanımını düşür.
                    master.write(day_path,arcname=f'{day}_GUN.zip',compress_type=zipfile.ZIP_STORED)
                    st.write(f'✅ {idx}/14 — {day} doğrulandı')
                master.writestr('MASTER_OZET.txt','HIZLI ON AILE KADAVRA KURTARMA\nCEKILIS=3038\nGUN=14\n')
            names=validate_zip(tmp)
            if len([n for n in names if n.endswith('_GUN.zip')])!=14:
                raise ValueError('Master içinde 14 günlük ZIP yok.')
            # İç arşivleri de ayrı ayrı test et; yalnız dış ZIP CRC yeterli değil.
            with zipfile.ZipFile(tmp) as master:
                for day in DAYS:
                    with master.open(f'{day}_GUN.zip') as src:
                        with zipfile.ZipFile(src) as inner:
                            bad=inner.testzip()
                            if bad: raise ValueError(f'{day}: iç ZIP bozuk: {bad}')
            os.replace(tmp,out)
            st.success('14/14 günlük ZIP ve Master bütünlük testinden geçti. Kurtarma dosyası hazır.')
        except Exception as exc:
            st.error(f'Kurtarma durduruldu; günlük .pkl kayıtlarına dokunulmadı: {exc}')
    out=STORE/'HIZLI_ON_14_GUN_AILE_KADAVRA_MASTER_KURTARILMIS.zip'
    if out.exists():
        try:
            validate_zip(out)
            with open(out,'rb') as master_file:
                st.download_button('⬇️ DOĞRULANMIŞ KURTARMA MASTER ZIP',master_file,out.name,'application/zip',width='stretch')
        except Exception as exc:
            st.error(f'Kurtarma dosyası doğrulanamadı: {exc}')
else:
    st.caption('Master için 14 günün .pkl kayıtları gerekli. Mevcut günler silinmez.')

st.caption('V3.2: 1 tuş = 1 gün. Tamamlanan gün diske yazılır; ağır tablolar session_state içinde tutulmaz.')
