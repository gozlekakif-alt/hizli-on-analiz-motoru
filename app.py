...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS coupons(id INTEGER PRIMARY KEY AUTOINCREMENT,target_draw_id INTEGER,kind TEXT,numbers TEXT,checked INTEGER DEFAULT 0,hits INTEGER DEFAULT 0,matched_numbers TEXT DEFAULT '')""")
    ...
    for name,ddl in [("kind","TEXT"),("checked","INTEGER DEFAULT 0"),("hits","INTEGER DEFAULT 0"),("matched_numbers","TEXT DEFAULT ''")]:
        if name not in cols: c.execute(f"ALTER TABLE coupons ADD COLUMN {name} {ddl}")
    cols={r[1] for r in c.execute("PRAGMA table_info(coupons)").fetchall()}
    if "coupon_type" in cols: c.execute("UPDATE coupons SET kind=COALESCE(kind,coupon_type)")
    if "hit_count" in cols: c.execute("UPDATE coupons SET hits=COALESCE(hit_count,hits,0)")
    c.commit(); c.close()
...
def check_and_roll():
    c=con()
    rows=c.execute("""SELECT c.id,c.target_draw_id,c.numbers,d.numbers FROM coupons c JOIN draws d ON d.draw_id=c.target_draw_id WHERE COALESCE(c.checked,0)=0""").fetchall()
    checked=[]
    for cid,target,cp,real in rows:
        a={int(x) for x in str(cp).split(",") if x.strip()}
        b={int(x) for x in str(real).split(",") if x.strip()}
        hit=sorted(a&b)
        c.execute("UPDATE coupons SET checked=1,hits=?,matched_numbers=? WHERE id=?",(len(hit),",".join(map(str,hit)),cid))
