...
def init_db():
    ...
    # Backward-compatible migration for an existing Cloud DB.
    cols={r[1] for r in c.execute("PRAGMA table_info(coupons)").fetchall()}
    if "kind" not in cols:
        c.execute("ALTER TABLE coupons ADD COLUMN kind TEXT")
        if "coupon_type" in cols:
            c.execute("UPDATE coupons SET kind=coupon_type WHERE kind IS NULL")
    if "hits" not in cols:
        c.execute("ALTER TABLE coupons ADD COLUMN hits INTEGER DEFAULT 0")
        if "hit_count" in cols:
            c.execute("UPDATE coupons SET hits=hit_count WHERE hits=0")
    if "checked" not in cols:
        c.execute("ALTER TABLE coupons ADD COLUMN checked INTEGER DEFAULT 0")
    c.commit()
    c.close()
...
for i,cp in enumerate(ana4,1):
    c.execute("""INSERT INTO coupons (target_draw_id,kind,numbers,checked,hits)
                 ...
                 WHERE NOT EXISTS (SELECT 1 FROM coupons
                                   WHERE target_draw_id=? AND kind=? AND numbers=?)""",
