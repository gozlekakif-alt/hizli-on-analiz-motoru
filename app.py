...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.1",
    page_icon="🎯",
    layout="wide",
)
...
st.title("🎯 Hızlı On Ultimate AI Studio V18.1")
...
def all_consecutive_blocks_from_set(draw_set, min_len=2, max_len=4):
    """Bir çekilişte bulunan tüm 2/3/4'lü ardışık alt blokları çıkarır."""
    values = sorted(set(int(n) for n in draw_set))
    present = set(values)
    blocks = []
...
def block_birth_engine_v17(df, lookback=500, target_time=None):
    ...
    sets = row_sets(work)
    current_set = sets[-1]
    current_blocks = all_consecutive_blocks_from_set(current_set, 2, 4)
    ...
    blocks_by_draw = [
        all_consecutive_blocks_from_set(s, 2, 4)
        for s in sets
    ]
    ...
    if not match_df.empty:
        ...
        for _, match in match_df.iterrows():
            ...
            weight = max(float(match["Benzerlik %"]), 1.0) / 100.0
            for block in all_consecutive_blocks_from_set(draw_set, 2, 4):
    ...
    length_bonus = out["Uzunluk"].map({2: 0.00, 3: 0.04, 4: 0.06})
...
def adaptive_weights_from_memory_v18(memory_df):
    """
    Kalıcı motor hafızasından senaryo performansına göre öğrenen katsayılar üretir.
    Veri azsa nötr katsayılar kullanır.
    """
    default = {
        "Taşıma": 1.0,
        "Yerine": 1.0,
    ...
    work = work.dropna(subset=["Isabet"])
    if len(work) < 8:
        return default
    ...
    scenario_map = {
        ...
        "Saat": ["İskelet", "İskelet Dengeli"],
        "Benzer": ["Benzer/Sürpriz", "Sürpriz/Benzer"],
        "Kısaİvme": ["Benzer/Sürpriz", "Sürpriz/Benzer"],
        "Bölge": ["İskelet", "İskelet Dengeli"],
        "Küme": ["Taşıma/Küme", "Blok Doğum"],
    }

    weights = {}
    for motor, scenarios in scenario_map.items():
        subset = work[work["Senaryo"].isin(scenarios)]
        if subset.empty:
            weights[motor] = 1.0
            continue
        ratio = float(subset["Isabet"].mean()) / overall
        weights[motor] = round(min(max(ratio, 0.55), 1.65), 3)
...
def evaluate_exact_target_draw_v181(df, items, target_draw_no):
    """Kuponları yalnızca üretildikleri tek hedef çekiliş ile değerlendirir."""
    if not items or target_draw_no is None:
        return pd.DataFrame()

    target = df[df["Cekilis_No"].astype(int) == int(target_draw_no)]
    if target.empty:
        return pd.DataFrame()
...
def target_badge_v181(target_draw, target_date, target_time):
    return f"#{int(target_draw)} — {target_date} {target_time}"
...
if page == "✅ Kontrol":
    st.success("Çalışan sürüm: V18.1 — Ana dosya: app.py")
    ...
    st.caption(
        "V17'nin bütün özellikleri korunur. V18; merkezi takvim, "
        "kalıcı öğrenme, güven puanları ve otomatik hata analiziyle "
        "kapalı bir öğrenme döngüsü kurar."
    )
    ...
    if len(df) < 100:
        ...
        st.subheader("4️⃣ V18 Nefes Puanı")
        st.dataframe(
            master18[
                [
                    "Sayı",
        ...
        st.subheader("5️⃣ Öğrenen motor katsayıları")
        st.dataframe(
            pd.DataFrame(
                [
                    {
        ...
        if st.button(
            ...
            st.session_state["v180_target_draw"] = int(calendar.get("Sonraki Çekiliş"))
            st.session_state["v180_target_date"] = str(calendar.get("Sonraki Tarih"))
            st.session_state["v180_target_time"] = str(calendar.get("Sonraki Saat"))
            ...
            st.warning(
                "🎯 BU KUPONLAR SADECE ŞU ÇEKİLİŞ İÇİNDİR: "
                + target_badge_v181(
                    st.session_state["v180_target_draw"],
                    st.session_state["v180_target_date"],
                    st.session_state["v180_target_time"],
                )
            )
        ...
        if items and start_draw:
            ...
            if evaluated.empty:
                target_draw = st.session_state.get("v180_target_draw", start_draw)
                target_date = st.session_state.get("v180_target_date", "")
                target_time = st.session_state.get("v180_target_time", "")
                st.info(
                    "Bu kuponlar yalnızca "
                ...
                if (
                    not settings_error
                    and st.button(
                        "🧠 SONUCU ÖĞREN VE GITHUB HAFIZASINA YAZ",
                        key="v180_learn",
                    )
                ):
                    old_memory = load_motor_memory_v17(
                        settings
                    )
                    ...
                    save_motor_memory_v17(
                        settings,
                        combined,
                    )
                    st.success(
                        "V18 motor hafızası güncellendi."
                    )
...
st.caption(
    "V18.1 V17/V18 özelliklerini korur; kuponları üretildikleri çekiliş no+tarih+saat ile kilitler ve yanlış çekilişle öğrenmeyi engeller. "
