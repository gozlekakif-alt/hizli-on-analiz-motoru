...
def _dna_ticket_brain(tab,pool,size=5):
    """16→4/5 ayrı Kolon Beyni.
    Genel puanın ilk N'ini almaz; farklı mekanizma rollerinden koltuk doldurur.
    """
    ...
    gap=C("Gap"); same=C("AYNI YAŞAM İZİ R"); kret=C("KÜME DÖNÜŞ R")
    ret=C("Return Survival"); sleep=C("Seri→Uzun Uyku")
    carry=C("Carry Survival"); cluster=C("Küme Yaşam")
    seq=C("ARDIŞIK/+2/+3 R"); micro=np.minimum(C("Mikro Bağımsız Kanıt"),4)/4.0
    fam=np.minimum(C("Bağımsız Aile"),4)/4.0; neg=C("Negatif Kanıt")
    dna=C("DNA Return")

    z["ROL_RETURN"]=0.28*dna+0.22*ret+0.18*kret+0.17*same+0.15*np.minimum(sleep,1)
    z["ROL_TRACE"]=0.34*same+0.22*kret+0.18*seq+0.14*micro+0.12*fam
    carry_gate=np.maximum(cluster,micro)
    z["ROL_CARRY"]=(0.38*carry+0.24*cluster+0.18*micro+0.12*seq+0.08*fam)*np.clip((carry_gate-0.30)/0.55,0,1)
    z["ROL_INDEPENDENT"]=0.34*micro+0.28*fam+0.18*kret+0.12*same+0.08*ret
    z["ROL_SURGEON"]=np.maximum.reduce([
        z["ROL_RETURN"].to_numpy(),z["ROL_TRACE"].to_numpy(),
        z["ROL_CARRY"].to_numpy(),z["ROL_INDEPENDENT"].to_numpy()
    ]) + 0.10*np.sort(np.vstack([
        z["ROL_RETURN"],z["ROL_TRACE"],z["ROL_CARRY"],z["ROL_INDEPENDENT"]
    ]).T,axis=1)[:,-2] - 0.18*neg

    roles=["ROL_RETURN","ROL_TRACE","ROL_INDEPENDENT","ROL_CARRY"]
    selected=[]; used_bands=Counter()
    for role in roles:
        q=z.sort_values([role,"ROL_SURGEON","Ana Puan"],ascending=False)
        for _,r in q.iterrows():
            n=int(r["Sayı"]); b=band(n)
            if n in selected or used_bands[b]>=2: continue
            # carry koltuğu tek kanıtla dolmasın
            if role=="ROL_CARRY" and float(r[role])<0.34: continue
            selected.append(n); used_bands[b]+=1; break
        if len(selected)>=size: break

    # 5. koltuk veya eksik koltuklar: mekanizma çatışmasını çözen cerrah.
    q=z.sort_values(["ROL_SURGEON","DNA Return","Ana Puan"],ascending=False)
    for _,r in q.iterrows():
        n=int(r["Sayı"]); b=band(n)
        if n in selected or used_bands[b]>=2: continue
...
def blind_test(df,ntest=40,min_train=120):
    ...
    for i in range(start,len(df)):
        ...
        rows.append({
            ...
            "DNA 4":len(hdna4),"DNA 5":len(hdna5),
            ...
            "DNA4 Sayıları":"-".join(map(str,dna4)),"DNA4 Doğruları":"-".join(map(str,hdna4)),
            "DNA5 Sayıları":"-".join(map(str,dna5)),"DNA5 Doğruları":"-".join(map(str,hdna5)),
