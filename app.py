...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.5.6.2 HF1",
    page_icon="🎯",
    layout="wide",
)
...
st.title("🎯 Hızlı On Ultimate AI Studio V18.5.6.2")
...
def consensus_coupon_v1856(items, master, latest_set, skeleton, size=7):
    ...
    carriers = carriers.sort_values(
        ["Devam Puanı 562", "Taşıma Kimlik", "Konsensüs 56"],
...
if page == "✅ Kontrol":
    ...
    if len(df) < 100:
        ...
        st.subheader("4️⃣ V18.5.6.2 Nefes Puanı")
        st.dataframe(
            master18[
                [
                    "Sayı",
        ...
        st.write("**🧩 V18.5.6.2 Blok Rejim / Mikro-Konum Kararı**")
        ...
        st.write("**🔁 V18.5.6.2 Elden Ele Taşıma Kimliği**")
        # UI paneli, kupon üretim fonksiyonundaki yerel master1856/latest_set
        # değişkenlerine erişemez. Panel için mevcut master18 ve son gerçek
        # çekilişten güvenli biçimde latest_set oluşturulur.
        _latest_set562 = set()
        if not dfa.empty:
            _last_row562 = dfa.iloc[-1]
            _latest_set562 = set(
                int(_last_row562[c]) for c in num_cols(dfa)
                if pd.notna(_last_row562[c])
            )
        _carry562 = carry_continue_break_v18562(master18, _latest_set562)
        _carry562 = _carry562[_carry562["Sayı"].astype(int).isin(_latest_set562)].sort_values(
            ["Devam Puanı 562","Taşıma Kimlik"], ascending=[False,False]
        )
        _dev = _carry562[_carry562["Taşıma Kararı 562"]=="DEVAM"]["Sayı"].astype(int).tolist()
        _bel = _carry562[_carry562["Taşıma Kararı 562"]=="BELİRSİZ"]["Sayı"].astype(int).tolist()
        _kir = _carry562[_carry562["Taşıma Kararı 562"]=="KIRILMA"]["Sayı"].astype(int).tolist()
        c1,c2,c3 = st.columns(3)
        c1.metric("DEVAM adayı", len(_dev))
        c2.metric("BELİRSİZ", len(_bel))
        c3.metric("KIRILMA adayı", len(_kir))
        st.caption("DEVAM: " + (" - ".join(map(str,_dev[:10])) if _dev else "yok"))
        st.caption("Çoklu Küçük rejimde kuponlar artık tek 10'luk bölgeye en fazla 3 sayı yığabilir; güçlü 2–3 mikro-bölge temsil edilir.")
        ...
        st.caption("V18.5.6.2: adaptif taşıma 0–7 + Devam/Kırılma kimliği + Çoklu Küçük rejimde 2–3 mikro-bölge dağıtımı ve tek-bölge yığılma freni.")
        q1, q2 = st.columns(2)
        with q1:
