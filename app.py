...
# --------------------------- UI ---------------------------
st.title("🧬 Hızlı On — Yaşam Kimliği MASTER APP V4.4 — KALICI ULTRA CPU SAFE + 14 GÜN")
    ...
    if st.button("▶️ SONRAKİ GÜNÜ HESAPLA",use_container_width=True,key="master14_next_day_persistent",type="primary",disabled=(_progress14>=14)):
        ...
        if len(_idxs)!=217:
            ...
            try:
                ...
                _tmp=_day_cache_path14(_next_date).with_suffix(".tmp")
                _tmp.write_bytes(_dayzip)
                _tmp.replace(_day_cache_path14(_next_date))
