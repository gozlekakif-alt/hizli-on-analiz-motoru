...
# ============================================================
# TELEFON İÇİN SABİT HIZLI PANEL
# Sekmelere saklanmaz: kupon ekle -> sonucu ekle -> kaçta kaç aynı yerde.
# ============================================================
with st.expander("⚡ HIZLI KUPON + YENİ ÇEKİLİŞ + KAÇTA KAÇ", expanded=True):
    st.caption("Telefonda ana akış: önce verilen/oynadığın kolonu HIZLI KUPON olarak kaydet; sonuç gelince aynı panelden çekilişi ekle. Tüm kayıtlı kolonların Kaçta Kaç / Tuttu / Kaçtı sonucu burada görünür.")

    st.markdown("#### 1) ⚡ Hızlı kupon ekle")
    qc1,qc2=st.columns([1,2])
    qdraw=qc1.number_input("Hedef çekiliş no",min_value=1,step=1,value=int(target_draw),key="top_q_draw")
    qlabel=qc2.text_input("Kupon adı",value="HIZLI KUPON",key="top_q_label")
    qtext=st.text_input("Kupon sayılarını yapıştır (4/5/7/8/9/10 vb.)",placeholder="39 51 7 38 28",key="top_q_nums")
    qnums=_coupon_numbers_from_text(qtext)
    if qtext:
        st.caption(f"Okunan kupon: {len(qnums)} sayı · {' - '.join(map(str,qnums))}")
    if st.button("➕ HIZLI KUPONU KAYDET",type="primary",use_container_width=True,key="top_save_coupon"):
        if not (2 <= len(qnums) <= 20):
            st.error("Kupon için 2–20 benzersiz sayı gir.")
        else:
            msg=_persist_manual_coupon(int(qdraw),qlabel,qnums)
            # Hedef snapshot'ı oturumda varsa yeni manuel kuponla birlikte güncelle.
            _led=st.session_state.get("snapshot_ledger",{}) or {}
            _k=str(int(qdraw))
            if _k in _led:
                _led[_k]=_attach_manual_coupons(_led[_k]); st.session_state["snapshot_ledger"]=_led
            if int(qdraw)==int(target_draw):
                st.session_state["v26_current_snapshot"]=_attach_manual_coupons(st.session_state.get("v26_current_snapshot",snapshot))
            st.success(msg)

    _qshow=_manual_coupons_for_draw(int(qdraw))
    if _qshow:
        st.dataframe(pd.DataFrame([{"Kupon":x["label"],"Sayılar":" - ".join(map(str,x["numbers"])),"Boyut":len(x["numbers"])} for x in _qshow]),use_container_width=True,hide_index=True)

    st.divider()
    st.markdown("#### 2) ➕ Yeni çekilişi ekle ve TÜM kolonları kontrol et")
    rd1,rd2,rd3=st.columns(3)
    rdno=rd1.number_input("Çekiliş no",min_value=1,step=1,value=int(target_draw),key="top_result_draw")
    rdate=rd2.text_input("Tarih",value=str(target_date),key="top_result_date")
    rtime=rd3.selectbox("Saat",SLOTS,index=SLOTS.index(target_time) if target_time in SLOTS else 0,key="top_result_time")
    rblob=st.text_area("20 sonucu yapıştır",height=150,placeholder="Çekiliş no: ...\n17.08.2026 - 23:52\n4\n5\n7\n...",key="top_result_blob")
    rnums=_result_numbers_from_text(rblob)
    if rblob:
        st.caption(f"Okunan sonuç: {len(rnums)}/20")
    _tok,_repo,_,_=github_config()
    rsavegh=st.checkbox("GitHub veri.txt'ye de kalıcı yaz",value=bool(_tok and _repo),key="top_result_save_gh")
    if st.button("✅ SONUCU EKLE + KAÇTA KAÇ GÖSTER",type="primary",use_container_width=True,key="top_add_result"):
        if len(rnums)!=20:
            st.error("Tam 20 benzersiz sonuç sayısı gerekli.")
        else:
            _row=pd.DataFrame([{"draw_no":int(rdno),"date":rdate,"time":rtime,"numbers":rnums}])
            ...
            if _row["_dt"].isna().any():
                ...
                _snap=find_snapshot_for_draw(int(rdno))
                if _snap is not None:
                    _summ,_,_=eval_snapshot(_snap,rnums)
                    st.session_state["last_added_hit_summary"]=_summ
                    _learned,_learn_msg,_=auto_learn_from_snapshot(_snap,rnums)
                ...
                st.session_state["last_added_hit_meta"]=(int(rdno),rdate,rtime)
                _old=st.session_state.get("master_extra_df",pd.DataFrame())
                st.session_state["master_extra_df"]=merge_dataframes(_old,_row)
                _local=persist_local_draws([{"draw_no":int(rdno),"date":rdate,"time":rtime,"numbers":rnums}])
                _msg="Oturuma eklendi · "+_local
                if rsavegh:
                    try: _msg += " · "+add_draw_to_github(rdno,rdate,rtime,rnums)
                    except Exception as e: _msg += f" · GitHub yazılamadı: {e}"
    ...
    if _last_top:
        ...
        if isinstance(_summ_top,pd.DataFrame) and not _summ_top.empty:
            st.markdown(f"### 🎯 #{_last_top[0]} SONUCU — VERİLEN KOLONLAR KAÇTA KAÇ TUTTU?")
            st.dataframe(_summ_top,use_container_width=True,hide_index=True)
        else:
            st.warning("Bu çekiliş için sonuçtan önce snapshot/kupon kaydı bulunamadı.")
