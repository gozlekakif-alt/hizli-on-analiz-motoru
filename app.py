...
def init_db():
    ...
    # COUPONS TABLOSUNU TEK ŞEMAYA KİLİTLE.
    # Eski sürümlerde coupon_type/hit_count NOT NULL kaldıysa yeni INSERT'ler
    # IntegrityError veriyordu. Eski tabloyu yerinde yamamak yerine güvenli
    # biçimde canonical tabloya taşıyoruz.
    exists=c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='coupons'").fetchone()
    if not exists:
        ...
        # Rebuild whenever old columns/schema remain.
        if cols != canonical:
            c.execute("DROP TABLE IF EXISTS coupons_new")
            c.execute("""CREATE TABLE coupons_new(
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                target_draw_id INTEGER,
                kind TEXT NOT NULL,
                numbers TEXT NOT NULL,
                checked INTEGER DEFAULT 0,
                hits INTEGER DEFAULT 0)""")
            kind_expr="kind" if "kind" in cols else ("coupon_type" if "coupon_type" in cols else "'ESKI'")
            hits_expr="hits" if "hits" in cols else ("hit_count" if "hit_count" in cols else "0")
            checked_expr="checked" if "checked" in cols else "0"
            target_expr="target_draw_id" if "target_draw_id" in cols else "NULL"
            numbers_expr="numbers" if "numbers" in cols else "''"
            c.execute(f"""INSERT INTO coupons_new(target_draw_id,kind,numbers,checked,hits)
                          SELECT {target_expr},
                                 COALESCE({kind_expr},'ESKI'),
                                 COALESCE({numbers_expr},''),
                                 COALESCE({checked_expr},0),
                                 COALESCE({hits_expr},0)
                          FROM coupons""")
            c.execute("DROP TABLE coupons")
            c.execute("ALTER TABLE coupons_new RENAME TO coupons")

    c.execute("""CREATE UNIQUE INDEX IF NOT EXISTS ux_coupon_exact
                 ON coupons(target_draw_id,kind,numbers)""")
    c.commit()
    c.close()
...
for i,cp in enumerate(ana4,1):
    c.execute("""INSERT OR IGNORE INTO coupons(target_draw_id,kind,numbers,checked,hits)
                 VALUES(?,?,?,0,0)""",
              (target,f"ANA-{i}",",".join(map(str,cp))))
for i,cp in enumerate(user4,1):
    c.execute("""INSERT OR IGNORE INTO coupons(target_draw_id,kind,numbers,checked,hits)
                 VALUES(?,?,?,0,0)""",
              (target,f"KOD-{i}",",".join(map(str,cp))))
c.commit(); c.close()
