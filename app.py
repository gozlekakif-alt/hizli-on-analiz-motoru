...
with daily:
    ...
    if state.empty:
        st.warning('Bu çekiliş için kayıt bulunamadı; veri bütünlüğünü kontrol edin.')
    else:
...
with paths:
    ...
    one=one.sort_values('cekilis').copy()
    one.insert(0,'secili_tarih',day)
    one.insert(1,'secili_sayi',number)
    st.dataframe(one[['secili_tarih','secili_sayi','cekilis','el','saat','gun_oncesi','cikti','gun_sonrasi','saat_oncesi','saat_sonrasi','uyku_el','gun_kesintisiz_oncesi','gun_kesintisiz_sonrasi']],hide_index=True,use_container_width=True)
    st.download_button('📥 Bu sayının 217 çekilişlik yaşamını CSV indir',one.to_csv(index=False).encode('utf-8-sig'),f'yasam_{day}_sayi_{number}.csv','text/csv',key='download_path')
    st.caption(f'Kayıt: {len(one)}/217. 01:02 köprü çekilişi dahil; saatlik 12-el kohort hesabına dahil değildir.')
...
with export:
    ...
    st.caption('Çekiliş öncesi 80 sayı tablosu 01:02 dahil tüm 217 çekilişi kapsar. Saatlik kohort tablosu yalnız 12 çekilişlik tam saatleri kapsar.')
