...
def _sniper_histories(df):
    ...
    for di in range(1,len(dates)):
        d=dates[di]; prev_date=dates[di-1]
        if not all(x in dm.get(d,{}) for x in SNIPER_INPUT+[SNIPER_TARGET]):
            continue
        if not all(x in dm.get(prev_date,{}) for x in PREV_NIGHT):
            continue
        day=dm[d]; prev=dm[prev_date]
        events.append({
            "date":d,"prev_date":pd,
...
def sniper_2317_prediction(df, topn=10):
    ...
    return {
        "target_date":d,"target_time":"23:17","previous_date":prev_date,
...
def sniper_walk_forward(df, ntest=60):
    ...
    for di in range(1,len(dates)):
        ...
        if all(x in dm.get(d,{}) for x in SNIPER_INPUT+[SNIPER_TARGET]) and \
           all(x in dm.get(prev_date,{}) for x in PREV_NIGHT):
            candidates.append(d)
...
st.title("🎯 Sayı Laboratuvarı V8.1.1 — 23:17 Hedef Motoru")
st.caption(
