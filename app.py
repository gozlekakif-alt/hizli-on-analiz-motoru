...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.5.10",
    page_icon="🎯",
    layout="wide",
)
...
st.title("🎯 Hızlı On Ultimate AI Studio V18.5.10")
...
if page == "✅ Kontrol":
    ...
    if len(df) < 100:
        ...
        st.subheader("4️⃣ V18.5.10 Nefes Puanı")
        st.dataframe(
            master18[
                [
                    "Sayı",
        ...
        st.write("**🧩 V18.5.10 Blok Rejim / Mikro-Konum Kararı**")
        ...
        st.write("**🔁 V18.5.10 Elden Ele Taşıma Kimliği**")
        ...
        st.write("**🌙 V18.5.10 Dinlenmiş Dönüş Motoru**")
        ...
        st.caption("🏆 V18.5.10 OYNA — MASTER 7: V18.5.9 Living çekirdeğini korur. Yeni olarak 4 bağımsız Geçiş uzmanı (Dengeli / Tekrar / Yerine Geçme / Saat-Sıcaklık) walk-forward test edilir ve ayrı 👑 ÜST MASTER katmanında Living ailesiyle birleştirilir.")
        ...
        if st.button(
            ...
            _upper_coupon510, _upper_explain510 = upper_meta_master_v18510(
                items, _transition_items510, size=v18_size, transition_weights=_transition_weights510
            )
            if _upper_coupon510:
                items.append({
                    "Senaryo":"👑 ÜST MASTER 7 — Living + 4 Geçiş",
                    "Kupon":_upper_coupon510,
                    "Üst Master Açıklama 510":_upper_explain510,
                    "Geçiş Uzmanları 510":_transition_items510,
                    "Geçiş Ağırlıkları 510":_transition_weights510,
                })
            ...
            _upper_item510 = next((x for x in items if str(x.get("Senaryo","")) .startswith("👑 ÜST MASTER")), None)
            if _upper_item510:
                st.markdown("### 👑 V18.5.10 ÜST MASTER — Living + 4 Geçiş")
                st.caption("Living Engine ve Geçiş motorlarının kendi formülleri değiştirilmez; yalnız üst karar katmanında bağımsız aileler olarak birleşir.")
                _tr_items510=_upper_item510.get("Geçiş Uzmanları 510",[])
                if _tr_items510:
                    for _j,_it in enumerate(_tr_items510,1):
                        st.info(f"Geçiş Uzmanı {_j} — {_it.get('Profil','')}: " + " - ".join(map(str,_it.get('Kupon',[]))))
                _ux=_upper_item510.get("Üst Master Açıklama 510")
                if isinstance(_ux,pd.DataFrame) and not _ux.empty:
                    st.dataframe(_ux, use_container_width=True, hide_index=True)
                _bs=st.session_state.get("v18510_transition_bt_summary")
                if isinstance(_bs,pd.DataFrame) and not _bs.empty:
                    st.caption("Dört Geçiş uzmanının geleceği görmeyen kısa walk-forward ağırlıkları:")
                    st.dataframe(_bs, use_container_width=True, hide_index=True)
    ...
    if not source_numbers:
        ...
        st.divider()
        st.subheader("🧪 V18.5.10 — 4 Geçiş Uzmanı Walk-Forward Test")
        st.caption("Dengeli, Tekrar, Yerine Geçme ve Saat/Sıcaklık motorları hedef sonucu görmeden tek tek ölçülür. Bu test ÜST MASTER ağırlıklarını doğrulamak içindir.")
