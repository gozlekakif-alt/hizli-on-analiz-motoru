...
# -------------------------------------------------------------------
# ARDIŞIK + BLOK MOTOR LABORATUVARI
# -------------------------------------------------------------------
st.title("🧪 Ardışık + Blok Motor Laboratuvarı V1")
st.caption("META final seçim kapalıdır. Yalnızca Ardışık + Blok uzmanı sızıntısız geçmiş test edilir.")

def _runs(nums):
    xs=sorted(set(map(int,nums))); out=[]; cur=[]
    for x in xs:
        if not cur or x==cur[-1]+1: cur.append(x)
        else:
...
def _bf(nums):
    ss=set(map(int,nums)); rr=_runs(ss); rn=set(x for r in rr for x in r)
    bands=[sum(10*b+1<=x<=10*b+10 for x in ss) for b in range(8)]
    return {"runs":rr,"run_nums":rn,"pairs":sum(len(r)-1 for r in rr),
            "triples":sum(max(0,len(r)-2) for r in rr),"max_run":max([len(r) for r in rr],default=1),
            "bands":bands,"dense":sum(c>=4 for c in bands)}

def _bsim(a,b):
    z=(.20*min(abs(a['pairs']-b['pairs'])/5,1)+.18*min(abs(a['triples']-b['triples'])/3,1)
       +.18*min(abs(a['max_run']-b['max_run'])/3,1)+.14*min(abs(a['dense']-b['dense'])/3,1)
       +.30*min(sum(abs(x-y) for x,y in zip(a['bands'],b['bands']))/20,1))
    return max(0.,1.-z)

def block_predict(engine,t,k=7,neighbors=55):
    if t<80: return list(range(1,k+1)),np.zeros(80),{"signal":0,"confidence":0,"neighbors":0,"runs":[],"avg":0}
    cur=np.where(engine.A[t-1]==1)[0]+1; cf=_bf(cur); cand=[]
    for j in range(max(10,t-750),t-1): # j+1 < t: future leakage impossible
        jf=_bf(np.where(engine.A[j]==1)[0]+1); sim=_bsim(cf,jf)
        if cf['max_run']==jf['max_run']: sim+=.06
        if cf['pairs']==jf['pairs']: sim+=.05
        cand.append((sim,j))
    cand.sort(reverse=True); chosen=cand[:min(neighbors,len(cand))]
    score=np.zeros(80); total=0.
    for sim,j in chosen:
        nxt=np.where(engine.A[j+1]==1)[0]+1; nf=_bf(nxt); w=max(sim,.02)**2
        prev=set((np.where(engine.A[j]==1)[0]+1).tolist())
        for n in nxt:
            score[n-1]+=w*(1.0 if n in nf['run_nums'] else .28)
            if n-1 in prev or n+1 in prev: score[n-1]+=w*.22
        total+=w
    if total: score/=total
    curset=set(cur.tolist())
    for n in range(1,81):
        if n-1 in curset or n+1 in curset: score[n-1]+=.04
    pred=(np.argsort(-score)[:k]+1).tolist(); sig=cf['pairs']+2*cf['triples']+max(0,cf['max_run']-2)
    av=float(np.mean([x[0] for x in chosen])) if chosen else 0.
    return pred,score,{"signal":int(sig),"confidence":round(min(100,10*sig+55*max(0,av-.45)),1),"neighbors":len(chosen),"runs":cf['runs'],"avg":round(av,3)}

def backtest(engine,tests,k,neighbors):
    rows=[]; start=max(100,len(engine.A)-tests)
    for t in range(start,len(engine.A)):
        pred,_,inf=block_predict(engine,t,k,neighbors); actual=set((np.where(engine.A[t]==1)[0]+1).tolist()); hit=sorted(set(pred)&actual); af=_bf(actual)
        rows.append({"Index":t,"Sinyal":inf['signal'],"Güven":inf['confidence'],"Tahmin":" - ".join(map(str,pred)),"İsabet":len(hit),"Tutturan":" - ".join(map(str,hit)) if hit else "Yok","Gerçek blok içi isabet":len(set(pred)&af['run_nums'])})
    return pd.DataFrame(rows)
...
try:
    ...
    st.error(f"Veri hazırlanamadı: {e}"); st.stop()
st.success(f"Veri hazır: {len(df)} çekiliş")

k=st.slider("Uzmanın aday sayısı",3,12,7)
neighbors=st.slider("Benzer geçmiş blok durumu",20,100,55,5)
pred,score,info=block_predict(eng,len(df),k,neighbors)
st.header("1️⃣ Canlı blok teşhisi")
a,b,c,d=st.columns(4); a.metric("Sinyal",info['signal']); b.metric("Güven",f"%{info['confidence']}"); c.metric("Benzer geçmiş",info['neighbors']); d.metric("Ort. benzerlik",info['avg'])
st.write("Son çekiliş ardışıkları:",info['runs'] if info['runs'] else "Yok")
st.info("Adaylar: "+" - ".join(map(str,pred)))
rank=pd.DataFrame({"Sayı":range(1,81),"Blok puanı":score}).sort_values("Blok puanı",ascending=False).reset_index(drop=True); rank.insert(0,"Sıra",range(1,81)); st.dataframe(rank,use_container_width=True,hide_index=True)

st.header("2️⃣ Sızıntısız geçmiş laboratuvarı")
tests=st.select_slider("Test adedi",options=[100,250,500,750],value=250)
if st.button("🧪 Ardışık + Blok motorunu test et",type="primary"):
    bt=backtest(eng,min(tests,len(df)-100),k,neighbors); rnd=k*20/80; avg=float(bt['İsabet'].mean()); active=bt[bt.Sinyal>=2]; strong=bt[bt.Sinyal>=4]
    a,b,c,d=st.columns(4); a.metric("Genel",f"{avg:.3f}/{k}"); b.metric("Rastgele",f"{rnd:.3f}/{k}",delta=f"{avg-rnd:+.3f}"); c.metric("Sinyal aktif",f"{active['İsabet'].mean():.3f}/{k}" if len(active) else "—"); d.metric("Güçlü sinyal",f"{strong['İsabet'].mean():.3f}/{k}" if len(strong) else "—")
    grp=bt.assign(SinyalGrubu=pd.cut(bt.Sinyal,[-1,1,3,5,999],labels=['0–1','2–3','4–5','6+']))
    sm=grp.groupby('SinyalGrubu',observed=True).agg(Test=('İsabet','size'),Ortalama=('İsabet','mean'),Maksimum=('İsabet','max'),BlokİçiOrt=('Gerçek blok içi isabet','mean')).reset_index(); sm['Rastgele']=rnd; sm['Net fark']=sm.Ortalama-rnd
    st.dataframe(sm,use_container_width=True,hide_index=True); st.dataframe(bt.sort_values('Index',ascending=False),use_container_width=True,hide_index=True)
    st.download_button("⬇️ Test CSV indir",bt.to_csv(index=False).encode('utf-8-sig'),"blok_motor_lab_test.csv","text/csv")
st.warning("Bu sürüm META kuponu üretmez. Önce bu uzmanı kanıtlayacağız; sonra diğer motorlara geçeceğiz.")
