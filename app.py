...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS coupons(id INTEGER PRIMARY KEY AUTOINCREMENT,target_draw_id INTEGER,kind TEXT,numbers TEXT,checked INTEGER DEFAULT 0,hits INTEGER DEFAULT 0)""")
    ...
    if "kind" not in cols:
        ...
        if "coupon_type" in cols: c.execute("UPDATE coupons SET kind=coupon_type WHERE kind IS NULL")
    if "hits" not in cols:
        c.execute("ALTER TABLE coupons ADD COLUMN hits INTEGER DEFAULT 0")
        if "hit_count" in cols: c.execute("UPDATE coupons SET hits=hit_count WHERE hits=0")
    if "checked" not in cols: c.execute("ALTER TABLE coupons ADD COLUMN checked INTEGER DEFAULT 0")
    c.commit(); c.close()
...
def load():
    ...
    c.close()
    if df.empty:
        return pd.DataFrame(columns=["draw_id","draw_time","date","numbers","Sayilar"])
    ...
    # Canonical day key required by same_day_history/build_pool.
...
def same_day_history(df):
    ...
    if "date" not in x.columns:
        if "draw_time" not in x.columns:
            return x.iloc[0:0].copy()
        x["draw_time"]=pd.to_datetime(x["draw_time"],dayfirst=True,errors="coerce")
        x["date"]=x["draw_time"].dt.date
    d=x.iloc[-1]["date"]
    return x[x["date"]==d].reset_index(drop=True)
