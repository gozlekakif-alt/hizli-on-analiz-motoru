...
def parse_txt(text: str, source=""):
    ...
    Bu okuyucu satır bazlıdır ve boşluk/BOM gibi farkları tolere eder.
    """
    out = []
    for line_no, raw in enumerate(text.splitlines(), start=1):
        ...
        # Önce gerçek repo biçimi: no ; tarih saat ; 20 sayı
        m = re.match(
            r"^\s*(\d+)\s*;\s*(\d{2}\.\d{2}\.\d{4})\s+(\d{2}:\d{2})\s*;\s*(.+?)\s*$",
            line
        )
        if m:
            try:
                ...
                # comma, whitespace or accidental semicolon between numbers tolerated
...
def load_repo(root: Path):
    ...
    # veri.txt first, because this is the canonical repo dataset
    files = sorted(files, key=lambda f: (0 if f.name.lower() == "veri.txt" else 1, str(f)))

    for f in files:
        try:
            raw = f.read_bytes()
            # UTF-8 first; fallback to common Turkish/Windows encodings
            text = None
            ...
            if f.suffix.lower() == ".txt":
                ds = parse_txt(text or "", str(f))
            elif f.suffix.lower() == ".csv":
                ds = parse_csv_bytes(raw, str(f))
            else:
                ds = []
            ...
            audit.append({
                ...
                "bytes": len(raw),
                "first_line_preview": first_nonempty[:140],
                "draws_parsed": len(ds)
            })
            all_draws.extend(ds)
...
def txt_summary(dfs, meta):
    lines = [
        "HIZLI ON ARASTIRMA LABORATUVARI V4",
...
# -------------------------- UI --------------------------
st.set_page_config(page_title="Hızlı On Araştırma Laboratuvarı V4", layout="wide")
st.title("Hızlı On — Araştırma Laboratuvarı V4")
st.caption("Repo verisini otomatik bulur. Dosya yükleme yok. Önce gün gün, sonra 14 gün bütün olarak analiz eder.")

ROOT = Path(".").resolve()

draws, audit = load_repo(ROOT)

if not draws:
    st.error("veri.txt bulundu fakat çekiliş satırları okunamadı.")
    st.write("Aşağıdaki tabloda özellikle `veri.txt` satırındaki **first_line_preview** alanını kontrol et.")
...
with tabs[12]:
    ...
    st.download_button(
        ...
        data=z,
        file_name="HIZLI_ON_ARASTIRMA_KUTUKLERI_V4.zip",
