...
st.set_page_config(
    page_title="Hızlı On Ultimate AI Studio V18.5.13 + META-20 V1",
    page_icon="🎯",
    layout="wide",
)
...
def meta20_v1(items, transition_items, df, target_time=None, memory_df=None, target_size=10):
    """META-20 V1 — bütün sayı bulucuların kanıtını tek 1–80 tablosunda toplar.

    Hedef sonucu kullanmaz. Önce 80 sayının tamamını puanlar, ilk 20 aday havuzunu
    oluşturur; sonra aynı sıralamadan 7/8/9/10 hedef büyüklüğünde final kupon seçer.
    Aynı kökten gelen tekrar oyları aile içinde tekilleştirilir ve sürü cezası uygulanır.
    """
    if df is None or df.empty:
        return [], pd.DataFrame(), pd.DataFrame()

    def _family(name, profile=None):
        s = str(name or "")
        if profile:
            return "GEÇİŞ:" + str(profile)
        if s.startswith("🏆 OYNA"):
            return "MASTER"
        if s.startswith("👑 ÜST MASTER"):
            return "ÜST_MASTER"
        if "İskelet" in s:
            return "LIVING:İSKELET"
        if "Blok Rejim" in s or "Blok Doğum" in s:
            return "LIVING:BLOK"
        if "Taşıma/Küme" in s:
            return "LIVING:TAŞIMA"
        if "Yenilenme" in s:
            return "LIVING:YENİLENME"
        if "Benzer/Sürpriz" in s:
            return "LIVING:BENZER"
        if "Birleşik Güç/Konsensüs" in s:
            return "LIVING:KONSENSÜS"
        return "LIVING:DİĞER"

    # Türetilmiş 5.12/5.13 seçicilerini ham kanıt olarak tekrar sayma.
    base_items = []
    for it in (items or []):
        name = str(it.get("Senaryo", ""))
        if not it.get("Kupon"):
            continue
        if name.startswith("🧠 V18.5.12") or name.startswith("🧬 V18.5.13") or name.startswith("🧠 META-20"):
            continue
        base_items.append(it)

    # Geçmiş senaryo formu; az örneği nötre küçült.
    scenario_weight = {}
    if isinstance(memory_df, pd.DataFrame) and not memory_df.empty and {"Senaryo", "Isabet"}.issubset(memory_df.columns):
        mem = memory_df.copy()
        mem["Isabet"] = pd.to_numeric(mem["Isabet"], errors="coerce")
        ...
        for name, g in mem.groupby(mem["Senaryo"].astype(str)):
            ...
            if not len(g):
                continue
            long_avg = float(g["Isabet"].mean())
            short_avg = float(g.tail(min(15, len(g)))["Isabet"].mean())
            blended = 0.68 * short_avg + 0.32 * long_avg
            reliability = min(1.0, len(g) / 30.0)
            raw = float(np.clip(0.70 + 0.30 * (blended / 1.75), 0.68, 1.38))
            scenario_weight[name] = 1.0 + reliability * (raw - 1.0)
    ...
    for it in base_items:
        ...
        evidence.append({
            "name": name,
            "family": _family(name),
            "coupon": set(int(x) for x in it.get("Kupon", []) if 1 <= int(x) <= 80),
            "weight": float(scenario_weight.get(name, 1.0)),
            "internal": {},
        })
    for j, it in enumerate(transition_items or [], start=1):
        profile = str(it.get("Profil", f"Uzman {j}"))
        internal = {int(k): float(v) for k, v in (it.get("Uzman Puanları 511", {}) or {}).items()}
        evidence.append({
            "name": f"Geçiş Uzmanı {j} — {profile}",
    ...
    smart = intelligent_score_table(df, target_time).copy()
    smart_idx = smart.set_index("Sayı") if not smart.empty and "Sayı" in smart.columns else pd.DataFrame()
    if not smart.empty and "Toplam Puan" in smart.columns:
        smin, smax = float(smart["Toplam Puan"].min()), float(smart["Toplam Puan"].max())
        smart_norm = {int(r["Sayı"]): (float(r["Toplam Puan"])-smin)/(smax-smin) if smax>smin else 0.5 for _,r in smart.iterrows()}
    else:
        smart_norm = {n:0.5 for n in range(1,81)}

    latest = set(int(df.iloc[-1][c]) for c in NUM_COLS)
    all_families = sorted(set(e["family"] for e in evidence))
    rows=[]
    for n in range(1,81):
        fam_best={}
        supporters=[]
        raw_support=0
        master_support=0
        transition_internal=0.0
        best_single_weight=0.0
        for e in evidence:
            if n not in e["coupon"]:
                continue
            raw_support += 1
            supporters.append(e["name"])
            fam=e["family"]
            w=float(e["weight"])
            fam_best[fam]=max(float(fam_best.get(fam,0.0)), w)
            best_single_weight=max(best_single_weight,w)
            if fam in {"MASTER","ÜST_MASTER"}:
                master_support += 1
            if fam.startswith("GEÇİŞ:"):
                transition_internal=max(transition_internal, float(e.get("internal",{}).get(n,0.0))/100.0)

        independent=len(fam_best)
        fam_sum=sum(fam_best.values())
        family_evidence=min(fam_sum/4.0,1.0)
        cross_evidence=min(independent/4.0,1.0)
        master_evidence=min(master_support/2.0,1.0)
        carry=1.0 if n in latest else 0.0
        minority_quality=(best_single_weight if independent in (1,2) else 0.0)
        herd=max(0.0,(raw_support-independent)/max(raw_support,1)) if raw_support else 0.0
        stat=float(smart_norm.get(n,0.5))
        score=(
            30.0*stat + 24.0*family_evidence + 15.0*cross_evidence
            + 10.0*master_evidence + 7.0*carry + 7.0*transition_internal
            + 7.0*minority_quality - 10.0*herd
        )
        rows.append({
            "Sayı":n,
            "META-20 Puan":round(score,3),
            "Genel Analiz":round(stat*100,2),
            "Bağımsız Aile":independent,
            "Ham Destek":raw_support,
            "Aile Kanıtı":round(family_evidence,3),
            "MASTER Kanıtı":round(master_evidence,2),
            "Geçiş İçi":round(transition_internal*100,2),
            "Taşıma":int(carry),
            "Sürü Cezası":round(herd,3),
            "Aileler":" | ".join(sorted(fam_best)),
            "Destekleyenler":" | ".join(supporters),
        })

    full=pd.DataFrame(rows).sort_values(["META-20 Puan","Bağımsız Aile","Genel Analiz","Sayı"],ascending=[False,False,False,True]).reset_index(drop=True)
    top20=full.head(20).copy()
    top20["META Sıra"]=np.arange(1,len(top20)+1)

    size=int(max(7,min(10,target_size)))
    selected=[]; reasons={}; band_counts=Counter(); signatures={int(r["Sayı"]):set(str(r["Aileler"]).split(" | "))-{""} for _,r in top20.iterrows()}
    remaining=set(int(x) for x in top20["Sayı"])
    max_band=3
    while remaining and len(selected)<size:
        best_n=None; best_adj=-1e9
        for n in sorted(remaining):
            row=top20[top20["Sayı"]==n].iloc[0]
            band=ten_band_name(n)
            if band_counts[band]>=max_band:
                continue
            trial=sorted(selected+[n]); run=best_run=1
            for a,b in zip(trial,trial[1:]):
                run=run+1 if b==a+1 else 1; best_run=max(best_run,run)
            if best_run>3:
                continue
            sim=0.0
            if selected:
                sig=signatures.get(n,set())
                sims=[]
                for s in selected:
                    sig2=signatures.get(s,set())
                    union=sig|sig2
                    sims.append(len(sig&sig2)/len(union) if union else 0.0)
                sim=max(sims) if sims else 0.0
            adj=float(row["META-20 Puan"]) - 5.0*sim
            if adj>best_adj:
                best_adj=adj; best_n=n
        if best_n is None:
            best_n=min(remaining,key=lambda n:int(top20.index[top20["Sayı"]==n][0]))
        selected.append(int(best_n)); remaining.remove(int(best_n)); band_counts[ten_band_name(int(best_n))]+=1
        r=top20[top20["Sayı"]==best_n].iloc[0]
        if int(r["Bağımsız Aile"])>=3:
            reasons[best_n]="çoklu bağımsız kanıt"
        elif float(r["MASTER Kanıtı"])>0:
            reasons[best_n]="MASTER destekli"
        elif int(r["Taşıma"])>0:
            reasons[best_n]="taşıma + meta puan"
        else:
            reasons[best_n]="META çeşitlilik/seçim"

    top20["Final Hedef"] = top20["Sayı"].isin(selected)
    top20["Seçim Nedeni META"] = top20["Sayı"].map(reasons).fillna("")
    return sorted(selected), top20, full
...
st.title("🎯 Hızlı On Ultimate AI Studio V18.5.13 + META-20 V1")
...
if page == "✅ Kontrol":
    ...
    if len(df) < 100:
        ...
        with q3:
            meta_target_size = st.selectbox(
                "🧠 META-20 hedef",
                [7, 8, 9, 10],
                index=3,
                key="meta20_target_size",
                help="META-20 ilk 20 aday havuzundan kaç sayılık final hedef kupon üretileceğini seçer.",
            )
        ...
        if st.button(
            ...
            # META-20 V1 — tüm analizleri 1–80 ortak beyinde birleştir; önce Top-20 havuz, sonra hedef 7/8/9/10.
            _meta_coupon, _meta_top20, _meta_full = meta20_v1(
                items, _transition_items511, df, target_time=v18_target_time,
                memory_df=memory, target_size=meta_target_size
            )
            if _meta_coupon:
                items.append({
                    "Senaryo":f"🧠 META-20 V1 — Hedef {int(meta_target_size)}",
                    "Kupon":_meta_coupon,
                    "META20 Top20":_meta_top20,
                    "META20 Tam80":_meta_full,
                    "META20 Hedef":int(meta_target_size),
                })
            ...
            _meta_item = next((x for x in items if str(x.get("Senaryo", "")).startswith("🧠 META-20 V1")), None)
            if _meta_item:
                st.markdown("### 🧠 META-20 V1 — Tek Beyin / 1–80 Ortak Analiz")
                st.caption("Living, MASTER, ÜST MASTER ve dört Geçiş uzmanının kanıtlarını aynı 80 sayılık tabloda toplar. Aynı kökten gelen oyları tekilleştirir, sürü cezası uygular; önce Top-20 aday havuzunu kurar, sonra seçtiğin 7/8/9/10 hedefe sıkıştırır. Hedef sonucu seçim sırasında kullanılmaz.")
                st.success(f"🎯 META hedef {int(_meta_item.get('META20 Hedef', len(_meta_item.get('Kupon',[]))))}: " + " - ".join(map(str,_meta_item.get("Kupon",[]))))
                _mt = _meta_item.get("META20 Top20")
                if isinstance(_mt,pd.DataFrame) and not _mt.empty:
                    _mc=[c for c in ["META Sıra","Sayı","META-20 Puan","Genel Analiz","Bağımsız Aile","Ham Destek","MASTER Kanıtı","Geçiş İçi","Taşıma","Sürü Cezası","Final Hedef","Seçim Nedeni META"] if c in _mt.columns]
                    st.dataframe(_mt[_mc],use_container_width=True,hide_index=True)
        ...
        if items and start_draw:
            ...
            if evaluated.empty:
                ...
                try:
                    _meta_eval = next((x for x in items if str(x.get("Senaryo", "")).startswith("🧠 META-20 V1")), None)
                    _actual_meta = df[(df["Tarih"].astype(str) == str(target_date)) & (df["Saat"].astype(str) == str(target_time))].tail(1)
                    if _meta_eval and not _actual_meta.empty:
                        _actual_set_meta=set(int(_actual_meta.iloc[0][c]) for c in NUM_COLS)
                        _top20_meta=_meta_eval.get("META20 Top20")
                        if isinstance(_top20_meta,pd.DataFrame) and not _top20_meta.empty:
                            _pool_meta=set(_top20_meta["Sayı"].astype(int))
                            _hits_meta=sorted(_pool_meta & _actual_set_meta)
                            _final_meta=set(int(x) for x in _meta_eval.get("Kupon",[]))
                            _final_hits=sorted(_final_meta & _actual_set_meta)
                            st.info(
                                f"🧠 META-20 sonuç analizi: Top-20 havuzda {len(_hits_meta)}/20 gerçek sayı yakalandı "
                                f"({ ' - '.join(map(str,_hits_meta)) if _hits_meta else 'yok' }). "
                                f"Hedef {len(_final_meta)} kupon: {len(_final_hits)}/{len(_final_meta)} "
                                f"({ ' - '.join(map(str,_final_hits)) if _final_hits else 'yok' })."
                            )
                except Exception:
                    pass
