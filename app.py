...
if len(done)==14:
    if st.button('14/14 HAZIR GÜNLERDEN MASTER OLUŞTUR',width='stretch'):
        out=STORE/'HIZLI_ON_14_GUN_AILE_KADAVRA_MASTER_V3_2.zip'
        with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED) as z:
            for day in DAYS:
                rr=load_day_result(day)
                z.writestr(f'{day}_GUN.zip',result_zip(rr))
                del rr
                gc.collect()
            z.writestr('MASTER_OZET.txt','HIZLI ON AILE KADAVRA V3.2\nCEKILIS=3038\nGUN=14\nMIMARI=DISK_BACKED_1_TUS_1_GUN\n')
    mp=STORE/'HIZLI_ON_14_GUN_AILE_KADAVRA_MASTER_V3_2.zip'
    if mp.exists(): st.download_button('⬇️ 14 GÜNLÜK MASTER ZIP',mp.read_bytes(),mp.name,'application/zip',width='stretch')
else:
    st.caption('Master 14 gün tamamlanınca açılır. Hazır günler yeniden hesaplanmaz.')
