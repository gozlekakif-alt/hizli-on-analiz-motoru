...
st.set_page_config(page_title="BANT TEK LAB V1", page_icon="📊", layout="wide")
APP_VERSION = "BANT TEK LAB V1 — Walk-forward bant kalibrasyonu"

# ===================================================================
# BANT TEK LAB V1
# ===================================================================

def _bant_features(engine, t):
    """
    1-80'i 8 adet 10'luk banda ayırır.
    Bir sayının kendi bandındaki basınç/soğuma/dönüş davranışını yalnız geçmişten ölçer.
    """
    if t < 140:
        return pd.DataFrame()
    ...
    h10 = A[max(0, t-10):t]
    h30 = A[max(0, t-30):t]
    h100 = A[max(0, t-100):t]

    for idx in range(80):
        band_id = idx // 10
        lo, hi = band_id*10, band_id*10 + 10

        # Bandın çekiliş başına ortalama üye sayısı
        b5 = float(h5[:, lo:hi].sum(axis=1).mean()) if len(h5) else 0.0
        b10 = float(h10[:, lo:hi].sum(axis=1).mean()) if len(h10) else 0.0
        b30 = float(h30[:, lo:hi].sum(axis=1).mean()) if len(h30) else 0.0
        b100 = float(h100[:, lo:hi].sum(axis=1).mean()) if len(h100) else 0.0

        trend = b5 - b30
        trend_long = b10 - b100

        # Son çekilişte bandın kaç elemanı vardı?
        last_band = int(A[t-1, lo:hi].sum())

        # Benzer bant basıncından sonra bu sayının gerçek dönüş oranı
        cases = hits = recent_cases = recent_hits = 0
        for j in range(max(30, t-420), t):
            if engine.draw_nos[j] != engine.draw_nos[j-1] + 1:
                continue
            prev5 = A[max(0,j-5):j, lo:hi]
            prev30 = A[max(0,j-30):j, lo:hi]
            if len(prev5) == 0 or len(prev30) == 0:
                continue
            pb5 = float(prev5.sum(axis=1).mean())
            pb30 = float(prev30.sum(axis=1).mean())
            ptrend = pb5 - pb30

            # Aynı rejim: yakın kısa dönem bant yoğunluğu + yakın trend
            if abs(pb5 - b5) <= 0.45 and abs(ptrend - trend) <= 0.40:
                cases += 1
                hits += int(A[j, idx])
                if j >= t-150:
                    recent_cases += 1
        ...
        base = float(A[max(0,t-300):t, idx].mean())
        p_regime = (hits + 7.0*base) / max(cases + 7, 1)
        p_recent = (recent_hits + 5.0*p_regime) / max(recent_cases + 5, 1)

        rows.append({
            "Sayı": idx+1,
            "Bant": f"{lo+1}-{hi}",
            "band_id": band_id,
            "b5": b5, "b10": b10, "b30": b30, "b100": b100,
            "trend": trend, "trend_long": trend_long,
            "last_band": last_band,
            "p_regime": p_regime, "p_recent": p_recent,
            "cases": cases, "recent_cases": recent_cases,
            "base": base,
        })
    ...
    # Band motorunda hem sıcaklaşma hem rejim içi gerçek dönüş ayrı kanaldır.
    d["recent_n"] = mm(d["p_recent"])
    d["regime_n"] = mm(d["p_regime"])
    d["trend_n"] = mm(d["trend"])
    d["trend_long_n"] = mm(d["trend_long"])
    d["pressure_n"] = mm(d["b5"])
    ...
    d = d.sort_values(
        ["BANT skor","p_recent","p_regime","trend"],
        ascending=False
    ).reset_index(drop=True)
    ...
    for t in range(max(160, int(start_t)), len(engine.A)):
        ...
        for _, r in tab.iterrows():
            ...
            rows.append({
                ...
                "Bant": r["Bant"],
                "Sıra": int(r["Sıra"]),
                "BANT skor": float(r["BANT skor"]),
                "b5": float(r["b5"]),
                "b10": float(r["b10"]),
                "b30": float(r["b30"]),
                "b100": float(r["b100"]),
                "trend": float(r["trend"]),
                "trend_long": float(r["trend_long"]),
                "last_band": int(r["last_band"]),
                "p_regime": float(r["p_regime"]),
...
def bant_calibration_report(wf):
    if wf.empty:
    ...
    d["Dönem"] = np.where(
        d["Index"] <= q1, "ERKEN",
        np.where(d["Index"] <= q2, "ORTA", "SON")
    )

    try:
        d["Skor dilimi"] = pd.qcut(d["BANT skor"], 10, duplicates="drop")
    except Exception:
        d["Skor dilimi"] = pd.cut(d["BANT skor"], 10)
    ...
    for k in [1,2,3,4,5,6,8,10]:
        ...
        row = {
            "TopK": k,
            "Aday": len(s),
            "Doğruluk": s["Doğru"].mean(),
            "Net_vs_%25": s["Doğru"].mean() - .25,
        }
        for era in ["ERKEN","ORTA","SON"]:
            z = s[s["Dönem"] == era]
            row[era] = z["Doğru"].mean() if len(z) else np.nan
        rr.append(row)
    return score, pd.DataFrame(rr)


def bant_strict_selector_from_history(wf, target_index, current_tab):
    ...
    for _, r in current_tab.iterrows():
        rank = int(r["Sıra"])
        score = float(r["BANT skor"])
        band_id = int(r["band_id"])
        tr = float(r["trend"])
        pr = float(r["p_recent"])

        h = hist[
            (hist["Sıra"] == rank) &
            ((hist["Sayı"]-1)//10 == band_id) &
            (np.abs(hist["BANT skor"]-score) <= .07) &
            (np.abs(hist["trend"]-tr) <= .35) &
        ...
        if len(h) < 70:
            h = hist[
                (hist["Sıra"] == rank) &
                ((hist["Sayı"]-1)//10 == band_id) &
                (np.abs(hist["BANT skor"]-score) <= .11) &
                (np.abs(hist["trend"]-tr) <= .55) &
                (np.abs(hist["p_recent"]-pr) <= .09)
            ].copy()

        ok = False
        acc = first = second = np.nan
        if len(h) >= 70:
            h = h.sort_values("Index")
            mid = len(h)//2
            first = float(h.iloc[:mid]["Doğru"].mean())
            second = float(h.iloc[mid:]["Doğru"].mean())
            acc = float(h["Doğru"].mean())
            ok = bool(acc >= .36 and first >= .32 and second >= .32)
        ...
        details.append({
            ...
            "Bant": r["Bant"],
            "Sıra": rank,
            "BANT skor": round(score,4),
            "trend": round(tr,4),
            "p_recent": round(pr,4),
            "Benzer geçmiş": int(len(h)),
            "Geçmiş doğruluk": round(acc,4) if np.isfinite(acc) else np.nan,
            "İlk yarı": round(first,4) if np.isfinite(first) else np.nan,
            "Son yarı": round(second,4) if np.isfinite(second) else np.nan,
            "KONUŞ": bool(ok),
        })
...
def bant_strict_walkforward(engine, start_t=160):
    wf = bant_walkforward_lab(engine, start_t)
    if wf.empty:
        return pd.DataFrame(), wf

    rows = []
    for t in sorted(wf["Index"].unique()):
        tab = _bant_features(engine, int(t))
        picks, _ = bant_strict_selector_from_history(wf, int(t), tab)
...
st.title("📊 BANT TEK LAB V1")
st.caption(
    "Yalnız BANT uzmanı çalışır. 1-80 sekiz adet 10'luk bantta incelenir. "
    "Yüksek bant yoğunluğu tek başına sinyal değildir; geçmiş rejimde gerçek isabet şarttır."
)

try:
    st.success(
        f"Veri hazır • {len(df)} çekiliş • son çekiliş #{int(df['Cekilis_No'].iloc[-1])}"
    )
except Exception:
    pass

if st.button("📊 BANT TAM WALK-FORWARD TESTINI CALISTIR", type="primary"):
    with st.spinner("BANT motoru bütün uygun çekilişlerde test ediliyor..."):
        strict_df, bant_wf = bant_strict_walkforward(eng, 160)
        score_sum, rank_sum = bant_calibration_report(bant_wf)

    st.session_state["bant_strict"] = strict_df
    st.session_state["bant_raw"] = bant_wf
    st.session_state["bant_score"] = score_sum
    st.session_state["bant_rank"] = rank_sum

if "bant_strict" in st.session_state:
    strict_df = st.session_state["bant_strict"]
    bant_wf = st.session_state["bant_raw"]
    score_sum = st.session_state["bant_score"]
    rank_sum = st.session_state["bant_rank"]

    active = strict_df[strict_df["Kupon boyu"] > 0].copy()
    full = active[active["Tam isabet"] == True].copy()

    c1,c2,c3,c4,c5 = st.columns(5)
    c1.metric("Test edilen çekiliş", len(strict_df))
    c2.metric("Aday satırı", len(bant_wf))
    c3.metric("KONUŞ", len(active))
    c4.metric("Verilen sayı", int(active["Kupon boyu"].sum()) if len(active) else 0)
    c5.metric("Tam kolon", len(full))

    st.subheader("BANT skor kalibrasyonu")
    ...
    st.subheader("Tüm walk-forward kararları")
    st.dataframe(
        strict_df.sort_values("Çekiliş", ascending=False),
        use_container_width=True,
        hide_index=True
    )

    st.download_button(
        "⬇️ 1) BANT WALK-FORWARD KARARLARI",
        strict_df.to_csv(index=False).encode("utf-8-sig"),
        file_name="BANT_TEK_LAB_WALKFORWARD.csv",
        mime="text/csv",
        key="dl_bant_wf",
    )

    st.download_button(
        "⬇️ 2) BANT TUM ADAY KALIBRASYONU",
        bant_wf.to_csv(index=False).encode("utf-8-sig"),
        file_name="BANT_ADAY_KALIBRASYON.csv",
        mime="text/csv",
        key="dl_bant_raw",
    )

    st.info(
        f"İki çıktı hazır: WALK-FORWARD={len(strict_df)} satır, "
        f"ADAY KALİBRASYON={len(bant_wf)} satır."
    )
