...
st.sidebar.markdown("### ➕ Yeni çekiliş ekle ve KALICI KAYDET")
with st.sidebar.form("add_live_draw",clear_on_submit=True):
    new_draw=st.number_input("Çekiliş no",min_value=1,value=int(max(base.draw.max(), saved_live.draw.max() if not saved_live.empty else 0))+1,step=1)
    new_dt=st.text_input("Tarih - saat","08.09.2026 00:47")
    new_nums=st.text_area("20 sayı","7 10 17 21 25 26 27 28 30 38 41 42 49 51 59 60 65 66 71 74",height=100)
    save_btn=st.form_submit_button("💾 SONUCU KAYDET",type="primary")
    if save_btn:
        try:
            dtv=pd.to_datetime(new_dt,dayfirst=True,errors="raise")
            nums=tuple(sorted({int(x) for x in new_nums.replace(","," ").split()}))
            if len(nums)!=20 or min(nums)<1 or max(nums)>80:
                st.error("Tam 20 farklı sayı girilmeli; aralık 1–80.")
            else:
                save_live_draw(new_draw,dtv,nums)
                st.success(f"#{int(new_draw)} kalıcı kaydedildi.")
                st.rerun()
        except Exception as e:
            st.error(f"Kayıt hatası: {e}")

# İstenirse toplu geçici yapıştırma da kullanılabilir.
st.sidebar.markdown("### Toplu/geçici veri")
live_text=st.sidebar.text_area("İstersen satır biçiminde yapıştır",height=100,
    placeholder="54261;08.09.2026 00:52;1,2,...")
live=parse_text(live_text) if live_text.strip() else pd.DataFrame(columns=["draw","dt","nums"])

saved_live=load_saved_live()
df=pd.concat([base,saved_live,live],ignore_index=True)
