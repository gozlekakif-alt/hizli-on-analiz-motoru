...
st.set_page_config(
    page_title="Hızlı On Ultimate V24 — Adaptif Birleşik Master",
    page_icon="🎯",
    layout="wide",
)
...
# ============================================================
# V24 — ADAPTİF BİRLEŞİK MASTER
# Birleşen yapı:
# - V21 yaşayan alt motor omurgası
# - Benzer Durum Arama motoru
# - Blok Basınç Arama motoru
# - Blok motoru yalnız güçlü blok karakterinde agresifleşir
# - Benzer motor istikrar katmanı olarak sürekli çalışır
# - Saat ana eksen olmaya devam eder
# - Tek motorun aşırı puanı tek başına karar veremez
# ============================================================

def v24_norm_series(s):
...
def v24_similar_search_scores(df):
    try:
        _, score_df = similar_state_next_scores(
            df,
            state_window=6,
            search_window=min(500, len(df)),
            top_matches=25,
        )
        if score_df is None or score_df.empty:
            raise ValueError("empty")
        out = score_df[["Sayı","Benzer Durum Puanı"]].copy()
        out = out.rename(columns={"Benzer Durum Puanı":"V24 Benzer"})
        return out
    except Exception:
        # V21'in kendi Benzer sinyaline geri dön.
        base = v21_submotors(df, str(df.iloc[-1].Saat))
        return base[["Sayı","Benzer"]].rename(columns={"Benzer":"V24 Benzer"})


def v24_block_pressure_scores(df, target_time=None):
    region_map = {}
    try:
        _, region_df, _ = block_pressure_engine_v184(
            df,
        ...
        if region_df is not None and not region_df.empty:
            region_map = region_df.set_index("Bölge")["Blok Bölge Puanı"].to_dict()
    except Exception:
        region_map = {}

    try:
        transition = predicted_block_number_scores(
            df,
            lookback=min(400, len(df)),
        )[["Sayı","Blok Geçiş Puanı"]].copy()
    except Exception:
        transition = pd.DataFrame({
            "Sayı": range(1,81),
            "Blok Geçiş Puanı": [0.0]*80,
        })

    out = pd.DataFrame({"Sayı": range(1,81)})
    out["Bölge Basıncı"] = out["Sayı"].map(
        lambda n: float(region_map.get(ten_band_name(int(n)), 0.0))
    )
    out = out.merge(transition, on="Sayı", how="left").fillna(0.0)
    out["V24 Blok"] = (
        0.68*v24_norm_series(out["Bölge Basıncı"])
        + 0.32*v24_norm_series(out["Blok Geçiş Puanı"])
    )
    return out


def v24_block_trigger(df):
    """
    Blok motorunun atak moda geçip geçmeyeceğini ölçer.
    Güçlü blok fazı yoksa Blok motoru düşük ağırlıkta kalır.
    """
    sets = [
        set(int(r[c]) for c in NUM_COLS)
        for _, r in df.tail(min(12,len(df))).reset_index(drop=True).iterrows()
    ]
    if len(sets) < 4:
        return 0.0, {"Son Blok":0.0,"Kısa Blok":0.0,"Taşıma":0.0}

    def block_strength(s):
        vals=sorted(s)
        runs=[]; cur=[]
        for n in vals:
            if not cur or n==cur[-1]+1:
                ...
                cur=[n]
        if len(cur)>=2: runs.append(cur)
        return float(sum(max(0,len(b)-1) for b in runs))

    strengths=[block_strength(s) for s in sets]
    recent=np.mean(strengths[-3:])
    prev=np.mean(strengths[-8:-3]) if len(strengths)>=8 else np.mean(strengths[:-3])

    carry = np.mean([
        len(sets[i-1] & sets[i]) for i in range(max(1,len(sets)-4),len(sets))
    ])

    growth = max(0.0, recent-prev)
    raw = (
        0.45*np.clip(recent/4.0,0,1)
        + 0.30*np.clip(growth/2.5,0,1)
        + 0.25*np.clip((carry-3.5)/4.0,0,1)
    )
    return float(np.clip(raw,0,1)), {
        "Son Blok":round(float(recent),2),
        "Kısa Blok":round(float(prev),2),
        "Taşıma":round(float(carry),2),
    }


def v24_adaptive_master(df, target_time=None):
    # V21 yaşayan omurga
    v21_coupon, v21_table = v21_live_master(
        df,
        target_time or str(df.iloc[-1].Saat),
    )

    similar = v24_similar_search_scores(df)
    block = v24_block_pressure_scores(df, target_time)
    trigger, trigger_meta = v24_block_trigger(df)

    table = v21_table.copy()
    table = table.merge(similar, on="Sayı", how="left")
    table = table.merge(
        block[["Sayı","V24 Blok","Bölge Basıncı","Blok Geçiş Puanı"]],
        on="Sayı",
        how="left",
    ).fillna(0.0)

    # Normalize V21 + old search engines.
    table["V21_n"] = v24_norm_series(table["V21 Puan"])
    table["Benzer_n"] = v24_norm_series(table["V24 Benzer"])
    table["Blok_n"] = v24_norm_series(table["V24 Blok"])

    # Benzer motor istikrar motoru: daima orta ağırlık.
    w_sim = 0.20

    # Blok motoru yalnız trigger kuvvetliyse agresif.
    w_block = 0.06 + 0.26*trigger

    # Kalan ağırlık V21 yaşayan omurgaya.
    w_v21 = max(0.50, 1.0 - w_sim - w_block)

    # Ortak destek bonusu.
    common = np.minimum(table["Benzer_n"], table["Blok_n"])
    consensus = (
        (table["Motor Oy"] >= 3).astype(float)
        if "Motor Oy" in table.columns
        else (table["Band Oy"] >= 3).astype(float)
        if "Band Oy" in table.columns
        else 0.0
    )

    table["V24 Puan"] = (
        w_v21*table["V21_n"]
        + w_sim*table["Benzer_n"]
        + w_block*table["Blok_n"]
        + 0.06*common
        + 0.04*consensus
    ) * 100.0

    # Aşırı yalnız motor tepesini frenle:
    lone_block = (table["Blok_n"] > 0.88) & (table["Benzer_n"] < 0.40)
    lone_sim = (table["Benzer_n"] > 0.88) & (table["Blok_n"] < 0.30)
    table.loc[lone_block, "V24 Puan"] -= 4.0*(1.0-trigger)
    table.loc[lone_sim, "V24 Puan"] -= 2.0

    table["V24 Puan"] = table["V24 Puan"].round(3)
    ...
    meta = {
        "Blok Tetik":round(trigger,3),
        "V21 Ağırlık":round(w_v21,3),
        "Benzer Ağırlık":round(w_sim,3),
        "Blok Ağırlık":round(w_block,3),
        **trigger_meta,
    }
    return coupon, ranked, meta


def v24_blind_test(df, test_count=50):
    work = df.sort_values("Cekilis_No").reset_index(drop=True)
    start = max(180, len(work)-int(test_count))
    rows=[]
    ...
    for i in range(start,len(work)):
        ...
        c24, _, meta = v24_adaptive_master(train, str(target.Saat))
        c21, _ = v21_live_master(train, str(target.Saat))

        # Eski Benzer ve Blok motorlarını da ayrı ölç.
        sim = v24_similar_search_scores(train)
        sim_c = sorted(
            sim.sort_values(["V24 Benzer","Sayı"],ascending=[False,True])
               .head(7)["Sayı"].astype(int).tolist()
        )
        blk = v24_block_pressure_scores(train, str(target.Saat))
        blk_c = sorted(
            blk.sort_values(["V24 Blok","Sayı"],ascending=[False,True])
               .head(7)["Sayı"].astype(int).tolist()
        )

        actual=set(int(target[c]) for c in NUM_COLS)
        h24=sorted(set(c24)&actual)
        h21=sorted(set(c21)&actual)
        hs=sorted(set(sim_c)&actual)
        hb=sorted(set(blk_c)&actual)

        rows.append({
            "Çekiliş":int(target.Cekilis_No),
            "Tarih":str(target.Tarih),
            "Saat":str(target.Saat),
            "V24 Kupon":"-".join(map(str,c24)),
            "V24 İsabet":len(h24),
            "V24 Tutan":"-".join(map(str,h24)),
            "V21 İsabet":len(h21),
            "Benzer İsabet":len(hs),
            "Blok İsabet":len(hb),
            "Blok Tetik":meta["Blok Tetik"],
            "Blok Ağırlık":meta["Blok Ağırlık"],
            "Avantaj V21":len(h24)-len(h21),
            "Sızıntı":"TEMİZ",
        })
...
if page == "✅ Kontrol":
    st.success("Çalışan sürüm: V24 ADAPTİF BİRLEŞİK MASTER")
...
elif page == "🧠 Adaptif Birleşik MASTER V24":
    st.header("🧠 V24 — Adaptif Birleşik MASTER")
    st.caption(
        "V21 yaşayan alt motor omurgası + Benzer Durum Arama + Blok Basınç Arama. "
        "Benzer motor istikrar katmanıdır. Blok motoru yalnız blok karakteri güçlendiğinde "
        "atak moda geçer. Saat ana eksen olmaya devam eder."
    )

    if len(df) < 200:
        st.warning("V24 için 200+ çekiliş önerilir.")
    else:
        next_no,next_date,next_time = next_draw_defaults(df)
        c24,t24,m24 = v24_adaptive_master(df,next_time)

        st.write(f"**Hedef:** #{next_no} — {next_date} {next_time}")
        st.success("🏆 V24 TEK 7'Lİ: " + " - ".join(map(str,c24)))

        m1,m2,m3,m4 = st.columns(4)
        m1.metric("Blok tetik",m24["Blok Tetik"])
        m2.metric("V21 ağırlık",m24["V21 Ağırlık"])
        m3.metric("Benzer ağırlık",m24["Benzer Ağırlık"])
        m4.metric("Blok ağırlık",m24["Blok Ağırlık"])

        st.caption(
            f"Son blok gücü {m24['Son Blok']} | "
            f"önceki kısa blok {m24['Kısa Blok']} | "
            f"son taşıma {m24['Taşıma']}"
        )

        cols=[
            "Sayı","V24 Puan","V21 Puan","V24 Benzer","V24 Blok",
            "Bölge Basıncı","Blok Geçiş Puanı"
        ]
        cols=[c for c in cols if c in t24.columns]
        st.dataframe(
            t24[cols].head(25),
            use_container_width=True,
            hide_index=True,
        ...
        st.divider()
        st.subheader("🧪 V24 karşılaştırmalı kör test")
        tn=st.selectbox(
            "Test adedi",
            [10,25,50,100],
            index=2,
        ...
        if st.button(
            ...
            with st.spinner(
                "V24, V21, Benzer ve Blok motoru aynı hedeflerde karşılaştırılıyor..."
            ):
                rr=v24_blind_test(df,tn)
            st.session_state["v24_result"]=rr

        rr=st.session_state.get("v24_result",pd.DataFrame())
        if isinstance(rr,pd.DataFrame) and not rr.empty:
            a,b,c,d=st.columns(4)
            a.metric("Test",len(rr))
            b.metric("V24 ort.",f"{rr['V24 İsabet'].mean():.2f}")
            c.metric("V21 ort.",f"{rr['V21 İsabet'].mean():.2f}")
            d.metric("Net avantaj",f"{rr['Avantaj V21'].mean():+.2f}")

            st.write(
                f"Benzer ort.: **{rr['Benzer İsabet'].mean():.2f}** — "
                f"Blok ort.: **{rr['Blok İsabet'].mean():.2f}**"
            )
            ...
            st.download_button(
                "⬇️ V24 test CSV",
                rr.to_csv(index=False).encode("utf-8-sig"),
                file_name=f"V24_ADAPTIF_BIRLESIK_TEST_{len(rr)}.csv",
                mime="text/csv",
                use_container_width=True,
                key="v24_csv",
            )
