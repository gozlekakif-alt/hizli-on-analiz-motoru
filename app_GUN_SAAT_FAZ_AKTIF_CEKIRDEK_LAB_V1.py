...
# UI
# ============================================================
st.title("🧠 GÜN × SAAT × FAZ AKTİF ÇEKİRDEK — LAB V1")
st.info(
...
tabs=st.tabs(["🧪 750 GÜN-İÇİ KÖR TEST","🎯 BUGÜNÜN AKTİF ÇEKİRDEĞİ","📊 SAAT/FAZ KARAKTERİ"])

with tabs[0]:
    st.subheader("750 walk-forward")
    st.caption(
    ...
    if st.button("🚀 750 TESTİ BAŞLAT",type="primary",use_container_width=True):
        with st.spinner("Gün–Saat–Faz kör testi çalışıyor..."):
            st.session_state.core_wf=walkforward(df,A,750)
    ...
    if isinstance(wf,pd.DataFrame) and len(wf):
        ...
        st.download_button(
            ...
            file_name="GUN_SAAT_FAZ_AKTIF_CEKIRDEK_750.csv",
