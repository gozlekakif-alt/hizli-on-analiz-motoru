...
def repeat_v2_candidates(engine, t):
    ...
    # SIKI V3: 500 testte kronolojik ilk %65 / son %35 ayrı kontrol edildi.
    # Basit ve dayanıklı fren: V2 sinyali + trans_same >= 0.26136.
    # Eğitim bölümünde %40.0, görülmemiş doğrulama bölümünde %41.2 doğruluk.
    tab["V3_sinyal"] = (
        tab["V2_sinyal"]
        & (tab["trans_same"] >= 0.26136)
    )

    sel = sorted(
        tab.loc[tab["V3_sinyal"], "Sayı"].astype(int).unique().tolist()
    )
...
# ============================================================
st.title("🔁 TEKRAR TEK LAB — SIKI V3")
st.caption(
...
st.info(
    "V2 tabanı korunur. SIKI V3: V2 + trans_same ≥ 0.26136. "
    "Eşik 500 testin ilk %65'inde seçilip son %35'inde ayrıca doğrulandı. "
...
if st.button("🚀 TESTİ BAŞLAT", type="primary", use_container_width=True):
    with st.spinner(f"{tests} çekilişlik TEKRAR SIKI V3 walk-forward çalışıyor..."):
        eng = RepeatEngine(df)
...
if isinstance(draw_df, pd.DataFrame) and not draw_df.empty:
    ...
    st.download_button(
        ...
        file_name="TEKRAR_SIKI_V3_WALKFORWARD.csv",
        mime="text/csv",
        use_container_width=True,
    )
    ...
    if isinstance(cand_df, pd.DataFrame) and not cand_df.empty:
        st.download_button(
            ...
            file_name="TEKRAR_SIKI_V3_ADAY_DETAY.csv",
            mime="text/csv",
            use_container_width=True,
        )
...
st.divider()
st.subheader("🎯 Güncel TEKRAR SIKI V3 kararı")
try:
