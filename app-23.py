...
def _unified_brain_observer(df, tab, char, size=16):
    """
    TEK BEYİN BAĞLANTI KATMANI — GÖZLEMCİ
    Motorları yan yana toplamaz; bağımsız mekanizma ailelerini birbirine bağlar.
    3-el blok yalnız destek/fren bağlamıdır, tek başına aday seçemez.
    """
    ...
    # GAP mekanizması: yalnız adayın kendi GAP uzmanı konuşur.
    gap_signal=pd.Series(0.0,index=z.index,dtype=float)
    for gcol,mask in [
        ("GAP-1 R",gap==1),("GAP-2 R",gap==2),("GAP-3 R",gap==3),
        ("GAP-4/5 R",(gap>=4)&(gap<=5)),("GAP-6+ R",gap>=6)
    ]:
        gap_signal=np.maximum(gap_signal, C(gcol)*mask.astype(float))

    # Birbirinden bağımsız kanıt yolları.
    return_path=(0.34*gap_signal+0.20*ret+0.16*rhythm+0.12*same+0.10*react+0.08*longsleep)
    carry_path=(0.38*carry+0.18*C("TAŞIMA R")+0.16*cluster+0.10*seq+0.10*core+0.08*slot)
    cluster_path=(0.34*cluster+0.20*np.maximum(C("KÜME TAŞIMA R"),C("KÜME DÖNÜŞ R"))+
                  0.16*rhythm+0.12*same+0.10*block+0.08*blockdeg)
    trace_path=(0.30*same+0.22*rhythm+0.16*slot+0.12*react+0.10*block+0.10*seq)

    # Günün ilk 3 elindeki blok, ancak başka bir yol da aynı adayı destekliyorsa güçlenir.
    other=np.maximum.reduce([return_path.to_numpy(),carry_path.to_numpy(),
                             cluster_path.to_numpy(),trace_path.to_numpy()])
    block_confirm=block.to_numpy()*np.clip((other-0.35)/0.45,0,1)

    # Çatışma: tek aile/konsensüs parlaması, negatif kanıt ve desteksiz blok cezalanır.
    independent=(0.34*fam+0.24*strong+0.22*micro+
                 0.20*np.minimum(
                     ((return_path>=0.42).astype(int)+(carry_path>=0.42).astype(int)+
                      (cluster_path>=0.42).astype(int)+(trace_path>=0.42).astype(int))/3.0,1.0))
    contradiction=0.42*neg + 0.10*((fam<=0.25)&(micro<=0.25)).astype(float)
    unsupported_block=0.08*((block>0.45)&(other<0.35)).astype(float)

    # En güçlü iki mekanizma + bağımsız doğrulama. Aynı aileyi çok kez saymıyoruz.
    paths=np.vstack([return_path,carry_path,cluster_path,trace_path]).T
    ps=np.sort(paths,axis=1)[:,::-1]
    z["Tek Beyin Skor"]=(0.34*ps[:,0]+0.22*ps[:,1]+0.24*independent+
                         0.12*block_confirm+0.08*C("Ana Yüzdelik")-
                         contradiction-unsupported_block)
    ...
    # Havuz16: tam 80 sayıdan seçilir, fakat bant tekelleşmesini sınırlar.
    q=z.sort_values(["Tek Beyin Skor","Bağımsız Aile","Mikro Bağımsız Kanıt","Ana Puan"],
                    ascending=False)
    chosen=[]; bc=Counter()
    for _,r in q.iterrows():
        n=int(r["Sayı"]); b=band(n)
    ...
    meta={"block":block_meta,
          "top_paths":z.sort_values("Tek Beyin Skor",ascending=False).head(16)
                       [["Sayı","Tek Beyin Skor","Tek Beyin Yol","3El Blok"]].to_dict("records")}
...
with tabs[4]:
    ...
    if isinstance(bt,pd.DataFrame) and not bt.empty:
        ...
        if "Birleşik Beyin Havuz16" in bt.columns:
            ...
            st.caption("Birleşik Beyin GÖZLEMCİDİR: GAP + yaşam izi + küme + survival + ardışık + 3-el blok + negatif kanıtı bağımsız yollar halinde bağlar; canlı MASTER'a henüz dokunmaz.")
